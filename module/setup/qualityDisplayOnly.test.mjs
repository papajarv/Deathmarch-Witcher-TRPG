...
test("wired weapon qualities do NOT get displayOnly", () => {
    ...
    /* Grappling IS wired (the CE grapple actions gate on it) — NOT displayOnly. */
    assert.equal(!!WEAPON_QUALITIES.grappling?.displayOnly, false);
...
test("armor qualities marked displayOnly cover the no-engine set", () => {
    ...
    /* `monsterResistance` was removed from the catalog entirely, so it no
     * longer belongs to the no-engine display-only set. */
