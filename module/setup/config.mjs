...
export const SKILL_MAP = Object.freeze({
    ...
    deduction:     skill("int"),
    education:     skill("int"),
    commonspeech:  skill("int", 2),  /* Core p.49: difficult skill — ×2 IP/rank */
    eldersp:       skill("int", 2),  /* Elder Speech (Elven / Hen Linge) — difficult skill, ×2 IP/rank */
    elderspskellige: skill("int", 2),/* Elder Speech (Skellige dialect) — difficult INT language, ×2 IP/rank */
    dwarven:       skill("int", 2),  /* Core p.49: difficult skill — ×2 IP/rank */
    nilfgaardian:  skill("int", 2),  /* Elder Speech (Nilfgaardian dialect) — difficult INT language, ×2 IP/rank */
    monster:       skill("int", 2),  /* Core p.49: difficult skill — ×2 IP/rank */
    socialetq:     skill("int"),
    streetwise:    skill("int"),
    tactics:       skill("int", 2),  /* Core p.49: difficult skill — ×2 IP/rank */
    teaching:      skill("int"),
    ...
    spellcast:     skill("will", 2),  /* Core p.49: difficult skill — ×2 IP/rank */
    resistmagic:   skill("will", 2),  /* Core p.49: difficult skill — ×2 IP/rank */
    resistcoerc:   skill("will"),
    ritcraft:      skill("will", 2),  /* Core p.49: difficult skill — ×2 IP/rank */

    alchemy:       skill("cra", 2),   /* Core p.49: difficult skill — ×2 IP/rank */
    cooking:       skill("cra", 1, { homebrew: "foodAndDrink" }),
    crafting:      skill("cra", 2),   /* Core p.49: difficult skill — ×2 IP/rank */
    disguise:      skill("cra"),
    firstaid:      skill("cra"),
    forgery:       skill("cra"),
    picklock:      skill("cra"),
    trapcraft:     skill("cra", 2)    /* Core p.49: difficult skill — ×2 IP/rank */
});
 ...
 * Core p.48). "bomb" is a Death March addition: an area weapon thrown
 * with Athletics against a scatter-on-fumble table; targets in the AoE
 * defend with Block (shield / Full Cover) or Reposition (leave the
 * radius). Bomb-specific fields live on WeaponData directly. Legacy
 * "thrown" is a dead migration-source value kept selectable so authored
 * items that predate the melee-with-range collapse still round-trip. */
export const WEAPON_TYPES = Object.freeze({
    ...
    bomb:   "WITCHER.Weapon.TypeBomb"
});

/* Bomb AoE template shape — picker on the weapon sheet when
 * weaponType === "bomb". "circle" reuses the shared `radius` field
 * (damageProperties) for its size; the other three read the bomb-
 * specific `bombTemplate.size` field (length for line/cone, side for
 * rect). Passed straight through to Foundry's MeasuredTemplate types
 * at throw-placement time. */
export const WEAPON_BOMB_SHAPES = Object.freeze({
    circle: "WITCHER.Weapon.Bomb.Shape.Circle",
    cone:   "WITCHER.Weapon.Bomb.Shape.Cone",
    line:   "WITCHER.Weapon.Bomb.Shape.Line",
    rect:   "WITCHER.Weapon.Bomb.Shape.Rect"
});
 ...
 * layering rule on p.154 ("max 1 heavy + 1 medium per location"). Clothes
 * is a Death March addition: plain clothing that occupies an armor slot
 * (blocks layering) but grants no SP. Shields are their own item type
 * (`shield`, not armor), so no shield entry here — legacy items still
 * using `armorType: "shield"` are filtered out defensively in the actor
 * armor calc (base.mjs, character.mjs) via the same location==="Shield"
 * check they've always used. */
export const ARMOR_TYPES = Object.freeze({
    clothes: "WITCHER.Armor.TypeClothes",
...
export const AVAILABILITY = Object.freeze({
    ...
    elderfolk:    "WITCHER.Gear.AvailElderFolk",
    relic:        "WITCHER.Gear.AvailRelic",
    goetia:       "WITCHER.Gear.AvailGoetia",
    experimental: "WITCHER.Gear.AvailExperimental",
...
export const FORMULA_SUBTYPES = Object.freeze({
    ...
    // Alchemical Item — a formula category that in RAW behaves like any other
    // (Alchemy roll + substances), but under Alchemy Reborn consumes NO base
    // (unlike potion/oil/bomb/decoction). The base UI is suppressed for it.
    alchemical: "WITCHER.Crafting.SubAlchemicalItem"
});
...
export const SPELL_DAMAGE_TYPES = Object.freeze({
    ...
    healHp:      "WITCHER.Magic.DamageTypeHealHp",
...
/* No `permanent` — these effects run invisibly in the background (no on-
 * target AE badge), so a permanent one could never be found to remove.
 * Author a real Active Effect if you need a lasting, visible/removable one. */
...
/* "Apply Every" — how often a spell's target-resource effect (Damage HP /
 * Heal HP / Ablation) re-applies over its duration. `instant` = once on cast;
 * the others create a ticking Active Effect (heal / damage action + cadence)
 * that fires each turn / minute / hour / day until the duration expires. */
export const SPELL_APPLY_EVERY = Object.freeze({
    instant: "WITCHER.Magic.ApplyEveryInstant",
    turn:    "WITCHER.Magic.ApplyEveryTurn",
    minute:  "WITCHER.Magic.ApplyEveryMinute",
    hour:    "WITCHER.Magic.ApplyEveryHour",
    day:     "WITCHER.Magic.ApplyEveryDay"
});
...
export const BRAWL_ACTIONS = Object.freeze({
    ...
    pushKick: { labelKey: "WITCHER.Brawl.PushKick", kind: "attack",  damage: "punch", half: true, fixedLoc: "torso", pushBackFormula: "body/3", wallRamming: true, wallRammingFormula: "1d6", note: "WITCHER.Brawl.NotePushKick" },
    ...
    /* Disarm — grapple action. Made at −4, must be in a grapple. Deals PUNCH
     * damage to the target's weapon ARM (forced location in brawlAttack), then
     * the weapon is knocked 1d6m in a random direction — or the attacker may
     * make a DC 18 Brawling check to snatch it for themselves instead (post-hit
     * rider). Defended with Brawling (or a Grappling weapon's skill) or Dodge/
     * Escape. Doable with a Grappling weapon in the melee dialog too. */
    disarm:   { labelKey: "WITCHER.Brawl.Disarm",   kind: "grapple", damage: "punch", ceToHitMod: -4, needsGrapple: true, isDisarm: true, note: "WITCHER.Brawl.NoteDisarm", rawNote: "WITCHER.Brawl.NoteDisarmRaw" },
     ...
     * Trip/Disarm become grapple-gated too under CE). */
    pin:      { labelKey: "WITCHER.Brawl.Pin",      kind: "grapple", status: "pinned", needsGrapple: true, note: "WITCHER.Brawl.NotePin" },
    /* Choke — only on a foe you have PINNED (needsPin). A HOLD that deals
     * 3 + melee-bonus suffocation THROUGH armor, +any Strangling-quality
     * bonus. Performed with Brawling or a Strangling weapon (NOT plain
     * grappling — enforced in the melee dialog via requiresQuality:strangling).
     * `isChoke` routes it to the maintenance handler: each turn you re-Choke to
     * keep it, spending a normal action (×1 damage) or the extra action (×2);
     * an un-maintained choke releases on your next turn. */
    choke:    { labelKey: "WITCHER.Brawl.Choke",    kind: "grapple", status: "chokeheld",   needsPin: true, isChoke: true, note: "WITCHER.Brawl.NoteChoke", rawNote: "WITCHER.Brawl.NoteChokeRaw" },
    /* Drag — usable by EITHER party in a grapple (holder OR grapplee). Not a
     * combat exchange: an opposed PHYSIQUE vs PHYSIQUE test (auto-rolled, no
     * defender prompt). On a win you pick a reachable location and drag the foe
     * (and yourself, still holding each other) there — distance = |physique-base
     * difference|, minimum 2m, capped by your remaining movement. `isDrag`
     * routes it to its own handler. */
    drag:     { labelKey: "WITCHER.Brawl.Drag",     kind: "grapple", needsGrappleAnyRole: true, isDrag: true, note: "WITCHER.Brawl.NoteDrag" },
    ...
    /* Trip — GRAPPLER-ONLY. Like Takedown (opposed Brawling / Grappling weapon
     * vs the foe's Brawling; on a success a NON-LETHAL punch to Stamina, still
     * soaked by SP; grapple MAINTAINED) — but only the TARGET goes prone, the
     * grappler stays on their feet (no selfProne). needsGrapple (holder-only),
     * NOT needsGrappleAnyRole: the grapplee can't trip their captor. */
    trip:     { labelKey: "WITCHER.Brawl.Trip",     kind: "grapple", damage: "punch", nonLethal: true, status: "prone", location: true, needsGrapple: true, note: "WITCHER.Brawl.NoteTrip", rawNote: "WITCHER.Brawl.NoteTripRaw" },
    ...
    push:     { labelKey: "WITCHER.Brawl.Push",     kind: "attack",  pushBackFormula: "phy/5floor", pickDirection: true, breaksHolds: true, needsGrapple: true, wallRamming: true, note: "WITCHER.Brawl.NotePush" },
    /* Slam — CE grapple finisher. Mechanically the CE Trip enhancement
     * (double kick damage + prone + 1d6m knockback + Stun save) but to a
     * RANDOM location instead of the legs. Grapple-gated like Push — usable
     * from INSIDE a grapple/pin by EITHER role (holder or grapplee), gated in
     * the brawl dialog via the same either-role check (see brawlDialog). The
     * double kick rides via a strikeMeta.dmgMult=2 injection in brawlAttack;
     * the 1d6m throw separates the pair, so it breaks the hold. */
    slam:     { labelKey: "WITCHER.Brawl.Slam",     kind: "grapple", damage: "kick", nonLethal: true, status: "prone", location: true, toHitMod: -3, pushBackFormula: "phy/5floor", pickDirection: true, triggerStunSave: { mod: 0 }, breaksHolds: true, needsGrapple: true, rammingFormula: "1d6", wallRamming: true, note: "WITCHER.Brawl.NoteSlam" },
    /* Takedown — usable by EITHER party in a grapple (holder OR grapplee).
     * Opposed Brawling (or a Grappling weapon) vs the foe's Brawling. On a
     * success: NON-LETHAL punch damage (to stamina, still soaked by SP — only
     * ramming ignores armour) and BOTH combatants go prone. The grapple hold is
     * MAINTAINED (prone no longer breaks a grapple), so there is no breaksHolds. */
    takedown: { labelKey: "WITCHER.Brawl.Takedown", kind: "grapple", damage: "punch", nonLethal: true, status: "prone", selfProne: true, location: true, needsGrappleAnyRole: true, note: "WITCHER.Brawl.NoteTakedown" },
     ...
     *   of at least one hold pair (grappled / pinned / chokeheld).
     ...
     *   visible statuses. Fast-path from the dock's Action menu, doesn't
     *   need a canvas target — the partner is discovered from the hold
     *   pair registry. */
...
export const BRAWL_GROUPS = Object.freeze([
    ...
    { labelKey: "WITCHER.Brawl.GroupGrapple", actions: ["grapple", "releaseGrapple", "push", "slam", "takedown", "pin", "choke", "throw", "trip", "drag", "escape", "reverseGrapple"] }
]);
 ...
 * stat-block. */
/* Death March house rule (supersedes the RAW p.175 split): every
 * non-humanoid monster is resistant to any damage that isn't silver-weapon
 * damage or fire — the meteorite-weakness column from the novel-rules table
 * is folded into "silver" so silver is the sole material gate. Fire bypass
 * lives in damageCalculator.mjs (applyNaturalResists). Humanoids carry no
 * weakness. GM can still override per monster in the sheet. */
...
/* Derive a slug from the label so we can key lang lookups (e.g. "Armor
 * Piercing" → "ArmorPiercing"). Stable across languages because we key
 * off the English label, which is the source of truth in this file. */
const _wqSlug = (s) => String(s ?? "").replace(/[^A-Za-z0-9]+/g, "");
const wq = (label, description, param = null, extra = null) => {
    const slug = _wqSlug(label);
    return Object.freeze({
    /* Lazy getters — labels & descriptions localize at READ time, so a
     * language swap (or a load that happens after Foundry lang is ready)
     * takes effect without rebuilding the frozen catalog. `labelEn`
     * exposes the raw English source so parsers (openCategoryBonuses)
     * can index both the localized AND English forms — otherwise
     * existing world items with English bonus text stop matching under a
     * localized client. */
    get label()       { return t(`WITCHER.WeaponQuality.${slug}.Label`,       label); },
    get description() { return t(`WITCHER.WeaponQuality.${slug}.Description`, description); },
    labelEn:     label,
     ...
     *   brawlingUnarmedStrike — Combat Extended "Brawling (Punch)/(Kick)": the
     *     OPPOSITE direction to addsDamageToUnarmed. When you ATTACK WITH this
     *     weapon, the wielder's punch ("punch") or kick ("kick") damage code is
     *     added to the WEAPON's damage. "" = off. The attack stays a weapon
     *     attack, so it keeps the weapon's damage type and lethality. Consumed
     *     in weaponAttackMixin. A weapon with this can't be equipped by an actor
     *     that fights with Natural Weapons (which replace the punch/kick dice). */
    ...
    brawlingUnarmedStrike:     String(extra?.brawlingUnarmedStrike ?? ""),
    ...
    /* ── House-rule hardwired hooks ─────────────────────────────────────
     *   fastDrawPenaltyDelta  — Quick Strike: shave N off the fast-draw
     *                           to-hit penalty (delta 2 turns −3 into −1).
     *   versatileActionSwitch — Versatile: extra-action penalty is −2 not
     *                           −3 when this action's type differs from the
     *                           previous action's type.
     *   spellDefenseBonus     — Warded (armor): +N to the defence roll when
     *                           resisting a spell.
     *   stealthDarkBonus      — Like a Shadow (armor): +N to Stealth checks
     *                           while in Dim Light −2 or darker.
     *   unmovingStagger       — Unmoving (armor): on a 0-damage hit, the
     *                           wearer may spend 5 STA to Stagger the attacker. */
    fastDrawPenaltyDelta:    Number(extra?.fastDrawPenaltyDelta)    || 0,
    versatileActionSwitch:   Boolean(extra?.versatileActionSwitch   ?? false),
    spellDefenseBonus:       Number(extra?.spellDefenseBonus)       || 0,
    stealthDarkBonus:        Number(extra?.stealthDarkBonus)        || 0,
    unmovingStagger:         Boolean(extra?.unmovingStagger         ?? false)
});
};
export const WEAPON_QUALITIES = Object.freeze({
    ...
    armorPiercing:         wq("Armor Piercing",          "Ignores the target's damage resistances — armor DR and any monster per-type resistance matching the weapon's damage type. Does NOT bypass immunity or the 'half from non-silver' / 'half from non-meteorite' weakness gates.",
                              null, { damageFlags: { armorPiercing: true } }),
    improvedArmorPiercing: wq("Improved Armor Piercing", "Ignores the target's damage resistances (as Armor Piercing) AND halves the armor's SP on hit. Does NOT bypass immunity or the 'half from non-silver' / 'half from non-meteorite' weakness gates.",
    ...
    /* Combat Extended per-limb Brawling weapons (a cestus for the fist, bladed
     * boots for the kick). When you ATTACK WITH the weapon, your punch or kick
     * damage code is added to the weapon's damage — the attack stays a weapon
     * attack (weapon's damage type + lethality). Can't be equipped alongside
     * Natural Weapons, which replace the punch/kick dice these fold in. */
    brawlingPunch:         wq("Brawling (Punch)",        "Attacking with this weapon adds your punch damage code to the weapon's damage. Requires a free fist; can't be used with Natural Weapons. (Combat Extended)",
                              null, { brawlingUnarmedStrike: "punch" }),
    brawlingKick:          wq("Brawling (Kick)",         "Attacking with this weapon adds your kick damage code to the weapon's damage. Can't be used with Natural Weapons. (Combat Extended)",
                              null, { brawlingUnarmedStrike: "kick" }),
    ...
    focus:                 wq("Focus",                   "A spellcasting focus. Casting a spell through this equipped gear reduces the spell's STA cost by its value (a spell never costs less than 1 STA). When you have more than one focus equipped you pick which to cast through.",
    ...
    injector:              wq("Injector",                "Can be charged (an action) with a dose of poison or elixir; a damaging hit drives the dose deep, making a poison 3 harder to resist or an elixir last 3 rounds longer. (GM-resolved — no charge-with-poison UI; rider note posts on hit.)",
                              null, { displayOnly: true }),
    greaterFocus:          wq("Greater Focus",           "A greater spellcasting focus. When you cast a spell through this equipped gear, its value is added as a bonus to your spellcasting roll.",
                              { type: "number", placeholder: "1" }),
    ...
    silver:                wq("Silver",                  "The weapon's whole damage counts as silver — no separate roll. Silver-weak monsters take FULL damage; every other target (characters, non-silver-weak monsters) takes HALF (silver's poor offensive edge). Combine with Meteorite to deal full to those other targets too.",
                              null,
                              { damageFlags: { isSilver: true } }),
    slowReload:            wq("Slow Reload",             "Takes a 1-action reload between shots."),
    repeating:             wq("Repeating",               "Combat Extended magazine crossbows (magazine 2+) only: firing re-cocks the weapon from its magazine, so you keep firing until the magazine runs dry. Without it, a magazine crossbow must be re-cocked (its full reload-action cost) before every shot."),
    ...
    /* ── House-rule rune qualities ──────────────────────────────────────
     * Both intended as runestone enchantments and both STACK across runes
     * (the enhancement derivation sums numeric quality values): three
     * Adrenalized(1) runes read as Adrenalized(3). */
    adrenalized:           wq("Adrenalized",             "Whenever you would gain an Adrenaline point, gain the listed number of additional points. Stacks across runes.",
                              { type: "number", placeholder: "1" }),
    durable:               wq("Durable",                 "When this weapon would be ablated (lose Reliability) for any reason, roll 1d6; on a 4, 5, or 6 it is not ablated."),
    ...
    footCharging:          wq("Charging",                "When used to Charge, this weapon deals an extra damage die per 2m moved, up to the maximum shown. This is in addition to the extra damage gained from movement during mounted combat. (EO's redefined ‘Charging’ — distinct from the legacy Core entry kept at the `charging` key.)",
                              { type: "number", placeholder: "3" }, { chargeBonusPerMeter: 0.5, chargingMode: "onfoot" }),
    ...
    twoHand:               wq("Two-Hand",                "When used as a two-handed weapon instead of a one-handed weapon, this weapon gains the indicated benefits.",
                              { type: "text", placeholder: "+1 WA, +1d6 Dmg" }),
    strangling:            wq("Strangling",              "The weapon can be used to execute a choke hold, gaining the indicated benefits. Suffocation bonuses stack onto the per-turn choke damage: \"+N suffocation\" adds a flat amount; \"×N suffocation\" multiplies the whole per-turn total (e.g. a garrote → \"×3 suffocation\").",
                              { type: "text", placeholder: "+2 WA, ×3 suffocation" }),
...
export const ARMOR_QUALITIES = Object.freeze({
    ...
    poorVision:           wq("Poor Vision",           "You have a reduced field of vision and lose scent tracking abilities, and you only recover HALF your normal Stamina from in-combat recovery (Take Breath). With Poor Vision, you also take a −2 to Awareness and ranged attacks, and a −1 penalty to all other vision-dependent checks. On many helms you can open a visor or mask as an Action to negate this, but that removes any resistance to piercing attacks the helmet has and reduces SP by 5.",
                              null, { armorHalvesCombatStaRecovery: true }),
    /* `visor` — Combat Extended, helmets. Marks a helm whose visor/mask can be
     * raised or lowered (right-click → Raise/Lower Visor, one Action). While the
     * visor is RAISED the helm's Restricted / Poor Vision effects are negated —
     * the STA-recovery halving, the −2 Awareness/ranged penalty, and the 90°
     * token vision angle all lift until it's lowered again. State lives in
     * `system.visorRaised`; the toggle + negation are wired in
     * mechanics/helmetVision.mjs. No standalone effect without a vision quality. */
    visor:                wq("Visor",                 "Combat Extended (helmets): this helm's visor or face mask can be raised or lowered as an Action (right-click the armor). While raised it negates the helm's Restricted / Poor Vision effects — the Stamina-recovery penalty, the −2 to Awareness and ranged attacks, and the restricted field of vision — but you lose the helm's protection to the face while it's up (narrative)."),
    /* Spellcasting foci (shared with WEAPON_QUALITIES). Worn armor carrying
     * these lets a caster (whose profession Can Use Foci) cast through it:
     * Focus reduces STA cost by its value (min 1); Greater Focus adds its
     * value to the spellcasting roll. Both are numeric-param qualities and can
     * also be granted by glyph enhancements. */
    focus:                wq("Focus",                 "A spellcasting focus. Casting a spell through this worn gear reduces the spell's STA cost by its value (a spell never costs less than 1 STA). When you have more than one focus equipped you pick which to cast through.",
                              { type: "number", placeholder: "1" }),
    greaterFocus:         wq("Greater Focus",         "A greater spellcasting focus. When you cast a spell through this worn gear, its value is added as a bonus to your spellcasting roll.",
                              { type: "number", placeholder: "1" }),
    regenerating:         wq("Regenerating",          "Whenever you heal HP, restore the listed number of additional HP (capped at your maximum). Stacks across glyphs.",
                              { type: "number", placeholder: "1" }),
    warded:               wq("Warded",                "+2 to your defense roll when you defend against a spell (magic resistance).",
                              null, { spellDefenseBonus: 2 }),
    likeAShadow:          wq("Like a Shadow",         "+2 to Stealth checks while you are in Dim Light −2 or darker.",
                              null, { stealthDarkBonus: 2 }),
    unmoving:             wq("Unmoving",              "When an attacker strikes you and deals 0 damage, you may spend 5 STA to immediately Stagger them.",
                              null, { unmovingStagger: true }),
    quickStrike:          wq("Quick Strike",          "While worn, your fast-draw attack penalty is −1 instead of −3.",
                              null, { fastDrawPenaltyDelta: 2 }),
    versatile:            wq("Versatile",             "While worn, when your extra action is a different action than your previous one this turn, the extra-action penalty is −2 instead of −3.",
                              null, { versatileActionSwitch: true }),
...
/* Weapon qualities that only make sense on RANGED weapons (weaponType
 * "ranged"), so the picker hides them on melee/thrown weapons. `repeating`
 * governs magazine-crossbow re-cocking — meaningless on anything you don't
 * fire from a chamber. */
const RANGED_ONLY_QUALITY_KEYS = new Set([
    "repeating"
]);
export function isRangedOnlyQuality(key) {
    return RANGED_ONLY_QUALITY_KEYS.has(key);
}
...
export const HOMEBREW = Object.freeze({
    ...
    // kind:"houseRule" hides the toggle from both the inline settings list and
    // the Homebrew Content editor — it's surfaced only through the House Rules
    // configuration menu. The underlying homebrew.splitMovement setting still
    // backs isHomebrewEnabled("splitMovement"), so every callsite is unchanged.
    splitMovement:     homebrew(false, "houseRule"),
    ...
    // Same "houseRule" placement as splitMovement — toggle lives in the House
    // Rules menu; the underlying homebrew.rawToxicity setting is unchanged.
    rawToxicity:       homebrew(true, "houseRule"),
...
export const EFFECT_TRIGGERS = Object.freeze([
    ...
    // onceOnApply: fire the modify ONCE at effect apply — roll the value,
    // add it to the target's SOURCE (a current-pool bump the player can
    // spend). On effect removal the same amount is subtracted, clamped at
    // zero — so a "drink +1d6 STA" potion grants some current STA the
    // player CAN use up; when the potion ends, whatever's left (or 0) is
    // reclaimed. Doesn't compile to a native change; handled by the
    // one-shot bump hooks in policy/tick-effects.js.
    { value: "onceOnApply",    labelKey: "WITCHER.Effect.WhenOnceOnApply", mode: "oneshot" },
...
export const COMBAT_MOD_TARGETS = Object.freeze([
    ...
    { key: "system.combatMods.runStaReduction",              labelKey: "WITCHER.Effect.CmRunSta" },
 ...
 * refills it. Returns the new { value, temp } numbers to write. HP value is NOT
 * clamped at 0 — it goes negative in store, which the system relies on for
 * calculations (overkill depth, death thresholds, etc.). Dying is still decided
 * by healthState's `hp <= 0` test, so any negative reads as dying; the display
 * bars clamp to 0 visually (see sheets). Temp never goes below 0. */
export function drainHp(hp, amount) {
    ...
    return { temp: temp0 - fromTemp, value: val0 - (dmg - fromTemp) };
}
...
export function effectTargetGroups() {
    ...
    for (const s of STATS) {
        ...
        /* Per-stat cap — the RAW ceiling on the base value (default 10
         * for characters). AE-targeting this raises or lowers what a
         * player can allocate into that stat via the sheet's +1 button.
         * Character-only field (monsters have no cap). */
        stats.push({ key: `system.stats.${s}.cap`,      label: `${name} — ${L("WITCHER.Effect.StatCapSuffix")}` });
    }
    const skills = [];
...
export const EFFECT_ACTION_TYPES = Object.freeze([
    ...
    // Grant vision — night / improved-night / dark. Read in the vision helpers
    // (mechanics/light-level.mjs): waives light penalties at its tier, and Dark
    // Vision also drives the token's Foundry vision mode + sight range. Inert (no
    // native change) — the vision layer reads it off the actor's active effects.
    { type: "vision", labelKey: "WITCHER.Effect.ActionVision", kind: "vision" },
    ...
    // Adrenaline STA cost delta — carries a signed integer that adjusts the
    // STA cost per adrenaline die spent (negative = cheaper, positive =
    // more expensive). Summed across all effects on the actor.
    { type: "adrenalineStaCostDelta", labelKey: "WITCHER.Effect.ActionAdrCostDelta", kind: "marker" }
]);
...
/* Vision tiers a `vision` action can grant — mirrors the race nightVision tiers,
 * cumulative (each also waives the lighter ones). */
export const VISION_TYPES = Object.freeze([
    { value: "night",    labelKey: "WITCHER.Vision.Type.Night" },
    { value: "improved", labelKey: "WITCHER.Vision.Type.Improved" },
    { value: "dark",     labelKey: "WITCHER.Vision.Type.Dark" }
]);

export function visionTypeOptions() {
    const L = (k) => game.i18n.localize(k);
    return VISION_TYPES.map(o => ({ value: o.value, label: L(o.labelKey) }));
}
 ...
 *
 * `rolledValues` — sparse map of `{ [actionIndex]: number }` produced by
 * WitcherActiveEffect._preCreate for actions authored with dice-formula
 * `value`s. Foundry's change engine casts change.value via `Number(raw) || 0`
 * (see foundry/common/documents/active-effect.mjs #castDelta), so a raw
 * "1d6+2" would silently become 0 — we substitute the pre-rolled result. */
export function compileActionsToChanges(actions, rolledValues = null) {
    if (!Array.isArray(actions)) return [];
    const rolls = rolledValues && typeof rolledValues === "object" ? rolledValues : null;
    const out = [];
    for (let i = 0; i < actions.length; i++) {
        ...
        // Substitute the pre-rolled result for a dice-formula value. The cache
        // key is the action's index in the flag array; a numeric override is
        // possible but currently unused (the pre-roll only fires for strings).
        if (rolls && typeof value === "string" && Object.prototype.hasOwnProperty.call(rolls, i)) {
            const cached = Number(rolls[i]);
            if (Number.isFinite(cached)) value = cached;
        }
...
/* True if a modify-action value string is a dice formula (contains a "d"
 * flanked by digits) rather than a plain number. Used at effect-create time
 * to decide whether to pre-roll the value into rolledValues. Keeps the
 * detection cheap: `Roll.validate` is more permissive but also more expensive
 * and would false-accept pure math like "2+3". */
export function isActionValueFormula(value) {
    if (typeof value !== "string") return false;
    const trimmed = value.trim();
    if (!trimmed) return false;
    if (/^[+-]?\d+(?:\.\d+)?$/.test(trimmed)) return false;
    return /\d\s*d\s*\d/i.test(trimmed);
}
...
export function applyConditionActions(actor) {
    ...
    for (const e of actor.allApplicableEffects()) {
        ...
        const rolled = e.flags?.[SYSTEM_ID]?.rolledValues;
        for (let index = 0; index < actions.length; index++) {
            const a = normalizeAction(actions[index]);
            if (!a || a.type !== "modify" || a.when !== "condition" || !a.target) continue;
            if (!effectOperation(a.op)) continue;
            /* Same guard as applyEventLedger: a spendable pool-current value must
             * never be re-applied at prep. A continuous "while X, +N adrenaline/HP"
             * buff on one of these re-adds every render and leaks into source via
             * the next read-modify-write — the exact snowball / un-settable bug.
             * Pool grants must be one-time writes, not prep buffs. */
            if (isPoolCurrentTarget(a.target)) continue;
            if (!evaluateCondition(a.condition, actor)) continue;
            const cur  = Number(foundry.utils.getProperty(actor, a.target)) || 0;
            // A dice/formula value coerces to 0 through applyOperation's
            // Number() cast, so substitute the pre-rolled cached result (the
            // same rolledValues cache the "always" path uses — condition
            // actions are pre-rolled too; see shouldPreRollAction). A plain
            // numeric value passes straight through.
            let val = actionValue(a.value);
            if (typeof val === "string" && val.trim() !== "") {
                const cached = Number(rolled?.[index]);
                val = Number.isFinite(cached) ? cached : 0;
            }
            const next = applyOperation(cur, a.op, val);
...
/* Spendable POOL-CURRENT values — resources gameplay code reads, modifies, and
 * writes back to SOURCE (hp / sta / adrenaline / toxicity / luck spent-and-
 * gained in play; shield / vigor player-set). An EVENT-modifier that targets one
 * of these must be a ONE-TIME real write to source (see applyEventModify), NOT an
 * every-prep ledger re-application. Re-applying at prep is catastrophic for a
 * spendable pool: (a) it becomes un-settable — set it to 0 and prep re-adds N×fires
 * on the next render; and (b) it snowballs — the next read-modify-write reads the
 * inflated PREPARED value and leaks it into source, which prep then re-inflates.
 * `.max` fields are NOT here: they're derived caps, not read-modify-written pools. */
export const POOL_CURRENT_TARGETS = Object.freeze(new Set([
    "system.adrenaline.value",
    "system.derivedStats.hp.value",
    "system.derivedStats.sta.value",
    "system.stats.toxicity.value",
    "system.stats.luck.value",
    "system.derivedStats.shield",
    "system.derivedStats.vigor"
]));
export function isPoolCurrentTarget(target) {
    return POOL_CURRENT_TARGETS.has(String(target ?? ""));
}
...
export function applyEventLedger(actor) {
    ...
    for (const e of actor.allApplicableEffects()) {
        ...
        for (const [indexStr, entry] of Object.entries(ledger)) {
            ...
            /* Pool-current targets are written to SOURCE once by applyEventModify
             * (never banked here), so they must not be re-applied at prep — that
             * re-application is exactly the "tick it down and it pops back up /
             * snowballs" bug for adrenaline & the other spendable pools. */
            if (isPoolCurrentTarget(a.target)) continue;
            ...
            if (isActionValueFormula(a.value)) {
                // Dice value: the tick engine already rolled it once per firing
                // and banked the running total in entry.amount. Apply that
                // accumulated sum ONCE (correct for add/subtract; a per-fire
                // dice value under set/multiply is ill-defined by design).
                const amt = Number(entry?.amount) || 0;
                cur = applyOperation(cur, a.op, amt);
            } else {
...
/* Base magic-tagged skills — the four WILL-based caster skills whose
 * level-ups can be paid for out of Magic IP in the character chrome's
 * magic-mode banner. Profession skills opt in independently via the
 * `isMagical` per-slot flag on `ProfessionData`. Kept as an ordered
 * frozen array so callers can `.includes()` cheaply. */
export const MAGIC_SKILLS = Object.freeze([
    "spellcast", "hexweave", "resistmagic", "ritcraft"
]);
...
function mergeQualityCatalog(defaults, override) {
    ...
    for (const [key, defEntry] of Object.entries(defaults)) {
        ...
        merged[key] = ovrEntry
            ? {
                /* Spread the FULL default entry first so EVERY behavioral field
                 * survives — reach* (Long/Superior/Extreme Reach), parry/spell/
                 * reliability/charge bonuses, requiresMinPhysique, addsDamageTo-
                 * Unarmed, etc. The qualities editor only exposes label /
                 * description / param / damageFlags / rider, so a saved override
                 * (which freezes a full-catalog snapshot) would otherwise drop
                 * all the un-listed fields — that's what silently killed weapon
                 * Reach. Then overlay only the editor-exposed fields. */
                ...defEntry,
...
export function getActiveWeaponQualities() {
    try {
        ...
        return normalizeValuelessSilver(mergeQualityCatalog(WEAPON_QUALITIES, override));
    } catch (_) { /* setting not registered yet during init */ }
    return WEAPON_QUALITIES;
}

/* Silver is a valueless quality — the whole hit counts as silver, there is no
 * separate silver dice. A GM catalog override saved back when Silver still had
 * its `{ type:"text", placeholder:"2d6" }` parameter would otherwise resurrect
 * a value box in the qualities picker (mergeQualityCatalog keeps the override's
 * param). Strip it so Silver stays valueless everywhere regardless of a stale
 * stored override — no world-data migration needed. damageFlags/description are
 * left untouched. */
function normalizeValuelessSilver(catalog) {
    const s = catalog?.silver;
    if (s?.param) return { ...catalog, silver: { ...s, param: null } };
    return catalog;
}

/* Detect an EQUIPPED-ARMOR quality by its DISPLAY LABEL (case-insensitive),
 * matching either the built-in quality OR a GM-authored custom quality that
 * shares the name. Custom qualities get generic keys (newQuality1, …) that no
 * engine hook can predict, so the hardwired house qualities (Quick Strike,
 * Like a Shadow, Versatile, Warded, Unmoving) are matched on their label.
 * Armor only — never shields or weapons. */
export function equippedArmorHasQualityLabeled(actor, labelEn) {
    const want = String(labelEn ?? "").trim().toLowerCase();
    if (!want) return false;
    const cat = getActiveArmorQualities();
    for (const it of (actor?.items ?? [])) {
        ...
        for (const q of qs) {
            const e = cat?.[q];
            const l1 = String(e?.label   ?? "").trim().toLowerCase();
            const l2 = String(e?.labelEn ?? "").trim().toLowerCase();
            if (l1 === want || l2 === want || String(q).toLowerCase() === want) return true;
        }
    }
    return false;
}
