...
/* The EO build generators under tools/ ship only in the packaging repo,
 * not in this play checkout. Read them optionally so their absence skips
 * the two generator-icon tests instead of crashing the whole file. */
...
const catalogSrc= readOpt(join(repoRoot, "tools", "gen-eo-catalog.mjs"));
const compsSrc  = readOpt(join(repoRoot, "tools", "gen-eo-components.mjs"));
...
test("No EO pack source references a Foundry icon that doesn't ship", () => {
    ...
    /* packs-src/ (the pre-build EO pack JSON) ships only in the packaging
     * repo, not this play checkout — nothing to scan here. */
    try { statSync(join(repoRoot, "packs-src")); }
    catch { return; /* skip — no pack sources in this checkout */ }
...
test("Catalog generator uses only verified-shipping icon paths", { skip: catalogSrc === null && "tools/gen-eo-catalog.mjs not present in this checkout" }, () => {
...
test("Components generator uses a real bag icon, not material-bag-yellow", { skip: compsSrc === null && "tools/gen-eo-components.mjs not present in this checkout" }, () => {
