...
test("S8 — handleHoldApply requires OWNER on the holder ONLY (no target-permission check)", () => {
    /* The sender must OWN the holder (their own PC) — that authorizes the hold,
     * exactly like attacking an NPC you don't own. It must NOT also require
     * permission on the TARGET: NPCs default to NONE ownership for players, so a
     * target check refused every player grapple (and, since _doApplyHoldLink
     * stamps the status AND runs positioning, that looked like "statuses can't
     * apply / tokens can't move"). */
    const block = sockSrc.match(/async function handleHoldApply[\s\S]+?_doApplyHoldLink\(holder, target, payload\.kind\);/)?.[0] ?? "";
    assert.match(block, /authorizeSocket\(payload, holder, OWNER\)/);
    assert.doesNotMatch(block, /authorizeSocket\(payload, target\)/);
});
