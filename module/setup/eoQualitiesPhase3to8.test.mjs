...
test("Phase 3 — adjacent attacks with Extreme Reach refuse + abort", () => {
    assert.match(attackMixinSrc, /reachAdjacentNoAttack/);
    /* Warn is now i18n'd via tFormat; the fallback string still carries the
     * `${weapon} (Extreme Reach)` shape, and the branch returns null (abort). */
    assert.match(attackMixinSrc, /ui\.notifications\?\.warn\(tFormat\("WITCHER\.Doc\.WeaponAttackMixin\.Notify\.ExtremeReachAdjacent"/);
    assert.match(attackMixinSrc, /\$\{weapon\.name\} \(Extreme Reach\)/);
});
...
test("Phase 4 — Nimble STA reduction is calculated and applied at spend time", () => {
    assert.match(attackMixinSrc, /nimbleAttackStaReduction/);
    assert.match(attackMixinSrc, /drawStaReduction/);
    /* Reworked: no post-hoc refund (which clamped at max and could net STA
     * gain). The reduction is stashed on the actor as `_nimbleExtraStaReduction`
     * and subtracted from the extra-action cost at spend time. */
    assert.match(attackMixinSrc, /this\._nimbleExtraStaReduction = \(this\.nextActionSlot === "extra"\)/);
    assert.match(attackMixinSrc, /Math\.max\(nimbleAttackRed, isFastDrawAttack \? nimbleDrawRed : 0\)/);
    /* Consumed (subtracted then zeroed) in combatRoundMixin at spend time. */
    const combatRoundSrc = readFileSync(new URL("../documents/mixins/combatRoundMixin.mjs", import.meta.url), "utf8");
    assert.match(combatRoundSrc, /_nimbleExtraStaReduction/);
});
...
test("Phase 6 — Charge tail appended to damage formula + chip", () => {
    ...
    /* Chip label is now i18n'd via tFormat; the fallback keeps the
     * `Charge ({chargeBonusSrc})` shape. */
    assert.match(attackMixinSrc, /label:\s*tFormat\("WITCHER\.Doc\.WeaponAttackMixin\.Dialog\.Button\.ChargeX", \{ chargeBonusSrc: chargeBonusSrc \}, "Charge \(\{chargeBonusSrc\}\)"\)/);
});
