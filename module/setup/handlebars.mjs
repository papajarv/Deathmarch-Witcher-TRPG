...
export function registerHandlebars() {
    ...
    /* localizeFormat — localize a key with {placeholder} substitutions.
     * Usage: {{localizeFormat "WITCHER.Some.Key" name=actor.name count=5}}
     * Wraps game.i18n.format so template authors can inject dynamic data
     * without breaking the translation cycle. */
    Handlebars.registerHelper("localizeFormat", (key, options) => {
    ...
    /* `hasFlair value` — true only for a REAL per-item flair override: a valid
     * 6-digit hex that isn't one of the OFF sentinels. "#000000" is the old
     * ColorField null-coerced default; "#b0a894" is the flair-override picker's
     * off-state default (which older builds silently persisted on every save,
     * so treating it as OFF retro-clears every wrongly-flagged item without a
     * migration). Coerces Color objects / null to a string first.
     * Used by flair-override.hbs + the item display views. */
    if (!Handlebars.helpers.hasFlair) {
        Handlebars.registerHelper("hasFlair", (v) => {
            const s = String(v ?? "").trim().toLowerCase();
            return /^#[0-9a-f]{6}$/.test(s) && s !== "#000000" && s !== "#b0a894";
        });
    }
    ...
    foundry.applications.handlebars.loadTemplates([
        ...
        `systems/${SYSTEM_ID}/templates/item/book.hbs`,

        // Shared item-sheet partials
        `systems/${SYSTEM_ID}/templates/item/partials/source-field.hbs`,
        `systems/${SYSTEM_ID}/templates/item/partials/flair-override.hbs`,
        ...
        `systems/${SYSTEM_ID}/templates/applications/weather-config-light.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/weather-config-calendar.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/satiety-dialog.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/ui-customizer-head.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/ui-customizer-section.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/ui-customizer-inventory.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/ui-customizer-fonts.hbs`,
        `systems/${SYSTEM_ID}/templates/applications/ui-customizer-css.hbs`,
