...
test("socketHook.runDefenseChoice threads attackerDamageFlags into defendWith/defendBySkill", () => {
    ...
    /* Signature has since grown a trailing `attackerUuid = ""` param, so
     * anchor on both required params surviving rather than the closing paren. */
    assert.match(socketSrc, /async function runDefenseChoice\([\s\S]+attackerDamageFlags\s*=\s*null[\s\S]*attackHitLocation\s*=\s*null/);
    /* Both options objects now also carry attackKind / attackerUuid, so
     * attackerDamageFlags is no longer the last key — just assert it's threaded. */
    assert.match(socketSrc, /defendWith\([^)]+attackerDamageFlags[^)]*\}\)/);
    assert.match(socketSrc, /defendBySkill\([^)]+attackerDamageFlags[^)]*\}\)/);
});
