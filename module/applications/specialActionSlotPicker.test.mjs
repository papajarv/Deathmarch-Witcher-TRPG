...
test("Picker source advertises the three slot kinds + extra-action STA surcharge", () => {
    ...
    // Extra-action button label includes the STA cost: the base cost is now
    // the homebrew-configurable hrExtraActionStaCost(), reduced by
    // combatMods.extraActionStaReduction (floored at 0).
    assert.match(src, /extraStaCost\s*=\s*Math\.max\(0,\s*hrExtraActionStaCost\(\)\s*-\s*\(Number\(actor\.system\?\.combatMods\?\.extraActionStaReduction\)\s*\|\|\s*0\)\)/);
    // Label is localized via tFormat with an {extraStaCost} token; fallback
    // string still spells out "Extra Action ({extraStaCost} STA)".
