...
import { isTokenAgainstWall } from "../mechanics/pushToken.mjs";
import { weaponUsesBrawlingSkill } from "../mechanics/grappleWeapon.mjs";
import { WEAPON_QUALITIES as WQ_CATALOG, equippedArmorHasQualityLabeled } from "../setup/config.mjs";
...
/* ── Auto-checked situational bonuses (the player can still uncheck) ──
 * These pre-tick attack-card toggles when the tactical situation obviously
 * warrants them, so a flanking / sneaking attacker doesn't have to remember
 * to tick the bonus every time. Both are best-effort: any failure returns
 * false so we NEVER wrongly pre-check a bonus. */

/** True when the attacker sits OUTSIDE the target token's vision cone.
 *  Mirrors the stealth spotting model (allowed vision angle + walls +
 *  scene levels). A target with full/unclipped 360° vision sees everywhere
 *  and is therefore never "out of sight". */
async function attackerOutOfTargetSight(aTok, dTok) {
    try {
        if (!aTok || !dTok || aTok === dTok) return false;
        if (canvas?.scene?.tokenVision === false) return false;   // vision off → don't guess
        const { getAllowedVisionAngle } = await import("../mechanics/stealth-hooks.mjs");
        const { buildSightPolygon, getTokenLevel, canLevelSee, externalRadiusOf } = await import("../mechanics/stealth-los.mjs");
        const angle = Number(getAllowedVisionAngle(dTok)) || 360;
        if (angle >= 360) return false;                            // full vision → always seen
        const dLevel = getTokenLevel(dTok);
        const aLevel = getTokenLevel(aTok);
        if (dLevel && aLevel && dLevel.id !== aLevel.id && !canLevelSee(dLevel, aLevel)) return true;
        const radius = canvas?.dimensions?.maxR ?? 100000;
        const poly = buildSightPolygon(
            { x: dTok.center?.x ?? 0, y: dTok.center?.y ?? 0, elevation: Number(dTok.document?.elevation) || 0 },
            dLevel,
            { angle, rotation: Number(dTok.document?.rotation) || 0, radius,
              externalRadius: externalRadiusOf(dTok) }
        );
        if (!poly) return false;
        // "In sight" if the attacker's center OR any bounding-box corner is
        // inside the cone (matches stealth's spotterCanSee corner test).
        const cx = aTok.center?.x ?? 0, cy = aTok.center?.y ?? 0;
        const hw = ((Number(aTok.w) || 0) / 2) - 2, hh = ((Number(aTok.h) || 0) / 2) - 2;
        const inSight = poly.contains(cx, cy)
            || poly.contains(cx - hw, cy - hh) || poly.contains(cx + hw, cy - hh)
            || poly.contains(cx + hw, cy + hh) || poly.contains(cx - hw, cy + hh);
        return !inSight;
    } catch (_) { return false; }
}

/** True when the attacker is stealthed and the target has NOT yet spotted
 *  them — i.e. a genuine ambush from concealment. */
async function attackerHasAmbush(attackerActor, targetActor) {
    try {
        if (!attackerActor || !targetActor) return false;
        const { isStealthed, isSpottedBy } = await import("../mechanics/stealth.mjs");
        return isStealthed(attackerActor) && !isSpottedBy(attackerActor, targetActor);
    } catch (_) { return false; }
}
...
import { getActiveWeatherModifiers, getEnvironmentalModifiersForActor } from "../mechanics/weather-modifiers.mjs";
...
import { hrBowsCannotStrongStrike, hrOffhandPenalty, hrExtraActionToHit, hrExtraActionStaCost,
         hrStrongStrikePenalty } from "../mechanics/house-rules-config.mjs";
 ...
 *  (RAW default). A Joint Attack already carries its own -3
 *  (STRIKE_TYPES.joint.toHit), so this is only added to non-joint single
 *  attacks. This is the SHIPPED default; the live value used by the
 *  attack roll comes from `hrOffhandPenalty()` so a GM can tune it via
...
/** The live ranged weather penalty for `actor` as { total, parts:[{label,value}] }.
 *  Read via getEnvironmentalModifiersForActor so it's SHELTER-GATED — an attacker
 *  whose token is inside a suppress-weather region (or an indoor/off scene) takes
 *  no weather ranged penalty. */
function weatherRangedPenalty(actor) {
    let recs = [];
    try { recs = getEnvironmentalModifiersForActor(actor, "ranged") ?? []; } catch (_) { recs = []; }
    const parts = recs
...
/** The live light-level ATTACK penalty (Darkness −2) read from the ATTACKER's own
 *  tile, as { total, parts:[{label,value}] }. Applies to melee and ranged alike
 *  (unlike the ranged-only weather penalty); already waived by the attacker's
 *  Dark Vision, since the records are token-scoped. */
function lightAttackPenalty(actor) {
    let mods = [];
    const token = actor?.getActiveTokens?.()?.[0] ?? null;
    try { mods = getActiveWeatherModifiers(undefined, token) ?? []; } catch (_) { mods = []; }
    const parts = mods
        .filter(m => m.target === "attack")
        .map(m => ({ label: L(m.label), value: Number(m.value) || 0 }))
        .filter(p => p.value);
    return { total: parts.reduce((s, p) => s + p.value, 0), parts };
}
...
/* Quick Strike (armor quality) — shaves 2 off the wearer's fast-draw penalty
 * (−3 → −1). Matched by label so it works with a custom OR built-in quality. */
function armorFastDrawDelta(actor) {
    return equippedArmorHasQualityLabeled(actor, "Quick Strike") ? 2 : 0;
}

/* Versatile (armor quality) — when the extra action is a DIFFERENT action than
 * the previous one this turn, the extra-action penalty is −2 instead of −3
 * (returns 1 = "shave 1 off"). Compares the last recorded action label
 * (system.combatRound.actionLabel — includes strike type + weapon/cast/etc.)
 * with this attack's label, so normal ≠ strong ≠ fast, and attack ≠ spell ≠
 * ranged all count as different. */
function versatileExtraReduction(actor, weapon, strikeKey) {
    if (!equippedArmorHasQualityLabeled(actor, "Versatile")) return 0;
    const last = String(actor?.system?.combatRound?.actionLabel ?? "").trim().toLowerCase();
    if (!last) return 0;                                            // nothing prior to differ from
    const cur = `attack: ${String(weapon?.name ?? "").trim().toLowerCase()} · ${String(strikeKey ?? "normal").toLowerCase()}`;
    return last !== cur ? 1 : 0;
}
...
function buildContent(ctx) {
    const { weapon, offhandChoices, aimMod, weather, lightAtk, baseRange, fastDraw, forcedExtra, usesAmmo, ammoChoices, selectedAmmoId, aimRank, adrenaline, canFullRound } = ctx;
    ...
    const strikeOption = ([key, s]) => {
        const bits = [];
        /* Per-strike to-hit penalty deliberately OMITTED from the
         * option label — the full modifier breakdown (strike penalty,
         * offhand, extra action, etc.) already renders at the bottom
         * of the dialog and appears live in the roll preview, so
         * duplicating "-3" in the picker just clutters the label. */
        ...
        let blocked = s.fullRound && !canFullRound;
        if (blocked) {
            bits.push(L("WITCHER.Attack.NeedsFullRound"));
        } else {
            /* Grapple-action contextual availability — MIRROR the brawl dialog:
             * show the action greyed WITH A REASON rather than hiding it, so the
             * player sees WHY it's unavailable on the grappling weapon. */
            const reason = _meleeGrappleGrey(key, s);
            if (reason) {
                blocked = true;
                bits.push(reason);
            } else if (key === "pin" && _revTargetActor
                       && !_revTargetActor.statuses?.has?.("prone")
                       && isTokenAgainstWall(_revTargetActor.getActiveTokens?.()?.[0])) {
                bits.push("Adjacent Wall");   // hint — Pin allowed because the foe's at a wall
            }
        }
    ...
    /* Reverse Grapple targets the SPECIFIC foe grappling you. `getHoldsSync` is
     * warm for the GM and for players once the holds-pip refresh has run. */
    const _revActorUuid = normalizedActorUuid(ctx.actor);
    /* Target comes from the tile-targeting overlay that opened this attack
     * (weaponAttack → openAttackDialog with `targetActor: forceDefender`).
     * That's the ONLY target source in this system — no reticle / T-targeting. */
    const _revTargetActor = ctx.targetActor ?? null;
    const _revTargetUuid = _revTargetActor ? normalizedActorUuid(_revTargetActor) : null;
    /* Grapple prereqs read the hold REGISTRY (normalized world uuids), NOT
     * ctx.actor.statuses: an unlinked token's "grappled"/"pinned" status lives
     * on its synthetic token actor, so a status read is a false negative when
     * ctx.actor is the world actor. The registry is representation-proof — this
     * is what makes Reverse Grapple actually appear on a Grappling weapon. */
    const _myHolds = _revActorUuid ? getHoldsSync(_revActorUuid) : [];
    /* Am I the TARGET (grapplee) of a hold of `kinds`? — used by Reverse. */
    const _heldBy = (kinds) => _myHolds.some(p =>
        kinds.includes(p.kind) && p.targetUuid === _revActorUuid);
    /* Am I the HOLDER of a hold of `kinds` on the CURRENTLY-TARGETED foe?
     * The grapple-chain follow-ups (pin/choke/throw/ride/disarm) are the
     * GRAPPLER's tools — only the holder, and only against the specific foe
     * they're holding, may perform them. The grapplee gets Escape / Reverse /
     * normal attacks instead. Reversing does NOT qualify a grapplee here. */
    const _holdingTargetIn = (kinds) => !!(_revActorUuid && _revTargetUuid) &&
        _myHolds.some(p => kinds.includes(p.kind)
            && p.holderUuid === _revActorUuid && p.targetUuid === _revTargetUuid);
    const _revTargetIsGrappler = !!(_revActorUuid && _revTargetUuid) &&
        _myHolds.some(p => p.kind === "grappled"
            && p.targetUuid === _revActorUuid && p.holderUuid === _revTargetUuid);
    /* You can't initiate a NEW grapple while already holding one (or a pin) —
     * a holder is committed to their current hold. Follow-ups (pin/choke/…)
     * are separately gated to the foe you actually hold via _holdingTargetIn. */
    const _alreadyHolding = _myHolds.some(p =>
        (p.kind === "grappled" || p.kind === "pinned") && p.holderUuid === _revActorUuid);
    /* Pin exclusivity: only one foe pinned at a time — hide Pin while pinning. */
    const _isPinner = _myHolds.some(p => p.kind === "pinned" && p.holderUuid === _revActorUuid);
    const passesPrereq = (s) => {
        const need = s?.prereq;
        if (!need) return true;
        /* Follow-ups require HOLDING the target (grappler-only), incl. after
         * escalation (pinned/chokeheld) so a pin→choke chain still qualifies. */
        if (need === "grappling") return _holdingTargetIn(["grappled", "pinned", "chokeheld"]);
        if (need === "anyHold")   return _holdingTargetIn(["grappled", "pinned", "chokeheld"]);
        /* Choke (prereq "pinning") — you must be the PINNER of the selected foe. */
        if (need === "pinning")   return _holdingTargetIn(["pinned"]);
        /* Reverse Grapple — the grapplee reverses their grappler. Available
         * while grappled OR pinned (a pin reverses the whole stack — the
         * grapple layer persists under a pin, so `_revTargetIsGrappler` covers
         * it). ONLY a chokehold blocks it (Escape the choke first). Gated to a
         * Grappling weapon via passesQuality, and to the foe holding you as the
         * selected target. reverseHold re-verifies the target at execution. */
        if (need === "grappledSelfOnly") {
            return _revTargetIsGrappler && !_heldBy(["chokeheld"]);
        }
        /* Either-role grapple (Takedown): in a grapple/pin/choke with the
         * SELECTED foe, as holder OR grapplee. */
        if (need === "inGrappleWith") {
            return !!(_revActorUuid && _revTargetUuid) && _myHolds.some(p =>
                (p.kind === "grappled" || p.kind === "pinned" || p.kind === "chokeheld")
                && ((p.holderUuid === _revActorUuid && p.targetUuid === _revTargetUuid)
                 || (p.targetUuid === _revActorUuid && p.holderUuid === _revTargetUuid)));
        }
        return true;
    };
    /* Contextual grey-out reason for a grapple action on the grappling weapon —
     * mirrors the brawl dialog's tails so the player sees WHY it's greyed rather
     * than the action silently vanishing. Returns null when the action is
     * available. (Structural gates — wrong quality, already pinning, grappling a
     * new foe — still HIDE the option in the filter below.) */
    /* Is the selected foe NOT adjacent (more than a tile away, Chebyshev with a
     * half-tile grace)? Theatre-of-mind (a side without a canvas token) returns
     * false — no measurable distance, trust the GM. Mirrors the weaponAttack
     * adjacency gates so the melee tree greys what the after-click gate refuses. */
    const _targetNotAdjacent = (target) => {
        const aTok = ctx.actor?.getActiveTokens?.()?.[0];
        const tTok = target?.getActiveTokens?.()?.[0];
        const gs   = Number(canvas?.scene?.grid?.size) || 0;
        if (!aTok || !tTok || gs <= 0) return false;
        const ac = { x: aTok.center?.x ?? aTok.x, y: aTok.center?.y ?? aTok.y };
        const tc = { x: tTok.center?.x ?? tTok.x, y: tTok.center?.y ?? tTok.y };
        return Math.max(Math.abs(ac.x - tc.x), Math.abs(ac.y - tc.y)) / gs > 1.5;
    };
    /* Grapple finishers that need the foe ADJACENT — a Grappling weapon with
     * reach may still Grapple / Trip / Drag at range, but NOT these. Shield
     * Bash rides along (Core p.164, strictly close-quarters). The grapple-THROW
     * is here; a hurled-weapon "throw" (s.thrown) is exempt below. */
    const _MELEE_CLOSE_ONLY = new Set([
        "pin", "choke", "chokehold", "throw", "disarm", "push", "slam", "takedown", "reverseGrapple", "bash"
    ]);
    const _meleeGrappleGrey = (key, s) => {
        if ((s.prereq === "grappling" || s.prereq === "anyHold") && !passesPrereq(s)) return "no grapple";
        if (s.prereq === "inGrappleWith" && !passesPrereq(s)) return "not grappling";
        if (s.prereq === "pinning" && !passesPrereq(s)) return "needs pinned target";
        /* Drag — a pin locks both parties in place; you can't drag while pinned. */
        if (key === "drag" && _myHolds.some(p => p.kind === "pinned"
            && (p.holderUuid === _revActorUuid || p.targetUuid === _revActorUuid))) return "pinned";
        if (s.prereq === "grappledSelfOnly" && !passesPrereq(s)) return _heldBy(["chokeheld"]) ? "choke first" : "not grappled";
        if (key === "slam" && ctx.actor?.statuses?.has?.("prone")) return "prone";
        if (key === "takedown") {
            if (ctx.actor?.statuses?.has?.("prone")) return "prone";
            if (_revTargetActor?.statuses?.has?.("prone")) return "target prone";
        }
        /* Trip — only the target goes prone, so no self check; can't trip a
         * foe already down. (Grappler-only greying comes from its `grappling`
         * prereq above.) */
        if (key === "trip" && _revTargetActor?.statuses?.has?.("prone")) return "target prone";
        if (key === "push" && _revTargetActor?.statuses?.has?.("prone")) return "target prone";
        if (key === "pin" && _revTargetActor
            && !(_revTargetActor.statuses?.has?.("prone")
                 || isTokenAgainstWall(_revTargetActor.getActiveTokens?.()?.[0]))) return "need prone/wall";
        /* Adjacency — the close-quarters finishers can only strike an adjacent
         * foe; a hurled-weapon throw (s.thrown) is a ranged attack and exempt.
         * Grapple / Trip / Drag are absent from _MELEE_CLOSE_ONLY, so a reach
         * grappling weapon can still perform them at distance. */
        if (_MELEE_CLOSE_ONLY.has(key) && !s?.thrown && _revTargetActor && _targetNotAdjacent(_revTargetActor)) return "too far";
        return null;
    };
    ...
    /* Some CE actions require the weapon to carry a specific quality — e.g.
     * Grapple only appears on a weapon with the Grappling quality (you can't
     * grapple with a plain sword; that's unarmed Brawling). Actions with no
     * `requiresQuality` pass through. */
    const passesQuality = (s) => {
        if (!s?.requiresQuality) return true;
        const need = String(s.requiresQuality).toLowerCase();
        /* Case-insensitive so a weapon storing the label ("Grappling") rather
         * than the key ("grappling") still satisfies the gate — mirrors
         * findGrappleWeapon's matching for the defense side. */
        if (weaponQualities.some(q => String(q).toLowerCase() === need)) return true;
        /* A Brawling-skill weapon can perform every grapple action unarmed
         * Brawling can — so it satisfies the grappling/strangling gate without
         * carrying the quality (the *** brawling-weapon rule). */
        if ((need === "grappling" || need === "strangling") && weaponUsesBrawlingSkill(weapon)) return true;
        return false;
    };
    ...
    /* Rely on the resolved `ctx.mode` rather than the raw quick slot — a CE
     * melee weapon in the quick slot resolves to melee mode and must NOT be
     * forced to the Throw strike (throw-only quick weapons already resolve to
     * `mode: "thrown"`, so they're still caught here). */
    const isThrownMode = canThrow && ctx.mode === "thrown";
    ...
    /* Tackling status (dock's Full Round → Tackle) — like Charging, locks the
     * melee picker to Tackle only. weaponAttack resolves the opposed-Physique
     * smash and strips the status. */
    const _isTackling = !!ctx.actor?.statuses?.has?.("tackling");
    ...
    if (isThrownMode) {
        ...
        basicOpts = entries
            .filter(([key]) => key === "tackle")
            .map(strikeOption).join("");
        if (basicOpts) defaultStrike = "tackle";
    } else {
        /* House rule: "Bows can't Strong Strike" — filter out any strike
         * whose category resolves to `strong` when this weapon is a bow
         * (ranged + arrow ammo, matching the same test in
         * allowsStrikeVariants). Off by default; strict-RAW behavior
         * still permits bows to Strong Strike. */
        const _bowNoStrong = hrBowsCannotStrongStrike()
            && weapon?.system?.weaponType === "ranged"
            && weapon?.system?.ammoType   === "arrow";
        basicOpts = entries
            ...
            .filter(([key, s]) => !s.meleeOnly && (variants || cat(key) === "normal") && passesPrereq(s) && passesQuality(s) && passesHefty(key) && key !== "charge"
                && !(_bowNoStrong && cat(key) === "strong"))
            .map(strikeOption).join("");
    }
    ...
    /* Charging / Tackling lock us to a single strike — no special attacks either. */
    const specialOpts = (melee && !isThrownMode && !_isCharging && !_isTackling)
        ? entries.filter(([key, s]) =>
                ...
                && passesQuality(s)
                && passesHefty(key)
                /* NOTE: the CONTEXTUAL gates (prereq not met, prone attacker /
                 * target, pin needs prone/wall) are no longer filtered here —
                 * they GREY the option with a reason in strikeOption instead, to
                 * mirror the brawl dialog. Only STRUCTURAL gates hide below. */
                /* Can't grapple the foe already grappling you (Reverse is that
                 * path), and can't grapple a SECOND foe while already holding
                 * one — a grappler's grappling actions stay on their grapplee. */
                && !(key === "grapple" && (_revTargetIsGrappler || _alreadyHolding))
                /* Pin exclusivity — hide Pin while you already pin someone. */
                && !(key === "pin" && _isPinner)
                /* Choke is applied ONCE — hide it entirely while you already
                 * choke the selected foe (it's maintained via the turn prompt). */
                && !(key === "chokehold" && _holdingTargetIn(["chokeheld"]))
                ...
                && key !== "throw"
                /* Charge + Tackle live in the Full Round menu — see filter
                 * comment on basicOpts above. Exclude here too so they don't
                 * appear in the Special Attacks optgroup; each shows only while
                 * its arming status (charging / tackling) is up. */
                && key !== "charge" && key !== "tackle")
            .map(strikeOption).join("")
        : "";
     ...
     *
     * Multi-swing strikes:
     *   Fast   → 2 swings, same weapon → shot-2 picker mirrors shot 1
     *   Joint  → 2 swings, off-hand for shot 2 → shot-2 picker is
     *            populated live in refresh() from the CURRENT off-hand
     *            selection's own damage-type list
     * Every other strike renders shot-1 only (or nothing if the weapon
     * is single-type). The shot-2 shell always ships in the HTML but
     * starts hidden; refresh() reveals + populates it when needed. */
    const weaponDamageTypes = (weapon?.system?.effective?.damageTypes ?? weapon?.system?.damageTypes ?? [])
        .filter(t => typeof t === "string" && t.length > 0);
    const buildDmgTypeRadios = (types, groupName) => types.map((t, i) => `
        <label class="wdm-atk-check">
            <input type="radio" name="${groupName}" value="${esc(t)}"${i === 0 ? " checked" : ""} />
            <span>${esc(L(DAMAGE_TYPES[t] ?? t))}</span>
        </label>`).join("");
    const showShot1 = weaponDamageTypes.length >= 2;
    /* Render the whole shell whenever the main weapon isn't ammo-based
     * — refresh() manages inner visibility (block hidden if BOTH shot
     * 1 and shot 2 are hidden; shown if either has content). The
     * shot-2 radios are populated per render pass from either the main
     * weapon (Fast) or the current off-hand selection (Joint). */
    const damageTypeBlock = !weapon?.system?.requiresAmmo ? `
        <div class="wdm-atk-dmgtype-block" data-dmgtype-block${showShot1 ? "" : ' style="display:none;"'}>
            <div class="wdm-atk-dmgtype-shot1" data-dmgtype-shot1${showShot1 ? "" : ' style="display:none;"'}>
                <div class="wdm-atk-section-label" data-dmgtype-shot1-label>${esc(L("WITCHER.Attack.DamageTypes"))}</div>
                <div class="wdm-atk-checks">${showShot1 ? buildDmgTypeRadios(weaponDamageTypes, "dmgtype-1") : ""}</div>
            </div>
            <div class="wdm-atk-dmgtype-shot2" data-dmgtype-shot2 style="display:none;">
                <div class="wdm-atk-section-label">${esc(L("WITCHER.Attack.DamageTypeShot2"))}</div>
                <div class="wdm-atk-checks" data-dmgtype-shot2-radios></div>
            </div>
        </div>` : "";
    ...
    // Some start pre-checked when the situation warrants (autoCheckSit) — the
    // player can still untick them before rolling.
    const autoSit = Array.isArray(ctx.autoCheckSit) ? ctx.autoCheckSit : [];
    const sitRows = ATTACK_MODIFIERS
        .filter(m => ranged || !m.rangedOnly)
        .map(m => {
            const v = (ranged && m.rangedMod != null) ? m.rangedMod : m.mod;
            const chk = autoSit.includes(m.value) ? " checked" : "";
            return `<label class="wdm-atk-check">
                <input type="checkbox" name="sit" value="${m.value}" data-mod="${v}"${chk} />
    ...
    const autoBracketKey = (Number.isFinite(targetDistanceM) && Number.isFinite(baseRange) && baseRange > 0)
        ...
        : "close";
    /* Point Blank modifier is weapon-type-dependent — must match the
     * resolver in the reader() below. Compute once and use it to
     * relabel the pointBlank option in the dropdown so the shown
     * modifier matches the applied one.
     *   Crossbow → +5   Bow / Thrown → −5   otherwise → RANGE_BRACKETS default */
    const _pbSkill    = String(ctx.weapon?.system?.skillKey ?? "").toLowerCase();
    const _isThrownPB = isThrownMode || _pbSkill === "athletics";
    const _defaultPB  = RANGE_BRACKETS.find(r => r.value === "pointBlank")?.mod ?? 5;
    const pointBlankModForWeapon = _pbSkill === "crossbow"
        ? 5
        : (_pbSkill === "archery" || _isThrownPB) ? -5 : _defaultPB;
        ...
        <div class="wdm-atk-field">
            ...
            <select name="range">
                ${RANGE_BRACKETS.map(r => {
                    ...
                    const effMod  = r.value === "pointBlank" ? pointBlankModForWeapon : r.mod;
                    const modTxt  = effMod ? ` (${signed(effMod)})` : "";
    ...
    const extraActionBlock = forcedExtra ? `
        <div class="wdm-atk-weather wdm-atk-extra">
            ...
            <span class="wdm-atk-weather-v">${signed(Math.min(0, hrExtraActionToHit() + (Number(cm.extraActionPenaltyReduction) || 0)))}, ${Math.max(0, hrExtraActionStaCost() - (Number(cm.extraActionStaReduction) || 0))} STA</span>
        </div>` : "";
    ...
    const fastDrawBlock = fastDraw ? `
        <div class="wdm-atk-weather wdm-atk-fastdraw">
            ...
            <span class="wdm-atk-weather-v">${signed(Math.min(0, fastDraw + (Number(cm.fastDrawPenaltyReduction) || 0) + armorFastDrawDelta(ctx.actor)))}</span>
        </div>` : "";
    ...
    const lightBlock = (lightAtk && lightAtk.parts.length) ? `
        <div class="wdm-atk-weather">
            <span class="wdm-atk-weather-k"><i class="fa-solid fa-moon"></i> ${esc(L("WITCHER.Attack.Light"))}</span>
            <span class="wdm-atk-weather-v">${signed(lightAtk.total)}</span>
            <div class="wdm-atk-weather-parts">${
                lightAtk.parts.map(p => `${esc(p.label)} ${signed(p.value)}`).join(" · ")
            }</div>
        </div>` : "";
...
function collect(root, ctx) {
    const { aimMod, weather, lightAtk, fastDraw, forcedExtra } = ctx;
    ...
    /* Weapon-type override for Point Blank (RAW Core p.165):
     *   Crossbows keep +5 — mechanical trigger, easy to aim at contact.
     *   Bows      get −5 — long draw + tangling on the target's own body.
     *   Thrown    get −5 — no swing arc to aim, target on top of you.
     * Applied only at the pointBlank band; other bands keep RANGE_BRACKETS.
     *
     * Thrown detection matches isThrownMode above: either the mode
     * is explicitly "thrown" (dual-mode toggle), the weapon uses
     * Athletics natively (RAW thrown-only weapons), or a melee
     * weapon with a range is being hurled from the quick slot. */
    let rangeMod = inanimate ? 0 : rangeDef.mod;
    if (!inanimate && rangeKey === "pointBlank") {
        const skill    = String(ctx.weapon?.system?.skillKey ?? "").toLowerCase();
        const modeStr  = String(ctx.mode ?? "").toLowerCase();
        const isThrown = modeStr === "thrown" || skill === "athletics";
        if (skill === "crossbow")                 rangeMod =  5;
        else if (skill === "archery" || isThrown) rangeMod = -5;
    }
    const range = { key: rangeKey, mod: rangeMod, label: L(rangeDef.labelKey) };
    ...
    /* Charge collapses movement + a forced strong strike into a single
     * strike type in this codebase. RAW: charge is a movement action
     * that forces the next attack to be a strong strike. Practically
     * that means the same to-hit hierarchy as a plain strong strike —
     * base from hrStrongStrikePenalty(), per-actor reduction from
     * combatMods.strongStrikePenaltyReduction. The legacy
     * `chargePenaltyReduction` field is retained on the data model and
     * in the AE-editor target list for backwards compatibility, but
     * it's no longer read here — migrate items granting it to
     * `strongStrikePenaltyReduction` instead. */
    const strikeRed = (strikeCat === "strong" || strikeCat === "charge") ? cm.strongStrikePenaltyReduction
                    : strikeCat === "joint"  ? cm.offhandPenaltyReduction
                    : 0;

    /* Strong-strike penalty and off-hand penalty both read live from
     * the House Rules panel so a GM can retune the base value; the
     * per-actor `strongStrikePenaltyReduction` / `offhandPenaltyReduction`
     * AE fields still fold on top via the `reduce()` helper. Same for
     * the Extra Action to-hit penalty. Charge falls under the strong
     * branch — the codebase's `charge` strike type is the "strong strike
     * that a charge forces you to make" in RAW terms. */
    if (strike.toHit) {
        const base = (strikeCat === "strong" || strikeCat === "charge") ? hrStrongStrikePenalty() : strike.toHit;
        add(L(strike.labelKey), reduce(base, strikeRed));
    }
    // Off-hand weapon attack: base -3 (a joint strike already includes its own -3).
    if (ctx.mainIsOffhand && !strike.offhand) add(L("WITCHER.Attack.OffHand"), reduce(hrOffhandPenalty(), cm.offhandPenaltyReduction));
    if (extraAction)  add(L("WITCHER.Attack.ExtraAction"), reduce(hrExtraActionToHit(), (Number(cm.extraActionPenaltyReduction) || 0) + versatileExtraReduction(ctx.actor, ctx.weapon, strikeKey)));
    ...
    /* CE: throwing a weapon that isn't designed for it takes a −4
     * penalty. The `throwing` quality is the "this weapon is
     * throwable-by-design" marker; weapons without it that get
     * hurled anyway (any melee weapon with a range under CE's
     * every-weapon-throwable rule) eat the penalty. Native thrown
     * weapons (skillKey === "athletics") — throwing knives, spears
     * — are exempt: they ARE throwing weapons by definition.
     *
     * `ceOn` + thrown-mode detection are recomputed here rather
     * than reused from `buildContent` — that scope is a separate
     * function, its locals aren't visible in `collect`. Guarded
     * against settings-not-ready races the same way. */
    let _ceOn = false;
    try { _ceOn = isCombatExtendedEnabled(); } catch (_) { /* soft */ }
    const _pbSkillCollect = String(ctx.weapon?.system?.skillKey ?? "").toLowerCase();
    const _modeStrCollect = String(ctx.mode ?? "").toLowerCase();
    const _isThrownCollect = _modeStrCollect === "thrown" || _pbSkillCollect === "athletics";
    if (_ceOn && _isThrownCollect) {
        const qs           = ctx.weapon?.system?.qualities ?? [];
        const hasThrowing  = Array.isArray(qs) && qs.includes("throwing");
        const nativeThrown = _pbSkillCollect === "athletics";
        if (!hasThrowing && !nativeThrown) {
            add(L("WITCHER.Attack.NotThrowable"), -4);
        }
    }
    if (fastDraw) add(L("WITCHER.Attack.FastDraw"), reduce(fastDraw, (Number(cm.fastDrawPenaltyReduction) || 0) + armorFastDrawDelta(ctx.actor)));
    if (ranged && weather.total) add(L("WITCHER.Attack.Weather"), weather.total);
    // CE Poor-Vision helmet (visor down): −2 to ranged attacks. Its own line so
    // it's visible in the breakdown and folded into modTotal → the roll.
    if (ranged && actorPoorVisionActive(ctx.actor)) add(L("WITCHER.Armor.PoorVisionPenalty"), POOR_VISION_PENALTY);
    if (lightAtk?.total) add(L("WITCHER.Attack.Light"), lightAtk.total);   // Darkness −2 from the attacker's tile
    ...
    /* Damage-type picker: one radio group per swing.
     *   dmgtype-1 → main weapon's damage type for shot 1
     *   dmgtype-2 → same weapon (Fast) or off-hand (Joint) for shot 2
     * Null entries mean "no override on this shot" (single-type weapon
     * or no picker rendered) → mixin falls back to the shot weapon's
     * full damage-type list. The flat `damageTypes` field is retained
     * for backward compat with any caller that still reads it — set
     * from shot 1's pick. */
    const pickedShot1 = root.querySelector('input[name="dmgtype-1"]:checked')?.value || null;
    const pickedShot2 = root.querySelector('input[name="dmgtype-2"]:checked')?.value || null;
    const damageTypesOverride    = pickedShot1 ? [pickedShot1] : null;
    const damageTypesByShotArray = [
        pickedShot1 ? [pickedShot1] : null,
        pickedShot2 ? [pickedShot2] : null
    ];
    ...
    return {
        ...
        damageTypesByShot: damageTypesByShotArray,
...
function refresh(root, ctx) {
    ...
    if (adrInput) {
        ...
        if (ro) ro.textContent = v > 0
            ? L("WITCHER.Attack.AdrenalineReadout").replace("{dice}", v).replace("{sta}", v * adrenalineStaPerDieFor(ctx.actor))
            : "";
    }
    ...
    /* Damage-type shot-2 picker — populated live based on strike category:
     *   Fast   → 2 swings, same weapon; mirror shot-1's radios
     *   Joint  → 2 swings, off-hand for shot 2; radios come from the
     *            CURRENT off-hand selection's damage-type list
     * Everything else → shot-2 hidden. Shot-1 label picks up a "1st"
     * qualifier when shot 2 is visible, so the player reads the two
     * pickers as a pair. Outer block visibility toggles on whether
     * EITHER shot has any radios to show. */
    const strikeCat = cat(strikeKey);
    const dtBlock  = root.querySelector("[data-dmgtype-block]");
    const dtShot1  = root.querySelector("[data-dmgtype-shot1]");
    const dtShot1L = root.querySelector("[data-dmgtype-shot1-label]");
    const dtShot2  = root.querySelector("[data-dmgtype-shot2]");
    const dtShot2R = root.querySelector("[data-dmgtype-shot2-radios]");
    if (dtBlock) {
        const shot1HasRadios = !!dtShot1?.querySelector('input[name="dmgtype-1"]');
        let shot2Types = [];
        if (strikeCat === "fast" && shot1HasRadios) {
            /* Same weapon on both swings → same choice set. */
            shot2Types = Array.from(dtShot1.querySelectorAll('input[name="dmgtype-1"]'))
                .map(r => r.value).filter(Boolean);
        } else if (strikeCat === "joint") {
            /* Off-hand's own type list — the two swings are different
             * weapons, so shot 2 is not necessarily the same choice set
             * as shot 1. */
            const offId = root.querySelector('[name="offhand"]')?.value
                        || ctx.offhandChoices?.[0]?.id;
            const off = offId ? ctx.actor?.items?.get?.(offId) : null;
            const offTypes = (off?.system?.effective?.damageTypes ?? off?.system?.damageTypes ?? [])
                .filter(t => typeof t === "string" && t.length > 0);
            if (offTypes.length >= 2) shot2Types = offTypes;
        }
        if (shot2Types.length) {
            /* Rebuild the shot-2 radios. Default the first option
             * checked; if the user already picked something and that
             * value is still valid, preserve their choice across the
             * refresh. */
            const prior = dtShot2R?.querySelector('input[name="dmgtype-2"]:checked')?.value;
            const html = shot2Types.map((t, i) => {
                const checked = prior ? (t === prior) : (i === 0);
                return `<label class="wdm-atk-check">
                    <input type="radio" name="dmgtype-2" value="${esc(t)}"${checked ? " checked" : ""} />
                    <span>${esc(L(DAMAGE_TYPES[t] ?? t))}</span>
                </label>`;
            }).join("");
            if (dtShot2R) dtShot2R.innerHTML = html;
            if (dtShot2)  dtShot2.style.display = "";
        } else if (dtShot2) {
            dtShot2.style.display = "none";
            if (dtShot2R) dtShot2R.innerHTML = "";
        }
        const shot2Visible = shot2Types.length > 0;
        /* Shot-1 label: neutral "Damage Types" alone; qualify with
         * "(1st Swing)" when shot 2 is also visible. */
        if (dtShot1L) {
            dtShot1L.textContent = shot2Visible
                ? L("WITCHER.Attack.DamageTypeShot1")
                : L("WITCHER.Attack.DamageTypes");
        }
        /* Outer block visible if either shot has content. */
        dtBlock.style.display = (shot1HasRadios || shot2Visible) ? "" : "none";
    }
...
export async function openAttackDialog(weapon, actor, opts = {}) {
    ...
    /* Quick-slot collapses to throw-only — the item was drawn to throw while the
     * main hand is busy. EXCEPTION (Combat Extended): a MELEE weapon in the quick
     * slot can still be swung in melee, so it stays dual-mode / melee-capable
     * rather than throw-only. RAW behaviour is unchanged. The off-hand −3 (see
     * isOffhandEquipped) still applies either way — this only unlocks the melee
     * OPTION, not a free hand. */
    const quickThrowOnly = inQuickSlot && !(setupCeOn && wType === "melee");
    const dualMode = monsterMode ? false : (rawDualMode && !quickThrowOnly);
    /* Default mode picks the weapon's primary use:
     *   - Throw-only quick slot → thrown (hands busy with the main weapon)
    ...
    const mode = quickThrowOnly ? "thrown"
                /* Caller (the dock Melee/Throw toggle) can preselect the
                 * dual-mode weapon's mode so the dialog opens matching the
                 * overlay the player just used. Player can still flip the
                 * in-dialog toggle. */
                : (dualMode && (opts.mode === "melee" || opts.mode === "thrown")) ? opts.mode
                : dualMode       ? "melee"
                :                  "melee";
    /* Ranged UI (range-bracket picker, weather ranged penalty,
     * distance auto-select, ranged-only situational mods) applies to
     * any ranged shot — PC or monster. Previously monsterMode stripped
     * this, so a monster bandit with a crossbow got a melee-shaped
     * dialog with no range bracket. Strike-variant tabs stay gated
     * on monsterMode (RAW p.153 keeps Strong/Fast/Feint/Joint PC-only). */
    const ranged    = quickThrowOnly ? true
                     : isRangedWeapon(weapon) || (dualMode && mode === "thrown");
    const variants  = monsterMode ? false
                     : quickThrowOnly ? false   /* thrown shots are normal-only */
                     : allowsStrikeVariants(weapon) || (dualMode && mode === "melee");
    const melee     = (wType === "melee" && !quickThrowOnly);
    ...
    const weather   = ranged ? weatherRangedPenalty(actor) : { total: 0, parts: [] };
    const lightAtk  = lightAttackPenalty(actor);   // Darkness −2, melee + ranged
    ...
    // chambered, so they get no picker here. Monster inline attacks stub
    // `weapon.usesAmmo = false` in monsterVirtualWeapon, so this branch is
    // still quiescent for natural attacks — only real ranged weapons dropped
    // onto a monster's Combat block flip it on under monsterMode.
    const usesAmmo = !!weapon.usesAmmo && !weapon.hasChamber;
    /* Coated arrow/bolt stacks (a 1-qty spinoff produced by the apply-
     * oil flow) are distinct item documents, so they naturally list as
     * separate entries here. Suffix the ammo name with the oil name so
     * the player can tell them apart — e.g. "Regular Arrow (Hanged
     * Man's Venom)". Plain stacks stay unlabeled. */
    const ammoChoices = usesAmmo
        ? (weapon.getEligibleAmmo?.() ?? []).map(e => {
            const oil = e.item?.system?.appliedOil;
            const suffix = (oil && oil.name) ? ` (${oil.name})` : "";
            return { id: e.item.id, name: `${e.item.name}${suffix}`, qty: e.qty };
        })
        : [];
    ...
    try {
         ...
         * enemy's token. */
        /* 0. Explicit target passed by the caller (the canvas tile-targeting
         *    overlay picks a token and routes it here as opts.targetActor, so
         *    the range bracket is auto-computed for that pick even though it
         *    never went through game.user.targets). */
        let dTok = opts.targetActor?.getActiveTokens?.()?.[0]
    ...
    /* Situational bonuses to PRE-CHECK on open (player can still uncheck):
     *   • hiddenAttacker ("Out of Target's View") — attacker sits outside the
     *     target's vision cone.
     *   • ambush — attacker is stealthed and the target hasn't spotted them.
     * Both need the two tokens; best-effort, silent-fail to no auto-check. */
    let autoCheckSit = [];
    try {
        const aTok = actor?.getActiveTokens?.()?.[0] ?? null;
        const targetActor = opts.targetActor
            ...
            ?? null;
        const dTok = targetActor?.getActiveTokens?.()?.[0]
            ?? Array.from(game.user?.targets ?? [])[0] ?? null;
        const dActor = targetActor ?? dTok?.actor ?? null;
        if (await attackerOutOfTargetSight(aTok, dTok)) autoCheckSit.push("hiddenAttacker");
        if (await attackerHasAmbush(actor, dActor))     autoCheckSit.push("ambush");
    } catch (_) { /* leave empty — no auto-checks */ }

    const ctx = { weapon, actor, targetActor: opts.targetActor ?? null, base: opts.base ?? { total: 0, chips: [] }, meleeBase: opts.meleeBase ?? null, dualMode, mode, ranged, variants, melee, offhandChoices, mainIsOffhand, aimMod, weather, lightAtk, baseRange, targetDistanceMeters, fastDraw, forcedExtra, usesAmmo, ammoChoices, selectedAmmoId, aimRank, aimBonus, adrenaline, canFullRound, excludedSpecials, targetPool, autoCheckSit };

    const content = buildContent(ctx);

    const result = await foundry.applications.api.DialogV2.wait({
        window: { title: `${L("WITCHER.Attack.DialogTitle")} — ${weapon.name}`, icon: "fa-solid fa-crosshairs", resizable: true },
        /* Fixed starting width so a long strike description WRAPS to the window
         * instead of expanding it to the note's single-line width. resizable lets
         * the player widen/narrow from the bottom-right corner. */
        position: { width: 480 },
        content,
