...
function injectBodyFigure(wrap) {
    ...
    svg.setAttribute("aria-label", "Body zones — click to cover with shield");
...
export async function openRaiseShieldDialog(actor, shield) {
    ...
    if (cv <= 0) {
        return await DialogV2.wait({
            ...
            content: `<p>This shield has a Cover Value of 0. Set its CV on the shield item sheet (Combat Extended).</p>`,
            ...
            <div class="wdm-rs-body">
                ...
                <div class="wdm-rs-sidebar">
                    <div class="wdm-rs-status">
                        Covering <span class="wdm-rs-count" data-rs-count>0</span>
                        <span style="font-size:0.6875rem;opacity:0.65;">of ${cv}</span>
                    </div>
                    <div class="wdm-rs-list" data-rs-list><em>(no locations picked)</em></div>
                    <div class="wdm-rs-head-warning" data-rs-headwarn style="display:none;">
                        <i class="fa-solid fa-eye-slash"></i>
                        Head covered → applies Restricted Vision (−2 to Block / Parry / Dodge until your next turn).
                    </div>
                    ...
                    ${fullCover ? "" : `<button type="button" class="wdm-rs-reset" data-rs-reset>Reset</button>`}
                </div>
            </div>
        </div>`;
    ...
    if (alreadyRaised) {
        buttons.push({
            action: "lower", label: "Lower Shield", icon: "fa-solid fa-shield-slash",
            callback: async () => {
                const upd = {
