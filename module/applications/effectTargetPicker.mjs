    ...
    const content = `<div class="wdm-target-picker">
        <div class="wdm-tp-bar">
          <select class="wdm-tp-cat">
            <option value="">${t("WITCHER.App.EffectTargetPicker.Text.AllCategories", "All categories")}</option>
            ${groups.map((g, i) => `<option value="${i}">${esc(g.label)}</option>`).join("")}
          </select>
          <input type="search" class="wdm-tp-search" placeholder="${t("WITCHER.App.EffectTargetPicker.Text.SearchParameters", "Search parameters…")}" autofocus />
        </div>
