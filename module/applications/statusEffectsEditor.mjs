...
import { STATUS_EFFECTS, DEFAULT_HIDDEN_ON_TOKEN_IDS } from "../setup/statusEffects.mjs";
...
function buildEditableRegistry() {
    const out = Object.fromEntries(
        STATUS_EFFECTS.map(s => [s.id, {
            name: s.name,
            img: s.img,
            rimColor: s.rimColor,
            /* Seed default for the token-ring visibility toggle.
             * `s.showOnToken` is written explicitly by the food & drink
             * and stress seed rows in setup/statusEffects.mjs; every
             * pure-RAW status has it undefined, which reads as visible.
             * Kept as an explicit `!== false` so a future entry with
             * `showOnToken: false` in the RAW seed would still respect
             * that intent. */
            showOnToken: s.showOnToken !== false
        }])
    );
    ...
    const NAME_FALLBACKS = {
        "drunk-1": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-1", "Drunk I"),
        "drunk-2": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-2", "Drunk II"),
        "drunk-3": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-3", "Drunk III"),
        "drunk-4": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-4", "Drunk IV"),
        "drunk-5": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-5", "Drunk V"),
        "drunk-6": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-6", "Drunk VI"),
        "drunk-7": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-7", "Drunk VII"),
        "drunk-8": t("WITCHER.App.StatusEffectsEditor.StatusName.drunk-8", "Drunk VIII"),
        gorged:   t("WITCHER.App.StatusEffectsEditor.StatusName.gorged",   "Gorged"),
        full:     t("WITCHER.App.StatusEffectsEditor.StatusName.full",     "Full"),
        fed:      t("WITCHER.App.StatusEffectsEditor.StatusName.fed",      "Fed"),
        peckish:  t("WITCHER.App.StatusEffectsEditor.StatusName.peckish",  "Peckish"),
        hungry:   t("WITCHER.App.StatusEffectsEditor.StatusName.hungry",   "Hungry"),
        famished: t("WITCHER.App.StatusEffectsEditor.StatusName.famished", "Famished"),
        hangover: t("WITCHER.App.StatusEffectsEditor.StatusName.hangover", "Hangover"),
        "food-sickness":        t("WITCHER.App.StatusEffectsEditor.StatusName.food-sickness",        "Food Sickness"),
        "break-indulgent":      t("WITCHER.App.StatusEffectsEditor.StatusName.break-indulgent",      "Indulgent"),
        "break-paranoid":       t("WITCHER.App.StatusEffectsEditor.StatusName.break-paranoid",       "Paranoid"),
        "break-scared":         t("WITCHER.App.StatusEffectsEditor.StatusName.break-scared",         "Scared"),
        "break-depressive":     t("WITCHER.App.StatusEffectsEditor.StatusName.break-depressive",     "Depressive"),
        "break-impulsive":      t("WITCHER.App.StatusEffectsEditor.StatusName.break-impulsive",      "Impulsive"),
        "break-self-harming":   t("WITCHER.App.StatusEffectsEditor.StatusName.break-self-harming",   "Self-Harming"),
        "break-selfish":        t("WITCHER.App.StatusEffectsEditor.StatusName.break-selfish",        "Selfish"),
        "break-violent":        t("WITCHER.App.StatusEffectsEditor.StatusName.break-violent",        "Violent"),
        "boon-stoic":           t("WITCHER.App.StatusEffectsEditor.StatusName.boon-stoic",           "Stoic"),
        "boon-optimistic":      t("WITCHER.App.StatusEffectsEditor.StatusName.boon-optimistic",      "Optimistic"),
        "boon-hopeful":         t("WITCHER.App.StatusEffectsEditor.StatusName.boon-hopeful",         "Hopeful"),
        "boon-defiant":         t("WITCHER.App.StatusEffectsEditor.StatusName.boon-defiant",         "Defiant"),
        "boon-focused":         t("WITCHER.App.StatusEffectsEditor.StatusName.boon-focused",         "Focused"),
        "boon-stalwart":        t("WITCHER.App.StatusEffectsEditor.StatusName.boon-stalwart",        "Stalwart"),
        "boon-determined-grit": t("WITCHER.App.StatusEffectsEditor.StatusName.boon-determined-grit", "Determined Grit"),
        "boon-unbreakable":     t("WITCHER.App.StatusEffectsEditor.StatusName.boon-unbreakable",     "Unbreakable"),
        "boon-smile-at-death":  t("WITCHER.App.StatusEffectsEditor.StatusName.boon-smile-at-death",  "Smile at Death")
    };
    ...
    for (const id of Object.keys(STATUS_CLAUSES)) {
        ...
        out[id] = {
            ...
            /* Homebrew IDs (food & drink, stress) default OFF-token,
             * matching the seed in setup/statusEffects.mjs so a first-
             * time open shows the checkbox unchecked and the diff-based
             * save doesn't treat "still unchecked" as a delta from
             * default (which would spuriously persist every homebrew
             * status). Any other clause-only id keeps the default "on"
             * behavior. */
            showOnToken: !DEFAULT_HIDDEN_ON_TOKEN_IDS.has(id)
        };
    }
    return out;
}
...
const ROLL_KEYS = () => [
    { key: "attack",    label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.Attack", "Attack") },
    { key: "defense",   label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.Defense", "Defense") },
    { key: "awareness", label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.AwarenessSight", "Awareness (sight)") },
    { key: "all",       label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.AllRolls", "All rolls") },
    { key: "verbal",    label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.VerbalCombat", "Verbal Combat") }
];
const END_KINDS = () => [
    { value: "none",     label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.None", "None") },
    { value: "stunSave", label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.StunSave", "Stun save") },
    { value: "skill",    label: t("WITCHER.App.StatusEffectsEditor.Dialog.Button.SkillCheckDC", "Skill check (DC)") }
];
...
export class StatusEffectsEditor extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static #rowFromClause(id, pres, clause) {
        ...
        return {
            ...
            /* Default TRUE — the historic behavior is "every status
             * shows on the token as a ring." GMs opt OUT for cosmetic
             * / narrative-only statuses that shouldn't clutter the
             * portrait ring (e.g. "narratively drunk" story-flags,
             * bookkeeping markers). */
            showOnToken: pres?.showOnToken !== false,
            isDefault: DEFAULT_IDS.has(id),
            description: clause?.description ?? "",
            stats,
            roll,
            dotAmount: clause?.dot?.amount == null ? "" : String(clause.dot.amount),
            dotBypassArmor: !!clause?.dot?.bypassArmor,
            dotEveryLocation: clause?.dot?.scope === "all-locations",
            /* Accept both flat integers AND dice formulas ("1d6",
             * "1d3+1") — stored as string so the formula survives
             * round-trip. Empty string = disabled. */
            dotAblateArmor: clause?.dot?.ablateArmor == null ? "" : String(clause.dot.ablateArmor),
            dotAblateArmorOnlyIfPenetrated: !!clause?.dot?.ablateArmorOnlyIfPenetrated,
            dotAblateWeapon: clause?.dot?.ablateWeapon == null ? "" : String(clause.dot.ablateWeapon),
            dotAblateWeaponOnlyIfPenetrated: !!clause?.dot?.ablateWeaponOnlyIfPenetrated,
            ...
            clearsAtOwnTurnEnd: clause?.clearsAt === "ownTurnEnd",
    ...
    static #entryFromRow(row) {
        ...
        /* Always persist `showOnToken` (both true and false) so the
         * save-time diff against the RAW default correctly detects
         * flips in EITHER direction: a food/drink status defaults to
         * `false` and is stored as `true` when the GM checks the box;
         * a RAW status defaults to `true` and is stored as `false`
         * when the GM unchecks. If we only wrote it when false, a GM
         * turning ON a homebrew ring would leave nothing to compare
         * against and the override entry would look identical to the
         * seed → the ring would never persist. */
        entry.showOnToken = row.showOnToken !== false;
        ...
        // dotAmount is stored as a string so dice formulas ("1d6+2") survive
        // round-trip. Persist as a Number when it's plainly numeric, else as
        // the trimmed formula string; the tick engine's rollOrFlat handles
        // both. Skip when blank / "0" so the diff still omits untouched rows.
        const amtRaw = String(row.dotAmount ?? "").trim();
        if (amtRaw && amtRaw !== "0") {
            const amt = /^[+-]?\d+(?:\.\d+)?$/.test(amtRaw) ? Number(amtRaw) : amtRaw;
            const dot = { amount: amt };
            if (row.dotBypassArmor) dot.bypassArmor = true;
            if (row.dotEveryLocation) dot.scope = "all-locations";
            /* Persist ablate values as Number when purely numeric
             * ("3" → 3), else as the trimmed formula string
             * ("1d6"). Same round-trip pattern as dotAmount so a
             * user entering a fixed value gets stored as an int
             * and one entering a formula keeps the string. Skip
             * empty / "0" so the diff still omits untouched rows. */
            const ablArm = String(row.dotAblateArmor ?? "").trim();
            if (ablArm && ablArm !== "0") {
                dot.ablateArmor = /^[+-]?\d+(?:\.\d+)?$/.test(ablArm) ? Number(ablArm) : ablArm;
            }
            if (row.dotAblateArmorOnlyIfPenetrated) dot.ablateArmorOnlyIfPenetrated = true;
            const ablWpn = String(row.dotAblateWeapon ?? "").trim();
            if (ablWpn && ablWpn !== "0") {
                dot.ablateWeapon = /^[+-]?\d+(?:\.\d+)?$/.test(ablWpn) ? Number(ablWpn) : ablWpn;
            }
            if (row.dotAblateWeaponOnlyIfPenetrated) dot.ablateWeaponOnlyIfPenetrated = true;
            entry.dot = dot;
        }
        ...
        // Turn-END wins when both are ticked — "turn end" is an explicit
        // override of a status that defaults to clearing at turn start.
        if (row.clearsAtOwnTurnEnd) entry.clearsAt = "ownTurnEnd";
        else if (row.clearsAtOwnTurn) entry.clearsAt = "ownTurnStart";
    ...
    #initWorking() {
        ...
        for (const id of DEFAULT_IDS) {
            ...
            const pres = {
                ...
                /* Presence-aware default: the override CAN explicitly
                 * store `false` (a homebrew status the GM left off) or
                 * `true` (a homebrew status the GM opted onto the
                 * portrait). Only when the key is absent do we fall
                 * back to `DEFAULT_PRESENTATION[id].showOnToken` — which
                 * carries the seed value from `buildEditableRegistry`
                 * (`false` for food & drink / stress, `true` for
                 * everything else). Using `o?.showOnToken !== false`
                 * here would flip every homebrew status to `true` on
                 * first editor open, defeating the whole "hidden by
                 * default" intent. */
                showOnToken: (o && ("showOnToken" in o)) ? o.showOnToken : DEFAULT_PRESENTATION[id].showOnToken
            };
        ...
        for (const [id, o] of Object.entries(override)) {
            ...
            rows.push(StatusEffectsEditor.#rowFromClause(id, { name: o.name, img: o.img, rimColor: o.rimColor, showOnToken: o.showOnToken !== false }, StatusEffectsEditor.#stripPresentation(o)));
        }
        this.#working = rows;
    }

    static #stripPresentation(entry) {
        const { name, img, rimColor, showOnToken, removed, ...clause } = entry;
        return clause;
    }
    ...
    #syncFromForm() {
        ...
        for (const key of Object.keys(rows).sort((a, b) => Number(a) - Number(b))) {
            ...
            next.push({
                ...
                /* Unchecked checkboxes don't POST at all, so the
                 * absence of `r.showOnToken` means "hidden". Present
                 * with a truthy value means "visible". */
                showOnToken: !!r.showOnToken,
                isDefault: r.isDefault === true || r.isDefault === "true",
                description: String(r.description ?? "").trim(),
                stats,
                roll,
                dotAmount: String(r.dotAmount ?? "").trim(),
                ...
                clearsAtOwnTurnEnd: !!r.clearsAtOwnTurnEnd,
    ...
    #statusView(row, index, skillKeys) {
        ...
        return {
            ...
            showOnToken: row.showOnToken !== false,
            ...
            clearsAtOwnTurnEnd: row.clearsAtOwnTurnEnd,
    ...
    static async #onAddStatus() {
        ...
        this.#working.push(StatusEffectsEditor.#rowFromClause(id, { name: t("WITCHER.App.StatusEffectsEditor.Text.NewStatus", "New Status"), img: DEFAULT_ICON }, { description: "" }));
        this.render();
    }
