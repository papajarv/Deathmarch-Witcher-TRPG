...
/* Localized display strings are built at render time (not module-load), so
 * that Foundry's i18n has already loaded lang files when consumers iterate. */
const PARAM_TYPES = () => [
    { value: "none",    label: t("WITCHER.App.QualitiesEditor.Dialog.Button.NoParameter", "No parameter") },
    { value: "percent", label: t("WITCHER.App.QualitiesEditor.Dialog.Button.Percent", "Percent (%)") },
    { value: "number",  label: t("WITCHER.App.QualitiesEditor.Dialog.Button.Number", "Number") },
    { value: "text",    label: t("WITCHER.App.QualitiesEditor.Dialog.Button.TextFormula", "Text / formula") }
];

const DAMAGE_FLAG_KEYS = () => [
    { key: "armorPiercing",         label: t("WITCHER.App.QualitiesEditor.Dialog.Button.ArmorPiercingNegatesDR", "Armor Piercing (negates DR)")           },
    { key: "improvedArmorPiercing", label: t("WITCHER.App.QualitiesEditor.Dialog.Button.ImprovedAPNegatesDRHalvesSP", "Improved AP (negates DR + halves SP)")  },
    { key: "ablating",              label: t("WITCHER.App.QualitiesEditor.Dialog.Button.Ablating1d62SPDamageOn", "Ablating (+1d6/2 SP damage on penetration, on top of the standard −1 SP chip)") },
    { key: "doubleAblation",        label: t("WITCHER.App.QualitiesEditor.Dialog.Button.DoubleAblationDoublesTheStandardChip", "Double Ablation (doubles the standard chip to −2 SP, e.g. Crushing Force)") },
    { key: "bypassesWornArmor",     label: t("WITCHER.App.QualitiesEditor.Dialog.Button.BypassesWornArmor", "Bypasses Worn Armor")                   },
    { key: "bypassesNaturalArmor",  label: t("WITCHER.App.QualitiesEditor.Dialog.Button.BypassesNaturalArmor", "Bypasses Natural Armor")                },
    { key: "bypassesShield",        label: t("WITCHER.App.QualitiesEditor.Dialog.Button.BypassesShieldQuen", "Bypasses Shield (Quen)")                },
    { key: "isSilver",              label: t("WITCHER.App.QualitiesEditor.Dialog.Button.CountsAsSilverVsMonsterResists", "Counts as Silver (vs monster resists)") },
    { key: "isMeteorite",           label: t("WITCHER.App.QualitiesEditor.Dialog.Button.CountsAsMeteoriteVsMonsterResists", "Counts as Meteorite (vs monster resists)") },
    { key: "deniesParry",           label: t("WITCHER.App.QualitiesEditor.Dialog.Button.CannotBeParriedEGCrushing", "Cannot Be Parried (e.g. Crushing Force)") }
];

const NUMERIC_EFFECT_KEYS = () => [
    { key: "parryPenaltyDelta",   label: t("WITCHER.App.QualitiesEditor.Dialog.Button.ParryPenaltyReduction", "Parry Penalty Reduction"),
      hint: t("WITCHER.App.QualitiesEditor.Hint.ParryPenaltyDelta",   "Shaves N off the −3 parry penalty when DEFENDING with this weapon (Parrying = 2).") },
    { key: "spellDCBonus",        label: t("WITCHER.App.QualitiesEditor.Dialog.Button.SpellDCBonus", "Spell DC Bonus"),
      hint: t("WITCHER.App.QualitiesEditor.Hint.SpellDCBonus",        "Spells cast through this weapon treat their target DC as +N (Greater Focus = 2).") },
    { key: "reliabilityBonus",    label: t("WITCHER.App.QualitiesEditor.Dialog.Button.ReliabilityBonus", "Reliability Bonus"),
      hint: t("WITCHER.App.QualitiesEditor.Hint.ReliabilityBonus",    "Added to the weapon's max Reliability when this quality is present (Meteorite = 5).") },
    { key: "chargeBonusPerMeter", label: t("WITCHER.App.QualitiesEditor.Dialog.Button.ChargeBonusMeter", "Charge Bonus / Meter"),
      hint: t("WITCHER.App.QualitiesEditor.Hint.ChargeBonusPerMeter", "Bonus damage dice per meter charged from a mount (Charging = 1, i.e. +1d6/m).") }
];

const STRING_EFFECT_KEYS = () => [
    { key: "skillOverride",       label: t("WITCHER.App.QualitiesEditor.Dialog.Button.AttackSkillOverride", "Attack Skill Override"),
      hint: t("WITCHER.App.QualitiesEditor.Hint.SkillOverride",       "Forces the weapon's to-hit roll to use this skillMap key (rarely used — the weapon's own skillKey covers most cases). Blank = use the weapon's default.") }
];

const TACTICAL_FLAG_KEYS = () => [
    { key: "ignoresRepositionDistance",
      label: t("WITCHER.App.QualitiesEditor.Text.IgnoresRepositionDistanceLongReachWeapon", "Ignores Reposition Distance (long-reach weapon — defender's Reposition doesn't void follow-up swings)") },
    { key: "addsDamageToUnarmed",
      label: t("WITCHER.App.QualitiesEditor.Text.AddsDamageToUnarmedBrawlingCestusSpikedG", "Adds Damage to Unarmed (Brawling — cestus / spiked gauntlet folds its damage into punches/kicks)") }
];

const RIDER_KINDS = () => [
    { value: "none",     label: t("WITCHER.App.QualitiesEditor.Dialog.Button.NoRider", "No rider")                            },
    { value: "auto",     label: t("WITCHER.App.QualitiesEditor.Dialog.Button.AutoAppliesOnEveryDamagingHit", "Auto (applies on every damaging hit)")},
    { value: "percent",  label: t("WITCHER.App.QualitiesEditor.Dialog.Button.PercentRollsD100VsParameter", "Percent (rolls d100 vs parameter)")   },
    { value: "stunSave", label: t("WITCHER.App.QualitiesEditor.Dialog.Button.StunSaveTargetRollsStunAt", "Stun Save (target rolls Stun at param)")}
];
...
export class QualitiesEditor extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext(options) {
        ...
        ctx.buttons = [{ type: "submit", icon: "fa-solid fa-floppy-disk", label: t("WITCHER.Common.Save", "Save") }];
        return ctx;
    }
    ...
    #catalogView(name, label, statusIds) {
        ...
        const rows = this.#working[name].map((row, index) => ({
            ...
            riderStatusOptions: [{ value: "", label: t("WITCHER.App.QualitiesEditor.Text.PickAStatus", "— pick a status —"), selected: !row.riderStatus }]
    ...
    static async #onAddQuality(event, target) {
        ...
        this.#working[name].push(QualitiesEditor.#rowFromEntry(key, { label: t("WITCHER.App.QualitiesEditor.Dialog.Button.NewQuality", "New Quality"), description: "" }, false));
        this.render();
    }
    ...
    static async #onResetCatalog(event, target) {
        ...
        const ok = await foundry.applications.api.DialogV2.confirm({
            ...
            content: tFormat("WITCHER.App.QualitiesEditor.Dialog.ResetConfirm", { name }, "<p>Discard your customizations to the {name} qualities and restore the system defaults? This takes effect when you Save.</p>"),
            modal: true,
            rejectClose: false
        }).catch(() => false);
        if (!ok) return;
