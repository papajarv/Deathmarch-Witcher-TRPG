...
export async function openOpenCategoryConfigDialog(item, qualityKey) {
    ...
    const parsed = parseOpenCategoryBonus(currentText)
        ?? { wa: 0, dmgDice: "", grantedQualities: [], grantedQualityValues: {}, raw: "" };
    ...
    const html = `
        <div class="occ-dialog">
            <div class="wdm-cfg-section">Flat bonuses <span class="wdm-cfg-sub">(applied when ${escapeText(label)} fires)</span></div>
            <div class="wdm-cfg-row occ-flat-row">
                <label class="wdm-cfg-field">
                    <span class="wdm-cfg-label">WA</span>
                    <input type="number" name="wa" value="${Number(parsed.wa) || 0}" step="1" />
                </label>
                <label class="wdm-cfg-field">
                    <span class="wdm-cfg-label">Damage dice</span>
                    <input type="text" name="dice" value="${escapeAttr(parsed.dmgDice || "")}"
                           placeholder="1d6" pattern="\\d+d\\d+" />
                </label>
            </div>

            <div class="wdm-cfg-section">Granted qualities <span class="wdm-cfg-sub">(fire post-hit when ${escapeText(label)} triggers — fill parameter values for those that have one)</span></div>
            <div class="occ-filter">
                <input type="search" name="q" placeholder="Filter qualities…" autocomplete="off" />
            </div>
    ...
    try {
        const result = await DialogV2.prompt({
            window: { title: `${label} — bonus`, contentClasses: ["occ-window"] },
            content: html,
            ok: {
                label: "Save",
                callback: (_event, button) => {
                    ...
                    return { wa, dice, grantedKeys, grantedVals };
                }
            },
...
function formatBonusString({ wa, dice, grantedKeys, grantedVals }, catalog) {
    const parts = [];
