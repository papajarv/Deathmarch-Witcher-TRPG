...
export class RingPortraitCropper extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static async _onApplyClick(event) {
        ...
        try {
            ...
            console.log(`${SYSTEM_ID} | RPC rasterized: ${dataUrl.length} chars; uploading…`);
            ui.notifications.info(tFormat("WITCHER.Notify.Crop.Rasterized", { kb: Math.round(dataUrl.length / 1024) }, "Crop: rasterized ({kb} KB), uploading…"));
            /* Upload the rasterized crop to a real file so we can write a
             * file path (not a data URL) to prototypeToken.texture.src.
             * Foundry v14's FilePathField sanitizer rejects data: URLs
             * silently on server writes — the actor.update would appear to
             * succeed locally but the token texture would stay on the raw
             * source image, and every re-sync from portrait-toxicity would
             * repeat the same failure. Uploading first side-steps that
             * whole class of "works on my local Foundry but not on the
             * server" surprises. Falls back to the data URL only if the
             * upload path is unavailable (no world context, no upload
             * permission), matching the pre-upload behavior on those
             * setups. */
            const cacheKey = `${this.sourceImage}|${this._cropState.tx}|${this._cropState.ty}|${this._cropState.scale}`;
            let textureSrc = dataUrl;
            try {
                const { persistCropAsFile } = await import("../chrome/integrations/portrait-toxicity.js");
                const uploaded = await persistCropAsFile(dataUrl, cacheKey);
                if (uploaded) {
                    textureSrc = uploaded;
                    console.log(`${SYSTEM_ID} | RPC uploaded to ${uploaded}`);
                } else {
                    console.warn(`${SYSTEM_ID} | RPC upload returned null — falling back to data URL (may not persist under v14 sanitizer)`);
                }
            } catch (err) {
                console.warn(`${SYSTEM_ID} | RPC upload threw — falling back to data URL`, err);
            }
