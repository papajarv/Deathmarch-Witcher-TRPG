...
export class StressConfigApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext(options) {
        ...
        ctx.values = {
            ...
            bookFailStressEnabled:      !!v.bookFailStressEnabled,
            bookFailStressAmount:       Math.max(0, Number(v.bookFailStressAmount) ?? 1)
        };
        ctx.buttons = [
            { type: "button", action: "resetDefaults", icon: "fa-solid fa-arrow-rotate-left", label: t("WITCHER.App.StressConfig.Dialog.Button.ResetToDefaults", "Reset to Defaults") },
    ...
    static async #onResetDefaults() {
        const ok = await foundry.applications.api.DialogV2.confirm({
            ...
            content: `<p>${t("WITCHER.App.StressConfig.Text.DiscardAllStressConfigurationOverridesAn", "Discard all stress configuration overrides and restore the homebrew defaults? This takes effect when you Save.")}</p>`,
            modal: true,
            rejectClose: false
        }).catch(() => false);
        if (!ok) return;
    ...
    static async #onSubmit(event, form, formData) {
        ...
        const merged = {
            ...
            bookFailStressEnabled:      !!raw.bookFailStressEnabled,
            bookFailStressAmount:       Math.max(0, Number(raw.bookFailStressAmount) || 0)
        };
