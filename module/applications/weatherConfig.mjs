...
import { LIGHT_TIER_PENALTY, getActiveLightPenalties } from "../mechanics/light-level.mjs";
...
const MOD_TARGETS = ["awareness", "ranged", "tracking", "stealth", "movement", "endurance", "narrative"];

/* Light tab: the penalty-bearing tiers (bright / Lit carry none) and the three
 * roll targets each can penalize. Mirrors LIGHT_TIER_PENALTY in light-level.mjs. */
const LIGHT_PEN_TIERS = ["dim1", "dim2", "dim3", "darkness", "pitch"];
const LIGHT_PEN_FIELDS = ["awareness", "attack", "defense"];
/* Condition tags that carry mechanical weight, in display order. */
const MOD_TAGS = ["fog", "wind", "precip", "snow", "storm", "heat", "hail"];

/* Canonical stored shape for one modifier rule — tiered (`byLevel`) or flat.
 * Used for BOTH the saved override and the seed comparison, so the two are
 * apples-to-apples: an unedited table serializes identically to the seed (→ the
 * override is cleared), and tiered rules keep their per-tier map instead of being
 * flattened to value:0 (the ±0 bug). */
function serializeModRule(r) {
    if (r?.byLevel && typeof r.byLevel === "object") {
        const byLevel = {};
        for (const k of [1, 2, 3]) if (r.byLevel[k] != null) byLevel[k] = Number(r.byLevel[k]) || 0;
        return { target: r.target, byLevel, label: r.label ?? "" };
    }
    return {
...
export class WeatherConfigApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static PARTS = {
        ...
        light:     { template: "systems/witcher-ttrpg-death-march/templates/applications/weather-config-light.hbs", scrollable: [""] },
    ...
    static #GM_ONLY_TABS = ["climate", "terrain", "modifiers", "light", "calendar"];
    ...
    #seedWorking() {
        return {
            ...
            light:     this.#liveLightPenalties(),
            calendar:  this.#liveCalendarBits(),
            map:       this.#liveMapSettings()
        };
    }

    /* Per-tier light penalties, override-merged, as a plain {tier:{field:val}} map
     * over the penalty-bearing tiers only (bright / Lit carry none). */
    #liveLightPenalties() {
        const live = getActiveLightPenalties();
        const out = {};
        for (const tier of LIGHT_PEN_TIERS) {
            const src = live[tier] ?? {};
            out[tier] = Object.fromEntries(LIGHT_PEN_FIELDS.map(f => [f, Number(src[f]) || 0]));
        }
        return out;
    }
    ...
    #liveModifiers() {
        ...
        for (const tag of MOD_TAGS) {
            out[tag] = (rules[tag] ?? []).map(r => {
                ...
                // Tiered (byLevel) rules — fog / precip / snow — carry a per-tier
                // value map with NO flat `value`. Preserve it losslessly (the old
                // code flattened it to value:0, which is what zeroed those rows).
                if (r.byLevel && typeof r.byLevel === "object") {
                    row.byLevel = {};
                    for (const k of [1, 2, 3]) {
                        const v = r.byLevel[k];
                        if (v != null && v !== "") row.byLevel[k] = Number(v) || 0;
                    }
                }
                return row;
            });
    ...
    async _preparePartContext(partId, context) {
        ...
        if (partId === "light")     this.#ctxLight(ctx);
        if (partId === "calendar")  this.#ctxCalendar(ctx);
        return ctx;
    }

    #ctxLight(ctx) {
        ctx.usingDefaults = !this.#hasOverride("lightPenaltiesOverride");
        ctx.lightFields = LIGHT_PEN_FIELDS.map(f => ({
            key: f,
            label: localize(`WITCHER.Weather.Target.${f.charAt(0).toUpperCase()}${f.slice(1)}`) || f
        }));
        ctx.lightTiers = LIGHT_PEN_TIERS.map(tier => ({
            tier,
            label: localize(`WITCHER.Light.Config.Tier.${tier}`) || tier,
            cells: LIGHT_PEN_FIELDS.map(f => ({
                name: `light.${tier}.${f}`,
                value: Number(this.#working.light[tier]?.[f]) || 0
            }))
        }));
    }
    ...
    #ctxModifiers(ctx) {
        ...
        ctx.tags = MOD_TAGS.map(tag => ({
            ...
            rows: (this.#working.modifiers[tag] ?? []).map((r, i) => ({
                ...
                label: r.label,
                byLevel: !!r.byLevel,
                bl1: r.byLevel ? (r.byLevel[1] ?? "") : "",
                bl2: r.byLevel ? (r.byLevel[2] ?? "") : "",
                bl3: r.byLevel ? (r.byLevel[3] ?? "") : ""
            }))
        }));
    }
    ...
    #syncFromForm() {
        ...
        if (data.light) {
            for (const tier of LIGHT_PEN_TIERS) {
                const vals = data.light[tier];
                if (!vals) continue;
                if (!this.#working.light[tier]) this.#working.light[tier] = {};
                for (const f of LIGHT_PEN_FIELDS) {
                    if (vals[f] !== undefined) this.#working.light[tier][f] = Number(vals[f]) || 0;
                }
            }
        }
        if (data.mod) {
            for (const [tag, rows] of Object.entries(data.mod)) {
                ...
                for (const idx of Object.keys(rows).sort((a, b) => Number(a) - Number(b))) {
                    const r = rows[idx];
                    if (r.bl) {
                        // Tiered row — per-tier value map. A BLANK tier input means
                        // "no rule at this level" (omitted), which is distinct from
                        // an explicit 0; both must round-trip (a stray 0 would show
                        // as ±0 at that tier).
                        const byLevel = {};
                        for (const k of [1, 2, 3]) {
                            const raw = r.bl[k];
                            if (raw != null && String(raw).trim() !== "") byLevel[k] = Number(raw) || 0;
                        }
                        arr.push({ target: r.target, byLevel, label: r.label ?? "" });
                    } else {
                        arr.push({
                            minLevel: Math.max(1, Number(r.minLevel) || 1),
                            target: r.target,
    ...
    static async #onAddMod(event, target) {
        ...
        // New rows are TIERED (per-intensity), matching the unified rule shape —
        // three blank L1/L2/L3 boxes for the GM to fill.
        (this.#working.modifiers[tag] ??= []).push({ target: "awareness", byLevel: {}, label: "" });
        this.render();
    }
    ...
    static async #onResetTab(event, target) {
        ...
        if (tab === "light") this.#working.light = fresh.light;
    ...
    static async #onSubmit(event, form, formData) {
        ...
        for (const tag of MOD_TAGS) {
            const rows = (w.modifiers[tag] ?? []).map(serializeModRule);
            modsOut[tag] = rows;
            // Serialize the SEED the same way so an unedited table compares EQUAL
            // and the override is cleared — instead of always differing (which was
            // making a broken override stick).
            const seed = (WEATHER_MODIFIER_RULES[tag] ?? []).map(serializeModRule);
            if (JSON.stringify(rows) !== JSON.stringify(seed)) modsDiffer = true;
        }
        await game.settings.set(SYSTEM_ID, "weatherModifiersOverride", modsDiffer ? modsOut : {});

        // Light penalties: store the per-tier table only when it differs from the
        // LIGHT_TIER_PENALTY seed; an unedited table clears the override.
        const lightOut = {};
        let lightDiffer = false;
        for (const tier of LIGHT_PEN_TIERS) {
            const rec = Object.fromEntries(LIGHT_PEN_FIELDS.map(f => [f, Number(this.#working.light[tier]?.[f]) || 0]));
            lightOut[tier] = rec;
            const seed = LIGHT_TIER_PENALTY[tier] ?? {};
            if (LIGHT_PEN_FIELDS.some(f => (Number(rec[f]) || 0) !== (Number(seed[f]) || 0))) lightDiffer = true;
        }
        await game.settings.set(SYSTEM_ID, "lightPenaltiesOverride", lightDiffer ? lightOut : {});
