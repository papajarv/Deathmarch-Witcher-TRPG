...
import { isHomebrewEnabled, isCESubsystemEnabled, isCombatExtendedEnabled } from "../../api/homebrew.mjs";
...
import { hrExtraActionStaCost, hrExtraActionToHit, hrAdrenalineToTempSta } from "../../mechanics/house-rules-config.mjs";
...
import { getActiveWeatherModifiers, getEnvironmentalModifiersForActor, weatherAdjustedMoveCap } from "../../mechanics/weather-modifiers.mjs";
...
import { canThrowWeapon } from "../../mechanics/weaponThrow.mjs";
...
<footer id="wou-dock">
  ...
  <div class="bs-grid">
    ...
    <section class="dock-state combat-state">
      ...
      <div class="stats-stack">
        ...
        <div class="action-budget">
          <button type="button" class="action-btn full-round" data-action="full-round" title="${t("WITCHER.Chrome.Dock.Text.FullRoundConsumesMovementAction", "Full Round (consumes Movement + Action)")}">
            <i class="fa-solid fa-arrows-rotate"></i>
            <span class="nm">${t("WITCHER.Chrome.Dock.Label.FullRound", "Full Round")}</span>
          </button>
          <div class="action-divider" aria-hidden="true"></div>
          <div class="action-row">
            <!-- Movement is a VISUAL TRACKER, not a button — spend movement on
                 the canvas (drag / WASD / overlay). It just shows spent/cap. -->
            <div class="action-btn is-tracker" data-role="movement-tracker" title="${t("WITCHER.Dock.MovementLabel", "Movement")}">
              <i class="fa-solid fa-shoe-prints"></i>
              <span class="nm">${t("WITCHER.Chrome.Dock.Label.Movement", "Movement")}</span>
            ...
            <!-- Clinch — a SEPARATE action appended to Movement by a dash.
                 Shown only with Combat Extended on; crossed out when you're
                 already clinching, clinched, or have moved this turn (painted
                 in paintActionBudget). -->
            <span class="mov-clinch-sep" data-bind="clinch-sep" aria-hidden="true" hidden>–</span>
            <button type="button" class="action-btn clinch-btn" data-action="clinch" data-bind="clinch-btn" hidden title="${t("WITCHER.Chrome.Dock.Label.Clinch", "Clinch")}">
              <i class="fa-solid fa-hands-holding"></i>
              <span class="nm">${t("WITCHER.Chrome.Dock.Label.Clinch", "Clinch")}</span>
            </button>
            <button type="button" class="action-btn" data-action="action" title="${t("WITCHER.Chrome.Dock.Text.Action", "Action")}">
              <i class="fa-solid fa-bullseye"></i>
              <span class="nm">${t("WITCHER.Chrome.Dock.Label.Action", "Action")}</span>
            </button>
            <button type="button" class="action-btn" data-action="extra-action" title="${t("WITCHER.Chrome.Dock.Label.ExtraAction", "Extra Action")} (${hrExtraActionStaCost()} STA, ${hrExtraActionToHit() >= 0 ? "+" : ""}${hrExtraActionToHit()} to hit)">
              <i class="fa-solid fa-bolt"></i>
              <span class="nm">${t("WITCHER.Chrome.Dock.Label.ExtraAction", "Extra Action")}</span>
            </button>
          </div>
        </div>
        <div class="counters-row">
          <div class="counter" data-kind="stress" title="${t("WITCHER.Chrome.Dock.Text.Stress", "Stress")}">
            <i class="fa-solid fa-brain"></i>
            <span class="val" data-bind="c-stress-cur">0</span>
            <span class="sep">/</span>
            <span class="mx"  data-bind="c-stress-max">0</span>
          </div>
          <div class="counter" data-kind="adrenaline" title="${t("WITCHER.Chrome.Dock.Text.AdrenalineClickToSpendOnTempHP", "Adrenaline — click to spend on temp HP")}">
            <i class="fa-solid fa-bolt"></i>
            <span class="val" data-bind="c-adrenaline-cur">0</span>
            <span class="sep">/</span>
            <span class="mx"  data-bind="c-adrenaline-max">3</span>
          </div>
          <div class="counter" data-kind="shield" title="${t("WITCHER.Chrome.Dock.Text.QuenShield", "Barrier")}">
      ...
      <div class="defense-col">
        <!-- Guard-stance picker. Currently a single Balanced state with
             ...
             wired yet. -->
        <button type="button" class="guard-btn" data-action="guard-stance" data-stance="balanced" title="${t("WITCHER.Chrome.Dock.Text.SwordGuardBalanced", "Sword guard: Balanced")}">
          <i class="fa-solid fa-sword guard-sword"></i>
          <span class="nm">${t("WITCHER.Chrome.Dock.Label.Balanced", "Balanced")}</span>
        </button>
        ...
        <div class="defense-row">
          <div class="sp-figure-wrap">
            <svg class="sp-figure" viewBox="0 0 100 220" xmlns="http://www.w3.org/2000/svg" aria-label="${t("WITCHER.Chrome.Dock.Text.ArmorByBodyLocation", "Armor by body location")}">
          ...
          <div class="defense-buttons">
            <button type="button" class="defense-btn" data-action="dodge" title="${t("WITCHER.Common.Dodge", "Dodge")}">
              <i class="fa-solid fa-person-running"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Dodge", "Dodge")}</span>
            </button>
            <button type="button" class="defense-btn" data-action="reposition" title="${t("WITCHER.Chrome.Dock.Dialog.Button.Reposition", "Reposition")}">
              <i class="fa-solid fa-arrows-up-down-left-right"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Rel", "Rel")}</span>
            </button>
            <button type="button" class="defense-btn" data-action="brawl" title="${t("WITCHER.Chrome.Dock.Text.Brawl", "Brawl")}">
              <i class="fa-solid fa-hand-fist"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Brawl", "Brawl")}</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <section class="sign-tray peace-signs">
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Skills", "Skills")}"     data-action="skills">
        <i class="fa-solid fa-sitemap"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Skills", "Skills")}</span>
      </div>
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Heal", "Heal")}"       data-action="heal">
        <i class="fa-solid fa-heart-pulse"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Heal", "Heal")}</span>
      </div>
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Initiative", "Initiative")}" data-action="initiative">
        <i class="fa-solid fa-bolt"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Initiative", "Initiative")}</span>
      </div>
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Awareness", "Awareness")}"  data-action="awareness">
        <i class="fa-solid fa-eye"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Awareness", "Awareness")}</span>
      </div>
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Brawl", "Brawl")}"      data-action="brawl">
        <i class="fa-solid fa-hand-fist"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Brawl", "Brawl")}</span>
      </div>
    </section>
    <section class="sign-tray combat-signs">
      ...
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Fumble", "Fumble")}"        data-action="fumble">
        <i class="fa-solid fa-face-frown-open"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Fumble", "Fumble")}</span>
      </div>
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.StunDeathSavingThrow", "Stun / Death Saving Throw")}"  data-action="saving-throw">
        <i class="fa-solid fa-dice-d20"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.StunDeath", "Stun/Death")}</span>
      </div>
      <div class="sign" title="${t("WITCHER.Chrome.Dock.Text.Initiative", "Initiative")}"    data-action="initiative">
        <i class="fa-solid fa-bolt"></i><span class="nm">${t("WITCHER.Chrome.Dock.Label.Init", "Init")}</span>
      </div>
    </section>
...
function renderSawtooth(svg, frac, tempFrac = 0) {
  ...
  /* Build into a fragment and swap in ONE `replaceChildren` — this renders 4×
   * per dock rebind (HP + 3 combat bars), so the old `innerHTML=""` + sequential
   * appends to the LIVE svg was up to 4 separate DOM mutations each. */
...
export function getLocationSP(actor) {
  ...
  /* Monster natural SP — a single flat `system.combat.armor` value is
   * applied to every body location, matching buildNaturalArmorShape in
   * socketHook.mjs (which the attack-damage pipeline already uses).
   * Without this, DoTs that don't bypass armor (burning: 5/loc, armor
   * soaks) landed a monster's full raw damage per location because
   * every SP lookup returned 0 — the character-shape fields below
   * (armorHead / armorUpper / armorLower) don't exist on MonsterData. */
  if (actor.type === "monster") {
    const flat = Number(s.combat?.armor) || 0;
    if (flat > 0) {
      sp.head     += flat;
      sp.torso    += flat;
      sp.leftArm  += flat;
  ...
  /* Race natural armor (characters) — a full-body SP the race grants, added
   * on top of worn armor on EVERY location, never a layer, never ablated.
   * Mirrors buildNaturalArmorShape (damage pipeline) and the character
   * sheet's per-location total, so the paperdoll matches what actually soaks. */
  if (actor.type === "character") {
    let raceNat = 0;
    for (const it of (actor.items?.contents ?? actor.items ?? [])) {
      if (it.type === "race") raceNat += Number(it.system?.naturalArmorSP) || 0;
    }
    if (raceNat > 0) for (const k of SP_LOCATIONS) sp[k] += raceNat;
  }

  /* Legacy character-shape natural-armor fields (armorHead/Upper/Lower).
   * Neither CharacterData nor any current schema declares these, so this
   * loop is a no-op today — kept in case an old world carries them and
   * so custom actor flavors that DO author natural armor at these paths
   * still fold into location SP. */
  ...
  /* Worn armor: MAX-of-layers per location (matches buildArmorShape
   * in socketHook.mjs — the tougher piece defines the effective SP,
   * not the sum). Track worn SP per loc separately and merge on top
   * of any natural / legacy SP already in `sp`, since natural armor
   * DOES stack under armor (hide + worn) but two WORN pieces don't
   * stack with each other. */
  const layers = { head: [], torso: [], leftArm: [], rightArm: [], leftLeg: [], rightLeg: [] };
  const items = actor.items?.contents ?? actor.items ?? [];
  for (const a of items) {
    if (a.type !== "armor" || !a.system?.equipped) continue;
    const sys = a.system;
    /* Prefer the post-enhancement effective per-location value (folds in
     * socketed glyph / armor-mod stopping bonuses) so the paperdoll +
     * DoT soak read the same SP an incoming attack would resolve
     * against — see buildArmorShape in socketHook.mjs for the identical
     * fallback pattern. Base `{loc}Stopping` is the fallback for legacy
     * or uncovered locations where effective.stopping isn't populated. */
    const eff = sys.effective?.stopping ?? {};
    const pieceSP = {
      head:     Number(eff.head?.value     ?? sys.headStopping)     || 0,
      torso:    Number(eff.torso?.value    ?? sys.torsoStopping)    || 0,
      leftArm:  Number(eff.leftArm?.value  ?? sys.leftArmStopping)  || 0,
      rightArm: Number(eff.rightArm?.value ?? sys.rightArmStopping) || 0,
      leftLeg:  Number(eff.leftLeg?.value  ?? sys.leftLegStopping)  || 0,
      rightLeg: Number(eff.rightLeg?.value ?? sys.rightLegStopping) || 0
    };
    for (const loc of Object.keys(layers)) {
      if (pieceSP[loc] > 0) layers[loc].push(pieceSP[loc]);
    }
  }
  /* Combine layers per the active armor model: RAW buffer table (Core p.155),
   * or the Combat Extended "stronger + ¼ weaker" fold when eoArmorModel is on.
   * MUST mirror buildArmorShape (socketHook.mjs) so the paperdoll / DoT soak
   * read the same SP an incoming attack resolves against. Natural / legacy SP
   * already in `sp` stacks underneath (hide + worn), as before. */
  const _ceArmor = isCESubsystemEnabled?.("eoArmorModel");
  for (const loc of Object.keys(layers)) {
    sp[loc] += combineLayeredSPFor(layers[loc], _ceArmor);
  }
  return sp;
}
...
export const RES_TYPES = [
  { key: "bludgeoning", letter: "B", get tip() { return t("WITCHER.Chrome.Dock.Tip.BludgeoningResistance", "Bludgeoning Resistance"); } },
  { key: "slashing",    letter: "S", get tip() { return t("WITCHER.Chrome.Dock.Tip.SlashingResistance",    "Slashing Resistance");    } },
  { key: "piercing",    letter: "P", get tip() { return t("WITCHER.Chrome.Dock.Tip.PiercingResistance",    "Piercing Resistance");    } },
  { key: "fire",        letter: "F", get tip() { return t("WITCHER.Damage.Fire",      "Fire");      } },
  { key: "lightning",   letter: "L", get tip() { return t("WITCHER.Damage.Lightning", "Lightning"); } },
  { key: "cold",        letter: "C", get tip() { return t("WITCHER.Damage.Cold",      "Cold");      } },
  { key: "acid",        letter: "A", get tip() { return t("WITCHER.Damage.Acid",      "Acid");      } }
];
...
export function getResistancesForLocation(actor, loc) {
  ...
  for (const a of items) {
    if (a.type !== "armor" || !a.system?.equipped) continue;
    const eff = a.system?.effective ?? {};
    /* Gate on EFFECTIVE SP at this loc (visor-aware: a raised visor drops head
     * SP by 5), mirroring buildArmorShape so the paperdoll agrees with what
     * actually soaks. Falls back to the base field for legacy items. */
    const sp = Number(eff.stopping?.[loc]?.value ?? a.system?.[stoppingField]) || 0;
    if (!(sp > 0)) continue;
    /* Per-location EFFECTIVE resistances (visor-aware AND enhancement zone-
     * scoped) — a raised visor clears head piercing here. Fall back to the
     * piece-wide base flags only when the derivation isn't present. */
    const locRes = eff.resistancesByLoc?.[loc] ?? null;
    for (const { key } of RES_TYPES) {
      if (locRes ? locRes[key] : a.system?.[key]) found.add(key);
    }
  }
  return found;
}
...
export async function decrementArmorSP(actor, loc) {
  ...
  const items = actor.items?.contents ?? actor.items ?? [];
  /* Chip EVERY worn layer at this location, not just the first. Layered armor
   * combines into one SP (combineLayeredSP), so reducing all layers by 1 drops
   * the combined SP by 1 — the pairwise differences stay constant. Chipping a
   * single piece usually leaves the combined SP untouched (the buffer table
   * absorbs it), so DoT/bomb ablation would never degrade a stack. This mirrors
   * the attack path (damageCalculator ablates every itemId) and the spell path
   * (applyArmorAblation). Natural armor stays a fallback UNDER the worn layers:
   * it's only chipped when no worn layer had SP to take the hit. */
  let chippedWorn = false;
  for (const a of items) {
    if (a.type !== "armor" || !a.system?.equipped) continue;
  ...
  if (chippedWorn) return;
  /* Monster natural SP is a single flat `system.combat.armor` value that
   * covers every location. Without this branch, burning + acid ablation
   * fired every turn against a monster but never chipped its hide — the
   * loops below only see character-shape natural fields. The tick-effect
   * ablate loop dedupes per-monster so this drains once per turn, not
   * once per struck location (six-fold on a burning). */
  if (actor.type === "monster") {
    const cur = Number(actor.system?.combat?.armor) || 0;
    if (cur > 0) {
      await actor.update({ "system.combat.armor": Math.max(0, cur - 1) });
    }
    return;
  }
...
async function openAdrenalineChooser(actor) {
  ...
  const hasTempSta       = hrAdrenalineToTempSta();
  // No extra options available → straight to the RAW temp-HP prompt (previous
  // behavior; the chooser only appears when there's an actual choice to make).
  if (!hasFlowAndEbb && !hasLightningFast && !hasTempSta) {
    await promptAdrenalineTempHp(actor);
    return;
  }
  const buttons = [
    { action: "tempHp", label: t("WITCHER.Chrome.Dock.Dialog.Button.TempHPRollNd6", "Temp HP (roll Nd6)"), default: true }
  ];
  if (hasTempSta)       buttons.push({ action: "tempSta", label: t("WITCHER.Chrome.Dock.Dialog.Button.TempSTA", "Temp STA (6 per die)") });
  if (hasFlowAndEbb)    buttons.push({ action: "flowAndEbb", label: t("WITCHER.Chrome.Dock.Dialog.Button.FlowAndEbbRefundNVigor", "Flow and Ebb — refund N vigor") });
  if (hasLightningFast) buttons.push({ action: "lightningFast", label: t("WITCHER.Chrome.Dock.Dialog.Button.LightningFastRollNd6BonusMovement", "Lightning Fast — roll Nd6 bonus movement") });
  buttons.push({ action: "cancel", label: t("WITCHER.Common.Cancel", "Cancel") });
  let choice = null;
  try {
    choice = await DialogV2.wait({
      window: { title: tFormat("WITCHER.Chrome.Dock.Dialog.SpendAdrenalineTitle", { name: actor.name }, `${actor.name} — spend adrenaline`) },
      content: `<div style="padding:8px 0; font-size:0.8125rem;"><p>${tFormat("WITCHER.Chrome.Dock.Dialog.SpendAdrenalineBody", { name: actor.name }, `Pick how ${actor.name} spends adrenaline.`)}</p></div>`,
  ...
  if (choice === "tempSta")       { await promptAdrenalineTempSta(actor); return; }
...
async function promptFlowAndEbb(actor) {
  ...
  const dice = await DialogV2.prompt({
    window: { title: tFormat("WITCHER.Dialog.Dock.FlowEbb", { actor: actor.name }, "Flow and Ebb — {actor}") },
    content: `<div style="padding:8px 0; font-size:0.8125rem;">
      <label style="display:flex; gap:10px; align-items:center;">
        <span style="min-width:60px;">${t("WITCHER.Chrome.Dock.Text.Dice", "Dice")}</span>
        <input type="number" name="d" min="1" max="${pool}" step="1" value="1" autofocus />
      </label>
      <p class="hint" style="opacity:0.7; margin-top:6px;">${t("WITCHER.Chrome.Dock.Text.Refund1VigorPerAdrenalineDieSpentCappedA", "Refund 1 vigor per adrenaline die spent (capped at vigor you've poured out this round). Costs 1 adrenaline + STA per die.")}</p>
    </div>`,
    ok: { label: t("WITCHER.Chrome.Dock.Dialog.Button.Spend", "Spend"), callback: (_e, b) => Number(b.form?.elements?.d?.value) || 0 },
    rejectClose: false
  }).catch(() => null);
...
async function promptLightningFast(actor) {
  ...
  if (pool <= 0) { ui.notifications?.info(t("WITCHER.Notify.Dock.NoAdrenaline", "No adrenaline to spend.")); return; }
  const dice = await DialogV2.prompt({
    window: { title: tFormat("WITCHER.Dialog.Dock.LightningFast", { actor: actor.name }, "Lightning Fast — {actor}") },
    content: `<div style="padding:8px 0; font-size:0.8125rem;">
      <label style="display:flex; gap:10px; align-items:center;">
        <span style="min-width:60px;">${t("WITCHER.Chrome.Dock.Text.Dice", "Dice")}</span>
        <input type="number" name="d" min="1" max="${pool}" step="1" value="1" autofocus />
      </label>
      <p class="hint" style="opacity:0.7; margin-top:6px;">${t("WITCHER.Chrome.Dock.Text.RollNd6AndAddTheTotalToThisRoundSMovemen", "Roll Nd6 and add the total to this round's movement cap. Costs 1 adrenaline + STA per die.")}</p>
    </div>`,
    ok: { label: t("WITCHER.Chrome.Dock.Dialog.Button.Spend", "Spend"), callback: (_e, b) => Number(b.form?.elements?.d?.value) || 0 },
    rejectClose: false
  }).catch(() => null);
...
async function promptAdrenalineTempHp(actor) {
  ...
  const staPer = hasWRPerk(actor, "calmMind")
    ? Math.floor(adrenalineStaPerDieFor(actor) / 2)
    : adrenalineStaPerDieFor(actor);
  const content = `<div class="wou-adr-prompt">
    <label>${t("WITCHER.Chrome.Dock.Text.AdrenalineDiceTempHP", "Adrenaline dice → temp HP")}</label>
  ...
  const dice = await DialogV2.prompt({
    ...
    content,
    ok: { label: t("WITCHER.Common.Roll", "Roll"), callback: (_e, btn) => Number(btn.form?.elements?.dice?.value) || 0 },
    rejectClose: false
  }).catch(() => null);
  if (dice == null) return;
  ...
  try {
    ...
    await roll.toMessage({
      ...
      flavor:  tFormat("WITCHER.Chrome.Dock.Chat.AdrenalineSurge", { spend, staCost, gained }, `Adrenaline surge — spent ${spend} adrenaline (${staCost} STA) for +${gained} temp HP`)
    });
  } catch (err) {
...
/** House rule (hrAdrenalineToTempSta): spend N adrenaline → gain a FIXED 6 temp
 *  STA per die. Unlike temp HP this is NOT a roll and costs NO stamina to
 *  convert; the buffer is spent before real STA and evaporates at the start of
 *  the actor's next turn (cleared in combatRoundMixin.resetCombatRound). */
async function promptAdrenalineTempSta(actor) {
  const DialogV2 = foundry?.applications?.api?.DialogV2;
  if (!DialogV2 || !actor) return;
  if (!isAdrenalineEnabled()) return;
  const STA_PER_DIE = 6;
  ...
  if (pool <= 0) { ui.notifications?.info(t("WITCHER.Notify.Dock.NoAdrenaline", "No adrenaline to spend.")); return; }
  const content = `<div class="wou-adr-prompt">
    <label>${t("WITCHER.Chrome.Dock.Text.AdrenalineDiceTempSTA", "Adrenaline dice → temp STA")}</label>
    <input type="number" name="dice" min="1" max="${pool}" step="1" value="${pool}" autofocus />
    <p class="hint">${tFormat("WITCHER.Chrome.Dock.Text.AdrenalineTempStaHint", { pool, per: STA_PER_DIE }, `Spend up to ${pool} adrenaline. Each die grants ${STA_PER_DIE} temporary STA (no STA cost). Fades on your next turn.`)}</p>
  </div>`;
  const dice = await DialogV2.prompt({
    window: { title: tFormat("WITCHER.Dialog.Dock.AdrenalineToTempSta", { actor: actor.name }, "Adrenaline → Temp STA — {actor}") },
    content,
    ok: { label: t("WITCHER.Chrome.Dock.Dialog.Button.Spend", "Spend"), callback: (_e, btn) => Number(btn.form?.elements?.dice?.value) || 0 },
    rejectClose: false
  }).catch(() => null);
  if (dice == null) return;
  const spend = Math.min(pool, Math.max(0, Math.round(dice)));
  if (spend <= 0) return;
  try {
    const gained  = spend * STA_PER_DIE;
    const curTemp = Math.max(0, Number(actor.system?.derivedStats?.sta?.temp) || 0);
    await actor.update({
      "system.adrenaline.value":       Math.max(0, pool - spend),
      "system.derivedStats.sta.temp":  curTemp + gained
    });
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor }),
      content: tFormat("WITCHER.Chrome.Dock.Chat.AdrenalineTempSta", { spend, gained }, `Adrenaline surge — spent ${spend} adrenaline for +${gained} temp STA (fades next turn).`)
    });
  } catch (err) {
    console.warn("witcher-ttrpg-death-march | adrenaline temp STA failed", err);
  }
}
...
/* Hit-location labels — lazy-localized at read time so consumers can do
 * `SP_LOC_LABELS.head` and get "Tête" in a French client. */
const SP_LOC_FALLBACKS = {
  head: "Head", torso: "Torso",
  leftArm: "L. Arm", rightArm: "R. Arm",
  leftLeg: "L. Leg", rightLeg: "R. Leg"
};
const SP_LOC_LABELS = new Proxy(SP_LOC_FALLBACKS, {
  get(target, prop) {
...
function renderSPPopContent(actor, loc) {
  ...
  return `
    ...
    <div class="sp-pop-row"><span>${t("WITCHER.Chrome.Dock.Text.Damage", "Damage")}</span><b>${multLabel}</b></div>
    <div class="sp-pop-row"><span>${t("WITCHER.Chrome.Dock.Text.Resist", "Resist")}</span><span class="sp-pop-res ${hasRes ? "" : "none"}">${resHtml}</span></div>
  `;
}
...
const ACTION_ECON_TIP = () =>
  '<div class="wcu-tip">' +
    `<strong>${t("WITCHER.Chrome.Dock.Tip.YourTurn", "Your Turn")}</strong>` +
    t("WITCHER.Chrome.Dock.Tip.EachRound", "Each round you get one Movement and one Action.") +
    `<div class="wcu-tip-row"><span>${t("WITCHER.Chrome.Dock.Text.Movement", "Movement")}</span><span>${t("WITCHER.Chrome.Dock.Tip.MovementDesc", "Up to your SPD in metres")}</span></div>` +
    `<div class="wcu-tip-row"><span>${t("WITCHER.Chrome.Dock.Text.Action", "Action")}</span><span>${t("WITCHER.Chrome.Dock.Tip.ActionDesc", "Attack, cast, defend…")}</span></div>` +
    `<div class="wcu-tip-row"><span>${t("WITCHER.Chrome.Dock.Text.ExtraAction", "Extra Action")}</span><span>${t("WITCHER.Chrome.Dock.Tip.ExtraActionDesc", "3 STA, −3 to hit")}</span></div>` +
    `<div class="wcu-tip-row"><span>${t("WITCHER.Chrome.Dock.Text.FullRound", "Full Round")}</span><span>${t("WITCHER.Chrome.Dock.Tip.FullRoundDesc", "Spends Movement + Action")}</span></div>` +
    `<div class="wcu-tip-row"><span>${t("WITCHER.Chrome.Dock.Tip.Defending", "Defending")}</span><span>${t("WITCHER.Chrome.Dock.Tip.DefendingDesc", "1st free, then 1 STA each")}</span></div>` +
    `<div class="wcu-tip-flavor">${t("WITCHER.Chrome.Dock.Tip.ShakeOffFlavor", "Shake off a condition from the Action menu: “Clear a condition” (Stand, Put Out Fire…) or “End a condition — roll” (an Endurance/Physique check). Most cost 1 action.")}</div>` +
  '</div>';
...
function renderVigorBar(host, vigor, spent) {
  ...
  host.hidden = false;
  host.setAttribute("aria-label", tFormat("WITCHER.Chrome.Dock.Text.VigorXY", { lit, total }, "Vigor {lit}/{total}"));
...
function renderSpellsRow(host, actor) {
  ...
  /* Characters: only PINNED spells appear (players curate via the pin
   * toggle on the character magic subtab). Monsters: no pin curation —
   * every embedded spell/hex/ritual on the monster shows up on the dock
   * medallion row, matching how monster attacks always appear. Same
   * cast-flow for both (actor.castSpell). */
  const isMonster = actor.type === "monster";
  const castables = items.filter(i =>
  ...
  if (isMonster) {
    ...
    spells = castables.filter(i => pinSet.has(i.id));
  }

  /* Flag the container empty so the peace-mode CSS (wou-pinned-peace) can hide
   * the row entirely rather than show a bare "— no pinned spells —" out of combat. */
  const container = host.closest(".identity-combat");
  if (container) container.classList.toggle("is-empty", !spells.length);

  if (!spells.length) {
    const label = isMonster ? "— no spells —" : "— no pinned spells —";
    host.innerHTML = `<div class="spells-empty">${label}</div>`;
    return;
  }
  ...
  for (const spell of spells) {
    ...
    btn.addEventListener("click", async (e) => {
      ...
      try {
        ...
        /* Snapshot combat state BEFORE the cast: an area spell's template
         * placement is async and transiently nulls game.combat, so the spend
         * afterwards would otherwise think the actor is out of combat and skip
         * the action. If they were genuinely in combat and on their turn now,
         * force the spend through once the cast resolves. */
        const forceSpend = !!(actor?._inActiveCombat && actor?._isMyTurn);
        const castAndSpend = async (forceTarget = null) => {
          const res = await actor.castSpell(spell, forceTarget ? { forceTarget } : {});
          if (!res) return;
          if (res.fullRound) {
            if (typeof actor.recordFullRound === "function") {
              await actor.recordFullRound(`Cast: ${spell.name}`, { force: forceSpend });
            }
          } else {
            await maybeSpendActionSlot(actor, `Cast: ${spell.name}`, { force: forceSpend });
          }
        };
        // Tile target-select for a single-target spell that has a RANGE but no
        // AoE template (area spells place their own template). Pick a target
        // within range → set it as the user's target (how castSpell resolves a
        // single-target spell) and cast. Cancel → cast as-is (self / touch /
        // theatre-of-mind). The overlay caps at 1× the spell's range.
        const spellTok = actor.getActiveTokens?.()?.[0] ?? null;
        const spellShape = String(spell.system?.areaShape ?? "none");
        const spellHasTemplate = ["cone", "radius", "cube", "line"].includes(spellShape);
        const spellRangeStr = String(spell.system?.range ?? "").trim();
        // "touch" / "self" are AREA SHAPES (system.areaShape), not range strings.
        const isTouchSpell   = spellShape === "touch" || /^touch$/i.test(spellRangeStr);
        const isSelfOrZero   = spellShape === "self"  || /^(self|0)$/i.test(spellRangeStr);
        // RANGED single-target spell → pick the target here (dock), capped at
        // range. TOUCH spells are deliberately NOT handled here: their melee-
        // style pick runs INSIDE castSpell after the cast dialog (see
        // castSpellMixin), so the overlay sequences like the area templates
        // rather than popping before the dialog. Self/0 cast as-is.
        if (spellTok && isTileTargetingEnabled() && !spellHasTemplate
            && !isTouchSpell && !isSelfOrZero && spellRangeStr) {
          const rangedSpell = { name: spell.name, type: "weapon", system: { weaponType: "ranged", range: spellRangeStr, qualities: [] } };
          const engaged = await beginWeaponTargeting(actor, spellTok, rangedSpell, {
            capAtRange: true,
            onPick: async (defenderActor) => {
              // Route the pick via forceDefender — NO Foundry target-lock / chevron.
              await castAndSpend(defenderActor);
            },
            onCancel: async () => { await castAndSpend(); }
          });
          if (engaged) return;
        }
        await castAndSpend();
...
function getAppliedOilForWeapon(weapon) {
  /* Ranged fall-through: if the weapon itself carries no coating, look
   * at whatever ammo it's about to fire (crossbow chambered round, bow
   * nocked round). The coating lives on the projectile per Core p.166,
   * so this is where the dock pill should read for a bow with a
   * poisoned arrow ready to loose. Melee / thrown weapons hit the
   * primary path unchanged. Same helper used elsewhere for ammo lookup
   * so the two paths can't drift. */
  let ao = weapon?.system?.appliedOil;
  if ((!ao || !ao.name) && weapon?.usesAmmo) {
    /* Chambered snapshot FIRST — a coated 1-qty bolt's source document is
     * deleted when it's loaded, so getSelectedAmmo/getLoadedAmmo can no longer
     * resolve it. reload() stashes the coating on system.loaded.appliedOil for
     * exactly this reason, so read that for a chambered round; otherwise fall
     * back to a live nocked/selected ammo (bows). */
    const snap = weapon?.system?.loaded?.appliedOil;
    if ((Number(weapon?.system?.loaded?.count) || 0) > 0 && snap?.name) {
      ao = snap;
    } else {
      const ammoAo = getLoadedAmmo(weapon)?.system?.appliedOil;
      if (ammoAo && ammoAo.name) ao = ammoAo;
    }
  }
  ...
  if (exp <= 0) {
    ...
    return { name: ao.name, effect, dur: { label: t("WITCHER.Chrome.Dock.Dialog.Button.UntilCleansed", "Until cleansed") } };
  }
...
function weaponListSig(actor) {
  ...
  return inlineSig + "||" + equipped.map(w => {
    ...
    /* Read the effective coating snapshot: weapon's own for melee /
     * thrown, or the loaded/nocked ammo's for ranged (mirrors the
     * getAppliedOilForWeapon fall-through). Including `oil?.name` in
     * the key means the sig flips when a coated ammo is loaded, when
     * the coating is cleansed on that ammo, or when the ammo changes
     * altogether — so the dock pill picks up the state without waiting
    ...
    const chargeKey = ao && Number(ao.charges) > 0
      ...
      : "";
    const nameKey = oil?.name ?? "";
    const oilKey = od
      ? `${od.label}:${Math.round(od.remaining)}:${chargeKey}:${nameKey}`
      : (nameKey ? `n${nameKey}` : "");
    ...
    /* Melee/Throw dock toggle — a flag that doesn't touch occupancy or any
     * other field already in the sig, so include it directly or the sig-skip
     * would suppress the row re-render and the toggle button would look stale
     * after a click. */
    const modeKey = `tm${w.getFlag?.("witcher-ttrpg-death-march", "dockThrowMode") ? 1 : 0}`;
    return `${w.id}:${w.name}:${occupancyOf(w)}:${oil?.name ?? ""}:${oilKey}:${ammoKey}:${relKey}:${modeKey}`;
  }).join("|");
}
...
const EXTRA_MOD_FIELD_HTML = () => `
  <div class="wdm-defense-prompt-mods"
    ...
    <label for="wdm-def-extra-mod" style="font-size:0.6875rem;letter-spacing:0.12em;text-transform:uppercase;color:#c8a878;flex:0 0 auto;">${t("WITCHER.Chrome.Dock.Text.OtherModifier", "Other modifier")}</label>
    ...
    <span style="font-size:0.6875rem;opacity:0.6;">${t("WITCHER.Chrome.Dock.Text.AppliedToDefenseRoll", "applied to the defense roll")}</span>
  </div>`;
  ...
  try {
    const action = await DialogV2.wait({
      ...
      content: `<div style="padding:0.5rem 0;display:flex;flex-direction:column;gap:0.5rem;">
          <p style="margin:0;"><strong>${t("WITCHER.Chrome.Dock.Text.Parry", "Parry")}</strong> — roll at −3; a success leaves the attacker Staggered.</p>
          <p style="margin:0;"><strong>${t("WITCHER.Chrome.Dock.Text.Block", "Block")}</strong> — roll at no penalty; absorbing the hit spends 1 SP.</p>
        </div>${EXTRA_MOD_FIELD_HTML()}`,
      buttons: [
        { action: "parry", label: t("WITCHER.Chrome.Dock.Dialog.Button.Parry3", "Parry (−3)"), default: true,
          callback: (_event, _button, dlg) => { captured = readExtraMod(dlg.element); return "parry"; } },
        { action: "block", label: t("WITCHER.Chrome.Dock.Dialog.Button.Block", "Block"),
...
async function promptDefenseModifier(label) {
  ...
  try {
    const action = await DialogV2.wait({
      ...
      buttons: [
        { action: "roll", label: tFormat("WITCHER.Chrome.Dock.Dialog.Button.RollX", { label: label }, "Roll {label}"), default: true,
...
function renderWeaponList(host, actor) {
  ...
  if (actor.type === "monster") {
    ...
    inline.forEach((attack, i) => {
      const name      = String(attack?.name || t("WITCHER.Chrome.Dock.Text.Attack", "Attack"));
      ...
      /* To-hit readout (`+N`). Attacks are skill-derived: 1d10 + governing
       * stat + the chosen skill's rank + WA. Resolve the live total off the
       * actor's CURRENT stats so the wound/dying penalty is already reflected;
       * fall back to just WA if the skill can't be read. */
      const attackToHit = (() => {
        let n = Number(attack?.weaponAccuracy) || 0;
        try {
          const v = actor?._readSkillValues?.(attack?.skill || "melee");
          if (v) n += (Number(v.statVal) || 0) + (Number(v.skillVal) || 0) + (Number(v.skillMod) || 0);
        } catch (_) { /* fall back to WA only */ }
        return n;
      })();
      const skillLabelText = `${attackToHit >= 0 ? "+" : ""}${attackToHit}`;
      const skillTip = t("WITCHER.Chrome.Dock.Tip.SkillAttackBonus", "To-hit — roll 1d10 + this (stat + weapon skill + WA; wound penalty applied automatically)");
      ...
      el.innerHTML = `
        <button type="button" class="weapon-main${exhausted ? " is-disabled" : ""}" ${exhausted ? "disabled" : ""}
                title="${escapeHTML(exhausted ? t("WITCHER.Chrome.Dock.Tip.ROFExhausted", "ROF exhausted this round.") : tFormat("WITCHER.Chrome.Dock.Tip.AttackWithROF", { name, rofLeft, rofMax }, `Attack with ${name} (ROF ${rofLeft}/${rofMax})`))}">
          <span class="weapon-icon-wrap">
            <i class="fa-solid fa-paw"></i>
          </span>
          <span class="weapon-text">
            <span class="weapon-name">${escapeHTML(name)}<span class="weapon-rof" title="${t("WITCHER.Chrome.Dock.Text.RateOfFireShotsRemainingThisRound", "Rate of Fire — shots remaining this round")}"> ROF ${rofLeft}/${rofMax}</span></span>
      ...
      el.querySelector(".weapon-main").addEventListener("click", async (e) => {
        ...
        try {
          const {
            buildMonsterVirtualWeapon,
            monsterAttackHasStamina,
            chargeMonsterAttackStamina
          } = await import("../../combat/monsterVirtualWeapon.mjs");
          /* Per-attack STA gate. Some modern stat blocks (MotR / L&L / EM)
           * print signature attacks with a listed stamina cost; refuse the
           * swing when the monster can't cover it, matching the ROF-
           * exhausted / no-actions guards above. Zero-cost attacks pass. */
          if (!monsterAttackHasStamina(actor, attack)) {
            const staCost = Number(attack?.staminaCost) || 0;
            const staCur  = Number(actor?.system?.derivedStats?.sta?.value) || 0;
            ui.notifications?.warn(
              tFormat("WITCHER.Notify.Dock.MonsterNoSta",
                { actor: actor.name, cost: staCost, cur: staCur },
                "{actor} needs {cost} STA for this attack (has {cur})."));
            return;
          }
          ...
          /* Capture combat membership BEFORE the async attack flow: weaponAttack
           * opens dialogs / canvas targeting during which `game.combat` can read
           * null transiently (same failure the spell-cast fix addressed). If we
           * re-checked isActorInActiveCombat AFTER, it could be false and the ROF
           * tick + action spend would be silently skipped. */
          const wasLiveCombatant = !!(actor?._inActiveCombat && actor?._isMyTurn);
          /* Run the swing + post-commit bookkeeping. Shared by the immediate
           * path and the canvas tile-targeting path below (onPick hands the
           * chosen defender via forceDefender). Own try/catch so a deferred
           * onPick attack — fired after the overlay resolves, outside this
           * handler's outer try — is still guarded. */
          const runInlineMonsterAttack = async (attackOpts = {}) => {
            try {
              const result = await actor.weaponAttack(vw, { monsterMode: true, ...attackOpts });
              /* Bail on a null return (no target / broken / off-turn) — no swing
               * happened, no ROF tick should be recorded. */
              if (result === null) return;
              /* Attack committed — drain the printed STA cost. Runs BEFORE the
               * ROF/action-slot bookkeeping so a mid-drain error still leaves
               * the swing counted (better to over-charge than under-count when
               * only one write can fail). */
              await chargeMonsterAttackStamina(actor, attack);
              ...
              if (wasLiveCombatant) {
                ...
                if (nextCount >= rofMax) {
                  await maybeSpendActionSlot(actor, `Attack: ${name}`, { force: true });
          ...
          /* Canvas tile-targeting — the SAME overlay PC weapons use, so monster
           * melee AND ranged natural attacks pick their target on the canvas.
           * The virtual weapon carries weaponType + range, so reach vs band
           * resolves exactly as for a PC weapon; on any failure (no token,
           * gridless, unparseable range, setting off) it returns false and we
           * fall through to the immediate attack. */
          const monTok = actor.getActiveTokens?.()?.[0] ?? null;
          if (monTok && isTileTargetingEnabled()) {
            try {
              const engaged = await beginWeaponTargeting(actor, monTok, vw, {
                onPick: (defenderActor) => runInlineMonsterAttack({ forceDefender: defenderActor })
              });
              if (engaged) return;   // the overlay owns the flow now
            } catch (err) {
              console.warn("witcher-ttrpg-death-march | monster inline tile targeting failed", err);
            }
          }
          await runInlineMonsterAttack();
        } catch (err) {
          console.warn("witcher-ttrpg-death-march | monster inline attack failed", err);
        }
      });
    ...
    /* Item-weapon attacks on a monster. A humanoid monster with a real
     * weapon dropped onto its Combat section renders as a monster-style
     * button (RAW p.153 — no Strong/Fast strike variants), sourced from the
     * weapon item's own damage/qualities/name. ROF is a per-monster override
     * stored as a flag on the item (`flags.<sys>.monsterRof`) and defaults
     * to 1. All embedded weapons/shields show regardless of `equipped` —
     * loot piles don't equip and neither do monster templates. */
    const monsterWeapons = (actor.items?.contents ?? actor.items ?? [])
      .filter(i => i.type === "weapon" || i.type === "shield");
    monsterWeapons.forEach((w) => {
      const name    = String(w.name || t("WITCHER.Chrome.Dock.Text.Weapon", "Weapon"));
      const dmg     = String(w.system?.damage || "");
      const rofMax  = Math.max(1, Number(w.getFlag?.("witcher-ttrpg-death-march", "monsterRof")) || 1);
      const key     = `weapon:${w.id}`;
      const rofUsed = Math.max(0, Number(firedThisRound?.[key]) || 0);
      const rofLeft = Math.max(0, rofMax - rofUsed);
      const qLabels = weaponQualityLabels(w) || [];
      const subline = qLabels.join(" · ");
      const exhausted = isActorInActiveCombat(actor) && rofLeft <= 0;

      /* Combat Extended — Raise Shield button on a monster's shield row, same
       * affordance the PC weapon rows carry. The dialog handles the per-round
       * slot check and head-cover → Restricted Vision side-effect; monsters
       * carry the guard schema so it persists exactly like a character. */
      const monRaiseBtn = (w.type === "shield" && isCESubsystemEnabled("raiseShield"))
        ? `<button type="button" class="weapon-raise-shield" title="${tFormat("WITCHER.Chrome.Dock.Tip.RaiseShield", { name: escapeHTML(w.name) }, `Raise ${escapeHTML(w.name)} (Combat Extended — cover hit locations equal to CV)`)}">
      ...
      el.className = "weapon-item is-monster-inline"
        + (exhausted ? " is-no-action" : "");
      el.dataset.monsterWeaponId = w.id;
      el.innerHTML = `
        ${monRaiseBtn}
        <button type="button" class="weapon-main${exhausted ? " is-disabled" : ""}" ${exhausted ? "disabled" : ""}
                title="${escapeHTML(exhausted ? t("WITCHER.Chrome.Dock.Tip.ROFExhausted", "ROF exhausted this round.") : tFormat("WITCHER.Chrome.Dock.Tip.AttackWithROF", { name, rofLeft, rofMax }, `Attack with ${name} (ROF ${rofLeft}/${rofMax})`))}">
          <span class="weapon-icon-wrap">
            <img src="${escapeHTML(w.img || "")}" alt="" style="width:100%;height:100%;object-fit:contain;" />
          </span>
          <span class="weapon-text">
            <span class="weapon-name">${escapeHTML(name)}<span class="weapon-rof" title="${t("WITCHER.Chrome.Dock.Text.RateOfFireShotsRemainingThisRound", "Rate of Fire — shots remaining this round")}"> ROF ${rofLeft}/${rofMax}</span></span>
            ${dmg || subline ? `<span class="weapon-qualities">${[
      ...
      el.querySelector(".weapon-main").addEventListener("click", async (e) => {
        ...
        /* Per-monster weapon STA gate — same rule as inline monster attacks,
         * cost stored on the weapon item as `flags.<sys>.monsterStaminaCost`
         * (per-monster, not global; two monsters holding the same sword can
         * carry different costs). Default 0 = free. */
        const wStaCost = Math.max(0, Number(w.getFlag?.("witcher-ttrpg-death-march", "monsterStaminaCost")) || 0);
        if (wStaCost > 0) {
          const wStaCur = Number(actor?.system?.derivedStats?.sta?.value) || 0;
          if (wStaCur < wStaCost) {
            ui.notifications?.warn(
              tFormat("WITCHER.Notify.Dock.MonsterNoSta",
                { actor: actor.name, cost: wStaCost, cur: wStaCur },
                "{actor} needs {cost} STA for this attack (has {cur})."));
            return;
          }
        }
        /* Capture combat membership BEFORE the async attack — game.combat can
         * transiently read null during weaponAttack's dialogs/targeting, which
         * would make the post-attack check skip the ROF tick + action spend
         * (same failure the spell-cast fix addressed). */
        const wasLiveCombatant = !!(actor?._inActiveCombat && actor?._isMyTurn);
        /* Run the swing + post-commit bookkeeping. Shared by the immediate
         * path and the canvas tile-targeting path below; own try/catch so a
         * deferred onPick attack is guarded. */
        const runMonsterWeaponAttack = async (attackOpts = {}) => {
          try {
            const result = await actor.weaponAttack(w, { monsterMode: true, ...attackOpts });
            if (result === null) return;
            /* Drain STA on committed swing (matches inline monster attack behavior). */
            if (wStaCost > 0 && actor.spendStamina) {
              try { await actor.spendStamina(wStaCost, { reason: "monster attack" }); }
              catch (err) { console.warn("witcher-ttrpg-death-march | monster weapon STA drain failed", err); }
            }
            if (wasLiveCombatant) {
              const cur = actor.getFlag("witcher-ttrpg-death-march", "monsterAttackUsed") ?? {};
              const nextCount = (Number(cur?.[key]) || 0) + 1;
              if (nextCount >= rofMax) {
                await maybeSpendActionSlot(actor, `Attack: ${name}`, { force: true });
                await actor.unsetFlag("witcher-ttrpg-death-march", "monsterAttackUsed");
              } else {
                await actor.setFlag("witcher-ttrpg-death-march", "monsterAttackUsed", { ...cur, [key]: nextCount });
              }
            }
          } catch (err) {
            console.warn("witcher-ttrpg-death-march | monster weapon attack failed", err);
          }
        };
        /* Canvas tile-targeting — a real weapon on a monster is a genuine
         * weapon item, so melee AND ranged route through the overlay exactly
         * like a PC weapon. Shields keep their immediate behavior (a shield
         * bash isn't a ranged/reach target pick). */
        const monWTok = actor.getActiveTokens?.()?.[0] ?? null;
        if (monWTok && isTileTargetingEnabled() && w.type === "weapon") {
          try {
            const engaged = await beginWeaponTargeting(actor, monWTok, w, {
              onPick: (defenderActor) => runMonsterWeaponAttack({ forceDefender: defenderActor })
            });
            if (engaged) return;   // the overlay owns the flow now
          } catch (err) {
            console.warn("witcher-ttrpg-death-march | monster weapon tile targeting failed", err);
          }
        }
        await runMonsterWeaponAttack();
      });
      /* Raise Shield (CE) — mirrors the PC weapon-row wiring. */
      const monRaise = el.querySelector(".weapon-raise-shield");
      if (monRaise) {
        monRaise.addEventListener("click", async (e) => {
          e.preventDefault(); e.stopPropagation();
          try {
            const { openRaiseShieldDialog } = await import("../../applications/raiseShieldDialog.mjs");
            await openRaiseShieldDialog(actor, w);
          } catch (err) {
            console.warn("witcher-ttrpg-death-march | monster raise shield open failed", err);
          }
        });
      }
      host.appendChild(el);
    ...
    if (!inline.length && !monsterWeapons.length) {
      host.innerHTML = `<div class="weapon-empty">— no attacks defined —</div>`;
    }
    /* Monster branch is self-contained: item-weapons already rendered above
     * as monster-style rows, so skip the PC-style Parry/Drop loop. */
    return;
  ...
  for (const w of equipped) {
    ...
    /* Fill / charge readout: for melee/thrown, read the weapon's own
     * appliedOil; for ranged with a coated round in the pipe, read the
     * loaded/nocked ammo's appliedOil — matches the fall-through in
     * getAppliedOilForWeapon so the bar and the pill agree. */
    let aoLive = w?.system?.appliedOil;
    if ((!aoLive || !aoLive.name) && w?.usesAmmo) {
      const ammoForOil = getLoadedAmmo(w);
      const ammoAo = ammoForOil?.system?.appliedOil;
      if (ammoAo && ammoAo.name) aoLive = ammoAo;
    }
    ...
    const HAND_TAG   = {
      right: t("WITCHER.Chrome.Dock.HandTag.Right", "(M)"),
      left:  t("WITCHER.Chrome.Dock.HandTag.Left",  "(O)"),
      both:  t("WITCHER.Chrome.Dock.HandTag.Both",  "(Both)"),
      quick: t("WITCHER.Chrome.Dock.HandTag.Quick", "(Q)")
    };
    const HAND_TITLE = {
      right: t("WITCHER.Chrome.Dock.HandTitle.Right", "Held in the main hand"),
      left:  t("WITCHER.Chrome.Dock.HandTitle.Left",  "Held in the off-hand"),
      both:  t("WITCHER.Chrome.Dock.HandTitle.Both",  "Two-handed — occupies both hands"),
      quick: t("WITCHER.Chrome.Dock.HandTitle.Quick", "Quick / off-hand slot")
    };
    ...
    if (usesAmmo) {
      if (w.hasChamber) {
        ...
        const title = loadedName
          ? tFormat("WITCHER.Chrome.Dock.Text.NextToFire", { name: escapeHTML(loadedName) }, "Next to fire: {name}")
          : t("WITCHER.Chrome.Dock.Text.ChamberEmpty", "Chamber empty");
        ...
        const nm  = sel ? escapeHTML(sel.name) : escapeHTML(t("WITCHER.Chrome.Dock.Text.NoAmmoInContainer", "no ammo in an equipped container"));
        const label = sel ? `${qty} · ${escapeHTML(sel.name)}` : `${qty}`;
        ammoBadge = `<span class="weapon-ammo ${qty > 0 ? "is-loaded" : "is-empty"}" title="${escapeHTML(tFormat("WITCHER.Chrome.Dock.Tip.Nocked", { name: nm }, `Nocked: ${nm}`))}"><i class="fa-solid fa-bullseye"></i> <span class="weapon-ammo-text">${label}</span></span>`;
      }
    }
    // Reload only when there's room — a full chamber hides the button.
    // Magazine model (CE): the reload button = COCK, so show it whenever the
    // weapon is un-cocked (even a FULL magazine still needs cocking) and hide
    // it once armed. RAW / classic chambers keep the room-based gate.
    const showReload = usesAmmo && w.hasChamber
        && (w.usesMagazine ? !w.isArmed : !chamberFull);
    // Magazine model (CE): a separate ＋ button feeds one bolt into the reservoir
    // (distinct from cocking). Shown when there's room + feedable ammo; disabled
    // out of actions in combat, since each fed bolt costs one action.
    const showFeed = usesAmmo && w.usesMagazine && !chamberFull
        && (w.getEligibleAmmo?.().length ?? 0) > 0;
    ...
    const mainTitle = isBroken
      ? tFormat("WITCHER.Chrome.Dock.Tip.WeaponBrokenAttack", { name: escapeHTML(w.name) }, `${escapeHTML(w.name)} is broken — repair it before attacking.`)
      : blockedByQuick
        ? t("WITCHER.Chrome.Dock.Tip.TwoHandBlockedAttack", "Two-handed weapons can't attack while a Quick item is equipped.")
        : noActions
          ? t("WITCHER.Chrome.Dock.Tip.NoActionsLeft", "No actions left this turn.")
          : tFormat("WITCHER.Chrome.Dock.Tip.AttackWithWeapon", { name: escapeHTML(w.name) }, `Attack with ${escapeHTML(w.name)}`);
    const parryTitle = isBroken
      ? tFormat("WITCHER.Chrome.Dock.Tip.WeaponBrokenDefend", { name: escapeHTML(w.name) }, `${escapeHTML(w.name)} is broken — can't parry or block.`)
      : blockedByQuick
        ? t("WITCHER.Chrome.Dock.Tip.TwoHandBlockedDefend", "Two-handed weapons can't defend while a Quick item is equipped.")
        : tFormat("WITCHER.Chrome.Dock.Tip.DefendWithWeapon", { name: escapeHTML(w.name) }, `Defend with ${escapeHTML(w.name)} — Parry or Block`);
    /* Combined gate used by both buttons. */
    const attackGate = blockedByQuick || isBroken;
    const defendGate = blockedByQuick || isBroken;
    el.innerHTML = `
      ${showReloadBar
        ? `<span class="weapon-reload-bar" title="${tFormat("WITCHER.Chrome.Dock.Tip.Reloading", { name: escapeHTML(w.name), progress: reloadProgress, needed: reloadNeeded }, `Reloading ${escapeHTML(w.name)} — ${reloadProgress}/${reloadNeeded} actions`)}">
      ...
      ${(w.type === "shield" && isCESubsystemEnabled("raiseShield"))
        ...
        : ""}
      ${canSwitchGrip
        ? `<button type="button" class="weapon-grip-mode${inTwoHandMode ? " is-two" : ""}" title="${inTwoHandMode ? tFormat("WITCHER.Chrome.Dock.Tip.WieldingTwoHanded", { name: escapeHTML(w.name) }, `Wielding ${escapeHTML(w.name)} two-handed — click to switch back to one-handed`) : tFormat("WITCHER.Chrome.Dock.Tip.WieldingOneHanded", { name: escapeHTML(w.name) }, `Wielding ${escapeHTML(w.name)} one-handed — click to switch to two-handed (Two-Hand quality bonus)`)}">
             <i class="fa-solid ${inTwoHandMode ? "fa-hands" : "fa-hand"}"></i>
             <span class="weapon-grip-mode-label">${inTwoHandMode ? "2H" : "1H"}</span>
           </button>`
        : ""}
      ${(isTileTargetingEnabled() && w.type === "weapon" && canThrowWeapon(w))
        ? (() => {
            const throwOn = !!w.getFlag?.("witcher-ttrpg-death-march", "dockThrowMode");
            return `<button type="button" class="weapon-throw-mode${throwOn ? " is-throw" : ""}" title="${
              throwOn
                ? tFormat("WITCHER.Chrome.Dock.Tip.WeaponModeThrow", { name: escapeHTML(w.name) }, `${escapeHTML(w.name)}: Throw mode — click to attack in melee instead`)
                : tFormat("WITCHER.Chrome.Dock.Tip.WeaponModeMelee", { name: escapeHTML(w.name) }, `${escapeHTML(w.name)}: Melee mode — click to throw instead`)}">
             <i class="fa-solid ${throwOn ? "fa-bullseye" : "fa-hand-fist"}"></i>
             <span class="weapon-throw-mode-label">${throwOn ? escapeHTML(t("WITCHER.Chrome.Dock.Text.ThrowMode", "Throw")) : escapeHTML(t("WITCHER.Chrome.Dock.Text.MeleeMode", "Melee"))}</span>
           </button>`;
          })()
        : ""}
      <button type="button" class="weapon-main${attackGate ? " is-disabled" : ""}" ${attackGate ? "disabled" : ""} title="${mainTitle}">
        ...
        <span class="weapon-text">
          ...
          ${(() => {
            ...
            const headTag = sr.headCovered ? ` <i class="fa-solid fa-eye-slash" title="${t("WITCHER.Chrome.Dock.Text.HeadCoveredRestrictedVisionActive", "Head covered — Restricted Vision active")}"></i>` : "";
      ...
      ${showFeed
        ? `<button type="button" class="weapon-feed-magazine${noActions ? " is-disabled" : ""}" ${noActions ? "disabled" : ""} title="${noActions ? t("WITCHER.Notify.Dock.NoActions", "No actions left this turn.") : tFormat("WITCHER.Chrome.Dock.Tip.LoadMagazine", { name: escapeHTML(w.name) }, `Load ${escapeHTML(w.name)}'s magazine`)}"><i class="fa-solid fa-plus"></i></button>`
        : ""}
      ${showReload
        ? `<button type="button" class="weapon-reload${noActions ? " is-disabled" : ""}" ${noActions ? "disabled" : ""} title="${noActions ? t("WITCHER.Notify.Dock.NoActions", "No actions left this turn.") : tFormat("WITCHER.Chrome.Dock.Tip.ReloadWeapon", { name: escapeHTML(w.name) }, `Reload ${escapeHTML(w.name)}`)}"><i class="fa-solid fa-arrows-rotate"></i></button>`
        : ""}
      ${canSheathe
    ...
    // Melee/Throw mode toggle (throwable melee weapons, CE tile-targeting on).
    // Flips a per-weapon flag; the main click opens the matching overlay and
    // preselects the attack dialog's mode. Re-renders via the item-update hook.
    const throwBtn = el.querySelector(".weapon-throw-mode");
    if (throwBtn) {
      throwBtn.addEventListener("click", async (e) => {
        e.preventDefault(); e.stopPropagation();
        try {
          const cur = !!w.getFlag?.("witcher-ttrpg-death-march", "dockThrowMode");
          await w.setFlag("witcher-ttrpg-death-march", "dockThrowMode", !cur);
        } catch (err) {
          console.warn("witcher-ttrpg-death-march | throw-mode toggle failed", err);
        }
      });
    }
    ...
    el.querySelector(".weapon-main").addEventListener("click", async (e) => {
      ...
      // Run the attack and spend the appropriate action budget. Shared by the
      // immediate path and the canvas tile-targeting path below.
      ...
      const runAttackAndSpend = async (attackOpts = {}) => {
        let res;
        try {
          if (typeof actor.weaponAttack === "function") res = await actor.weaponAttack(w, attackOpts);
        ...
        if (!res) return;
        /* Include the strike type in the label so Versatile can tell a strong
         * attack from a fast/regular one (it compares last action vs current). */
        const _atkLabel = `Attack: ${w.name} · ${res.declaration?.strike ?? "normal"}`;
        if (res.declaration?.strikeMeta?.fullRound) await actor.recordFullRound?.(`Charge: ${w.name}`, { postCharge: true });
        else if (res.declaration?.extraAction) await actor.recordExtraAction?.(_atkLabel);
        else await maybeSpendActionSlot(actor, _atkLabel);
      };

      // Canvas tile-targeting: click the weapon → highlight its range on the
      // canvas → click a tile with a target to attack it. Only for real
      // weapons on a gridded canvas with the attacker present; bombs keep
      // their own AoE template flow, and the world setting can disable this
      // entirely (falls back to the immediate attack below). The picked
      // token's actor drives weaponAttack via forceDefender.
      try {
        const isBomb = w?.system?.weaponType === "bomb";
        const token = actor.getActiveTokens?.()?.[0] ?? null;
        if (!isBomb && token && isTileTargetingEnabled()) {
          // Dual-mode (throwable melee) weapons carry a Melee/Throw toggle;
          // read it so both the overlay and the attack resolve as that mode.
          // Pure melee / ranged weapons leave mode undefined (defaults apply).
          const throwable = canThrowWeapon(w);
          const attackMode = throwable
            ? (w.getFlag?.("witcher-ttrpg-death-march", "dockThrowMode") ? "thrown" : "melee")
            : undefined;
          const engaged = await beginWeaponTargeting(actor, token, w, {
            mode: attackMode,
            onPick: (defenderActor) => runAttackAndSpend({ forceDefender: defenderActor, mode: attackMode, tileTargeting: true })
          });
          if (engaged) return;   // the overlay owns the flow now
        }
      } catch (err) {
        console.warn("witcher-ttrpg-death-march | tile targeting failed", err);
      }

      await runAttackAndSpend();
    });
    ...
    // Load Magazine button (magazine crossbows only) → feed ONE bolt into the
    // reservoir without cocking. The action cost (one per bolt in combat, free
    // out of it) is charged inside feedMagazine itself, so no accounting here —
    // just an early warn for the disabled state to keep the UX snappy.
    el.querySelector(".weapon-feed-magazine")?.addEventListener("click", async (e) => {
      e.preventDefault(); e.stopPropagation();
      if (actor.hasActionSlot === false) {
        ui.notifications?.warn(t("WITCHER.Notify.Dock.NoActions", "No actions left this turn."));
        return;
      ...
      try {
        await feedMagazineWithPrompt(w);
      } catch (err) {
        console.warn("witcher-ttrpg-death-march | magazine load failed", err);
      }
    });
...
async function runBrawlAction(actor, forceDefender = null) {
  if (typeof actor?.brawlAttack !== "function") return;
  let res = null;
  try { res = await actor.brawlAttack(forceDefender ? { forceDefender } : {}); }
  ...
  try {
    ...
    else if (res.declaration?.actionMeta?.fullRound) await actor.recordFullRound?.(brawlLabel, { postCharge: !!actor.statuses?.has?.("charging") });
    else if (res.declaration?.extraAction) await actor.recordExtraAction?.(t("WITCHER.Chrome.Dock.Label.Brawl", "Brawl"));
...
/** Brawl with the SAME tile target-select as a weapon attack: paint the reach
 *  (unarmed melee = 1 tile), pick a target on the canvas, and resolve the brawl
 *  against it via forceDefender — honouring the clinch/pin hold-lock exactly
 *  like `beginWeaponTargeting` does for weapons. Falls back to an untargeted
 *  brawl only when tile targeting is off. Shared by the combat-state Brawl
 *  button AND the peace-tray Brawl sign so the two can't diverge. */
async function runBrawlWithTargeting(actor) {
  const brawlTok = actor?.getActiveTokens?.()?.[0] ?? null;
  if (brawlTok && isTileTargetingEnabled()) {
    const unarmed = { name: "Brawl", type: "weapon", system: { weaponType: "melee", range: "", qualities: [] } };
    const engaged = await beginWeaponTargeting(actor, brawlTok, unarmed, {
      onPick: async (defenderActor) => { await runBrawlAction(actor, defenderActor); },
      /* No fallback attack — a cancelled target-select just closes the overlay
       * (mirrors melee weapons); you MUST click a valid target to fire. */
      onCancel: async () => {}
    });
    if (engaged) return;
  }
...
async function runPeaceSignAction(action, actor) {
  switch (action) {
    case "brawl": {
      await runBrawlWithTargeting(actor);
      return;
    }
    case "sober-up": {
    ...
    case "initiative": {
      // Refuse when the actor is already in the active combat. This button
      // rolls an actor INTO combat; re-clicking it on a combatant re-rolls
      // their slot and reshuffles the order mid-fight. Re-rolls belong to the
      // combat tracker, not the peace-mode dock.
...
/** Roll a skill as a BLIND roll (GM sees the result; players see only that
 *  a blind roll was made). Threads v14's `messageMode: "blind"` through
 *  extendedRoll → ChatMessage.create — Foundry then auto-whispers to GMs
 *  and hides dice results from other clients. No hook interception
 *  needed; the older Hooks.once("preCreateChatMessage") pattern was
 *  fragile against any unrelated chat message firing between the hook
 *  register and rollSkill's card (e.g. an AE-tick card mid-roll would
 *  hijack the tag). */
async function rollBlindSkill(actor, skill, opts = {}) {
  if (typeof actor.rollSkill !== "function") return;
  return actor.rollSkill(skill, { ...opts, messageMode: "blind" });
}

/** Awareness from the dock: opens a compact prompt that SHOWS the modifiers
 *  currently in effect on the actor's Perception — the live weather + light
 *  penalties for where they stand (fog, rain, darkness, snow blindness, …),
 *  auto-applied and read-only — plus a free custom-modifier field. Everything
 *  shown folds into the (blind) skill roll; there are no opt-in toggles. Falls
 *  back to a plain blind roll if DialogV2 is unavailable.
 *
 *  Light + snow-blindness are positional / self-gating, so they always apply;
 *  plain weather penalties (fog/rain/storm) are dropped when the actor is
 *  sheltered (indoor / weather-off scene), matching the exposure model. */
async function rollAwarenessWithModifiers(actor) {
  const DialogV2 = foundry?.applications?.api?.DialogV2;
  const sign = (n) => (n >= 0 ? `+${n}` : `${n}`);

  // The env penalties in effect on this actor's Awareness — shown READ-ONLY. The
  // skill roll (rollSkillCheck) auto-applies these itself, so the dialog only
  // collects a CUSTOM modifier and must not pass them again (or they'd double).
  let mods = [];
  try {
    mods = getEnvironmentalModifiersForActor(actor, "awareness")
      .map(m => ({ label: game.i18n.localize(m.label), value: Number(m.value) || 0 }));
  } catch (_) { /* weather/calendar not ready */ }

  // Roll blind, passing ONLY the custom modifier (env mods are auto-applied).
  const roll = (manual) => rollBlindSkill(actor, "awareness",
    manual ? { situational: manual, situationalParts: [{ label: t("WITCHER.Chrome.Dock.Dialog.Button.Situational", "Situational"), value: manual }] } : {});

  if (!DialogV2) return roll(0);

  const rows = mods.length
    ? mods.map(m => `
        <div class="wou-skillmod-row">
          <span class="wou-skillmod-name">${escapeHTML(m.label)}</span>
          <span class="wou-skillmod-val">${sign(m.value)}</span>
        </div>`).join("")
    : `<p class="wou-skillmod-empty">${t("WITCHER.Chrome.Dock.Text.NoWeatherOrLightConditionsAffectAwarenes", "No weather or light conditions affect Awareness right now.")}</p>`;

  const content = `
    <div class="wou-skillmod-prompt">
      <div class="wou-skillmod-head">${t("WITCHER.Chrome.Dock.Text.ModifiersInEffect", "Modifiers in effect")}</div>
      ${rows}
      <label class="wou-skillmod-manual">
        <span>${t("WITCHER.Chrome.Dock.Text.OtherSituationalModifier", "Other situational modifier")}</span>
  ...
  const result = await DialogV2.prompt({
    ...
    ok: {
      ...
      callback: (_e, btn) => ({ manual: Number(btn.form.elements.manual?.value) || 0 })
    },
    rejectClose: false
  }).catch(() => null);

  if (result == null) return;   // cancelled
  return roll(result.manual);
}
...
function showOilPopover(bar) {
  ...
  const timeLine = label
    ? tFormat("WITCHER.Chrome.Dock.Text.LabelRemaining", { label }, `${label} remaining`)
    : t("WITCHER.Chrome.Dock.Text.ActiveUntilCleansed", "active until cleansed");
  pop.innerHTML =
...
const ACTION_OPTIONS = () => [
  { key: "skill",  icon: "fa-hand",           label: t("WITCHER.Chrome.Dock.Dialog.Button.UseASkill", "Use a Skill") },
  { key: "draw",   icon: "fa-hand-fist",      label: t("WITCHER.Chrome.Dock.Dialog.Button.DrawPickUpItem", "Draw / Pick Up Item") },
  { key: "verbal", icon: "fa-comments",       label: t("WITCHER.Chrome.Dock.Dialog.Button.InitiateVerbalCombat", "Initiate Verbal Combat") }
];
// Full-round options (Core p.152) — use the whole turn (lock all three slots).
const FULL_ROUND_OPTIONS = () => [
  { key: "run",      icon: "fa-person-running", label: t("WITCHER.Chrome.Dock.Dialog.Button.RunSPD3", "Run (SPD×3)") },
  { key: "charge",   icon: "fa-person-rays",    label: t("WITCHER.Chrome.Dock.Dialog.Button.ChargeSPD3MoveStrongStrike", "Charge (SPD×3 move + strong strike, -3 to hit)") },
  { key: "tackle",   icon: "fa-person-falling", label: t("WITCHER.Chrome.Dock.Dialog.Button.TackleSPD3", "Tackle (SPD×3 move + opposed Physique smash)") },
  { key: "dodge",    icon: "fa-shield-halved",  label: t("WITCHER.Chrome.Dock.Dialog.Button.ActivelyDodge", "Actively Dodge") },
  { key: "aim",      icon: "fa-crosshairs",     label: t("WITCHER.Chrome.Dock.Dialog.Button.Aim1RoundMax3", "Aim (+1/round, max +3)") },
  { key: "recovery", icon: "fa-lungs",          label: t("WITCHER.Chrome.Dock.Dialog.Button.RecoveryActionRegainRECSTA", "Recovery Action (regain REC STA)") }
];
...
/** Tackle full-round flow. Mirrors openChargeFlow: grant SPD×3 movement and set
 *  the `tackling` status. The MELEE attack dialog then restricts to Tackle, and
 *  weaponAttack resolves it as an opposed-Physique smash (win → both prone, lose
 *  → only you) + strips the status once the strike commits. */
async function openTackleFlow(actor) {
  if (!actor) return;
  if (actor._inActiveCombat) {
    try {
      await actor.update({ "system.combatRound.runUsed": true });
    } catch (err) {
      console.warn("witcher-ttrpg-death-march | tackle run-budget grant failed", err);
      return;
    }
  }
  try {
    if (!actor.statuses?.has?.("tackling")) {
      await actor.toggleStatusEffect?.("tackling", { active: true });
    }
    ui.notifications?.info?.(
      `${actor.name} is tackling — SPD×3 movement + next melee attack is an opposed-Physique smash.`
    );
  } catch (err) {
    console.warn("witcher-ttrpg-death-march | tackling status apply failed", err);
  }
}
...
async function maybeSpendActionSlot(actor, label, { force = false } = {}) {
  // `force` lets a caller that pre-verified combat membership (before an async
  // cast that transiently nulls game.combat) spend anyway; otherwise gate live.
  if (!actor || (!force && !isActorInActiveCombat(actor))) return;
  if (typeof actor.spendActionSlot !== "function") return;
  try { await actor.spendActionSlot(label, { force }); }
...
async function openActionMenu(actor, slot) {
  ...
  if (slot === "action" || slot === "extra") {
    ...
    const actionOpts = isHeld
      ? [{ key: "escape", icon: "fa-arrow-right-from-bracket", label: t("WITCHER.Chrome.Dock.Dialog.Button.EscapeDodgeEscapeVsBrawling", "Escape (Dodge/Escape vs Brawling)") }, ...ACTION_OPTIONS()]
      : ACTION_OPTIONS();
    sections.push({
      group: "normal",
      title: slot === "extra" ? t("WITCHER.Chrome.Dock.Text.ExtraActionCost", "Extra Action — 3 STA, at −3") : t("WITCHER.Chrome.Dock.Text.Action", "Action"),
      opts: actionOpts
    });
    ...
    const clearOpts = selfClearOptions().map(o => actor.statuses?.has?.(o.id)
      ...
      : { key: `clear:${o.id}`, icon: o.icon, label: o.label, info: tFormat("WITCHER.Chrome.Dock.Info.RequiresCondition", { status: o.statusName }, `Requires the ${o.statusName} condition`) });
    ...
    // Visor raise/lower — helms with the CE `visor` quality. One cell per
    // equipped such helm; with MORE THAN ONE the cell names the helm so the
    // player knows which they're toggling. Costs the slot like any action.
    const visorHelms = (actor.items?.contents ?? actor.items ?? [])
      .filter(it => it?.type === "armor" && it.system?.equipped && hasVisorQuality(it));
    if (visorHelms.length) {
      const multi = visorHelms.length > 1;
      const visorOpts = visorHelms.map(it => {
        const up = isVisorRaised(it);   // raised → next action lowers it; down → raises
        const verb = up
          ? t("WITCHER.Chrome.Dock.Label.LowerVisor", "Lower Visor")
          : t("WITCHER.Chrome.Dock.Label.RaiseVisor", "Raise Visor");
        // Raise (visor down → opening it, you regain sight) = eye; Lower
        // (visor up → closing the mask down over the face) = mask.
        return { key: `visor:${it.id}`, icon: up ? "fa-mask" : "fa-eye", label: multi ? `${verb}: ${it.name}` : verb };
      });
      sections.push({ group: "visor", title: t("WITCHER.Dialog.Dock.Visor", "Visor"), opts: visorOpts });
    }
  }
  if (slot === "full") {
    ...
    // Temp STA (house-rule buffer) is spendable, so it un-stuns — mirror the
    // _stunned getter (value + temp) so the menu doesn't grey out actions the
    // player can actually afford.
    const staTemp = Math.max(0, Number(actor.system?.derivedStats?.sta?.temp) || 0);
    const stunned = staMax > 0 && (staVal + staTemp) === 0;
    // Being choked chokes off your breath — you can't catch it back. Recovery
    // is greyed while the actor carries the `chokeheld` status.
    const choked = actor.statuses?.has?.("chokeheld") === true;
    const fullOpts = FULL_ROUND_OPTIONS().map(o => {
      if (o.key === "recovery" && choked)
        return { ...o, info: t("WITCHER.Chrome.Dock.Text.ChokedNoRecovery", "You can't recover while you're being choked.") };
      if (stunned && o.key !== "recovery")
        return { ...o, info: t("WITCHER.Chrome.Dock.Text.StunnedRecoverFirst", "Stunned at 0 STA — recover first") };
  ...
  await DialogV2.wait({
    ...
    content,
    buttons: [{ action: "close", label: t("WITCHER.Common.Cancel", "Cancel"), default: true }],
    rejectClose: false,
        ...
        btn.addEventListener("click", async (ev) => {
          ...
          try {
            if (btn.dataset.group === "clear") {
              ...
              else if (btn.dataset.key === "tackle") {
                /* Like Charge — arm the Tackle: grant SPD×3 movement + set the
                 * `tackling` status so the next MELEE attack's dialog restricts
                 * to Tackle (an opposed-Physique smash). Close the menu first. */
                dlg?.close?.();
                await openTackleFlow(actor);
                return;
              }
              ...
              if (ability) {
                const name = String(ability.name || t("WITCHER.Chrome.Dock.Text.SpecialAbility", "Special Ability")).trim();
            ...
            } else if (btn.dataset.group === "visor") {
              // Spend the slot first; only flip the visor if it was spent (a
              // failed spend in combat leaves the helm as-is). Out of combat
              // recordAction is free, so it just toggles.
              const item = actor.items.get(btn.dataset.key.slice("visor:".length));
              if (item) {
                const spent = slot === "extra"
                  ? await actor.recordExtraAction(label)
                  : await actor.recordAction(label);
                if (spent) await item.update({ "system.visorRaised": !isVisorRaised(item) });
              }
...
/** Start a clinch from the dock's Clinch button. Picks a target via the
 *  click-to-clinch overlay (tokens within 1 tile; falls back to the user's
 *  manual target), then applies the clinch — which consumes the whole
 *  movement (see holdLink _doApplyHoldLink). Guards re-check the crossed
 *  conditions (already clinched, or moved this turn) in case the painted
 *  button state is momentarily stale. */
async function startClinch(actor) {
  if (!actor) return;
  const cr = actor.system?.combatRound ?? {};
  const moved = !!cr.movementUsed || (Number(cr.movementMeters) || 0) > 0;
  if (moved) {
    ui.notifications?.warn(t("WITCHER.Notify.Dock.ClinchMovementSpent", "Clinch requires your full unspent movement action — you've already moved this turn."));
    return;
  }
  if (actor.statuses?.has?.("clinched")) {
    ui.notifications?.warn(t("WITCHER.Notify.Dock.ClinchAlreadyClinched", "You can't clinch while you're clinched — break free first."));
    return;
  }
  try {
    const { isInGrappleFamily } = await import("../../mechanics/holdModifiers.mjs");
    if (isInGrappleFamily(actor)) {
      ui.notifications?.warn(t("WITCHER.Notify.Dock.ClinchInGrapple", "You can't clinch while in a grapple — break free first."));
      return;
    }
  } catch (_) { /* registry unreachable — allow */ }
  let target = null;
  try {
    const { showClinchOverlay } = await import("../../mechanics/clinch.mjs");
    const picked = await showClinchOverlay(actor);
    if (picked === undefined) target = [...(game.user?.targets ?? [])][0]?.actor ?? null;
    else { if (picked === null) return; target = picked; }
  } catch (_) {
    target = [...(game.user?.targets ?? [])][0]?.actor ?? null;
  }
  if (!target) {
    ui.notifications?.warn(t("WITCHER.Notify.Dock.ClinchNeedsTarget", "Clinch requires a single targeted token."));
    return;
  }
  try {
...
async function promptMovement(actor) {
  ...
  const clinchBlockedTooltip = movementAlreadySpent
      ? ` title="${t("WITCHER.Chrome.Dock.Text.YouVeAlreadySpentMovementThisTurnClinchR", "You've already spent movement this turn — clinch requires your full unspent movement action.")}"`
      : "";
  ...
  const breakClinchRadio = breakClinchAvailable
      ? `<label><input type="radio" name="moveAction" value="breakClinch"> ${t("WITCHER.Chrome.Dock.Text.BreakClinchStepOutCosts1m", "Break clinch (step out — costs 1m)")}</label>`
      : "";
  ...
  const walkRadio = showWalkOption
      ? `<label><input type="radio" name="moveAction" value="walk" checked> ${t("WITCHER.Chrome.Dock.Text.WalkRun", "Walk / Run")}</label>`
      : "";
  ...
  const actionRadios = showRadios
      ? `<fieldset class="wou-move-actions"><legend>${t("WITCHER.Chrome.Dock.Text.MoveAction", "Move action")}</legend>
           ${walkRadio}
  ...
  const repositionNote = spd > 0
      ? `<p class="hint">${t("WITCHER.Chrome.Dock.Text.RepositionDefensive", "Reposition (defensive):")} <strong>${repositionPrior}</strong> / ${spd}m used this round. Half-SPD (${Math.floor(spd/2)}m) per defense; total capped by SPD.</p>`
      : "";
  ...
  const result = await DialogV2.prompt({
    ...
    content,
    ok: { label: t("WITCHER.Common.Confirm", "Confirm"), callback: (_e, btn) => ({
    ...
    render: (_e, dlg) => {
        ...
        const setMetersState = () => {
            ...
            if (picked === "clinch") {
                ...
                metersInput.title = t("WITCHER.Dialog.Dock.ClinchLocked", "Clinch consumes your full movement action — distance is not chosen.");
            } else if (picked === "breakClinch") {
                metersInput.disabled = true;
                metersInput.classList.add("is-blocked");
                metersInput.title = t("WITCHER.Dialog.Dock.BreakClinchLocked", "Break clinch costs 1 metre of movement.");
            } else {
  ...
  if (moveAction === "clinch") {
    ...
    /* Target selection: a click-to-clinch overlay of every token within one
     * tile of the actor. Falls back to the user's manual target when there's
     * nothing in reach to highlight. `null` = the user cancelled the overlay
     * (Esc / empty click) → abort quietly. */
    let target = null;
    try {
        const { showClinchOverlay } = await import("../../mechanics/clinch.mjs");
        const picked = await showClinchOverlay(actor);
        if (picked === undefined) {
            ...
            if (picked === null) return;   // cancelled — no warning
            target = picked;
        }
    } catch (_) {
        target = [...(game.user?.targets ?? [])][0]?.actor ?? null;
...
function paintActionBudget(dock, cr, inCombat = false, sta = null, opts = {}) {
  ...
  const OFF_TURN_TITLE = t("WITCHER.Chrome.Dock.Text.OffTurnTitle", "Not your turn — wait for the active combatant.");
  ...
  const STUN_TITLE = t("WITCHER.Chrome.Dock.Text.StunTitle", "Stunned at 0 STA — only a Recovery action is available");
  ...
  if (full) {
    ...
    full.title = offTurn ? OFF_TURN_TITLE
      : c.fullRound ? (c.fullRoundLabel || t("WITCHER.Chrome.Dock.Text.FullRound", "Full Round"))
      : dirty
        ? t("WITCHER.Chrome.Dock.Text.FullRoundUnavailable", "Full Round unavailable — you've already moved or acted this turn")
        : t("WITCHER.Chrome.Dock.Text.FullRoundHint",        "Full Round (uses your whole turn — locks all three)");
  }
  const move = budget.querySelector('.action-btn[data-role="movement-tracker"]');
  if (move) {
    ...
    // Weather footing (wind/rain/snow) trims the cap, floored at half speed —
    // computed by the caller (actor in scope) and passed via opts.moveCap.
    const cap     = Number.isFinite(opts.moveCap) ? opts.moveCap : spd * runMul + lfBonus;
    ...
    move.title = offTurn ? OFF_TURN_TITLE
      ...
      : c.movementUsed
        ...
        : moveLockedByAction
          ? t("WITCHER.Chrome.Dock.Text.MovementUnavailable", "Movement unavailable — you've already acted this turn (enable Split Movement to interleave)")
          : moved > 0
            ? tFormat("WITCHER.Chrome.Dock.Text.MovedSoFar", { m: moved }, "Moved {m}m so far (split movement)")
            : t("WITCHER.Chrome.Dock.Text.MovementHint", "Movement (declare distance, up to SPD)");
  }
  /* Clinch — separate CE-only button appended to Movement by a dash. Shown
   * only with Combat Extended on (opts.clinchShow); crossed out (inert) when
   * already clinching / clinched / moved this turn (opts.clinchCrossed,
   * computed by the caller where the actor is in scope). */
  const clinchBtn = budget.querySelector('[data-bind="clinch-btn"]');
  const clinchSep = budget.querySelector('[data-bind="clinch-sep"]');
  if (clinchBtn) {
    const show = !!opts.clinchShow;
    clinchBtn.hidden = !show;
    if (clinchSep) clinchSep.hidden = !show;
    if (show) {
      const crossed = !!opts.clinchCrossed;
      clinchBtn.classList.toggle("is-crossed", crossed);
      clinchBtn.title = crossed
        ? (opts.clinchReason || t("WITCHER.Chrome.Dock.Text.ClinchUnavailable", "Clinch unavailable — you're already clinching, clinched, or have moved this turn"))
        : t("WITCHER.Chrome.Dock.Text.ClinchHint", "Clinch an adjacent foe — consumes your whole movement action");
    }
  }
  ...
  if (act) {
    ...
    act.title = offTurn ? OFF_TURN_TITLE
      : stunned ? STUN_TITLE
      : c.actionUsed ? (c.actionLabel || t("WITCHER.Chrome.Dock.Text.ActionUsed", "Action used")) : t("WITCHER.Chrome.Dock.Text.Action", "Action");
  }
  ...
  if (extra) {
    ...
    extra.title = offTurn ? OFF_TURN_TITLE
      : stunned ? STUN_TITLE
      : c.extraUsed
        ? (c.extraLabel || t("WITCHER.Chrome.Dock.Text.ExtraActionUsed", "Extra action used"))
        : extraLocked
          ? t("WITCHER.Chrome.Dock.Text.ExtraActionLocked", "Use your action first — the extra action is a second action (3 STA, at −3)")
          : t("WITCHER.Chrome.Dock.Text.ExtraActionAvailable", "Extra Action (3 STA, at −3)");
  }
...
function wireActionButtons(dock, actor) {
  ...
  budget.querySelectorAll(".action-btn").forEach(btn => {
    ...
    clone.addEventListener("click", async (e) => {
      ...
      /* Movement is a tracker now (no data-action) — spend it on the canvas. */
      if (action === "action")            await openActionMenu(actor, "action");
        ...
        /* Crossed = in a grapple / clinch, or moved this turn → inert. Don't
         * silently swallow the click — surface WHY (the button's title is the
         * reason string) so it never reads as "nothing happened". */
        if (clone.classList.contains("is-crossed")) {
          ui.notifications?.info(clone.title || t("WITCHER.Chrome.Dock.Text.ClinchUnavailable", "Clinch unavailable this turn."));
          return;
        }
        await startClinch(actor);
      }
    });
  });
}
...
export function rebindDock() {
  ...
  /* getLocationSP folds in the combined CURRENT SP (so current-SP edits +
   * ablation already move the sig). Append the raw per-location current + MAX
   * stopping of every equipped armor so a MAX-only edit (which doesn't change
   * the combined current SP) still triggers a rebind — layer membership and the
   * per-zone readout depend on max. Otherwise the paperdoll stayed stale until
   * a re-equip. */
  const _armorSpRaw = (actor?.items?.contents ?? actor?.items ?? [])
    .filter(i => i.type === "armor" && i.system?.equipped)
    .map(i => {
      const s = i.system ?? {};
      return `${i.id}:` + ["head", "torso", "leftArm", "rightArm", "leftLeg", "rightLeg"]
        .map(l => `${s[`${l}Stopping`] ?? ""}/${s[`${l}MaxStopping`] ?? ""}`).join(",");
    }).join(";");
  const armorSig = JSON.stringify(getLocationSP(actor)) + "|" + _armorSpRaw;
  /* Spell-row sig: for characters this is the pinned list (empty when
   * unpinned). For monsters we surface EVERY embedded castable, so
   * signature must fold in the actor's spell-item ids so drop/delete
   * of a monster spell triggers a rebind. */
  const pinSig   = actor?.type === "monster"
    ? JSON.stringify((actor.items?.contents ?? actor.items ?? [])
        .filter(i => i.type === "spell" || i.type === "hex" || i.type === "ritual")
        .map(i => i.id).sort())
    : JSON.stringify(actor?.flags?.["witcher-ttrpg-death-march"]?.pinnedSpells ?? []);
  ...
  // Computed here where the actor is in scope (paintActionBudget only gets the cr).
  const cr0     = data.combatRound ?? {};
  const cr0spd  = Number(cr0.spd) || 0;
  const cr0mul  = cr0.runUsed ? 3 : 1;
  const cr0bon  = Number(cr0.lightningFastBonus) || 0;
  const moveCap = actor ? weatherAdjustedMoveCap(cr0spd, cr0mul, cr0bon, actor) : cr0spd * cr0mul + cr0bon;
  /* Clinch button (CE-only): crossed out when the actor is in a grapple or
   * clinch (by status — see below), or has moved at all this turn (movement
   * spent forfeits the clinch action). */
  const clinchShow    = !!actor && isCombatExtendedEnabled();
  const movedThisTurn = !!cr0.movementUsed || (Number(cr0.movementMeters) || 0) > 0;
  let clinchCrossed   = false;
  let clinchReason    = "";
  if (clinchShow) {
    /* Greying is driven PURELY by the actor's own statuses — NOT the hold
     * registry. Statuses are folded into the rebind signature (statusSig)
     * and each apply/clear fires create/deleteActiveEffect → an immediate
     * dock rebind, so the greyed state tracks the hold live and CLEARS the
     * moment the hold ends. The registry, by contrast, lives on a hidden
     * per-world registry actor whose flag writes never rebind this dock, so
     * a registry-based check would leave the button stale-greyed after a
     * hold cleared — and a stale is-crossed button silently eats the click.
     * Both ROLES are covered by their status:
     *   target side  → grappled / pinned / chokeheld / clinched
     *   holder side  → isGrappling / isPinning / isChoking
     * The clinch AGGRESSOR carries no status but has spent their movement,
     * so `movedThisTurn` already greys them. */
    const has = (s) => { try { return !!actor.statuses?.has?.(s); } catch (_) { return false; } };
    /* clinched = you're being clinched; isClinching = you're the aggressor.
     * BOTH sides now carry a status, so either role greys the button. */
    const inClinch  = has("clinched") || has("isClinching");
    const inGrapple = has("grappled") || has("pinned") || has("chokeheld")
                   || has("isGrappling") || has("isPinning") || has("isChoking");
    clinchCrossed = movedThisTurn || inClinch || inGrapple;
    /* Grey it out WITH the specific reason (mirrors the other crossed-out
     * dock/brawl actions) — grapple is checked before the clinched case so
     * a pinned/chokeheld actor sees the grapple reason, not the generic one. */
    if (inGrapple)          clinchReason = t("WITCHER.Notify.Dock.ClinchInGrapple", "You can't clinch while in a grapple — break free first.");
    else if (inClinch)      clinchReason = t("WITCHER.Notify.Dock.ClinchAlreadyClinched", "You can't clinch while you're clinched — break free first.");
    else if (movedThisTurn) clinchReason = t("WITCHER.Chrome.Dock.Text.ClinchUnavailable", "Clinch unavailable — you're already clinching, clinched, or have moved this turn");
  }
  paintActionBudget(dock, data.combatRound, inCombat, data.sta, { offTurn, moveCap, clinchShow, clinchCrossed, clinchReason });
  ...
  dock.querySelectorAll('.defense-btn').forEach(btn => {
    ...
    clone.addEventListener("click", async (e) => {
      ...
      if (action === "brawl") {
        // Same tile target-select as a weapon (unarmed melee reach = 1 tile),
        // via the shared helper so the combat button and the peace-tray sign
        // behave identically. In token combat you MUST click a valid target to
        // fire; cancelling closes the overlay without an untargeted attack.
        await runBrawlWithTargeting(actor);
        return;
      }

      // Dodge and Reposition are first-class defensive actions (not generic
      // skill rolls): defendBySkill posts a t("WITCHER.Common.Dodge", "Dodge")/"Reposition — defense" card
      ...
      const def = action === "dodge" ? { skill: "dodge", label: t("WITCHER.Common.Dodge", "Dodge") }
                : action === "reposition" ? { skill: "athletics", label: t("WITCHER.Chrome.Dock.Dialog.Button.Reposition", "Reposition") }
                : null;
