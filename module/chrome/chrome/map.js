...
export function injectMapPanel() {
  ...
  el.setAttribute("aria-label", t("WITCHER.Chrome.Map.Text.Map", "Map"));
  document.body.appendChild(el);
  panelEl = el;

  if (!hooksWired) {
    /* Bind to the VIEWED actor (getPanelActor honors GM "View as" / "Lock as")
     * so GM-side re-renders fire on the impersonated player's item
     * changes, not just on the GM's own character. */
    const viewedId = () => getPanelActor("map")?.id;
    const ownsItem = (it) => it?.parent?.id === viewedId();
    Hooks.on("updateUser",  (u) => { if (u.id === game.user.id)               rerenderIfOpen(); });
    Hooks.on("updateActor", (a) => { if (a.id === viewedId())                 rerenderIfOpen(); });
    ...
    Hooks.on(PANEL_OVERRIDE_HOOK, (key) => { if (key === "map") rerenderIfOpen(); });
...
function positionBounds() {
  ...
  panelEl.style.top = `calc(${top}px / var(--wdm-size-map, var(--wdm-chrome-bars-scale, 1)))`;
  panelEl.style.bottom = `calc(${bottom}px / var(--wdm-size-map, var(--wdm-chrome-bars-scale, 1)))`;
  panelEl.style.left = `calc(${left}px / var(--wdm-size-map, var(--wdm-chrome-bars-scale, 1)))`;
  panelEl.style.right = `calc(${right}px / var(--wdm-size-map, var(--wdm-chrome-bars-scale, 1)))`;
  ...
  if (tab) {
    ...
    panelEl.style.setProperty("--map-close-x", `calc(${tabCenterX - left}px / var(--wdm-size-map, var(--wdm-chrome-bars-scale, 1)))`);
  }
}
...
function render() {
  if (!panelEl) return;
  // Honor GM "View As" — the map panel's item pool is the VIEWED
  // character's inventory, not the user's own. Falls back to no actor
  // when nothing is assigned.
  const actor = getPanelActor("map");
  ...
  panelEl.innerHTML = `
    <button id="wou-map-close" type="button" aria-label="${t("WITCHER.Chrome.Map.Text.CloseMap", "Close map")}" title="${t("WITCHER.Chrome.Map.Text.Close", "Close")}">
      <i class="fa-solid fa-chevron-up"></i>
    </button>

    <header class="wou-map-header">
      ${active ? `<span class="wou-map-title">${escapeText(active.name)}</span>` : ""}
      ${game.user?.isGM ? `<div class="wou-map-viewas wou-viewtools">${renderViewPanelAsPicker("map")}${renderViewAsPicker()}</div>` : ""}
    </header>
    ...
    <section class="wou-map-canvas">
      ${active
        ? (imgUrl
            ...
            : `<div class="wou-map-empty">${escapeText(active.name)} has no map image configured.<br><span class="dim">${t("WITCHER.Chrome.Map.Text.SetOneViaTheItemSCogwheel", "Set one via the item's cogwheel.")}</span></div>`)
        : `<div class="wou-map-empty">${t("WITCHER.Chrome.Map.Text.SelectAMapItemFromTheToolbar", "Select a map item from the toolbar.")}</div>`}
    </section>
  `;
  ...
  wireToolbar();
  wirePanZoom();
  // GM "View as" picker — re-render the map panel on selection so the new
  // character's maps replace the previous list.
  wireViewAsPicker(panelEl, () => render());
  wireViewPanelAsPicker(panelEl, "map", () => render());
}
