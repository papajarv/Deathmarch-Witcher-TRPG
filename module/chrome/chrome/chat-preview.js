...
import { carouselShouldShow } from "./carousel-initiative.js";
import { getChatPreviewConfig } from "./chat-preview-config.mjs";

/* Body classes set while a full-screen chrome overlay is open. Used only for
 * the "hide previews while a menu is open" client option — by default previews
 * show over the overlay (chrome.css lifts them above it). */
...
/**
 * Chat-card previews in the right-side sidebar lane.
 *
 * When a new ChatMessage fires and the chat tab isn't currently expanded,
 * inject a compact "square" preview card into our own `#wdm-chat-previews`
 * strip. That strip is positioned (chrome.css) on the RIGHT — the lane the
 * sidebar/chat occupies when open — as a bottom-anchored column, newest at
 * the bottom. Each card auto-dismisses after a few seconds or on click.
 *
 * The cards are styled to read as tight, scannable boxes (sender, one-line
 * title, result, action buttons) rather than the wide notification pills.
 */
 ...
 * <button class="damage">${t("WITCHER.Chrome.ChatPreview.Text.Damage", "Damage")}</button> on attack flavors doesn't
...
function buildPreviewElement(message) {
  ...
  if (attackSummaryHtml) {
    ...
    /* Suppress the primary paragraph when it just echoes the roll
       total — otherwise the initiative preview stacks
           14
           1d10 + 4 = 14
       for the same value. The roll line already shows both the
       formula AND the total, so dropping the paragraph collapses
       the card to the one-liner the user asked for. Only skips
       when the primary is either a bare number (initiative /
       simple rolls) or exactly matches one of the rolled totals. */
    const primaryTrim = primary?.trim() ?? "";
    const isDupeOfTotal = primaryTrim
        && rollLines.some(r => String(r.total).trim() === primaryTrim);
    const showPrimary = primary && !isDupeOfTotal;
    li.innerHTML = `
      <span class="wou-chat-preview-sender">${esc(senderLabel(message))}</span>
      <div class="wou-chat-preview-text">
        ${subtitle ? `<div class="wou-chat-preview-subtitle">${esc(subtitle)}</div>` : ""}
        ${showPrimary ? `<p>${esc(primary)}</p>` : ""}
        ${rollsHtml}
        ${verdictHtml}
...
function appendActionButtons(li, message) {
  /* Dedupe by data-action — the attack card sometimes has the SAME action
   * button in two places (the body's .wdm-attack-damage and the summary
   * action slot). One preview proxy per logical action is enough. */
  const seenActions = new Set();
  const textEl = li.querySelector(".wou-chat-preview-text");
  if (!textEl) return;
  const row = document.createElement("div");
  row.className = "wou-chat-preview-actions";
  /* One-way attach latch. The row is appended on the first proxy and NEVER
   * re-appended. This matters because `syncPreviewToMessage` (on
   * updateChatMessage) removes the current action row and rebuilds via a fresh
   * appendActionButtons call, while THIS call's deferred injected-scan passes
   * (rAF / setTimeout) may still fire afterwards. Without the latch, those
   * stale passes re-appended their detached row → a second action row →
   * duplicate buttons. Once superseded (row detached after attach), every add
   * bails. */
  let attached = false;
  const attachRow  = () => { if (!attached) { attached = true; textEl.appendChild(row); } };
  const superseded = () => attached && !row.isConnected;
  const dropIfEmpty = () => { if (!row.querySelector("button")) row.remove(); };

  /* Build one forwarding proxy. `finder` locates the matching live button in
   * #chat-log at click time — clicking the proxy fires the system's bound
   * handler on the real card. One-shot: if the live button is consumed
   * (data-consumed / disabled) or gone after the click, the proxy is dropped
   * so the preview stays in sync. */
  const addProxy = (label, finder) => {
    if (superseded()) return;   // this generation's row was replaced — abandon
    const proxy = document.createElement("button");
    proxy.type = "button";
    proxy.className = "wou-chat-preview-action";
    proxy.textContent = label || "Action";
    ...
    proxy.addEventListener("click", (ev) => {
      ...
      if (live) {
        ...
        setTimeout(() => {
          const still = finder();
          if (!still || still.dataset.consumed === "1" || still.disabled) { proxy.remove(); dropIfEmpty(); }
        }, 50);
      } else {
        proxy.remove();
        dropIfEmpty();
  ...
  /* 1) Persisted buttons from flavor + content. Matched live by ordinal +
   *    class (the original scheme) so duplicate-action cards resolve right. */
  let ordinal = 0;
  for (const html of [message.flavor, message.content]) {
    ...
    tmp.querySelectorAll("button").forEach((b) => {
      ...
      addProxy((b.textContent || "").trim(), () => findLiveButton(message.id, n, cls));
    });
  }

  /* 2) Buttons INJECTED into the live card at render time
   *    (renderChatMessageHTML) rather than saved in message.content — e.g.
   *    the "Assign Target" affordance on a targetless attack. createChatMessage
   *    (which spawned this preview) fires BEFORE that render, so the loop above
   *    can't see them. Scan the live card over the next frame(s) and mirror any
   *    new data-action button, forwarding by its data-action. */
  const mirrorInjected = () => {
    if (superseded() || !li.isConnected) return;   // replaced / preview expired
    /* Never attached our own row yet, but the preview already shows one → a
     * newer generation (a syncPreviewToMessage rebuild) owns it; don't add a
     * competing second row. */
    if (!attached && li.querySelector(".wou-chat-preview-actions")) return;
    const card = liveCard(message.id);
    if (!card) return;
    card.querySelectorAll("button[data-action]").forEach((b) => {
      const action = b.dataset.action;
      if (!action || seenActions.has(action)) return;
      seenActions.add(action);
      addProxy((b.textContent || "").trim(),
               () => liveCard(message.id)?.querySelector(`button[data-action="${cssEscape(action)}"]`) ?? null);
    });
  };
  requestAnimationFrame(mirrorInjected);
  setTimeout(mirrorInjected, 150);   // backstop for injections that run after a hook chain
}

/** The live chat-message element in the log for `messageId`, or null. */
function liveCard(messageId) {
  if (!messageId) return null;
  return document.querySelector(`#chat-log li.chat-message[data-message-id="${messageId}"], #chat .chat-message[data-message-id="${messageId}"]`);
}

/** CSS.escape with a defensive fallback for attribute-selector values. */
...
/** True for the chat message Foundry creates when initiative is rolled.
 *  Core stamps `flags.core.initiativeRoll = true` on it (see the client's
 *  Combat#rollInitiative → client/documents/combat.mjs). */
...
function showPreviewFor(message) {
  ...
  /* Client preferences (chrome/chat-preview-config.mjs). Read live so changes
   * in the Chat Previews dialog take effect on the next message. */
  const cfg = getChatPreviewConfig();
  if (!cfg.enabled) return;
  /* "Only me" — preview just the messages the current user authored (their own
   * action feedback), skipping everyone else's chatter. */
  if (cfg.from === "mine") {
    const authorId = message.author?.id ?? (typeof message.user === "string" ? message.user : message.user?.id);
    if (authorId !== game.userId) return;
  }
  /* "Hide when a full-screen menu is open" — otherwise previews float over it. */
  if (cfg.whenMenuOpen === "hide" && isChromeOverlayOpen()) return;
  /* Initiative rolls are already surfaced by the carousel bar whenever it's
   * showing for THIS combat's type (token combat vs theater-of-the-mind)
   * and audience — so the middle-screen preview would just double up. Drop
   * it in that case. When the carousel isn't active for this user/combat,
   * keep the preview: it's then the only initiative feedback on screen. */
  if (isInitiativeRollMessage(message) && game.combat && carouselShouldShow(game.combat)) return;
  ...
  /* Previews DO show over full-screen chrome overlays (inventory, character,
   * crafting, …) — otherwise a player browsing their inventory with chat
   * closed would never see the feedback for the actions they take THERE (food,
   * drink, reading a book, …). The compact right-side cards are lifted above
   * the overlay by chrome.css (only while one is open), so they read as small,
   * unobtrusive feedback toasts rather than being hidden behind it. */

  /* Combat cards (flags.<sys>.category === "combat") get their compact
   * <summary> folded into the content AFTER creation (appendAttackResult →
   * updateChatMessage). Building the preview now would show the fuller legacy
   * layout for a beat, then collapse to the mirror once the summary lands — a
   * visible "expand then shrink" flash. Defer these until the summary appears
   * (or a short fallback fires) so they render compact from the first paint. */
  if (isPendingCombatSummary(message)) { scheduleDeferredBuild(message); return; }
  buildAndShow(message);
}

/** Build + float a preview card for `message` (assumes the gates passed). */
function buildAndShow(message) {
  ...
  const ttl = Math.max(1000, (Number(getChatPreviewConfig().lifetimeSec) || 11) * 1000);
  setTimeout(() => li.remove(), ttl);
}

/** Re-check the live show/hide gates before a DEFERRED build fires (state may
 *  have changed since the message was created — chat opened, config toggled). */
function passesLiveGates(message) {
  if (isChatVisible() || !userCanSeeMessage(message)) return false;
  const cfg = getChatPreviewConfig();
  if (!cfg.enabled) return false;
  if (cfg.whenMenuOpen === "hide" && isChromeOverlayOpen()) return false;
  return true;
}

/** True while a combat card hasn't had its <summary> wrapped in yet. */
function isPendingCombatSummary(message) {
  const sysId = game.system?.id;
  const cat = message?.getFlag?.(sysId, "category") ?? message?.flags?.[sysId]?.category;
  if (cat !== "combat") return false;
  return !String(message?.content ?? "").includes("wdm-attack-card-summary");
}

/* Ids of combat cards whose preview we're holding until the summary lands,
 * mapped to a fallback timer that builds them anyway if it never does. */
const _pendingBuild = new Map();
const DEFER_FALLBACK_MS = 600;

function scheduleDeferredBuild(message) {
  if (_pendingBuild.has(message.id)) return;
  const timer = setTimeout(() => {
    _pendingBuild.delete(message.id);
    if (passesLiveGates(message)) buildAndShow(message);
  }, DEFER_FALLBACK_MS);
  _pendingBuild.set(message.id, timer);
}

/* On updateChatMessage: if we deferred this combat card and its summary has now
 * landed, build it compact from the first paint. Returns true when handled so
 * the normal in-place sync path is skipped. */
function resolvePendingBuild(message) {
  const timer = _pendingBuild.get(message.id);
  if (timer == null) return false;
  if (!String(message?.content ?? "").includes("wdm-attack-card-summary")) return false;
  clearTimeout(timer);
  _pendingBuild.delete(message.id);
  if (!isChatVisible() && userCanSeeMessage(message)) buildAndShow(message);
  return true;
}
...
export function installChatPreviews() {
  ...
  Hooks.on("updateChatMessage", (message) => {
    /* A deferred combat card whose summary just landed → build it now (compact
     * from the first paint). Otherwise keep an existing preview in sync. */
    if (resolvePendingBuild(message)) return;
    syncPreviewToMessage(message);
  });
}
