...
import { hrCritSpDowngradeMode } from "../../mechanics/house-rules-config.mjs";
...
const SEVERITY = () => ({
    simple:    { label: t("WITCHER.Chrome.CriticalRoll.Dialog.Button.Simple", 'Simple'),    beatBy: 7,  bonus: 3 },
    complex:   { label: t("WITCHER.Chrome.CriticalRoll.Dialog.Button.Complex", 'Complex'),   beatBy: 10, bonus: 5 },
    difficult: { label: t("WITCHER.Chrome.CriticalRoll.Dialog.Button.Difficult", 'Difficult'), beatBy: 13, bonus: 8 },
    deadly:    { label: t("WITCHER.Chrome.CriticalRoll.Dialog.Button.Deadly", 'Deadly'),    beatBy: 15, bonus: 10 }
});
...
const LOC_LABEL_FALLBACKS = {
    head: 'Head', torso: 'Torso',
    rightArm: 'Right Arm', leftArm: 'Left Arm',
    rightLeg: 'Right Leg', leftLeg: 'Left Leg'
};
const LOC_LABEL = new Proxy(LOC_LABEL_FALLBACKS, {
    get(target, prop) {
        if (!(prop in target)) return undefined;
        return t(`WITCHER.Chrome.HitLocation.Full.${String(prop)}`, target[prop]);
    }
});
...
function detectBalancedWeapon(actor) {
    ...
    for (const w of actor.items ?? []) {
        ...
        if (qs.includes("balanced")) hits.push(w.name || t("WITCHER.Chrome.CriticalRoll.Text.BalancedWeaponFallback", "Balanced Weapon"));
    }
    /* Natural Weapons (race) count too — an unarmed crit uses the race's
     * natural-weapon qualities, so a Balanced claw/bite sharpens the wound. */
    const race = (actor.items ?? []).find(i => i.type === "race" && i.system?.naturalWeapons);
    if (race && Array.isArray(race.system?.naturalWeaponQualities) && race.system.naturalWeaponQualities.includes("balanced")) {
        hits.push(race.name || t("WITCHER.Chrome.CriticalRoll.Text.BalancedWeaponFallback", "Balanced Weapon"));
    }
...
export async function openCriticalDialog(actor = null) {
    ...
    const sevOpts = Object.entries(SEVERITY())
        .map(([k, v]) => `<option value="${k}">${tFormat("WITCHER.Chrome.CriticalRoll.Text.SeverityOption", { label: v.label, beatBy: v.beatBy, bonus: v.bonus }, `${v.label} — beat by ${v.beatBy}+ · +${v.bonus} bonus dmg`)}</option>`)
        .join('');
    ...
    const balancedWarn    = balancedMeta?.multiple
        ? `<div style="margin-left:22px;opacity:0.7;font-size:0.625rem;color:#c8a878;">${t("WITCHER.Chrome.CriticalRoll.Text.MultipleWeaponsEquippedVerifyTheCritWasF", "Multiple weapons equipped — verify the crit was from")} <em>${balancedMeta.name}</em> before rolling.</div>`
        : "";
    ...
    const decimationHint    = decimationOn ? ` <span style="opacity:0.7;font-size:0.6875rem;">— ${t("WITCHER.Chrome.CriticalRoll.Text.SeverityPlusOneTier", "severity +1 tier")}</span>` : "";

    const locOpts = `
        <option value="head">${t("WITCHER.Chrome.CriticalRoll.Text.Head", "Head")}</option>
        <option value="torso">${t("WITCHER.Common.Torso", "Torso")}</option>
        <option value="rightArm">${t("WITCHER.Chrome.CriticalRoll.Text.RightArm", "Right Arm")}</option>
        <option value="leftArm">${t("WITCHER.Chrome.CriticalRoll.Text.LeftArm", "Left Arm")}</option>
        <option value="rightLeg">${t("WITCHER.Chrome.CriticalRoll.Text.RightLeg", "Right Leg")}</option>
        <option value="leftLeg">${t("WITCHER.Chrome.CriticalRoll.Text.LeftLeg", "Left Leg")}</option>
    `;

    const content = `
        <form style="display:flex; flex-direction:column; gap:8px;">
            <label><input type="checkbox" name="aimed"> ${t("WITCHER.Chrome.CriticalRoll.Text.AimedAttack", "Aimed Attack")}</label>
            <div class="form-group">
                <label>${t("WITCHER.Chrome.CriticalRoll.Text.Severity", "Severity")}</label>
                <select name="severity">${sevOpts}</select>
            </div>
            <div class="form-group" data-loc-row style="display:none;">
                <label>${t("WITCHER.Chrome.CriticalRoll.Text.Location", "Location")}</label>
                <select name="location">${locOpts}</select>
            </div>
            <label><input type="checkbox" name="balanced" ${balancedChecked}> ${t("WITCHER.Chrome.CriticalRoll.Text.BalancedWeapon", "Balanced Weapon (+1 aimed / +2 unaimed)")}${balancedHint}</label>
            ${balancedWarn}
            ${stableAimBonus > 0 ? `<label><input type="checkbox" name="stableAim" ${stableAimChecked} data-aim="${stableAimBonus}"> ${t("WITCHER.Chrome.CriticalRoll.Text.StableAim", "Stable Aim — add Aim bonus")}${stableAimHint}</label>` : ""}
            ${decimationOn ? `<label><input type="checkbox" name="decimation" ${decimationChecked}> ${t("WITCHER.Chrome.CriticalRoll.Text.CriticalDecimation", "Critical Decimation")}${decimationHint}</label>` : ""}
        </form>
    `;
    ...
    const variantRow = variant
        ? `<div><b>${t("WITCHER.Chrome.CriticalRoll.Text.Variant", "Variant:")}</b> ${variant}</div>`
        : '';

    const sideSuffix =
        locationFinal === 'leftArm' || locationFinal === 'leftLeg'   ? ` (${t("WITCHER.Chrome.CriticalRoll.Text.SideLeft", "Left")})`  :
        locationFinal === 'rightArm' || locationFinal === 'rightLeg' ? ` (${t("WITCHER.Chrome.CriticalRoll.Text.SideRight", "Right")})` : '';
    ...
    const woundName = previewEntry
        ...
        : tFormat("WITCHER.Chrome.CriticalRoll.Text.NoWoundAssigned", { loc: locLabel, variant: variant ? ` (${variant})` : '' }, `${locLabel}${variant ? ` (${variant})` : ''} — no wound assigned`);

    const html = `
        <div style="line-height:1.4;">
            <h3 style="margin:0 0 6px 0;">${tFormat("WITCHER.Chrome.CriticalRoll.Text.CriticalHeader", { severity: sev.label, wound: woundName }, `${sev.label} Critical — ${woundName}`)}</h3>
            <div><b>${t("WITCHER.Chrome.CriticalRoll.Text.Location", "Location:")}</b> ${locLabel}</div>
            ${variantRow}
            <div><b>${t("WITCHER.Chrome.CriticalRoll.Text.BeatDefenseBy", "Beat defense by:")}</b> ${sev.beatBy}+</div>
            <div><b>${t("WITCHER.Chrome.CriticalRoll.Text.BonusDamage", "Bonus damage:")}</b> +${sev.bonus}</div>
            <div><b>${t("WITCHER.Chrome.CriticalRoll.Text.Roll", "Roll:")}</b> ${rollFormula} → ${rollText}</div>
            ${decimation ? `<div style="opacity:0.75;font-size:0.6875rem;color:#c8a878;"><i class="fa-solid fa-arrow-up"></i> ${tFormat("WITCHER.Chrome.CriticalRoll.Text.CriticalDecimationLine", { from: severity, to: sevKey }, `Critical Decimation: ${severity} → <b>${sevKey}</b>`)}</div>` : ""}
            ${stableAim && aimAdd > 0 ? `<div style="opacity:0.75;font-size:0.6875rem;color:#bdc878;"><i class="fa-solid fa-crosshairs"></i> ${tFormat("WITCHER.Chrome.CriticalRoll.Text.StableAimLine", { aimAdd }, `Stable Aim: +${aimAdd} to crit-severity roll`)}</div>` : ""}
            <button type="button" data-action="wou-apply-crit"
                ...
                ${t("WITCHER.Chrome.CriticalRoll.Text.ApplyCriticalWound", "Apply Critical Wound")}
            </button>
        </div>
    `;
...
async function applyCritFromButton(button) {
    ...
    const promptPick = async (candidates, defaultId) => {
        ...
        const chosen = await DialogV2.prompt({
            ...
            content: `<form>
                <div class="form-group">
                    <label>${t("WITCHER.Chrome.CriticalRoll.Text.Target", "Target")}</label>
                    <select name="actorId">${opts}</select>
                </div>
            </form>`,
            modal: true,
            ok: {
                label: t("WITCHER.Chrome.CriticalRoll.Dialog.Button.Apply", 'Apply'),
...
export async function autoApplyCriticalWound({ actor, severity, locationKey, attackMessageUuid = null, suppressStunSave = false, damagePenetrated = true }) {
    ...
    /* House Rule: SP-based downgrade. When the caller flagged this
     * hit as NOT penetrating the target's Stopping Power at the hit
     * location (finalDamage === 0 → armor fully soaked the strike),
     * downgrade the wound per the GM's chosen mode.
     *
     *   "greaterToLesser" — force `lesser = true`, converting a
     *                       greater head/torso wound to its lesser
     *                       variant (limbs are already lesser-only,
     *                       so this mode is a no-op there).
     *   "anyToSimple"     — force `severity = "simple"`, dropping
     *                       every crit tier to Simple regardless
     *                       of the rolled tier.
     *   "off" / default   — no change; RAW behaviour. */
    if (!damagePenetrated) {
        const mode = hrCritSpDowngradeMode();
        if (mode === "greaterToLesser") {
            lesser = true;
        } else if (mode === "anyToSimple") {
            severity = "simple";
            lesser   = true;
        }
    }
    ...
    if (attackMsg) {
        ...
        try {
            ...
            await appendAttackResult(attackMsg, {
                fragment: woundLine,
                summaryAdd: { label: tFormat("WITCHER.Chrome.CriticalRoll.Summary.CritApplied", { actor: actor.name, entry: entry.name }, `${actor.name}: ${entry.name}`), kind: "crit", icon: "fa-skull-crossbones" }
            });
        } catch (err) {
        ...
        if (aeGrant > 0 && hasWRPerk(actor, "perserver")) {
            ...
            const banner =
                `<div class="wdm-attack-rider wdm-wr-rider" data-wr-rider="perserver">` +
                    ...
                    `<strong>${t("WITCHER.Chrome.CriticalRoll.Text.Perserver", "Perserver")}</strong> — ${escWoundAttr(messageText)}` +
                `</div>`;
            if (attackMsg) {
