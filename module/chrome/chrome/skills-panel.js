...
function injectInlineStyles() {
  ...
  s.textContent = `
    ...
    #wou-skills-pop .wou-sp-mod {
      display: flex; justify-content: center;
      padding: 0.375rem 0.75rem;
      border-top: 1px dotted rgba(140,133,121,0.16);
    }
    #wou-skills-pop .wou-sp-mod label {
      display: inline-flex; align-items: center; gap: 0.4rem;
      font-size: 0.625rem; letter-spacing: 0.20em; text-transform: uppercase;
      color: var(--wdm-amber-hi, #c8a878);
    }
    #wou-skills-pop .wou-sp-mod-input {
      width: 3.25rem; text-align: center;
      background: rgba(24, 20, 16, 0.35);
      border: 1px solid rgba(140,133,121,0.28);
      color: var(--wdm-amber-bright, #d6a050);
      font-weight: 700; font-size: 0.8125rem;
      padding: 0.15rem 0.25rem; border-radius: 2px;
    }
    #wou-skills-pop .wou-sp-mod-input:focus {
      outline: none; border-color: var(--wdm-amber-bright, #d6a050);
    }
  `;
...
function getPopover() {
  ...
  pop.setAttribute("aria-label", t("WITCHER.Chrome.SkillsPanel.Text.Skills", "Skills"));
...
function renderPopover(actor) {
  ...
  // Preserve the one-off modifier input value across re-renders. `renderPopover`
  // rewrites the whole popover innerHTML on every actor/item hook, which would
  // otherwise wipe a typed-in-progress mod (e.g. type "+3", an unrelated effect
  // tick fires, the input snaps back to 0 before the user clicks a skill).
  const modInput = pop.querySelector(".wou-sp-mod-input");
  const preservedMod = modInput ? modInput.value : null;
  const modWasFocused = modInput && modInput === document.activeElement;
  ...
  const panelsHtml = STATS.map(stat => {
    ...
    if (skills.length === 0) {
      ...
      listHtml = skills.map(sk => {
        ...
        const aeTitle = aeBonus !== 0 ? ` title="${escapeHTML(tFormat("WITCHER.Chrome.SkillsPanel.Tip.BaseFromEffects", { base, sign: aeBonus >= 0 ? " +" : " ", bonus: aeBonus }, `Base ${base}${aeBonus >= 0 ? " +" : " "}${aeBonus} from effects`))}"` : "";
  ...
  pop.innerHTML = `
    ...
    ${panelsHtml}
    ${profPanelHtml}
    <div class="wou-sp-mod">
      <label>
        <span>${t("WITCHER.Chrome.SkillsPanel.Text.MOD", "MOD")}</span>
        <input type="number" step="1" class="wou-sp-mod-input" value="0" title="${t("WITCHER.Chrome.SkillsPanel.Text.OneOffModifierAppliedToTheNextSkillRollO", "One-off modifier applied to the next skill roll only; resets after rolling.")}" />
      </label>
    </div>
    <div class="wou-sp-foot">
      ${t("WITCHER.Chrome.SkillsPanel.Text.Click", "Click")} <kbd>${t("WITCHER.Chrome.SkillsPanel.Text.SkillKbd", "SKILL")}</kbd> = ${t("WITCHER.Chrome.SkillsPanel.Text.Roll", "roll")}
    </div>`;

  if (preservedMod != null) {
    const nextInput = pop.querySelector(".wou-sp-mod-input");
    if (nextInput) {
      nextInput.value = preservedMod;
      if (modWasFocused) nextInput.focus();
    }
  }
}
...
/** Active actor: honors GM "View As" override, then falls back to selected
 *  token, then first owned character. Uses the shared getAssignedActor
 *  (chrome/lib/actor.js) so a GM eavesdropping on a player via View As
 *  sees THAT player's skills — not the GM's own assigned character. */
function getActiveActor() {
  const viewed = getAssignedActor();
...
/* Read Foundry v14's canonical chat visibility setting. `core.messageMode`
 * ("public" | "gm" | "blind" | "self" | "ic") is what the sidebar
 * Public/GM/Blind/Self dropdown writes to and what ChatMessage._preCreate
 * consumes via the `messageMode` operation option (see foundry_copy
 * chat-message.mjs line 538). The old `core.rollMode` key is legacy
 * and only bridged for messages carrying rolls. Fall back to "public"
...
/* Read + reset the one-off modifier input. Empty / non-numeric → 0. The
 * input clears back to 0 after every roll so a stray value from an earlier
 * check can't silently ride along on the next click. */
function consumeModInput() {
  const el = document.querySelector(`#${POPOVER_ID} .wou-sp-mod-input`);
...
async function rollSkill(actor, skillKey) {
  try {
    if (typeof actor.rollSkill === "function") {
      /* Thread the sidebar's current visibility through so the dock's
       * skill roll respects the same Public/GM/Blind/Self toggle as
       * every other roll surface. */
      const situational = consumeModInput();
      await actor.rollSkill(skillKey, { messageMode: currentMessageMode(), situational });
    } else {
...
async function rollProfessionSkill(actor, skillName) {
  try {
    ...
    /* Same visibility threading as the plain-skill path — respect
     * Foundry's Public / GM / Blind / Self chat mode toggle. */
    const situational = consumeModInput();
    await actor.rollProfessionSkill(slot, { messageMode: currentMessageMode(), situational });
  } catch (e) {
...
function onClick(e) {
  ...
  // 4. Profession-skill row click → dispatch via professionMixin, then dismiss
  ...
  if (profSkill) {
    ...
    close();                // roll picked → dismiss the panel
    return;
  }

  // 5. Skill row click → roll skill, then dismiss
