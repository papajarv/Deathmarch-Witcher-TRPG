...
function installClickHandlers() {
  ...
  /* Right-click a player card → that user's configuration sheet (Foundry's
   * usual User Config: name, colour, avatar, assigned character). The GM may
   * open any player's; a non-GM may only open their own (and Foundry would
   * reject rendering someone else's sheet anyway). Suppresses the browser
   * context menu on the card. */
  document.addEventListener("contextmenu", (event) => {
    const rowEl = event.target.closest("#scene-controls .wou-player-row[data-user-id]");
    if (!rowEl) return;
...
async function assignCharacter(userId, actorId) {
  ...
  try {
    ...
    if (prevActor && prevActor.id !== actorId) {
      await prevActor.update({ [`ownership.${userId}`]: new foundry.data.operators.ForcedDeletion() });
    }
