...
function ensureMount() {
    ...
    strip.innerHTML = `
        ...
        <span class="wou-weather-paused" data-bind="weather-paused" hidden></span>
    `;
...
function refreshNow() {
    ...
    let paused = false;
    try { paused = !!game.settings.get("witcher-ttrpg-death-march", "clockPaused"); }
    catch (_) { /* unregistered during early boot */ }
    strip.classList.toggle("is-paused", paused);
    const pausedEl = strip.querySelector('[data-bind="weather-paused"]');
    if (pausedEl) {
        pausedEl.hidden = !paused;
        pausedEl.textContent = paused ? (localize("WITCHER.Weather.TimeIsPaused") || "Time is Paused") : "";
    }
}
...
/* The current light-level indicator line — always shown (even in benign
 * weather), player + GM facing. Reads the viewer's own token tile, else scene
 * ambient. */
function lightingHtml() {
    let li = null;
    try { li = getActiveLightInfo(); } catch (_) { /* not ready */ }
    if (!li) return "";
    const cls = (li.tier === "pitch" || li.tier === "darkness") ? "is-bane"
              : String(li.tier).startsWith("dim") ? "is-warn" : "";
    const pen = li.awareness
        ? `<span class="wou-wm-light-pen">${signed(li.awareness)} ${localize("WITCHER.Weather.Target.Awareness")}</span>` : "";
    const waived = li.waived
        ? `<span class="wou-wm-light-waived">${localize("WITCHER.Light.SeenByVision")}</span>` : "";
    return `
        <div class="wou-wm-light ${cls}">
            <i class="fa-solid ${li.icon}"></i>
            <span class="wou-wm-light-label">${localize(li.label)}</span>
            ${pen}${waived}
        </div>`;
}

/* Current weather NAME + its penalties listed beneath it (a flat list, not
 * per-target boxes). Light-tier penalties are excluded — they show in the Light
 * Level indicator above. */
function weatherDisplayHtml() {
    let d = null;
    try { d = getActiveWeatherDisplay(); } catch (_) { /* not ready */ }
    if (!d) return "";
    const temp = d.temp != null ? ` <span class="wou-wm-wx-temp">${d.temp}°</span>` : "";
    const pens = d.penalties.length
        ? d.penalties.map(p => `
            <div class="wou-wm-wx-pen ${p.value < 0 ? "is-bane" : p.value > 0 ? "is-boon" : ""}">
                <span class="wou-wm-wx-val">${signed(p.value)}</span>
                <span class="wou-wm-wx-tgt">${localize(p.targetLabel)}</span>
                <span class="wou-wm-wx-desc">${localize(p.label)}</span>
            </div>`).join("")
        : `<div class="wou-wm-fx-empty">${localize("WITCHER.Weather.NoWeatherPenalties") || "No weather penalties."}</div>`;
    return `
        <div class="wou-wm-wx">
            <div class="wou-wm-wx-head"><i class="fa-solid ${d.icon}"></i> <span class="wou-wm-wx-name">${d.label}</span>${temp}</div>
            ${pens}
        </div>`;
}

function modifiersHtml() {
    const light = lightingHtml();
    const weather = weatherDisplayHtml();
    const notes = getActiveWeatherNotes();
    const noteRows = notes.map(n =>
        `<div class="wou-wm-fx-note">${localize(n)}</div>`
    ).join("");
    return light + weather + noteRows;
}
...
function openInfoMenu(anchor) {
    ...
    el.innerHTML = `
        <div class="wou-wm-section">
            <div class="wou-wm-head">${localize("WITCHER.Chrome.Weather.Text.CurrentTime") || "Current time"}</div>
...
export function wireWeather() {
    ...
    Hooks.on("updateSetting", (setting) => {
        ...
        if (k === "witcher-ttrpg-death-march.weatherBiome"
            ...
            || k === "witcher-ttrpg-death-march.clockPaused"
            || k === "witcher-ttrpg-death-march.weatherPins") {
            scheduleRefresh();
