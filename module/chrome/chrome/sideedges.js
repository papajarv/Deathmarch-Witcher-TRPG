...
/* The collapse button is a fixed control on #scene-controls — NOT inside the
   per-category tools menu (where it used to scroll with the tool list and read
   as just another tool).  It borrows the tool-button classes for styling but
   carries no data-action, so Foundry's tool dispatcher ignores it. Its grid
   placement (row 2, below the tool menus and above the party dock) lives in
   leftbar.css `#wou-controls-close` — DOM insertion order here is irrelevant to
   where it renders. */
const LEFT_CLOSE_HTML = () => `<button type="button" id="wou-controls-close" class="control ui-control tool icon fa-solid fa-caret-left" aria-label="${t("WITCHER.Chrome.Sideedges.Text.CollapseTools", "Collapse Tools")}" data-tooltip="${t("WITCHER.Chrome.Sideedges.Text.CollapseTools", "Collapse Tools")}"></button>`;
...
function wireLeft() {
  ...
  // Inject the close button into #scene-controls (grid placement in leftbar.css
  // puts it in row 2, below the tool menus and above the party dock). Foundry
  // re-renders the controls when the user switches categories, so we re-inject
  // on every renderSceneControls.
