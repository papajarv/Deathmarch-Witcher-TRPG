...
function buildPaperGraphics(seed = "", height = NOTE_HEIGHT) {
    ...
    shadow.drawRoundedRect(5, 8, NOTE_WIDTH, height, 5);
    shadow.endFill();
    g.addChild(shadow);
    ...
    paper.drawRoundedRect(0, 0, NOTE_WIDTH, height, 3);
    paper.endFill();
    g.addChild(paper);
    ...
    for (let i = 0; i < 6; i++) {
        ...
        vignette.drawRoundedRect(inset, inset, NOTE_WIDTH - inset * 2, height - inset * 2, 3);
    }
    g.addChild(vignette);
    ...
    frame.drawRect(7, 7, NOTE_WIDTH - 14, height - 14);
    g.addChild(frame);
    ...
    fl.moveTo(m, height - m - len).lineTo(m, height - m).lineTo(m + len, height - m);
    fl.moveTo(NOTE_WIDTH - m - len, height - m).lineTo(NOTE_WIDTH - m, height - m).lineTo(NOTE_WIDTH - m, height - m - len);
    g.addChild(fl);
...
class ParchmentRenderer {
    ...
    _build() {
        ...
        // Built (and measured) BEFORE the paper so the body can be laid out
        // beneath it and the paper sized to the total.
        ...
        // Body — wordWrap keeps it inside the paper width; htmlToText already
        // converts <br>/<p>/<li>/<h*> to newlines, so paragraph breaks survive.
        // NO mask: the paper grows to fit the text (see noteHeight below)
        // instead of clipping it — that clip was the "notes cut off" bug.
        ...
        // Dynamic paper height: enough to hold the measured body plus the
        // bottom button row, floored at the original NOTE_HEIGHT so short
        // notes keep their proportions.
        const noteHeight = Math.max(
            NOTE_HEIGHT,
            Math.ceil(bodyTop + body.height + 14 + BTN_H + PAD)
        );
        const btnY = noteHeight - PAD - BTN_H;

        c.hitArea = new PIXI.Rectangle(0, 0, NOTE_WIDTH, noteHeight);

        // Paper backing sized to the dynamic height, behind everything else.
        c.addChild(buildPaperGraphics(this.entry.id, noteHeight));
        c.addChild(title);
