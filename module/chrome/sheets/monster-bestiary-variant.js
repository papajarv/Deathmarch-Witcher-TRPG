...
Hooks.on("renderWitcherMonsterSheet", (app, element /*, context */) => {
  ...
  pill.dataset.tooltip = on
    ? t("WITCHER.Sheet.MonsterBestiaryVariant.Tip.On", "This actor has its own bestiary entry, separate from its compendium source.")
    : t("WITCHER.Sheet.MonsterBestiaryVariant.Tip.Off", "Default: this actor groups with its compendium source. Click to split into its own bestiary entry.");
  pill.innerHTML = `
    <input type="checkbox" ${on ? "checked" : ""} />
    <i class="fa-solid fa-dragon"></i>
    <span>${t("WITCHER.Sheet.MonsterBestiaryVariant.Text.BestiaryVariant", "Bestiary variant")}</span>
  `;
