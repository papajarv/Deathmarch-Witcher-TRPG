...
Hooks.on("renderWitcherMonsterSheet", (app, element /*, context */) => {
  ...
  slot.dataset.tooltip = mountItem
    ? tFormat("WITCHER.Chrome.MonsterMount.Tip.Linked", { name: mountItem.name, mod: formatMod(controlMod) }, `Linked: ${mountItem.name}  ·  Control mod ${formatMod(controlMod)}`)
    : t("WITCHER.Chrome.MonsterMount.Tip.MountableBeast", "Mountable beast — check to drop a mount item");
...
function renderInnerHTML({ enabled, mountItem, controlMod }) {
  const checkbox = `
    ...
    <i class="fa-solid fa-horse"></i><span class="lbl">${t("WITCHER.Sheet.MonsterMount.Text.Mount", "Mount")}</span>
  `;
  ...
  if (mountItem) {
    return `
      ...
      <span class="wou-mount-mod">${tFormat("WITCHER.Sheet.MonsterMount.Text.CtrlMod", { mod: escapeText(formatMod(controlMod)) }, `Ctrl ${escapeText(formatMod(controlMod))}`)}</span>
      <a class="wou-mount-unlink" data-action="unlink-mount" data-tooltip="${t("WITCHER.Sheet.MonsterMount.Text.UnlinkMount", "Unlink mount")}"><i class="fa-solid fa-xmark"></i></a>
    `;
  }
