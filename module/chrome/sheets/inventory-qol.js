...
function showPopup(anchorEl, qualityName, descHtml) {
    ...
    popup.innerHTML = `
        ...
        <div class="qol-quality-popup-body">${descHtml || `<em>${t("WITCHER.Sheet.InventoryQol.Text.NoDescriptionFound", "No description found.")}</em>`}</div>
    `;
...
function buildPanelHTML(container) {
    ...
    if (!items.length) {
        return `${weightBar}
            <p class="qol-empty-msg">${t("WITCHER.Sheet.InventoryQol.Text.EmptyDragItemsHereToStoreThem", "Empty — drag items here to store them.")}</p>`;
    }
    ...
    const rows = items.map(item => `
        <div class="qol-stored-item">
            ...
            <button class="qol-eject-btn" data-item-uuid="${item.uuid}"
                    title="${t("WITCHER.Sheet.InventoryQol.Text.RemoveFromContainer", "Remove from container")}" type="button">
...
function setupContainers($html, actor) {
    $html.find(".list-item").each((_, el) => {
        ...
        const $btn = $(`<a class="qol-contents-btn" title="${t("WITCHER.Sheet.InventoryQol.Text.ShowHideContents", "Show/hide contents")}">
