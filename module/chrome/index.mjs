...
import { registerSceneLevelsBridge } from "./chrome/scene-levels.js";
...
import { installLootDisplay } from "./chrome/loot-display.js";
import { installMapLoot } from "../canvas/mapLoot.mjs";

/* ── Ready-time injectors ─────────────────────────────────────────────── */

import { migrateBestiarySchemaIfNeeded } from "./lib/bestiary.js";
import { VIEWER_OVERRIDE_HOOK, getAssignedActor, isActorInActiveCombat } from "./lib/actor.js";
...
import { registerCarouselInitiative } from "./chrome/carousel-initiative.js";
...
export function wireChromeReady() {
    ...
    /* Carousel initiative tracker — anchored under the scene nav, drags
     * with nav collapse/expand via ResizeObserver. Also patches the
     * sidebar combat tracker to hide secret / stealthed / GM-hidden
     * combatants from non-GMs. See chrome/carousel-initiative.js. */
    registerCarouselInitiative();
    ...
    Hooks.on("combatRound",  scheduleRebindDock);   // round-based duration display
    /* Per-turn events fire for EVERY client on EVERY turn change and each drove
     * a full ~30-write dock rebind — a big FPS hit for players NOT in the fight
     * (whose dock doesn't change on a turn move between other combatants). Gate
     * the frequent per-turn hooks on the viewed actor actually being a
     * combatant: only then can this dock's off-turn paint have flipped. The
     * peace↔war edges (create/delete/combatStart) and round-based durations
     * above stay ungated because they DO change the dock for everyone.
     *
     * updateCombat also fires on non-turn combat edits (e.g. a combatant's
     * initiative), so keep it — but likewise only when this client's actor is
     * in the combat it describes. */
    const rebindIfInCombat = () => { if (isActorInActiveCombat(getAssignedActor())) scheduleRebindDock(); };
    Hooks.on("updateCombat", rebindIfInCombat);
    Hooks.on("combatTurn",   rebindIfInCombat);
    ...
    Hooks.on("combatTurnChange", rebindIfInCombat);
    ...
    //
    // `controlToken` fires once PER token, so a box-select / drag / ESC-deselect
    // of many tokens would rebind the WHOLE dock N times — the multi-select
    // stutter. The dock is actor-keyed (see the updateActor gate above), so
    // rebind only when the assigned actor the dock displays actually changes:
    // at most one rebind for the entire multi-token operation.
    let _lastDockCtrlActorId = null;
    Hooks.on("controlToken", () => {
        const aid = getAssignedActor()?.id ?? null;
        if (aid === _lastDockCtrlActorId) return;
        _lastDockCtrlActorId = aid;
        scheduleRebindDock();
    });

    /* Freeze all chrome CSS animations while the tab is hidden (see the
     * `.wou-tab-hidden` rule in styles/chrome.css). The dock's always-on
     * blur/box-shadow "breathing" animations otherwise keep advancing while
     * hidden and force one big re-raster of every blurred layer the instant
     * the tab is shown again — the sharp 0-FPS stall on tab-return, worst on
     * weak GPUs. One compositor-only body-class toggle; no layout cost. */
    document.addEventListener("visibilitychange", () => {
        document.body.classList.toggle("wou-tab-hidden", document.hidden);
    }, { passive: true });
