...
import { STRIKE_TYPES, ATTACK_LOCATIONS, rollHitLocation, EXTRA_ACTION,
         getActiveWeaponQualities, WEAPON_QUALITIES, ARMOR_QUALITIES, shieldBashDamage,
         RANGE_BRACKETS }
    from "../../setup/config.mjs";
import { isAdrenalineEnabled, adrenalineStaPerDie, adrenalineStaPerDieFor } from "../../api/adrenaline.mjs";
import { isCombatExtendedEnabled } from "../../api/homebrew.mjs";
import { hasWRPerk, wrHeroic } from "../../api/witcherReborn.mjs";
import { getActiveOpenCategoryBonuses, sumOpenCategoryWa, damageTailFromOpenCategory, grantedQualitiesFromOpenCategory, grantedQualityValuesFromOpenCategory, strangleSuffocation } from "../../mechanics/openCategoryBonuses.mjs";
...
import { actorPoorVisionActive, POOR_VISION_PENALTY } from "../../mechanics/helmetVision.mjs";
import { emitApplyDamage, emitApplyStatus, emitAppendAttackFragment, emitReduceReliability, qualitiesToDamageFlags, requestDefenseFromOwner } from "../../setup/socketHook.mjs";
import { critSeverityFromDelta } from "../../combat/critSeverity.mjs";
import { hrExtraActionToHit, hrOffhandPenalty, hrStrongStrikePenalty, hrCritBrackets, hrCritBonusLadders, hrNewSilverRules }
    from "../../mechanics/house-rules-config.mjs";
import { getActorTarget } from "../../chrome/chrome/context-menu-actor.js";
import { getResearchTier, bestiaryKeyFor, MAX_RESEARCH } from "../../chrome/lib/bestiary.js";
...
function renderCardSummary(parts, actionHtml = "") {
    let chipHtml;
    if (!parts.length) {
        chipHtml = `<span class="wdm-card-sum-part">${t("WITCHER.Doc.WeaponAttackMixin.Text.Attack", "Attack")}</span>`;
    } else {
...
async function _doAppendAttackResult(attackMsg, { fragment = "", summaryAdd = "", summaryAction = null, wrapperSelector = null } = {}) {
    if (!attackMsg) return;
    ...
    if (!card) {
        ...
        /* Hoist a Roll Damage button out of the just-wrapped body and into the
         * summary action slot, so it stays reachable on the COLLAPSED header
         * (wrapping otherwise buries it in the body). Strip the body copy to
         * avoid a duplicate when expanded. Only fires on the FIRST wrap of a
         * card that carries its own damage button (e.g. a brawl attack folded
         * via appendAttackResult); weapon cards are wrapped by the render
         * rebuild before any append, so this branch never runs for them. */
        try {
            const dmgBtn = bodyEl.querySelector('button[data-action="wdm-roll-damage"]');
            if (dmgBtn && !card.dataset.summaryActionHtml) {
                const clone = dmgBtn.cloneNode(true);
                clone.classList.add("wdm-card-sum-roll");
                card.dataset.summaryActionHtml = clone.outerHTML;
                dmgBtn.remove();
            }
        } catch (_) { /* best-effort — button just stays in the body */ }
    }
    const body     = card.querySelector(".wdm-attack-card-body");
    const summaryEl = card.querySelector(".wdm-attack-card-summary");
    if (fragment) {
        /* wrapperSelector — when set, append fragment INSIDE the first
         * matching element under the card body instead of at body
         * top-level. Enables per-victim nesting (bombs pre-post an
         * outer per-victim <details>, then per-location breakdowns
         * append into that victim's container). Falls back to body
         * append when the selector doesn't match (defensive — the
         * container may not have been posted yet under a race). */
        const container = wrapperSelector ? body.querySelector(wrapperSelector) : null;
        (container ?? body).insertAdjacentHTML("beforeend", fragment);
    }
    ...
    /* Deadly Focus is a PRE-damage crit-severity upgrade. Clearing the action
     * slot (summaryAction === "") is the signal that damage has been rolled —
     * so drop any lingering Deadly Focus rider from the body too; it's too late
     * to use once damage is rolled, and it shouldn't sit after the hit
     * breakdown. Covers both the owner and GM-relay damage-roll paths (both
     * append with summaryAction: ""). No-op for casts / cards without it. */
    if (summaryAction === "") {
        try { tmp.querySelectorAll('[data-wr-rider="deadlyFocus"]').forEach(n => n.remove()); }
...
function breakdownStatusMods(actor, bucket) {
    ...
    for (const id of actor.statuses) {
        const clause = _clauseFor(id, actor);
...
function attackRollFlavor({ actorName, weaponName, subtitle, chips = [], damage, qualities = [], qualityKeys = [], qualityValues = {}, note = "", hitLocation = null, weaponUuid = "", ammoUuid = "", ammoOilBonusDamage = 0, ammoOilTarget = "", ammoOilName = "" }) {
    ...
    const hitLocHtml = hitLocation?.label
        ? `<div class="wdm-attack-hit-loc"><i class="fa-solid fa-bullseye"></i>` +
              `<span class="wdm-attack-hit-loc-k">${t("WITCHER.Doc.WeaponAttackMixin.Text.HitLocation", "Hit Location")}</span>` +
     ...
     * Suppressed under the new silver rules — the legacy silver quality is
     * disabled and no separate silver roll should be captured. */
    const silverAttr = (damage?.silverFormula && !hrNewSilverRules())
        ? ` data-silver-formula="${esc(damage.silverFormula)}"` : "";
    ...
    /* Ammo UUID — set on ranged shots so the GM-side handleApplyDamage
     * reads the oil bonus off the arrow/bolt's appliedOil instead of the
     * bow's (Core p.166: the projectile carries the coating). Melee /
     * thrown / off-hand paths leave this empty, keeping the weapon's own
     * appliedOil as the source. */
    const ammoUuidAttr = ammoUuid ? ` data-ammo-uuid="${esc(ammoUuid)}"` : "";
    /* Inline snapshot of the fired ammo's coating. Captured at attack
     * roll time because spendShot deletes the 1-qty coated stack before
     * the player clicks Roll Damage — reading appliedOil off the ammo
     * document at that point would fail. Empty when the shot ammo has
     * no coating (bonus = 0 skips the fold on the GM side). */
    const oilBonusAttr = ammoOilBonusDamage > 0 ? ` data-ammo-oil-bonus="${esc(ammoOilBonusDamage)}"` : "";
    const oilTargetAttr = ammoOilTarget      ? ` data-ammo-oil-target="${esc(ammoOilTarget)}"` : "";
    const oilNameAttr   = ammoOilName        ? ` data-ammo-oil-name="${esc(ammoOilName)}"`   : "";
...
export async function applyQualityRiders(target, qualityKeys, qualityValues, locKey, speaker, opts = {}) {
    ...
    /* Attacker actor uuid, resolved from the attack's speaker — passed to
     * emitApplyStatus so the GM socket authorizes a rider status onto a
     * GM-owned NPC (the player has no OBSERVER on it) via source ownership. */
    const _riderSrcUuid = (() => {
        const sp = speaker ?? {};
        const a = sp.actor ? game.actors?.get?.(sp.actor)
    ...
    for (const key of qualityKeys) {
        ...
        if (rider.kind === "auto") {
            try {
                await emitApplyStatus({ targetUuid: target.uuid, statusId: rider.statusId, action: "apply", sourceActorUuid: _riderSrcUuid });
                ...
                await emit(frag, { label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.TargetStatus", { target: target.name, label }, `${target.name}: ${label}`), kind: "status", icon: "fa-droplet" });
                lines.push(tFormat("WITCHER.Doc.WeaponAttackMixin.Line.StatusApplied", { target: target.name, label, key }, `${target.name}: ${label} applied (${key})`));
            } catch (err) {
        ...
        if (rider.kind === "percent") {
            ...
            const rollLine =
                `<div class="wdm-attack-rider ${hit ? "is-hit" : "is-miss"}">` +
                    ...
                    `<em>${hit ? t("WITCHER.Doc.WeaponAttackMixin.Text.Applies", "applies") : t("WITCHER.Doc.WeaponAttackMixin.Text.NoEffect", "no effect")}</em>` +
                    `<details class="wdm-attack-rider-detail"><summary>roll detail</summary>${dieRender}</details>` +
                `</div>`;
            await emit(rollLine, hit ? { label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.TargetStatus", { target: target.name, label }, `${target.name}: ${label}`), kind: "status", icon: "fa-droplet" } : "");
            if (hit) {
                try {
                    await emitApplyStatus({ targetUuid: target.uuid, statusId: rider.statusId, action: "apply", sourceActorUuid: _riderSrcUuid });
                    lines.push(tFormat("WITCHER.Doc.WeaponAttackMixin.Line.StatusAppliedPct", { target: target.name, label, key, pct }, `${target.name}: ${label} applied (${key} ${pct}%)`));
                } catch (err) {
                    console.warn(`witcher-ttrpg-death-march | apply ${rider.statusId} to ${target.name} failed`, err);
                }
            }
        ...
        if (rider.kind === "stunSave") {
            ...
            const frag =
                `<div class="wdm-stun-prompt">` +
                    ...
                    `<i class="fa-solid fa-dice-d10"></i> ${t("WITCHER.Doc.WeaponAttackMixin.Text.RollStunSave", "Roll Stun Save")}</button>` +
                `</div>`;
            await emit(frag, {
                label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.StunSave", { name: target.name, mod: modifier ? ` ${signed(modifier)}` : "" }, `${target.name}: Stun save${modifier ? ` ${signed(modifier)}` : ""}`),
                kind: "status",
                icon: "fa-dice-d10"
            });
            lines.push(tFormat("WITCHER.Doc.WeaponAttackMixin.Line.StunSavePrompted", { name: target.name, mod: signed(modifier) }, `${target.name}: Stun save prompted (${signed(modifier)})`));
        }
    }
    return lines;
}
...
async function rollDamageFromButton(btn) {
    ...
    /* Target resolution runs BEFORE the button is consumed and dice roll.
     * If no target is set — canvas, stamped defender, tokenless — the
     * fallback picker fires; cancelling it leaves the button clickable
     * so the player can pick a target and try again. Priorities:
     *   1. Stamped defender flag on the attack message (weaponAttackMixin
     *      writes this when the attack was rolled with a live target).
     *   2. Live canvas token target on the current user.
     *   3. Tokenless combat-tracker target (per-user actorTargetUuid). */
    let _preRollTargets = [];
    try {
        const stampedUuid = msg?.getFlag?.(SYSTEM_ID, "defenderUuid");
        if (stampedUuid) {
            const stamped = await fromUuid(stampedUuid);
            if (stamped) _preRollTargets = [stamped];
        }
    } catch (_) { /* no stamped defender */ }
    if (!_preRollTargets.length) {
        _preRollTargets = Array.from(game.user?.targets ?? [])
            .map(t => t?.actor)
            .filter(a => !!a);
    }
    if (!_preRollTargets.length) {
        try {
            ...
            if (tokenlessTarget) _preRollTargets = [tokenlessTarget];
        } catch (_) { /* no actor-target flag */ }
    }
    /* Still no target after all three canonical resolvers. Rather than
     * popping a picker here (which duplicates the "Assign Target" button
     * that already lives on the attack card), notify the user to use
     * that button and bail — button stays clickable so they can Assign
     * Target then click Roll Damage again. */
    if (!_preRollTargets.length) {
        ui.notifications?.warn(t("WITCHER.Doc.WeaponAttackMixin.Notify.NoTargetSetUseTheAssign", "No target set — use the Assign Target button on the attack card first, then Roll Damage."));
        return;
    }

    /* Now that a target is committed, mark the button consumed. A double-
     * click during the pre-target picker window is stopped by the
     * `consumed` guard at the top of this function (the second click sees
     * consumed==="1" set by the first fire's downstream update). */
    ...
    const critSeverity = stampedSev ?? (delta != null ? critSeverityFromDelta(delta, hrCritBrackets()) : null);
    try {
        /* Bestiary L6 expert bonus, folded INTO the visible damage roll.
         * The attacker's mastery of a fully-researched creature adds a flat
         * +4 to damage against it — baked in as a roll term so it shows in
         * the dice tray and is part of roll.total, not a silent post-roll
         * add. Resolve the attacker (token-actor first so unlinked
         * combatants read their own bestiary flags) and an eligibility test
         * shared with the per-target apply loop below. The roll carries +4
         * when ANY committed target qualifies — exact for the normal
         * single-target swing; a mixed multi-target swing peels it back off
         * non-expert targets in the apply loop. */
        let _attacker = null;
        try {
            if (speaker?.token && speaker?.scene)
                _attacker = game.scenes?.get(speaker.scene)?.tokens?.get(speaker.token)?.actor ?? null;
            if (!_attacker && speaker?.actor) _attacker = game.actors?.get(speaker.actor) ?? null;
        } catch (_) { _attacker = null; }
        const isExpertTarget = (a) => {
            try {
                return !!(a?.type === "monster" && _attacker
                    && getResearchTier(_attacker, bestiaryKeyFor(a)) >= MAX_RESEARCH);
            } catch (_) { return false; }
        };
        const expertBonusInRoll = _preRollTargets.some(isExpertTarget) ? 4 : 0;
        const damageFormula = expertBonusInRoll ? `(${formula}) + ${expertBonusInRoll}` : formula;
        ...
        if (msg) {
            ...
            /* Foundry's ChatMessage permissions default to update:"OWNER",
             * and getUserLevel grants OWNER only to the message author.
             * When a player owns the attacker but the GM authored the
             * card (e.g. the attack was rolled from the combat dock on
             * their behalf), the player-side msg.update below throws and
             * only console.warns — the dice tray never appears on the
             * card and the click looks like a no-op. Route the mutation
             * through the GM in that case; local path still runs when
             * the current user actually owns the message. */
            ...
            if (canModifyMsg) {
                ...
                /* Non-author non-GM click: relay to the GM so the damage
                 * block still lands on the card. Handler is gated on
                 * attacker-ownership (mirrors the isAttacker || isGM
                 * check that lets the click reach us in the first place). */
                await emitAppendAttackFragment({
                    attackMessageUuid: msg.uuid,
                    fragment: stampedBlock,
                    summaryAction: ""
                });
            }
        }
        ...
        /* Targets were already resolved at the top of this function
         * BEFORE dice rolled — see `_preRollTargets` above. That covers
         * the canonical order (stamped defenderUuid → canvas targets →
         * tokenless flag → actor-picker fallback), and cancelling the
         * picker returns early so we don't reach this point without a
         * target. Use those directly. */
        let targets = _preRollTargets;
        ...
        if (targets.length && isDamaging) {
            ...
            /* Ammo UUID for ranged shots — the GM-side oil fold reads
             * appliedOil off THIS uuid (the fired arrow/bolt) instead of
             * the weapon's for ranged. Empty on melee/thrown attacks so
             * the weapon path stays authoritative. */
            const ammoUuid = btn.dataset.ammoUuid || "";
            /* Inline coating snapshot — captured at attack time so the
             * fold survives spendShot deleting the ammo document before
             * this click fires (see the attackRollFlavor call site).
             * Zero-bonus means "no coating on this shot". */
            const ammoOilBonus  = Number(btn.dataset.ammoOilBonus)  || 0;
            const ammoOilTarget = btn.dataset.ammoOilTarget || "";
            const ammoOilName   = btn.dataset.ammoOilName   || "";
            ...
            /* Non-lethal is normally derived from the strike (Pommel etc.); a
             * card may also flag it directly via data-non-lethal (brawl /
             * natural-weapon non-lethal → stamina). */
            const nonLethalHit = btn.dataset.nonLethal === "1" || !!strikeMeta?.nonLethal;
            for (const actor of targets) {
                /* The bestiary L6 expert +4 is already inside roll.total
                 * (folded into the visible damage roll above). If THIS target
                 * isn't one the attacker is an expert on — a mixed
                 * multi-target swing, or a Stand-Aside reroute onto a
                 * different creature — peel the +4 back off for this target. */
                const expertKeep = isExpertTarget(actor) ? expertBonusInRoll : 0;
                const weaponDamageForTarget = Math.max(0, roll.total - (expertBonusInRoll - expertKeep));
                try {
                    ...
                    await emitApplyDamage({
                        ...
                        weaponDamage:      weaponDamageForTarget,
                        ...
                        ammoOilTarget,
                        ammoOilName,
                        /* Non-lethal strikes (e.g. Pommel) route their damage to
                         * stamina instead of HP — see handleApplyDamage. */
                        nonLethal:         nonLethalHit,
                        attackMessageUuid: msg?.uuid ?? null
                    });
                } catch (err) {
                    console.warn(`witcher-ttrpg-death-march | apply damage to ${actor.name} failed`, err);
                }
                if (expertKeep > 0 && msg?.uuid) {
                    try {
                        await appendAttackResult(msg, {
                            fragment: `<div class="wdm-attack-rider"><i class="fa-solid fa-book-skull"></i> <strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.BestiaryExpertTitle", "Bestiary expert")}</strong>: ${tFormat("WITCHER.Doc.WeaponAttackMixin.Text.BestiaryExpertLine", { name: esc(actor.name) }, `+4 damage vs {name} — as an expert on this creature you know exactly where to strike.`)}</div>`,
                            summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Text.BestiaryExpertChip", "Expert: +4 dmg"), kind: "info", icon: "fa-book-skull" }
                        });
                    } catch (_) { /* rider is cosmetic — never block damage */ }
                }
                if (strikeAppliesStatus && actor?.uuid) {
                    ...
                    try {
                        if (HOLDS.includes(sid)) {
                            ...
                            await emitApplyStatus({ targetUuid: actor.uuid, statusId: sid, action: "apply", attackMessageUuid: msg?.uuid ?? null });
                        }
                        if (msg?.uuid) {
            ...
            /* Stamp what we applied so post-hoc handlers can compute
             * top-up damage against the SAME targets with the same
             * location / damage-type / weapon context. Currently
             * consumed by upgradeDeadlyFocusFromButton to retroactively
             * apply the crit-bonus delta when Deadly Focus is clicked
             * AFTER Roll Damage. Every field is a plain scalar / array
             * so the flag round-trips cleanly through Foundry storage. */
            if (msg && targets.length) {
                try {
                    const appliedTypes = typeKeysRaw.length
                        ? typeKeysRaw
                        : (types ? types.toLowerCase().split(/[\s·,/]+/).filter(Boolean) : []);
                    await msg.setFlag(SYSTEM_ID, "damageAppliedParams", {
                        targetUuids:     targets.map(t => t.uuid),
                        appliedSeverity: critSeverity ?? null,
                        locationKey:     locKey,
                        locationLabel:   locLabel,
                        damageTypes:     appliedTypes,
                        /* Weapon qualities that were on the initial apply.
                         * A retroactive top-up (Deadly Focus post-damage)
                         * needs to re-supply these so the calculator
                         * routes the delta through the SAME flags —
                         * without `isSilver: true` on a silver-quality
                         * weapon, the crit-bonus delta gets halved by
                         * the resistNonSilver stage, which the original
                         * damage apply had bypassed. */
                        qualities:       qualityKeys,
                        qualityValues,
                        weaponUuid
                    });
                } catch (err) {
                    console.warn(`${SYSTEM_ID} | stamp damageAppliedParams failed`, err);
                }
            }
        }
    } catch (err) {
        console.error("witcher-ttrpg-death-march | damage roll failed", err);
        ui.notifications?.error(t("WITCHER.Doc.WeaponAttackMixin.Notify.DamageRollFailedSeeConsole", "Damage roll failed — see console."));
    }
}
...
async function rollStunSaveFromButton(btn) {
    ...
    if (!actor?.rollStunSave) {
        ui.notifications?.warn(t("WITCHER.Doc.WeaponAttackMixin.Notify.CannotRollStunSaveForThat", "Cannot roll Stun save for that target."));
        return;
    }
    btn.disabled = true;
    try { await actor.rollStunSave({ modifier: mod }); }
        ...
        ui.notifications?.error(t("WITCHER.Doc.WeaponAttackMixin.Notify.StunSaveFailedSeeConsole", "Stun save failed — see console."));
        btn.disabled = false;
    }
}

/** GM-only picker to stamp a defender on an attack card that resolved
 *  without a target. Lists actors on the current scene first, then any
 *  other world characters/monsters, so common cases (something on the
 *  battlefield) land near the top. On confirm, writes the chosen actor's
 *  UUID to the message's `defenderUuid` flag — every downstream reader
 *  (rollDamageFromButton at :762, applyQualityRiders, hold-link riders)
 *  already picks up that flag as the authoritative defender. */
/** Resolve a defense for an attack card whose target is being assigned AFTER
 *  the roll (targetless attack → "Assign Target"). Prompts the picked defender
 *  (routed to their owner via the same socket relay the live attack + Stand
 *  Aside use), recomputes ONLY the defender-dependent verdict (HIT/MISS/CRIT)
 *  from the stored attack total, folds the defense + verdict into the card, and
 *  stamps `defenderUuid`/`defenseTotal`/`critSeverity` so Roll Damage lands on
 *  them. Every attacker-owned value (roll, mods, weapon, qualities, oils,
 *  damage formula, hit location) is preserved — nothing is re-rolled.
 *
 *  Self-contained on purpose: it duplicates the live flow's verdict markup
 *  rather than sharing it, so the normal targeted-attack path is untouched. */
async function resolveAssignedTargetDefense(msg, defenderActor) {
    if (!msg || !defenderActor) return;

    /* Attacker data stamped on the card at post time — read, never recomputed. */
    const attackTotal       = Number(msg.getFlag(SYSTEM_ID, "attackTotal"));
    const attackerUuid      = msg.getFlag(SYSTEM_ID, "attackerUuid") ?? "";
    const attackerName      = msg.getFlag(SYSTEM_ID, "attackerName") ?? msg?.speaker?.alias ?? t("WITCHER.Doc.WeaponAttackMixin.Text.Attacker", "Attacker");
    const attackWeaponUuid  = msg.getFlag(SYSTEM_ID, "attackWeaponUuid") ?? "";
    const attackWeaponName  = msg.getFlag(SYSTEM_ID, "attackWeaponName") ?? t("WITCHER.Doc.WeaponAttackMixin.Text.Weapon", "Weapon");
    ...
    /* Legacy cards (rolled before attackTotal was always stamped) can't be
     * re-resolved — fall back to the old behavior (stamp defender only). */
    if (!Number.isFinite(attackTotal)) {
        await msg.setFlag(SYSTEM_ID, "defenderUuid", defenderActor.uuid);
        ui.notifications?.info(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.TargetSet", { name: defenderActor.name }, `Target set to ${defenderActor.name}.`));
        return;
    }

    /* Weapon-derived defense gating: damage flags (deniesParry etc.) + whether
     * the shot is ranged/thrown (block-only). */
    let attackerDamageFlags = null;
    let firedRanged = false;
    try {
        const wpn = attackWeaponUuid ? await fromUuid(attackWeaponUuid) : null;
        const qs  = wpn?.system?.effective?.qualities ?? wpn?.system?.qualities ?? [];
        attackerDamageFlags = qualitiesToDamageFlags(qs);
        firedRanged = wpn ? isRangedWeapon(wpn) : false;
    } catch (_) { /* no weapon — permissive defaults */ }

    /* Engagement id suppresses the defender's standalone card so the defense
     * shows inline on the attack card. Reuse the card's if present. */
    let engagementId = msg.getFlag(SYSTEM_ID, "engagementId") ?? "";
    if (!engagementId) {
        engagementId = `assign-${msg.id}-${defenderActor.id}`;
        try { await msg.setFlag(SYSTEM_ID, "engagementId", engagementId); } catch (_) { /* non-fatal */ }
    }

    let defChoice;
    try {
        defChoice = await requestDefenseFromOwner({
            defenderActor,
            attackerName,
        ...
        console.warn(`${SYSTEM_ID} | assign-target defense request failed`, err);
        return;
    }
    if (!defChoice || !defChoice.action) return;   // dialog dismissed with no defense

    /* ── Verdict — defender-derived ONLY; attackTotal is the stored roll. ── */
    const defenseTotal = Number.isFinite(defChoice.defenseTotal) ? Number(defChoice.defenseTotal) : 10;
    const delta = attackTotal - defenseTotal;
    const sev   = critSeverityFromDelta(delta, hrCritBrackets());
    ...
    let defActionTitle = ({
        parry:      t("WITCHER.Doc.WeaponAttackMixin.DefAction.Parry", "Parry"),
        block:      t("WITCHER.Doc.WeaponAttackMixin.DefAction.Block", "Block"),
        dodge:      t("WITCHER.Common.Dodge", "Dodge"),
        reposition: t("WITCHER.Doc.WeaponAttackMixin.DefAction.Reposition", "Reposition"),
        none:       t("WITCHER.Doc.WeaponAttackMixin.DefAction.TookTheHit", "Took the hit"),
        stunned:    t("WITCHER.Doc.WeaponAttackMixin.DefAction.Stunned", "Stunned")
    })[defActionRaw] ?? (defActionRaw.charAt(0).toUpperCase() + defActionRaw.slice(1));
    if (defActionRaw === "brawlBlock") {
        const _gw = defChoice?.itemId ? defenderActor?.items?.get?.(defChoice.itemId) : null;
        defActionTitle = _gw?.name ?? t("WITCHER.Common.Brawling", "Brawling");
    }
    ...
    const chipRowHtml = (Array.isArray(defChoice.defenseChips) && defChoice.defenseChips.length)
        ? `<div class="wdm-attack-defense-chips" style="display:flex;flex-wrap:wrap;gap:3px;margin-top:3px;">${
            defChoice.defenseChips.filter(c => c && c.value != null && c.value !== "")
        ...
        : "";
    const defenseLabel = (isStunnedHit || isDc10Pass)
        ? `<div class="wdm-attack-defense"><span class="wdm-attack-defense-k">${t("WITCHER.Doc.WeaponAttackMixin.Text.Defense", "Defense")}</span> <span class="wdm-attack-defense-v"><b>${isStunnedHit ? "Stunned" : (defChoice.timedOut ? "No response" : "Took the hit")}</b> — DC <b>10</b></span></div>`
        : `<div class="wdm-attack-defense"><span class="wdm-attack-defense-k">${t("WITCHER.Doc.WeaponAttackMixin.Text.Defense", "Defense")}</span> <span class="wdm-attack-defense-v"><b>${esc(defActionTitle)}</b> → <b>${defenseTotal}</b></span>${chipRowHtml}${defChoice.defenseBody ? `<div class="wdm-attack-defense-body">${defChoice.defenseBody}</div>` : ""}</div>`;

    const verdictClass = delta > 0 ? (sev ? `is-hit is-crit is-crit-${sev}` : "is-hit") : "is-miss";
    const verdictHead  = delta > 0 ? (sev ? `HIT — ${sev.toUpperCase()} CRIT` : "HIT") : "MISS";
    const verdictSub   = delta > 0 ? `attacker beat by ${delta}` : delta === 0 ? "tie — defender wins" : `defender beat by ${Math.abs(delta)}`;
    const verdictRolls = `<span class="wdm-attack-verdict-rolls">attack <b>${attackTotal}</b> vs ${(isStunnedHit || isDc10Pass) ? "DC <b>10</b>" : `defense <b>${defenseTotal}</b>`}</span>`;
    const verdictHtml =
        `${defenseLabel}` +
    ...
    /* Crit banks 1 adrenaline for the attacker (matches the live flow). */
    if (delta > 0 && sev && isAdrenalineEnabled()) {
        try {
            const attacker = attackerUuid ? await fromUuid(attackerUuid) : null;
            const cur = Number(attacker?.system?.adrenaline?.value);
            const cap = Number(attacker?.system?.stats?.body?.value) || 0;
            if (attacker && Number.isFinite(cur)) {
                const nextAdr = Math.min(cur + 1, cap);
                if (nextAdr > cur) await attacker.update({ "system.adrenaline.value": nextAdr });
            }
        } catch (_) { /* best-effort */ }
    }

    /* Recalculate the WHOLE card in ONE write. Keep the attacker's original body
     * verbatim (roll, mods, chips, hit location), strip every defender-derived
     * block, then rebuild from scratch:
     *   body    = attacker flavor + defense block + verdict
     *   summary = verdict chip + (HIT → Roll Damage button; MISS → nothing)
     * A miss therefore has NO Roll Damage affordance anywhere — no surgical
     * append, no stored summary-action to leak the button back. */
    const root = document.createElement("div");
    root.innerHTML = String(msg.content ?? "");
    const existingCard = root.querySelector("details.wdm-attack-card");
    const bodyHost = existingCard?.querySelector(".wdm-attack-card-body") ?? root;

    /* Grab the ORIGINAL Roll Damage button (with its data-* attributes intact)
     * to re-surface in the summary — ONLY on a hit. Search the WHOLE card: on a
     * targetless card the button may already sit in the summary action slot,
     * not the body, so a body-only lookup would miss it and drop the button. */
    let summaryActionHtml = "";
    if (delta > 0) {
        const dmgBtn = root.querySelector('button[data-action="wdm-roll-damage"]');
        if (dmgBtn) {
            const clone = dmgBtn.cloneNode(true);
            clone.classList.add("wdm-card-sum-roll");
            summaryActionHtml = clone.outerHTML;
        }
        /* Witchers Reborn rider buttons that ride next to Roll Damage in the
         * summary on a hit — the same ones the live attack flow emits. */
        try {
            const attacker = attackerUuid ? await fromUuid(attackerUuid) : null;
            if (attacker) {
                // Cat · Deadly Focus — crit upgrade (crit landed AND upgradeable).
                if (sev && sev !== "deadly" && wrHeroic(attacker) === "deadlyFocus") {
                    summaryActionHtml +=
                        `<button type="button" class="wdm-attack-damage-roll wdm-attack-deadly-focus wdm-card-sum-roll" data-action="wdm-deadly-focus" ` +
                            `data-attacker-uuid="${esc(attacker.uuid)}" data-attack-msg-uuid="${esc(msg.uuid)}" ` +
                            `data-current-severity="${esc(sev)}" data-wr-rider="deadlyFocus">` +
                            `<i class="fa-solid fa-crosshairs"></i> Deadly Focus</button>`;
                }
                // Bear · Forceful Blow — strong-strike double-roll. Inherits the
                // Roll Damage button's data-* so the pipeline folds identically.
                const isStrongStrike = attackKind === "strong" || attackKind === "strongAttack" || attackKind === "charge";
                if (isStrongStrike && hasWRPerk(attacker, "forcefulBlow") && dmgBtn) {
                    const btnHtml = dmgBtn.outerHTML;
                    const grab = (attr) => { const m = btnHtml.match(new RegExp(`${attr}="([^"]*)"`)); return m ? m[1] : ""; };
                    const dataAttrs = ["data-formula", "data-types", "data-loc-mult", "data-loc-label", "data-loc-key", "data-strike", "data-silver-formula", "data-weapon-uuid", "data-quality-keys", "data-quality-values"]
                        .map(a => { const v = grab(a); return v ? `${a}="${v}"` : ""; }).filter(Boolean).join(" ");
                    summaryActionHtml +=
                        `<button type="button" class="wdm-attack-damage-roll wdm-card-sum-roll" data-action="wdm-forceful-blow" ` +
                            `data-attacker-uuid="${esc(attacker.uuid)}" ${dataAttrs}>` +
                            `<i class="fa-solid fa-hammer"></i> Forceful Blow — Roll Damage ×2, keep higher (5 STA)</button>`;
                }
            }
        } catch (_) { /* riders are best-effort */ }
    }
    /* Strip all defender-derived blocks + every damage affordance; what's left
     * is the attacker's own flavor. */
    bodyHost.querySelectorAll('.wdm-attack-damage, .wdm-attack-damage-note, .wdm-attack-defense, .wdm-attack-verdict, .wdm-card-sum-roll, button[data-action="wdm-roll-damage"]')
        .forEach(n => n.remove());
    const attackerBodyHtml = bodyHost.innerHTML;

    /* A fumbled roll (stamped on the card, or freshly re-resolved) shows FUMBLE
     * on the pip instead of reverting to MISS/HIT. */
    const isFumble = !!msg.getFlag(SYSTEM_ID, "fumble");
    const verdictLabel = isFumble ? "FUMBLE" : (delta > 0 ? (sev ? `${sev.toUpperCase()} CRIT` : "HIT") : "MISS");
    const verdictKind  = isFumble ? "fumble" : (delta > 0 ? (sev ? "crit" : "hit") : "miss");
    const parts = [{ label: verdictLabel, kind: verdictKind }];

    const details = document.createElement("details");
    details.className = "wdm-attack-card";
    details.open = false;
    details.dataset.summaryParts = JSON.stringify(parts);
    if (summaryActionHtml) details.dataset.summaryActionHtml = summaryActionHtml;
    const sumEl = document.createElement("summary");
    sumEl.className = "wdm-attack-card-summary";
    sumEl.innerHTML = renderCardSummary(parts, summaryActionHtml);
    const bodyEl = document.createElement("div");
    bodyEl.className = "wdm-attack-card-body";
    bodyEl.innerHTML = attackerBodyHtml + verdictHtml;
    details.append(sumEl, bodyEl);
    ...
    try {
        await msg.update({
            ...
            /* critSeverity is HIT-only — a miss must never leave a crit stamp the
             * damage pipeline could read. */
            [`flags.${SYSTEM_ID}`]: { defenderUuid: defenderActor.uuid, defenseTotal, critSeverity: (delta > 0 ? (sev ?? null) : null), critDelta: delta }
        });
    } catch (err) {
        console.warn(`${SYSTEM_ID} | assign-target card rebuild failed`, err);
    }
}

async function openAssignTargetDialog(msg) {
    const DialogV2 = foundry?.applications?.api?.DialogV2;
    if (!DialogV2 || !msg) return;

    /* Permission-aware picker. GM sees every actor. Players only see
     * actors they have at least LIMITED permission on — the actors they
     * could reasonably know about in the fiction. Prevents a player from
     * clicking Roll Damage → picking an NPC the GM has hidden and
     * shortcutting the reveal. */
    const user       = game.user;
    const isGM       = !!user?.isGM;
    const LIMITED    = CONST.DOCUMENT_OWNERSHIP_LEVELS?.LIMITED ?? 1;
    const canPick    = (a) => isGM || (a?.testUserPermission?.(user, LIMITED) ?? false);

    const sceneActorSet = new Set();
    try {
        for (const t of canvas?.scene?.tokens ?? []) {
            const a = t.actor;
            if (a && canPick(a)) sceneActorSet.add(a);
        }
    } catch (_) { /* no scene */ }

    const world = (game.actors?.contents ?? [])
        .filter(a => a && (a.type === "character" || a.type === "monster"))
        .filter(a => canPick(a))
        .filter(a => !sceneActorSet.has(a));

    const sceneList = [...sceneActorSet].sort((a, b) => a.name.localeCompare(b.name));
    const worldList = world.sort((a, b) => a.name.localeCompare(b.name));

    if (!sceneList.length && !worldList.length) {
        ui.notifications?.warn(isGM
            ? t("WITCHER.Doc.WeaponAttackMixin.Notify.NoActorsGM",     "No actors available to assign as a defender.")
            : t("WITCHER.Doc.WeaponAttackMixin.Notify.NoActorsPlayer", "No actors you can target — ask the GM to reveal an NPC or place a token you have access to."));
        return;
    }

    const monsterTag  = ` · ${t("WITCHER.Doc.WeaponAttackMixin.Text.MonsterTag", "monster")}`;
    const optRow = (a) => `<option value="${esc(a.uuid)}">${esc(a.name)}${a.type === "monster" ? monsterTag : ""}</option>`;
    const sceneGroup = sceneList.length
        ? `<optgroup label="${t("WITCHER.Doc.WeaponAttackMixin.Text.OnCurrentScene", "On the current scene")}">${sceneList.map(optRow).join("")}</optgroup>`
        : "";
    const worldGroup = worldList.length
        ? `<optgroup label="${t("WITCHER.Doc.WeaponAttackMixin.Text.WorldActors", "World actors")}">${worldList.map(optRow).join("")}</optgroup>`
        : "";

    const content = `
        <p style="margin:0 0 0.5em;">${t("WITCHER.Doc.WeaponAttackMixin.Text.AssignTargetBlurb", "Pick who this attack lands on. No live target token is required — the picked actor is stamped on the attack card as the defender, so Roll Damage applies to them.")}</p>
        <select id="wdm-assign-target-select" style="width:100%;">
            <option value="">${t("WITCHER.Doc.WeaponAttackMixin.Text.PickAnActor", "— pick an actor —")}</option>
            ${sceneGroup}
            ${worldGroup}
        </select>
    `;
    ...
    try {
        chosen = await DialogV2.prompt({
            window: { title: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Title.AssignAttackTarget", "Assign Attack Target") },
            content,
            ok: {
                label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.Assign", "Assign"),
                callback: (event, button, dialog) =>
                    dialog.element.querySelector("#wdm-assign-target-select")?.value ?? ""
            },
            rejectClose: false
        });
    } catch (_) { return; }
    ...
    /* Assigning a target to a targetless attack now runs the FULL resolution:
     * prompt the picked defender, recompute the HIT/MISS/CRIT verdict against
     * the stored attack total, fold the defense + verdict into the card, and
     * stamp the defender so Roll Damage lands on them — all attacker values
     * preserved. (Falls back to a plain defender stamp for legacy cards that
     * predate the always-stamped attackTotal.) */
    let defenderActor;
    try { defenderActor = await fromUuid(chosen); }
    catch (_) { defenderActor = null; }
    if (!defenderActor) {
        ui.notifications?.error(t("WITCHER.Doc.WeaponAttackMixin.Notify.FailedToAssignTheTargetSee", "Failed to assign the target — see console."));
        return;
    }
    try {
        await resolveAssignedTargetDefense(msg, defenderActor);
    } catch (err) {
        console.warn(`${SYSTEM_ID} | assign-target: resolve failed`, err);
        ui.notifications?.error(t("WITCHER.Doc.WeaponAttackMixin.Notify.FailedToAssignTheTargetSee", "Failed to assign the target — see console."));
    }
}
...
export function installAttackChatHandlers() {
    Hooks.on("renderChatMessageHTML", (msg, el) => {
        ...
        /* "Assign Target" affordance: if the attack resolved without a
           defenderUuid stamped on the message and there IS a damage
           button on the card, inject a button next to it so the picker
           can be opened. Available to the attacker (owner of the card's
           speaker) AND the GM. Players are permission-filtered inside
           the picker itself (`openAssignTargetDialog` shows only actors
           they have LIMITED+ on), so surfacing the button to non-GMs
           doesn't leak hidden actors. Once assigned, Roll Damage picks
           up the flag via the same defenderUuid path pre-targeted
           attacks use — no separate wiring needed. */
        const alreadyStamped = !!msg?.getFlag?.(SYSTEM_ID, "defenderUuid");
        if ((isAttacker || isGM) && !alreadyStamped) {
            for (const rollBtn of btns) {
                if (rollBtn.parentElement?.querySelector('[data-action="wdm-assign-target"]')) continue;
                const assignBtn = document.createElement("button");
                assignBtn.type = "button";
                assignBtn.className = "wdm-attack-assign-target";
                assignBtn.dataset.action = "wdm-assign-target";
                assignBtn.title = isGM
                    ? t("WITCHER.Doc.WeaponAttackMixin.Tip.AssignDefenderGM",     "Assign a defender for this attack")
                    : t("WITCHER.Doc.WeaponAttackMixin.Tip.AssignDefenderPlayer", "Assign a defender (limited to actors you can see)");
                assignBtn.innerHTML = `<i class="fa-solid fa-crosshairs"></i> ${t("WITCHER.Doc.WeaponAttackMixin.Text.AssignTarget", "Assign Target")}`;
                assignBtn.addEventListener("click", (e) => {
                    e.stopPropagation();
                    openAssignTargetDialog(msg);
                });
                rollBtn.parentElement.insertBefore(assignBtn, rollBtn);
            }
        }
         ...
         * they can't spend the attacker's STA. The button now lives in the
         * summary action slot (`.wdm-card-sum-action`) instead of a body
         * rider div, so the strip removes just the button if no wr-rider
         * wrapper is present. */
        const fbBtn = el.querySelector?.('button[data-action="wdm-forceful-blow"]');
        if (fbBtn) {
            if (!isAttacker && !isGM) {
                (fbBtn.closest(".wdm-wr-rider") ?? fbBtn).remove();
        ...
        /* Witchers Reborn — Deadly Focus button on the attack card.
         * User spec: sits inline next to Roll Damage in the card's
         * SUMMARY (the "header" — the always-visible row of the
         * <details> outer wrapper). Do NOT leave a copy in the body.
         * Click opens a picker that lets the attacker upgrade the crit
         * severity by any number of tiers they can afford. If Roll
         * Damage has already fired, the handler retroactively applies
         * the crit-bonus delta as a top-up (see the damageAppliedParams
         * flag stamp in rollDamageFromButton). */
        {
            const dfButtons = Array.from(el.querySelectorAll?.('button[data-action="wdm-deadly-focus"]') ?? []);
            if (dfButtons.length) {
                if (!isAttacker && !isGM) {
                    dfButtons.forEach(b => b.remove());
                } else {
                    /* Keep ONE, drop any accidental duplicates from
                     * previous render/append passes. */
                    const dfBtn = dfButtons[0];
                    for (let i = 1; i < dfButtons.length; i++) dfButtons[i].remove();
                    /* Anchor priority: summary action slot > any Roll
                     * Damage button. Falls back to leaving it in place
                     * only if nothing else exists. */
                    const summaryAction = el.querySelector?.(".wdm-card-sum-action");
                    const rollBtn       = el.querySelector?.('button[data-action="wdm-roll-damage"]');
                    let anchor = summaryAction ?? rollBtn?.parentElement ?? null;
                    if (anchor && anchor !== dfBtn.parentElement) {
                        anchor.appendChild(dfBtn);
                    }
                    if (!dfBtn.dataset.wired) {
                        dfBtn.dataset.wired = "1";
                        dfBtn.addEventListener("click", (e) => {
                            e.stopPropagation();
                            openDeadlyFocusPicker(dfBtn, msg);
                        });
                    }
                }
            }
                ...
                if (defender && defTok) {
                    ...
                    if (eligible.length) {
                        ...
                        div.innerHTML =
                            ...
                            `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.StandAside", "Stand Aside")}</strong> — ` +
                            ...
                            `<button type="button" class="wdm-attack-damage-roll" data-action="wdm-stand-aside" ` +
                                ...
                                t("WITCHER.Doc.WeaponAttackMixin.Text.StandAsideBtn", "Stand Aside — spend 1 adrenaline") +
                            `</button>`;
...
async function rollChargeProneFromButton(btn, msg) {
    ...
    if (!attackerUuid || !targetUuid) {
        ui.notifications?.warn?.(t("WITCHER.Doc.WeaponAttackMixin.Notify.ChargeProneNoActor", "Charge prone: missing actor references."));
        return;
    }
    ...
    try {
        ...
        if (attackerWins) {
            try {
                await emitApplyStatus({ targetUuid: target.uuid, statusId: "prone", action: "apply", sourceActorUuid: attacker?.uuid });
            } catch (err) {
        ...
        const fragment =
            `<div class="wdm-attack-rider">` +
                ...
                `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.ChargeProne", "Charge → Prone")}</strong>: ` +
        ...
        if (msg) {
            ...
            await appendAttackResult(msg, {
                fragment,
                summaryAdd: {
                    label: attackerWins ? t("WITCHER.Doc.WeaponAttackMixin.Text.ChargeProned", "Charge: target proned") : t("WITCHER.Doc.WeaponAttackMixin.Text.ChargeProneFailed", "Charge: prone attempt failed"),
        ...
        ui.notifications?.warn?.(t("WITCHER.Doc.WeaponAttackMixin.Notify.ChargeProneRollFailed", "Charge prone: roll failed."));
    }
}
...
async function autoRollBashOutcome(attacker, target, msg) {
    ...
    try {
        ...
        const buildFormula = (basic, cv) => {
            const parts = ["1d10"];
            if (basic) parts.push(String(basic));
            if (cv)    parts.push(String(cv));
            return parts.join(" + ");
        };
        ...
        if (attackerWins) {
            ...
            try {
                ...
                if (meters > 0 && attackerToken && targetToken?.document?.uuid) {
                    ...
                    const wall  = res?.hitWall ? t("WITCHER.Doc.WeaponAttackMixin.Text.CutShortWall", " — cut short against a wall") : "";
                    ...
                    /* Shoved into a wall — flat 1d6 ramming, bludgeoning to the
                     * torso, bypassing worn/natural armour but NOT a magic shield
                     * (Quen/Barrier soaks it). Same shape as Push Kick / Slam. */
                    if (res?.hitWall) {
                        try {
                            const rr   = await new Roll("1d6").evaluate();
                            const wdmg = Math.max(0, Math.floor(Number(rr.total) || 0));
                            if (wdmg > 0) {
                                await emitApplyDamage({
                                ...
                                pushLine += `<div style="color:#a44"><b>${esc(target.name)}</b> slams into the wall — <b>${wdmg}</b> ramming damage (torso, ignores armor).</div>`;
                            }
                        } catch (err) { console.warn("witcher-ttrpg-death-march | bash wall ramming failed", err); }
                    }
                } else if (meters > 0) {
            ...
            if (bigWin) {
                try {
                    await emitApplyStatus({ targetUuid: target.uuid, statusId: "staggered", action: "apply", sourceActorUuid: attacker?.uuid });
                    staggerLine = `<div style="color:#a84"><b>${esc(target.name)}</b> is <b>${t("WITCHER.Doc.WeaponAttackMixin.Text.Staggered", "Staggered")}</b> (delta ${delta}).</div>`;
                } catch (err) {
        ...
        const fragment =
            `<div class="wdm-attack-rider">` +
                ...
                `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.BashPhysiqueVsPhysique", "Bash → Physique vs Physique")}</strong>` +
                `<div class="wdm-bash-rolls" style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;margin-top:0.375rem;">` +
                    ...
                    `</div>` +
                `</div>` +
                `<div style="margin-top:0.375rem;">${t("WITCHER.Doc.WeaponAttackMixin.Text.Totals", "Totals:")} <b>${aTotal}</b> vs <b>${dTotal}</b> — delta <b>${delta >= 0 ? "+" : ""}${delta}</b>.</div>` +
        ...
        if (msg) {
            await appendAttackResult(msg, {
                fragment,
                summaryAdd: {
                    label: attackerWins ? t("WITCHER.Doc.WeaponAttackMixin.Text.BashPushed", "Bash: pushed") : t("WITCHER.Doc.WeaponAttackMixin.Text.BashHeld", "Bash: held"),
                    kind:  attackerWins ? "hit" : "miss",
        ...
        ui.notifications?.warn?.(t("WITCHER.Doc.WeaponAttackMixin.Notify.BashRollFailed", "Bash outcome: roll failed."));
    }
}
...
function findStandAsideCandidates(defender, defToken, attackerUuid = "") {
    ...
    for (const tok of (canvas?.tokens?.placeables ?? [])) {
        ...
        // Per-actor STA cost — respects any adrenalineStaCostDelta AEs on this
        // specific candidate (a Manticore under a "cheap adrenaline" buff can
        // Stand Aside even when a base-cost one couldn't).
        const staCost = adrenalineStaPerDieFor(a);
...
async function applyStandAsideFromButton(btn) {
    ...
    if (ae < 1) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.StandAsideNeeds1AdrenalineX", { actor: actor.name, ae: ae }, "Stand Aside needs 1 adrenaline — {actor} has {ae}."));
        return;
    }
    if (sta < staCost) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.StandAsideNeedsXSTAX", { staCost: staCost, actor: actor.name, sta: sta }, "Stand Aside needs {staCost} STA — {actor} has {sta}."));
        return;
    }
    btn.dataset.consumed = "1";
    ...
    try {
        ...
        btn.closest(".wdm-wr-rider")?.replaceWith(
            (() => {
                ...
                div.innerHTML =
                    `<i class="fa-solid fa-shield-heart"></i> ` +
                    `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.StandAside", "Stand Aside")}</strong> — ${esc(leapLine)}${esc(verdictText)}`;
                return div;
            })()
        );
    } catch (err) {
...
async function applyRiposteStrikeFromButton(btn) {
    ...
    const sta = Number(actor.system?.derivedStats?.sta?.value) || 0;
    if (sta < staCost) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.RiposteNeedsXSTAXHas", { staCost: staCost, actor: actor.name, sta: sta }, "Riposte needs {staCost} STA — {actor} has {sta}."));
        return;
    }
    btn.dataset.consumed = "1";
    ...
    try {
        ...
        btn.closest(".wdm-wr-rider")?.replaceWith(
            (() => {
                ...
                div.innerHTML =
                    `<i class="fa-solid fa-shield-heart"></i> ` +
                    `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.Riposte", "Riposte")}</strong> — ${esc(actor.name)} spent ${staCost} STA and ` +
...
/** Picker dialog: shows every affordable crit-tier target and lets the
 *  attacker pick how far to bump. Cost is by TIER DISTANCE: the Nth step from
 *  your current tier costs N adrenaline (+ per-die STA), so one tier is always
 *  1 and picking Deadly from Simple costs 1+2+3 = 6. Steps the attacker can't
 *  afford are disabled with a reason. */
async function openDeadlyFocusPicker(btn, msg) {
    if (btn?.dataset?.consumed === "1") return;
    const attackerUuid = btn?.dataset?.attackerUuid;
    const attacker = attackerUuid ? await fromUuid(attackerUuid) : null;
    if (!attacker) return;
    /* Current severity — prefer live message flag (survives DF chains),
     * fall back to the dataset the initial render stamped. */
    const attackMsgUuid = btn?.dataset?.attackMsgUuid;
    const targetMsg     = attackMsgUuid ? await fromUuid(attackMsgUuid) : msg;
    const currentSev    = targetMsg?.getFlag?.(SYSTEM_ID, "critSeverity")
                       ?? btn?.dataset?.currentSeverity
                       ?? "simple";
    /* Tier ladder. Cost is TIER-DISTANCE based, not per-destination: the Nth
     * step away from your current tier costs N adrenaline. So a single-tier bump
     * is always 1 (difficult→deadly = 1) while a full simple→deadly is 1+2+3 = 6.
     * Cumulative tallies built below. */
    const STEPS = [
        { from: "simple",    to: "complex"   },
        { from: "complex",   to: "difficult" },
        { from: "difficult", to: "deadly"    }
    ];
    const idxOf = (sev) => STEPS.findIndex(s => s.from === sev);
    const start = idxOf(currentSev);
    if (start < 0 || start >= STEPS.length) {
        ui.notifications?.info(t("WITCHER.Doc.WeaponAttackMixin.Notify.NoFurtherCritTierToUpgrade", "No further crit tier to upgrade to."));
        return;
    }
    const staPerDie = adrenalineStaPerDieFor(attacker);
    const aeAvail   = Number(attacker.system?.adrenaline?.value) || 0;
    const staAvail  = Number(attacker.system?.derivedStats?.sta?.value) || 0;
    /* Build the ladder of {target severity, cumulative ae, cumulative sta,
     * affordable, reason}. */
    const rows = [];
    let cumAe = 0;
    for (let i = start; i < STEPS.length; i++) {
        cumAe += (i - start + 1);   // Nth step FROM the current tier costs N adrenaline
        const cumSta = cumAe * staPerDie;
        let reason = "";
        if (aeAvail < cumAe)  reason = `needs ${cumAe} adrenaline (have ${aeAvail})`;
        else if (staAvail < cumSta) reason = `needs ${cumSta} STA (have ${staAvail})`;
        rows.push({
            targetSev: STEPS[i].to,
            cumAe, cumSta,
            affordable: !reason,
            reason
        });
    ...
    const DialogV2 = foundry?.applications?.api?.DialogV2;
    if (!DialogV2) return;
    /* Radios in the same NAME group participate in one RadioNodeList
     * so the DialogV2 callback can read the selected value straight
     * off `button.form.elements`. The first affordable row is pre-
     * checked so hitting Enter doesn't return a null selection. */
    let firstAffordable = -1;
    const optionsHtml = rows.map((r, i) => {
        if (firstAffordable < 0 && r.affordable) firstAffordable = i;
        const checked = firstAffordable === i ? " checked" : "";
        const label = r.targetSev[0].toUpperCase() + r.targetSev.slice(1);
        const cost  = `${r.cumAe} adrenaline · ${r.cumSta} STA`;
        return `<label style="display:flex;gap:0.5em;align-items:center;padding:0.25em 0;${r.affordable ? "" : "opacity:0.5;"}">` +
            `<input type="radio" name="wdm-df-target" value="${esc(r.targetSev)}" data-ae="${esc(String(r.cumAe))}" data-sta="${esc(String(r.cumSta))}"${r.affordable ? checked : " disabled"} />` +
            `<span style="min-width:5.5em;font-weight:600;">${esc(label)}</span>` +
            `<span style="opacity:0.85;">${esc(cost)}</span>` +
            `${r.reason ? `<span style="opacity:0.7;margin-left:auto;font-size:0.75em;">— ${esc(r.reason)}</span>` : ""}` +
            `</label>`;
    }).join("");
    if (firstAffordable < 0) {
        ui.notifications?.warn(t("WITCHER.Doc.WeaponAttackMixin.Notify.DeadlyFocusCantAfford", "You can't afford any Deadly Focus tier upgrade right now."));
        return;
    }
    let chosen = null;
    try {
        chosen = await DialogV2.prompt({
            window: { title: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Title.DeadlyFocusPickTargetSeverity", "Deadly Focus — pick target severity"), icon: "fa-solid fa-crosshairs" },
            content: `<div style="margin-bottom:0.5em;font-size:0.85em;opacity:0.85;">` +
                tFormat("WITCHER.Doc.WeaponAttackMixin.Text.DeadlyFocusIntro", { current: esc(currentSev[0].toUpperCase() + currentSev.slice(1)), ae: aeAvail, sta: staAvail }, `Current: <b>${esc(currentSev[0].toUpperCase() + currentSev.slice(1))}</b>. You have <b>${aeAvail}</b> adrenaline · <b>${staAvail}</b> STA. Cost is cumulative — jumping 2 tiers spends both step costs.`) +
                `</div>` +
                `<div class="wdm-df-picker">${optionsHtml}</div>`,
            ok: {
                label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.Upgrade", "Upgrade"),
                /* Match the (event, button) shape used elsewhere in the
                 * codebase (see enhancementSlots.mjs:266). Read the
                 * checked radio via the form; RadioNodeList's `.value`
                 * shortcut returns the selected value. */
                callback: (_event, dialogButton) => {
                    const form = dialogButton?.form;
                    if (!form) return null;
                    const picked = form.querySelector('input[name="wdm-df-target"]:checked');
                    if (!picked) return null;
                    return {
                        targetSev: picked.value,
                        aeCost:    Number(picked.dataset.ae) || 0,
                        staCost:   Number(picked.dataset.sta) || 0
                    };
                }
            },
            rejectClose: false
        ...
        console.warn(`${SYSTEM_ID} | Deadly Focus picker failed`, err);
        return;
    }
    if (!chosen) return;
    /* Route through the existing debit + apply + card-update path. */
    await upgradeDeadlyFocusFromButton(btn, msg, {
        targetSev: chosen.targetSev,
        aeCost:    chosen.aeCost,
        staCost:   chosen.staCost,
        startSev:  currentSev
    });
}

async function upgradeDeadlyFocusFromButton(btn, msg, choice) {
    if (btn?.dataset?.consumed === "1") return;
    if (!choice?.targetSev || !(choice.aeCost > 0)) return;
    const attackerUuid = btn?.dataset?.attackerUuid;
    const attackMsgUuid = btn?.dataset?.attackMsgUuid;
    const nextSeverity = choice.targetSev;
    const aeCost       = Number(choice.aeCost) || 0;
    const staCost      = Number(choice.staCost) || 0;
    if (!attackerUuid) return;
    const attacker = await fromUuid(attackerUuid);
    if (!attacker) return;
    const ae = Number(attacker.system?.adrenaline?.value) || 0;
    if (ae < aeCost) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.DeadlyFocusNeedsXAdrenalineX", { aeCost: aeCost, attacker: attacker.name, ae: ae }, "Deadly Focus needs {aeCost} adrenaline — {attacker} has {ae}."));
        return;
    }
    const staCur = Number(attacker.system?.derivedStats?.sta?.value) || 0;
    if (staCur < staCost) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.DeadlyFocusNeedsXSTAX", { staCost: staCost, attacker: attacker.name, staCur: staCur }, "Deadly Focus needs {staCost} STA — {attacker} has {staCur}."));
        return;
    }
    btn.dataset.consumed = "1";
    btn.disabled = true;
    try {
        ...
        /* Retroactive crit-bonus top-up.
         *
         * If Roll Damage already fired for this attack (rollDamageFromButton
         * stamps `damageAppliedParams` on the message when it applies), the
         * damage that landed already included the OLD crit bonus. Just
         * updating the message flag now would be silent — the damage
         * was based on the old severity. Fold the DELTA into a follow-up
         * emitApplyDamage: weaponDamage=0 + critBonus=delta means the
         * calculator's stage pipeline only routes the delta through SP
         * (which bypasses crit bonus anyway) and the location multiplier,
         * so multi-target hits and Left-Leg × 0.5 shots stay coherent. */
        const applied = targetMsg?.getFlag?.(SYSTEM_ID, "damageAppliedParams") ?? null;
        if (applied && Array.isArray(applied.targetUuids) && applied.targetUuids.length) {
            for (const tUuid of applied.targetUuids) {
                try {
                    const tActor = await fromUuid(tUuid);
                    if (!tActor) continue;
                    /* Look up the target's immuneToOrganCrits so the ladder
                     * matches what the ORIGINAL apply used — the calc
                     * itself re-derives the correct bonus for the given
                     * severity when we pass critSeverity through, but
                     * sending an explicit critBonus keeps the delta
                     * deterministic regardless of any post-hoc ladder
                     * config change. */
                    const immuneOrgan = !!tActor?.system?.combat?.immuneToOrganCrits
                        || tActor?.system?.category === "elementa"
                        || tActor?.system?.category === "specter";
                    /* Read the same live ladder pair the socket's initial
                     * apply will use, so an upgrade delta matches what a
                     * fresh crit at nextSeverity would grant. */
                    const ladders = hrCritBonusLadders();
                    const ladder = immuneOrgan ? ladders.noOrgans : ladders.normal;
                    const oldBonus = ladder[applied.appliedSeverity] ?? 0;
                    const newBonus = ladder[nextSeverity]           ?? 0;
                    const delta    = Math.max(0, newBonus - oldBonus);
                    if (delta <= 0) continue;
                    await emitApplyDamage({
                        targetUuid:        tUuid,
                        weaponDamage:      0,
                        silverDamage:      0,
                        critBonus:         delta,
                        damageTypes:       Array.isArray(applied.damageTypes) ? applied.damageTypes : [],
                        locationKey:       applied.locationKey ?? "",
                        locationLabel:     applied.locationLabel ?? "",
                        /* Re-supply the original apply's quality flags so
                         * the calculator routes the crit-bonus delta
                         * through the SAME resist / bypass logic. Silver-
                         * quality weapons short-circuit the crit-bonus
                         * halving via `isSilver: true` (see damageCalc
                         * applyNaturalResists); without these flags, a
                         * +7 delta from Simple→Deadly gets halved to +3
                         * against a silver-weak target. */
                        qualities:         Array.isArray(applied.qualities) ? applied.qualities : [],
                        qualityValues:     applied.qualityValues ?? {},
                        weaponUuid:        applied.weaponUuid ?? "",
                        attackMessageUuid: targetMsg.uuid,
                        /* Mark this as a top-up so downstream logging /
                         * chat cards can distinguish "delta from Deadly
                         * Focus upgrade" from a fresh damage apply. */
                        isCritUpgradeTopUp: true
                    });
                } catch (err) {
                    console.warn(`${SYSTEM_ID} | DF top-up damage failed for ${tUuid}`, err);
                }
            }
            /* Refresh the stamp so a hypothetical second DF chain (rare —
             * only if the GM re-arms via macro) starts from the new
             * severity as the baseline. */
            try {
                await targetMsg.setFlag(SYSTEM_ID, "damageAppliedParams", {
                    ...applied,
                    appliedSeverity: nextSeverity
                });
            } catch (_) { /* non-fatal */ }
        }
        ...
        if (targetMsg) {
             ...
             *
             * ALSO update the summary-parts JSON on `.wdm-attack-card`
             * so the crit CHIP in the collapsed header shows the new
             * severity, and re-render the summary from the updated
             * parts + persisted action HTML. Without this the body
             * verdict block reads "COMPLEX CRIT" but the pip in the
             * collapsed header still reads "SIMPLE CRIT". */
            const cur = String(targetMsg.content ?? "");
            const nextLabel = nextSeverity[0].toUpperCase() + nextSeverity.slice(1);
            let upgradedVerdict = cur
                .replace(/HIT — \w+ CRIT/i, `HIT — ${nextLabel.toUpperCase()} CRIT`)
                .replace(/is-crit-\w+/g, `is-crit-${nextSeverity}`);
            try {
                const parseDoc = document.createElement("div");
                parseDoc.innerHTML = upgradedVerdict;
                const card = parseDoc.querySelector("details.wdm-attack-card");
                if (card) {
                    /* Rewrite the crit chip in data-summary-parts. */
                    let parts = [];
                    try { parts = JSON.parse(card.dataset.summaryParts || "[]"); }
                    catch (_) { parts = []; }
                    for (const p of parts) {
                        if (p && typeof p === "object" && p.kind === "crit") {
                            p.label = `${nextLabel.toUpperCase()} CRIT`;
                        }
                    }
                    card.dataset.summaryParts = JSON.stringify(parts);
                    /* Re-render the summary from the fresh parts +
                     * persisted action HTML. renderCardSummary is
                     * defined at the top of this file (it's the same
                     * function _doAppendAttackResult calls). */
                    const sumEl = card.querySelector(".wdm-attack-card-summary");
                    if (sumEl) {
                        sumEl.innerHTML = renderCardSummary(parts, card.dataset.summaryActionHtml || "");
                    }
                    upgradedVerdict = parseDoc.innerHTML;
                }
            } catch (err) {
                console.warn(`${SYSTEM_ID} | DF summary chip rewrite failed`, err);
            }
        ...
        const notice = `<div class="wdm-attack-rider wdm-wr-rider-result"><i class="fa-solid fa-crosshairs"></i> <strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.DeadlyFocus", "Deadly Focus")}</strong> — ${attacker.name} upgrades crit to <strong>${nextSeverity[0].toUpperCase() + nextSeverity.slice(1)}</strong>${applied ? " <em>(retroactive top-up applied)</em>" : ""} (spent ${aeCost} adrenaline + ${staCost} STA).</div>`;
        if (targetMsg) {
...
async function rollPirouetteFromButton(btn, msg) {
    ...
    if (ae <= 0) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.PirouetteXHasNoAdrenaline", { attacker: attacker.name }, "Pirouette: {attacker} has no adrenaline."));
        return;
    }
    const staPer  = adrenalineStaPerDieFor(attacker);
    const staPool = Math.floor(sta / staPer);
    const maxDice = Math.min(ae, staPool);
    if (maxDice <= 0) {
        ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.PirouetteNeedsAtLeastXSTA", { staPer: staPer, attacker: attacker.name, sta: sta }, "Pirouette needs at least {staPer} STA per die — {attacker} has {sta}."));
        return;
    }
    const DialogV2 = foundry?.applications?.api?.DialogV2;
    if (!DialogV2) return;
    const dice = await DialogV2.prompt({
        window: { title: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Title.PirouetteAdrenalineDice", "Pirouette — adrenaline dice") },
        content:
            `<div style="padding:8px 0; font-size:0.8125rem;">` +
                `<label style="display:flex; gap:10px; align-items:center;">` +
                    `<span style="min-width:80px;">${t("WITCHER.Doc.WeaponAttackMixin.Text.Dice", "Dice")}</span>` +
                    `<input type="number" name="dice" min="1" max="${maxDice}" step="1" value="1" autofocus />` +
                `</label>` +
                `<p class="hint" style="opacity:0.7; margin-top:6px;">${tFormat("WITCHER.Doc.WeaponAttackMixin.Text.PirouetteHint", { maxDice, staPer, name: attacker.name }, `Up to ${maxDice} (limited by adrenaline pool + STA @ ${staPer}/die). Each die adds +1 to <strong>${attacker.name}</strong>'s next attack's feint bonus (base +3).`)}</p>` +
            `</div>`,
        ok: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.Spin", "Spin"), callback: (_e, b) => Number(b.form?.elements?.dice?.value) || 0 },
        rejectClose: false
    }).catch(() => null);
    ...
    try {
        ...
        const fragment =
            `<div class="wdm-attack-rider wdm-wr-rider-result">` +
                ...
                `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.Pirouette", "Pirouette")}</strong> — ${esc(attacker.name)}'s next attack at <strong>+${total}</strong> ` +
            ...
            if (summaryMsgId) {
                ...
                if (summaryMsg) {
                    ...
                    await summaryMsg.update({
                        content: tFormat(
                            "WITCHER.Doc.WeaponAttackMixin.Chat.FeintDetailed",
                            { attacker: esc(attacker.name), target: esc(targetName), total },
                            "<em><strong>{attacker}</strong> feints — next attack against <strong>{target}</strong> at <strong>+{total}</strong>.</em>"
                        )
                    });
                }
            }
...
async function rollForcefulBlowDamageFromButton(btn, msg) {
    ...
    try {
        ...
        const keptLabel = totalA >= totalB ? t("WITCHER.Doc.WeaponAttackMixin.Text.RollA", "Roll A") : t("WITCHER.Doc.WeaponAttackMixin.Text.RollB", "Roll B");
        const diceHtmlA = await rollA.render();
        const diceHtmlB = await rollB.render();

        /* No separate silver-damage roll: the Silver Weapon Trait makes the
         * WHOLE strike count as silver (folded via damageFlags downstream), so
         * there is no second damage value for Forceful Blow to roll. */
        const silverTotal = 0;
        const types    = btn.dataset.types || "";
        const typeKeysRaw = (() => {
            try {
        ...
        const note = locMult !== 1
            ? `<div class="wdm-attack-damage-note">${esc(game.i18n.localize("WITCHER.Attack.SubtractSP"))} ×${esc(locMult)}${locLabel ? ` (${esc(locLabel)})` : ""}</div>`
            : "";
        const silverBlockHtml = "";

        const damageBlockHtml =
            `<div class="wdm-attack-damage-roll-block" data-rider="forcefulBlow">` +
                `<div class="wdm-attack-damage-roll-head">` +
                    `<i class="fa-solid fa-hammer"></i> <strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.ForcefulBlow", "Forceful Blow")}</strong> — ${esc(head)}${types ? ` <span class="wdm-attack-damage-roll-types">— ${esc(types)}</span>` : ""} ` +
        ...
        if (msg) {
             ...
             *
             * Same author-only permission gate as rollDamageFromButton
             * (see there for the full rationale): route the update via
             * the GM when the current user isn't the message author. */
            const canModifyMsg = !!msg?.canUserModify?.(game.user, "update");
            if (canModifyMsg) {
                try {
                    const tmp = document.createElement("div");
...
function defaultDeclaration(baseTotal, forcedExtra = false, aimBonus = 0, offhandPenalty = 0, overrides = {}) {
    ...
    if (strikeMeta?.toHit) {
        /* Strong-strike penalty lives in the House Rules panel so a
         * GM can retune it globally; every other strike uses the
         * strike-table constant. `strong` covers both RAW's `strong`
         * key and CE's `strongAttack` key — both map to category
         * `strong` and both are the entry a house-rule tuneable
         * should override. */
        const strikeCat = ({ strong: "strong", strongAttack: "strong" })[strikeKey] ?? strikeKey;
        const _toHit = (strikeCat === "strong") ? hrStrongStrikePenalty() : strikeMeta.toHit;
        modTotal += _toHit;
        chips.push({ label: game.i18n.localize(strikeMeta.labelKey ?? `WITCHER.Attack.Strike${strikeKey[0].toUpperCase() + strikeKey.slice(1)}`), value: _toHit });
    }
    if (forcedExtra) {
        const _extra = hrExtraActionToHit();
        modTotal += _extra;
        chips.push({ label: game.i18n.localize("WITCHER.Attack.ExtraActionForced"), value: _extra });
    }
    if (offhandPenalty) {
...
export const weaponAttackMixin = (Base) => class extends Base {
    ...
    async weaponAttack(weapon, options = {}) {
        ...
        /* Pinner lock: while pinning someone you can only act against that foe.
         * Refuse an attack aimed at anyone else. */
        if (options.forceDefender && holdTargetBlockReason(this, options.forceDefender) === "pinning") {
            ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.PinnerLocked", { name: this.name }, "{name} is pinning someone — they can only act against the pinned foe."));
            return null;
        }
        /* Pinned-attacker lock: a pinned actor's ONLY melee option is a Reverse
         * against the foe holding them. Registry-based (representation-proof).
         * Early refuse if the target isn't a foe holding you; the post-dialog
         * check then limits the action itself to the Reverse. */
        const _selfPinned = isPinned(this);
        if (_selfPinned && !(options.forceDefender && isHeldByFoe(this, options.forceDefender))) {
            ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.PinnedOnlyReverse", { name: this.name }, "{name} is pinned — the only melee move is a Reverse against the foe holding them."));
            return null;
        }

        /* Bomb subtype gets its own dispatcher — spell-like AoE flow
         * with template placement + range clamp + opposed defense
         * (block / reposition only). Delegates and returns; the rest
         * of the strike pipeline never fires for a bomb. */
        if (weapon.type === "weapon" && weapon.system?.weaponType === "bomb") {
            const { throwBomb } = await import("../../mechanics/bombs.mjs");
            return throwBomb(this, weapon);
        }
        ...
        /* Tackling (dock's Full Round → Tackle) — the dialog restricted the melee
         * picker to Tackle; strip the status once the strike commits. */
        const _wasTackling = !!this.statuses?.has?.("tackling");
        ...
        if (relMax > 0 && relVal <= 0) {
            ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.XIsBrokenRepairItBefore", { weapon: weapon.name }, "{weapon} is broken — repair it before attacking."));
            return null;
        }
                ...
                if (DialogV2) {
                    ...
                    try {
                        go = await DialogV2.confirm({
                            window: { title: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Title.AttackWithoutATarget", `Attack without a target?`) },
                            content:
                                `<p>${tFormat("WITCHER.Doc.WeaponAttackMixin.Text.AttackNoTargetPrompt", { weapon: esc(weapon.name) }, `You haven't targeted anyone. Roll an attack with <strong>${esc(weapon.name)}</strong> anyway?`)}</p>` +
                                `<p style="opacity:0.7;font-size:0.6875rem;">${t("WITCHER.Doc.WeaponAttackMixin.Text.TargetTip", "Tip: click an enemy token to target it (T-key on a hovered token), or set a target from the combat tracker's context menu.")}</p>`,
                            rejectClose: false
                        });
                    } catch (_) { go = false; }
                    if (!go) return null;
        ...
        if (this._inActiveCombat && !options.skipActionGate) {
            if (!this._isMyTurn) {
                ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.NotTheirTurn", { name: this.name }, `${this.name} can't attack — not their turn.`));
                return null;
            }
            ...
            if (this.nextActionSlot === null) {
                ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.XHasNoActionsLeftThis", { this: this.name }, "{this} has no actions left this turn."));
                return null;
            }
        }
        ...
        if (weapon.usesAmmo) {
            ...
            if (!ready) {
                /* Magazine crossbow with bolts fed but not cocked: steer the
                 * player to the Reload (Cock) button rather than the generic
                 * "not loaded" (which reads as "no ammo at all"). */
                const notCocked = weapon.usesMagazine && !weapon.isArmed && (weapon.getChamberRounds?.()?.length ?? 0) > 0;
                const key = notCocked ? "WITCHER.Attack.NotCocked"
                          : weapon.hasChamber ? "WITCHER.Attack.NotLoaded" : "WITCHER.Attack.NoAmmo";
        ...
        const baseChipsFor = (p) => [
            ...
            p.sv ? { label: t("WITCHER.Common.Skill", "Skill"), value: p.sv.skillVal } : null,
            p.sv && p.sv.skillMod ? { label: t("WITCHER.Common.Mod", "Mod"), value: signed(p.sv.skillMod) } : null,
        ...
        if (options.skipDialog) {
            ...
            /* Base off-hand penalty from the House Rules panel (RAW
             * default -3). `Math.min(0, ...)` guards against a reduction
             * that would flip the value positive — a reduction can zero
             * out the penalty but not turn it into a bonus. */
            const offhandPenalty = isOffhandWeapon(weapon) ? Math.min(0, hrOffhandPenalty() + offRed) : 0;
            ...
            decl = await openAttackDialog(weapon, this, {
                ...
                /* Dual-mode (throwable melee) preselect from the dock's
                 * Melee/Throw toggle — opens the dialog in the chosen mode. */
                mode: options.mode,
                /* Explicit defender (canvas tile-targeting overlay, ripostes,
                 * etc.) so the dialog auto-computes the range bracket + distance
                 * for it without a game.user.targets reticle. */
                targetActor: options.forceDefender ?? null
            });
            if (!decl) return null;   // player cancelled
        }
        /* Pinned attacker: the ONLY melee option is a Reverse against the foe
         * holding you — refuse anything else (the reverse gate already limited
         * WHO the reverse can target). */
        if (_selfPinned && decl?.strike !== "reverseGrapple") {
            ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.PinnedOnlyReverse", { name: this.name }, "{name} is pinned — the only melee move is a Reverse against the foe holding them."));
            return null;
        }
        /* Can't slam while you're on the ground — Slam needs you upright. */
        if (decl?.strike === "slam" && this.statuses?.has?.("prone")) {
        ...
        /* Strip the tackling status once the strike commits (the dialog already
         * gave us strike === "tackle"; no promotion needed). */
        if (_wasTackling) {
            try { await this.toggleStatusEffect?.("tackling", { active: false }); }
            catch (_) { /* best-effort */ }
        }
        ...
        if (decl?.strike === "push" && _defenderActor?.statuses?.has?.("prone")) {
        ...
        if (decl?.strike === "takedown") {
            ...
            if (_defenderActor?.statuses?.has?.("prone")) {
                ui.notifications?.warn(t("WITCHER.Brawl.TakedownTargetProne", "You can't take down a target that's already prone."));
                return null;
            }
        }
        /* Trip needs the TARGET standing (only the target goes prone). */
        if (decl?.strike === "trip" && _defenderActor?.statuses?.has?.("prone")) {
            ui.notifications?.warn(t("WITCHER.Brawl.TripTargetProne", "You can't trip a target that's already prone."));
            return null;
        }
        /* Drag (grappling weapon) — NOT a weapon strike: an opposed PHYSIQUE test.
         * Requires an active grapple on the target; the whole flow lives in
         * mechanics/drag.performDrag (opposed roll → reachable overlay → hold-
         * preserving move). Skip the weapon roll entirely. Spends the action only
        ...
        if (decl?.strike === "drag") {
            ...
            const su = normalizedActorUuid(this);
            const tu = _defenderActor ? normalizedActorUuid(_defenderActor) : null;
            /* Either party may Drag — in a grapple/pin/choke with the target as
             * holder OR grapplee. */
            const inGrapple = !!(su && tu) && getHoldsSync(su).some(p =>
                ["grappled", "pinned", "chokeheld"].includes(p.kind)
                && ((p.holderUuid === su && p.targetUuid === tu) || (p.targetUuid === su && p.holderUuid === tu)));
            if (!inGrapple) {
                ui.notifications?.warn(t("WITCHER.Brawl.DragNeedsGrapple", "Drag requires an active grapple with the target."));
                return null;
            }
            const { performDrag } = await import("../../mechanics/drag.mjs");
            const res = await performDrag(this, _defenderActor);
            if ((res?.dragged ?? 0) > 0) { try { await this.recordAction?.(t("WITCHER.Brawl.Drag", "Drag")); } catch (_) {} }
            return null;
        }
        /* Tackle (full round) — NOT a weapon strike: an opposed PHYSIQUE smash.
         * You ALWAYS go prone; the target only on a win. No weapon roll, no
         * damage. Auto-rolled, no defence menu. */
        if (decl?.strike === "tackle") {
            if (!_defenderActor) {
                ui.notifications?.warn(t("WITCHER.Brawl.TackleNeedsTarget", "Tackle needs a target — pick the foe you're smashing into."));
                return null;
            }
            const { rollWitcherD10 } = await import("../../rolls/extendedRoll.mjs");
            const physBase = (a) => (Number(a.system?.stats?.body?.value) || 0)
                + (Number(a.system?.skills?.body?.physique?.value) || 0);
            const myRoll = await rollWitcherD10(), theirRoll = await rollWitcherD10();
            const myTotal = physBase(this) + (Number(myRoll?.total) || 0);
            const theirTotal = physBase(_defenderActor) + (Number(theirRoll?.total) || 0);
            const won = myTotal > theirTotal;
            /* You always hit the dirt; the foe only if you won the contest. Both
             * routed through the GM so cross-actor prone lands with permission. */
            try { await emitApplyStatus({ targetUuid: this.uuid, statusId: "prone", action: "apply" }); } catch (_) {}
            if (won) { try { await emitApplyStatus({ targetUuid: _defenderActor.uuid, statusId: "prone", action: "apply" }); } catch (_) {} }
            const speaker = ChatMessage.getSpeaker({ actor: this });
            const verdict = won
                ? `<div style="color:#4a4">${tFormat("WITCHER.Doc.BrawlMixin.Text.TackleWon", { name: esc(this.name), target: esc(_defenderActor.name) }, `<b>${esc(this.name)}</b> smashes into <b>${esc(_defenderActor.name)}</b> — <b>both</b> go down.`)}</div>`
                : `<div style="color:#a44">${tFormat("WITCHER.Doc.BrawlMixin.Text.TackleLost", { name: esc(this.name), target: esc(_defenderActor.name) }, `<b>${esc(_defenderActor.name)}</b> holds their ground — only <b>${esc(this.name)}</b> hits the dirt.`)}</div>`;
            await ChatMessage.create({
                speaker,
                content:
                    `<div class="wdm-attack-card">` +
                        tFormat("WITCHER.Doc.BrawlMixin.Text.TackleHeader", { name: esc(this.name), my: myTotal, target: esc(_defenderActor.name), their: theirTotal }, `<b>${esc(this.name)}</b> — Tackle: Physique <b>${myTotal}</b> vs <b>${esc(_defenderActor.name)}</b>'s <b>${theirTotal}</b>.`) +
                        verdict +
                    `</div>`
            });
            try { await this.recordFullRound?.(t("WITCHER.CombatExtended.Action.Tackle", "Tackle")); } catch (_) {}
            return null;
        }
        /* Choke (Strangling weapon) — only on a foe you have PINNED. One action's
         * worth of suffocation per Choke; re-invoke as an extra action for more,
         * and the turn-start prompt (mechanics/choke) upkeeps it automatically. */
        if (decl?.strike === "chokehold") {
            const { pinsTarget } = await import("../../mechanics/holdModifiers.mjs");
            if (!_defenderActor || !pinsTarget(this, _defenderActor)) {
                ui.notifications?.warn(t("WITCHER.Brawl.ChokeNeedsPin", "Choke can only be used on a foe you have PINNED first."));
                return null;
            }
        }
        /* Pin only lands on a foe already PRONE or backed against a WALL. */
        if (decl?.strike === "pin" && _defenderActor) {
            const { isTokenAgainstWall } = await import("../../mechanics/pushToken.mjs");
            const dTok = _defenderActor.getActiveTokens?.()?.[0] ?? null;
            const ok = _defenderActor.statuses?.has?.("prone") || (dTok && isTokenAgainstWall(dTok));
            if (!ok) {
        ...
        /* Companion list for defenses that DON'T use an item — dodge,
         * reposition (escape), brawlBlock, resistMagic. The joint-
         * attack rule applies to actions too: if you dodge the first
         * shot you can't dodge the second (only one dodge available
         * per exchange). Parry / block are absent from this list
         * because their per-item constraint is already covered by
         * `_usedDefenseItemIds` — a defender with two swords can
         * still parry both shots with different weapons. */
        const _usedDefenseActions = [];
        ...
        /* CE Poor-Vision helmet (visor down): −2 to ranged attacks. The dialog
         * path adds this as its own modifier line; the skipDialog path builds
         * the declaration without the dialog (weather/mods zeroed), so fold it
         * in here for ranged shots. Guarded on skipDialog so we never double-
         * count the dialog's contribution. */
        if (options.skipDialog && firedRanged && actorPoorVisionActive(this)) {
            decl.modTotal = (Number(decl.modTotal) || 0) + POOR_VISION_PENALTY;
            decl.grandMod = (Number(decl.grandMod) || 0) + POOR_VISION_PENALTY;
        }
        ...
        if (!firedRanged && !isThrowStrike && decl.strike !== "feint" && _defenderActor) {
            ...
            if (attackerToken && targetToken && gridSize > 0) {
                 ...
                 * additive for long/superior/extreme reach. */
                ...
                /* Measured through the shared helper rather than counted here,
                 * because the TARGETING OVERLAY offers this exact set and the
                 * two must not disagree — an overlay that lets you click a foe
                 * the gate then refuses is worse than no overlay. On a gridded
                 * scene this is the same Chebyshev tile count as before; on a
                 * gridless one it is straight-line metres, which is the only
                 * thing there is to measure. */
                const { separationMetres, meleeReachMetres, graceMetres, isGridless } =
                    await import("../../mechanics/gridDistance.mjs");
                const distM  = separationMetres(attackerToken, targetToken);
                const reachM = meleeReachMetres(reachExtendMeters);
                /* Half-tile grace — a target at least half within reach still
                 * counts ("half tiles are still tiles"). Matches the overlay. */
                if (distM != null && distM > reachM + graceMetres()) {
                    const reachLabel = isGridless()
                        ? `${reachM}m`
                        : (reachExtendMeters > 0
                            ? `${1 + Math.floor(reachExtendMeters / gridMeters)} tiles / ${gridMeters + reachExtendMeters}m`
                            : `1 tile / ${gridMeters}m`);
                    const awayLabel = isGridless()
                        ? `${Math.round(distM)}m`
                        : `${Math.round(distM / gridMeters)} tiles`;
                    ui.notifications?.warn(
                        `${weapon.name} — target is ${awayLabel} away, out of reach (${reachLabel}).`
                    );
                    return null;
                }
            }
        ...
        /* ── CE ranged-grapple follow-up gate ─────────────────────────
         * A Grappling weapon with extended reach can INITIATE a grapple (and
         * Trip / Drag) at range, but the "close" wrestling follow-ups — pin /
         * choke / throw / disarm / push / slam / takedown / reverse — need the
         * foe pulled adjacent first (Drag them in). Refuse those beyond a tile.
         * CE-only; RAW and non-grapple strikes are untouched, and grapple /
         * trip / drag fall through (allowed at reach). */
        if (isCombatExtendedEnabled?.() && _defenderActor && !isThrowStrike) {
            /* `!isThrowStrike` guards a hurled WEAPON throw (decl.strike "throw"
             * with strikeMeta.thrown) — that's a ranged attack and must stay
             * usable at distance; only a grapple-THROW (thrown false) is gated. */
            const RANGE_GRAPPLE_CLOSE_ONLY = new Set([
                "pin", "choke", "chokehold", "throw", "disarm", "push", "slam", "takedown", "reverseGrapple"
            ]);
            if (RANGE_GRAPPLE_CLOSE_ONLY.has(decl.strike)) {
                const aTok = this?.getActiveTokens?.()?.[0];
                const tTok = _defenderActor?.getActiveTokens?.()?.[0];
                const gs   = Number(canvas?.scene?.grid?.size) || 0;
                if (aTok && tTok && gs > 0) {
                    const ac = { x: aTok.center?.x ?? aTok.x, y: aTok.center?.y ?? aTok.y };
                    const tc = { x: tTok.center?.x ?? tTok.x, y: tTok.center?.y ?? tTok.y };
                    const tiles = Math.max(Math.abs(ac.x - tc.x), Math.abs(ac.y - tc.y)) / gs;
                    if (tiles > 1.5) {
                        ui.notifications?.warn(t("WITCHER.Attack.RangeGrappleTooFar", "Too far to grapple in close — Drag them adjacent before you can pin, choke, throw, or slam."));
                        return null;
                    }
                }
            }
        ...
        /* ── Always-adjacent melee gate ───────────────────────────────
         * A shield bash (Core p.164) and the grapple finishers Slam and
         * Takedown are strictly close-quarters — they can never reach a
         * non-adjacent foe, in RAW or CE, no matter what reach quality the
         * item carries or whether a grapple pair has drifted apart (movement
         * or a ranged grapple). Refuse them beyond a tile. The CE grapple
         * gate above already covers slam/takedown under CE; this backstops
         * RAW and the drifted-pair case. Non-listed strikes fall through. */
        const ALWAYS_ADJACENT_STRIKE = {
            bash:     "WITCHER.Attack.BashTooFar",
            slam:     "WITCHER.Attack.SlamTakedownTooFar",
            takedown: "WITCHER.Attack.SlamTakedownTooFar"
        };
        if (_defenderActor && Object.prototype.hasOwnProperty.call(ALWAYS_ADJACENT_STRIKE, decl.strike)) {
            const aTok = this?.getActiveTokens?.()?.[0];
            const tTok = _defenderActor?.getActiveTokens?.()?.[0];
            const gs   = Number(canvas?.scene?.grid?.size) || 0;
            if (aTok && tTok && gs > 0) {
                ...
                const tiles = Math.max(Math.abs(ac.x - tc.x), Math.abs(ac.y - tc.y)) / gs;
                if (tiles > 1.5) {
                    const fallback = decl.strike === "bash"
                        ? "Too far — a shield bash can only strike an adjacent target."
                        : "Too far — you can only slam or take down an adjacent target.";
                    ui.notifications?.warn(t(ALWAYS_ADJACENT_STRIKE[decl.strike], fallback));
                    return null;
                }
            }
        }
        ...
        if (isThrowStrike && _defenderActor) {
            ...
            if (attackerToken && targetToken) {
                ...
                try {
                    ...
                    /* Extreme bracket is `frac: 2` — 2× listed range. Plus a
                     * half-tile grace so a target whose tile is half within
                     * Extreme still counts (matches the targeting overlay). */
                    const _gm = Number(canvas?.scene?.grid?.distance) || 1.5;
                    const maxThrowMeters = Number(baseRange) > 0 ? Number(baseRange) * 2 + _gm * 0.5 : null;
        ...
        /* ── Ranged bracket auto-apply (skipDialog only) ─────────────────
         * The attack dialog computes the range bracket (pointBlank / close /
         * medium / long / extreme) from token distance vs. the weapon's
         * printed range and folds its to-hit modifier into decl.modTotal.
         * skipDialog callers (monster attack rows, scripted macros)
         * bypass the dialog — without this block a monster archer would
         * fire at pointBlank +5 across a battlefield, and a target 40m
         * from a 20m-range crossbow would be treated the same as a
         * point-blank shot. Reuse the same bracket math the dialog uses
         * so the two paths produce identical numbers, and gate max range
         * at Extreme (2× printed range) with a friendly warning. */
        if (options.skipDialog && firedRanged && !isThrowStrike && _defenderActor) {
            const attackerToken = this?.getActiveTokens?.()?.[0];
            const targetToken   = _defenderActor?.getActiveTokens?.()?.[0];
            if (attackerToken && targetToken) {
                let distanceMeters = null;
                ...
                } catch (_) { /* fall through to pixel-math backstop */ }
                if (!Number.isFinite(distanceMeters)) {
                    try {
                        const a = attackerToken.center ?? attackerToken;
                        const b = targetToken.center   ?? targetToken;
                        ...
                        const gd = Number(canvas?.scene?.grid?.distance) || 1.5;
                        distanceMeters = (px / sz) * gd;
                    } catch (_) { /* trust the GM */ }
                }
                try {
                    const { resolveWeaponRange } = await import("../../applications/attackDialog.mjs");
                    const baseRange = await resolveWeaponRange?.(weapon, this, null);
                    if (Number.isFinite(distanceMeters) && Number.isFinite(baseRange) && baseRange > 0) {
                        /* Max range: Extreme = 2× base. Beyond that, the
                         * shot fails — same behavior as the throw gate. */
                        if (distanceMeters > baseRange * 2) {
                            ui.notifications?.warn(
                                `${weapon.name} — target is ${Math.round(distanceMeters)}m away, ` +
                                `out of range (max ${Math.round(baseRange * 2)}m at Extreme).`
                            );
                            return null;
                        }
                        /* Pick the tightest bracket that still contains the
                         * distance — same logic the dialog uses (attackDialog
                         * .mjs autoBracketKey around line 523). */
                        let bracket = null;
                        if (distanceMeters <= 0.5) {
                            bracket = RANGE_BRACKETS.find(b => b.value === "pointBlank");
                        } else {
                            for (const b of RANGE_BRACKETS) {
                                if (b.frac == null) continue;
                                if (distanceMeters <= baseRange * b.frac) { bracket = b; break; }
                            }
                            if (!bracket) bracket = RANGE_BRACKETS.find(b => b.value === "extreme");
                        }
                        if (bracket && bracket.mod) {
                            decl.modTotal = (Number(decl.modTotal) || 0) + bracket.mod;
                            decl.grandMod = (Number(decl.grandMod) || 0) + bracket.mod;
                            if (Array.isArray(decl.chips)) {
                                decl.chips.push({
                                    label: game.i18n.localize(bracket.labelKey) ?? bracket.value,
                                    value: bracket.mod
                                });
                            }
                            /* Persist for the card / follow-up flows that
                             * read `decl.range.mod` (dialog path stamps
                             * the same field). */
                            if (decl.range) decl.range.mod = bracket.mod;
                        }
                    }
                } catch (_) { /* range unparseable — no bracket, no gate */ }
            }
        }
        ...
        const damageFor = (w, adr = 0, ammoItem = null) => {
            ...
            /* Combat Extended Brawling (Punch)/(Kick): attacking with this
             * weapon folds the wielder's punch or kick damage CODE into the
             * weapon's damage (a cestus fist, bladed boots). The unarmed code
             * already carries the Melee Bonus, so when it fires we DON'T also
             * add the weapon's own MB — that would double-count it. */
            const brawlCat = getActiveWeaponQualities?.() ?? WEAPON_QUALITIES;
            const brawlQs  = w.type === "weapon" ? (w.system?.effective?.qualities ?? w.system?.qualities ?? []) : [];
            let brawlStrike = "";
            for (const q of brawlQs) { const s = String(brawlCat[q]?.brawlingUnarmedStrike ?? ""); if (s) { brawlStrike = s; break; } }

            const applyMB = w.type === "weapon"
                && !isRangedShot
                && (w.system?.appliesMeleeBonus !== false)
                && !brawlStrike;   // the unarmed code below brings the MB instead
            const mb = applyMB ? (Number(this.system?.derivedStats?.meleeBonus) || 0) : 0;
            let base = mb !== 0
                ? `${d} ${mb > 0 ? "+" : "-"} ${Math.abs(mb)}`
                : d;
            if (brawlStrike) {
                // Punch = 1d6 + MB, Kick = 1d6 + MB + 4 (Core p.48). Natural
                // Weapons can't be equipped alongside these qualities, so the
                // core 1d6 always applies (no natural-weapon override to honor).
                const mbLive = Number(this.system?.derivedStats?.meleeBonus) || 0;
                const flat   = mbLive + (brawlStrike === "kick" ? 4 : 0);
                const unarmed = flat !== 0 ? `1d6 ${flat > 0 ? "+" : "-"} ${Math.abs(flat)}` : "1d6";
                base = `${base} + ${unarmed}`;
            }
        ...
        /* Close Quarters fires when the attacker and THIS target share a
         * grapple / pin / choke-hold / clinch pair — as EITHER role (the
         * grappler attacking the grapplee OR the grapplee attacking the
         * grappler). Read from the hold REGISTRY, not `target.statuses`: only
         * the grappled/pinned SIDE carries the status, so a status read misses
         * the grapplee-attacks-holder case AND unlinked-token holds. A bystander
         * in range can't piggyback — the pair must be between these two actors. */
        let _isCloseQ = false;
        try {
            const { normalizedActorUuid } = await import("../../mechanics/holdLink.mjs");
            const { getHoldsSync }        = await import("../../mechanics/holdRegistry.mjs");
            const attackerUuid = normalizedActorUuid(this);
            const targetUuid   = _targetActor ? normalizedActorUuid(_targetActor) : null;
            if (attackerUuid && targetUuid) {
                _isCloseQ = getHoldsSync(attackerUuid).some(p =>
                    ["grappled", "pinned", "clinched", "chokeheld"].includes(p.kind)
                    && ((p.holderUuid === attackerUuid && p.targetUuid === targetUuid)
                     || (p.targetUuid === attackerUuid && p.holderUuid === targetUuid)));
            }
        } catch (_) { /* no bonus if registry unreachable */ }
        ...
        const reachDistance = (reachInfo.reachExtendMeters > 0 || reachInfo.reachAdjacentPenalty < 0)
            ...
            : null;
        /* "Adjacent" = within ONE grid tile. Use the scene's grid distance
         * rather than a hardcoded 1.5 m so the adjacent-reach penalty fires on
         * scenes whose grid isn't 1.5 m/tile (a 1-tile target at, e.g., 5-unit
         * grids reads as "> 1.5" and silently skipped the penalty). */
        const _adjMeters = Number(canvas?.scene?.grid?.distance) || 1.5;
        const reachIsAdjacent = (reachDistance != null) && (reachDistance <= _adjMeters + 0.01);
        ...
        if (reachIsAdjacent && reachInfo.reachAdjacentPenalty < 0) {
            ...
            if (reachInfo.reachAdjacentNoAttack && !options.skipDialog) {
                ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.ExtremeReachAdjacent", { weapon: weapon.name }, `${weapon.name} (Extreme Reach) — can't attack an adjacent opponent.`));
                return null;
            }
            reachAdjacentChip = reachInfo.reachAdjacentPenalty;
            if (Array.isArray(decl.chips)) {
                decl.chips.push({ label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.AdjacentReach", "Adjacent (Reach)"), value: signed(reachAdjacentChip) });
            }
            if (reachInfo.reachAdjacentPommelOnly) {
                reachAdjacentNote = t("WITCHER.Doc.WeaponAttackMixin.Text.AdjacentPommelOnly", "Adjacent target — pommel strike only (Superior Reach).");
            } else if (reachInfo.reachAdjacentNoAttack) {
                reachAdjacentNote = t("WITCHER.Doc.WeaponAttackMixin.Text.AdjacentExtremeReach", "Adjacent target — Extreme Reach normally refuses; GM allowed it.");
            }
        }
        ...
        if (physiqueChip !== 0 && Array.isArray(decl.chips)) {
            decl.chips.push({ label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.PhysiqueUnder", "Physique (under)"), value: signed(physiqueChip) });
        }
         ...
         *                                 is via Extra Action.
         * We stash the reduction on `_nimbleExtraStaReduction` below and let
         * recordExtraAction subtract it from the extra-action STA cost at spend
         * time (reduce the cost, floored at 0 — NOT a post-hoc refund). */
        ...
        if (isRanged) {
            ...
            if (rangedPenaltyChip !== 0 && Array.isArray(decl.chips)) {
                decl.chips.push({ label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.ArmorRangedPenalty", "Armor Ranged Penalty"), value: signed(rangedPenaltyChip) });
            }
        }
        ...
        if (isGrounded && actorMounted && !options.skipDialog) {
            ui.notifications?.warn(tFormat("WITCHER.Doc.WeaponAttackMixin.Notify.GroundedMounted", { weapon: weapon.name }, `${weapon.name} (Grounded) — can't fire while mounted or in a vehicle.`));
            return null;
        }
        ...
        if (decl.strike === "charge") {
            ...
            if (best.perMeter > 0) {
                ...
                /* Meters moved this turn:
                 *   Token combat (a token on the canvas in an active encounter) →
                 *     read the ACTUAL distance moved this turn from
                 *     combatRound.movementMeters (recorded by canvas-movement on
                 *     every in-combat move). Fully automatic — NO prompt.
                 *   Theatre-of-mind (no token / not in combat) → prompt for it
                 *     (default 4m, the average half-Run), unless a scripted attack
                 *     suppressed dialogs (skipDialog → no bonus, nothing to measure). */
                const hasToken   = !!this?.getActiveTokens?.()?.[0];
                const autoMeters = !!this?._inActiveCombat && hasToken;
                let meters = 0;
                if (autoMeters) {
                    meters = Math.max(0, Math.floor(Number(this?.system?.combatRound?.movementMeters) || 0));
                } else if (!options.skipDialog) {
                    const meterPrompt = `<form><div class="form-group"><label>${t("WITCHER.Doc.WeaponAttackMixin.Text.MetersMoved", "Meters moved")}</label><input type="number" name="meters" min="0" step="1" value="4"/></div></form>`;
                    meters = 4;
                    try {
                        const DialogV2 = foundry?.applications?.api?.DialogV2;
                        if (DialogV2) {
                ...
                if (modeOk) {
                    ...
                    chargeBonusMeters = meters;
                    /* EO "up to the maximum shown": the per-item quality value on
                     * the winning charge quality caps the extra dice. footCharging
                     * carries a numeric max; cavalry / legacy charging have no
                     * value (→ 0 → uncapped, matching their wording). */
                    const chargeVals = weapon?.system?.effective?.qualityValues ?? weapon?.system?.qualityValues ?? {};
                    const chargeCap  = Number(chargeVals[best.key]) || 0;
                    if (chargeCap > 0) chargeBonusDice = Math.min(chargeBonusDice, chargeCap);
                    /* Auto mode with no tracked movement → say so, rather than
                     * silently applying a 0-die "charge". */
                    if (autoMeters && meters <= 0) {
                        reachAdjacentNote = (reachAdjacentNote ? reachAdjacentNote + " " : "")
                            + `${weapon.name}: no movement tracked this turn — charge bonus 0 (move before charging).`;
                    }
                } else {
        ...
        if (chargeBonusDice > 0 && mainDamage.display) {
            ...
            if (Array.isArray(decl.chips)) {
                decl.chips.push({ label: tFormat("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.ChargeX", { chargeBonusSrc: chargeBonusSrc }, "Charge ({chargeBonusSrc})"), value: `${chargeBonusMeters}m → +${chargeBonusDice}d6` });
            }
        }
        ...
        /* Fast strike launched via canvas tile-targeting: after each shot, prompt
         * the attacker to pick the NEXT shot's target on the canvas (RAW: Fast's
         * two attacks may hit different foes). Only engages when a tile pick
         * started this attack (`options.tileTargeting`), the strike is Fast
         * (multi-shot), and the dialog didn't already assign per-shot targets. */
        const _isFastStrike = decl?.strike === "fast" || decl?.strike === "fastAttack";
        const _tileFastPrompt = !!options.tileTargeting && !!options.forceDefender
            && !_perShotDefenders && _isFastStrike && attacks > 1;
        /* Prefer the CONTROLLED token belonging to this actor (the one the attack
         * was launched from); fall back to its first active token. Using the wrong
         * token would paint the second-shot overlay from the wrong position. */
        const _tileToken = _tileFastPrompt
            ? (canvas?.tokens?.controlled?.find?.(t => t?.actor === this)
               ?? this.getActiveTokens?.()?.[0] ?? null)
            : null;
        const _pickNextTileTarget = async () => {
            if (!_tileToken) return null;
            try {
                const { beginWeaponTargeting } = await import("../../policy/weapon-target-overlay.mjs");
                return await new Promise((resolve) => {
                    let done = false;
                    const fin = (v) => { if (!done) { done = true; resolve(v); } };
                    Promise.resolve(beginWeaponTargeting(this, _tileToken, weapon, {
                        mode: options.mode,
                        onPick:   (a) => fin(a ?? null),
                        onCancel: () => fin(null)
                    })).then(engaged => { if (!engaged) fin(null); }).catch(() => fin(null));
                });
            } catch (_) { return null; }
        };

        /* Per-shot RANGE for ranged attacks whose shots hit DIFFERENT targets
         * (Fast tile-targeting, or dialog per-shot targets). The dialog/skipDialog
         * folded the FIRST target's range band into decl.modTotal; each later shot
         * needs its OWN band. We compute a DELTA (this shot's mod − the first
         * shot's mod) and add it to that shot's roll. Melee / thrown are unaffected. */
        const _baselineRangeMod = Number(decl.range?.mod) || 0;
        const _perShotRange = firedRanged && !isThrowStrike && (_tileFastPrompt || !!_perShotDefenders);
        const _rangeModForTarget = async (defActor) => {
            const aTok = _tileToken ?? this.getActiveTokens?.()?.[0];
            const tTok = defActor?.getActiveTokens?.()?.[0];
            if (!aTok || !tTok) return { mod: 0 };
            let dist = null;
            try {
                const g = canvas?.grid;
                if (typeof g?.measureDistance === "function") {
                    const d = g.measureDistance(aTok.center ?? aTok, tTok.center ?? tTok);
                    if (Number.isFinite(Number(d))) dist = Number(d);
                }
            } catch (_) { /* pixel backstop below */ }
            if (!Number.isFinite(dist)) {
                try {
                    const a = aTok.center ?? aTok, b = tTok.center ?? tTok;
                    const px = Math.hypot((a?.x ?? 0) - (b?.x ?? 0), (a?.y ?? 0) - (b?.y ?? 0));
                    const sz = Number(canvas?.scene?.grid?.size) || 100;
                    const gd = Number(canvas?.scene?.grid?.distance) || 1.5;
                    dist = (px / sz) * gd;
                } catch (_) { return { mod: 0 }; }
            }
            let baseRange = null;
            try {
                const { resolveWeaponRange } = await import("../../applications/attackDialog.mjs");
                baseRange = await resolveWeaponRange?.(weapon, this, null);
            } catch (_) { /* unparseable → no bracket */ }
            if (!Number.isFinite(baseRange) || baseRange <= 0) return { mod: 0 };
            if (dist > baseRange * 2) return { outOfRange: true, dist, max: baseRange * 2 };
            let bracket = null;
            if (dist <= 0.5) bracket = RANGE_BRACKETS.find(b => b.value === "pointBlank");
            else {
                for (const b of RANGE_BRACKETS) { if (b.frac == null) continue; if (dist <= baseRange * b.frac) { bracket = b; break; } }
                if (!bracket) bracket = RANGE_BRACKETS.find(b => b.value === "extreme");
            }
            return { mod: Number(bracket?.mod) || 0, label: bracket ? (game.i18n.localize(bracket.labelKey) ?? bracket.value) : "" };
        };
        ...
        for (let i = 0; i < attacks; i++) {
            /* Per-shot defender resolution.  Joint keeps both shots on the
             * first-assigned target (RAW: two attacks against the SAME opponent).
             * Fast reads its own index — shot 0 → [0], shot 1 → [1] with fallback
             * to [0].  A tile-targeting Fast strike instead prompts for each
             * later shot's target on the canvas AFTER the previous shot lands.
             * Missing/cancelled → the remaining shots are skipped. */
            if (_tileFastPrompt && i > 0) {
                /* Prompt for this shot's target on the canvas (after the previous
                 * shot resolved). Cancelling / no pick keeps the previous target,
                 * so a Fast strike still fires both attacks (RAW) rather than
                 * silently dropping the second. */
                const next = await _pickNextTileTarget();
                if (next) _defenderActor = next;
            } else if (_perShotDefenders) {
            ...
            /* Per-shot range: recompute this shot's range band for its (possibly
             * different) target and carry the delta vs the first shot's band into
             * shotGrandMod below. Out-of-range → skip this shot BEFORE any defense
             * prompt or roll. Shot 0 keeps the baseline already in decl.modTotal. */
            let _shotRangeDelta = 0;
            if (_perShotRange && i > 0) {
                const rr = await _rangeModForTarget(_defenderActor);
                if (rr.outOfRange) {
                    ui.notifications?.warn(`${weapon.name} — shot ${i + 1} target is ${Math.round(rr.dist)}m away, out of range (max ${Math.round(rr.max)}m).`);
                    continue;
                }
                _shotRangeDelta = (Number(rr.mod) || 0) - _baselineRangeMod;
            }
            ...
            if (_willPrompt && !isFeintRoll && cantDefend) {
                ...
                try {
                    ...
                    //
                    // Gate on the STRIKE TYPE (`sMeta.offhand === true`)
                    // rather than "did the offhand weapon resolve" — the
                    // rule is triggered by choosing Joint Attack, not by
                    // whether Foundry successfully resolved the specific
                    // off-hand weapon. Guarding on `!!offhandWeapon`
                    // silently skipped the disallow list when the off-hand
                    // reference went stale (unlinked token variants,
                    // deleted item mid-turn, etc.), leaving the defender
                    // free to parry both shots with the same item — the
                    // exact symptom we're fixing.
                    const isJoint = sMeta.offhand === true;
                    const disallowedItemIds = isJoint ? [..._usedDefenseItemIds] : [];
                    /* Item-less defense lockout (dodge, escape, brawlBlock,
                     * resistMagic). Each can be used AT MOST ONCE per joint
                     * attack — dodging the first shot means you can't
                     * dodge the second (you already spent your dodge). */
                    const disallowedActions = isJoint ? [..._usedDefenseActions] : [];
                    ...
                    /* Track item-less defenses for the same joint-attack
                     * lockout — every action listed here can be used at
                     * most once per exchange. `parry`/`block` are not
                     * added here because their reuse is already gated by
                     * `_usedDefenseItemIds` (same item can't parry twice
                     * but a different sword can). `none`/`stunned` are
                     * not real defenses so they don't consume a slot. */
                    const _act = decl._defenseChoice?.action;
                    if (_act && _act !== "none" && _act !== "stunned"
                        && _act !== "parry" && _act !== "block") {
                        _usedDefenseActions.push(_act);
                    }
                } catch (err) {
            ...
            const baseTypeKeys = usesAmmo
                ...
                : wTypeKeys;
            /* Player picked a subset of the shot weapon's damage types
             * in the attack dialog (e.g. "spear can slash OR pierce,
             * dealing piercing this swing"). Intersect the shot
             * weapon's full list with the picker's selection so target
             * resistances / weaknesses land against the right type(s).
             *
             * Source precedence (per-shot):
             *   1. decl.damageTypesByShot[i] — Fast/Joint pick their
             *      own type per swing (2 pickers in the dialog).
             *   2. decl.damageTypes — legacy single-pick, applied to
             *      EVERY shot (backwards compat for callers that still
             *      pass the flat shape).
             *   3. null → fall back to the shot weapon's full type
             *      list (RAW default).
             *
             * Intersect-empty guard: if the picked type isn't in the
             * shot weapon's list (e.g. Joint main = slash/pierce, pick
             * pierce; off-hand = bash → intersect empty), don't drop
             * the shot to no types — fall back to the shot weapon's
             * full list so the swing still lands as SOMETHING sensible. */
            const perShotOverride = Array.isArray(decl.damageTypesByShot?.[i])
                                 && decl.damageTypesByShot[i].length
                ? decl.damageTypesByShot[i] : null;
            const flatOverride = Array.isArray(decl.damageTypes) && decl.damageTypes.length
                ? decl.damageTypes : null;
            const dtOverride = perShotOverride ?? flatOverride;
            let typeKeys;
            if (dtOverride) {
                const intersect = baseTypeKeys.filter(t => dtOverride.includes(t));
                typeKeys = intersect.length ? intersect : baseTypeKeys;
            } else {
                typeKeys = baseTypeKeys;
            }
            ...
            if (!isFeintRoll && hasWRPerk(this, "sting")) {
                ...
                if (stingFires) {
                    ...
                    if (hasIAP) {
                        ...
                        qualities.push(t("WITCHER.Doc.WeaponAttackMixin.Text.StingPlus5", "Sting +5"));
                    } else if (hasAP) {
                        riderPayload.keys = riderPayload.keys.filter(k => k !== "armorPiercing");
                        riderPayload.keys.push("improvedArmorPiercing");
                        qualities.push(t("WITCHER.Doc.WeaponAttackMixin.Text.StingImprovedAP", "Sting → Improved AP"));
                    } else {
            ...
            const shotGrandMod = prof.baseTotal + decl.modTotal + flatMod + statusAtk + ceHoldAtk + activelyDodgeAtk
                               ...
                               + reachAdjacentChip + physiqueChip + rangedPenaltyChip + _shotRangeDelta;
            ...
            if (activelyDodgeAtk) attackerStatusChips.push({ label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.TargetActivelyDodging", "Target Actively Dodging"), value: "-2" });
            ...
            const shotChips = [
                ...
                throwRangeStr ? { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.Range", "Range"), value: throwRangeStr } : null,
                ...
                /* Per-shot range correction (Fast/per-shot ranged shots hitting a
                 * DIFFERENT-distance target than the first). decl.chips carries the
                 * first target's band; this chip adjusts the card to THIS shot's
                 * band so the displayed total matches the roll. */
                _shotRangeDelta ? { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.RangeThisTarget", "Range (this target)"), value: signed(_shotRangeDelta) } : null,
                flatMod ? { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.AtkMod", "Atk Mod"), value: signed(flatMod) } : null,
                ...attackerStatusChips,
                ...targetStatusChips,
            ...
            /* Ranged shots stamp the fired projectile's coating onto the
             * damage button so the GM-side oil fold applies to the
             * arrow/bolt (Core p.166: the projectile carries the
             * coating, not the bow). Melee/thrown/off-hand paths leave
             * these empty — the weapon's own appliedOil stays
             * authoritative for those.
             *
             * TWO sources depending on weapon shape:
             *   Bow (no chamber): shotAmmo is the live ammo item —
             *     read its system.appliedOil directly.
             *   Crossbow (chamber): the round was already stamped
             *     into weapon.system.loaded.appliedOil at reload time
             *     (reloadMixin.reload), which survives the source
             *     ammo document being deleted when a 1-qty coated
             *     stack drained on reload.
             *
             * We inline the snapshot at attack-roll time in both cases
             * because spendShot decrements/deletes the source during
             * the same flow, so a fromUuid lookup at Roll-Damage-click
             * time can't be relied on. */
            let shotAmmoForOil = null;
            let shotAmmoOil = null;
            if (!isOff && weapon?.usesAmmo) {
                if (isBow && shotAmmo) {
                    shotAmmoForOil = shotAmmo;
                    shotAmmoOil    = shotAmmo?.system?.appliedOil ?? null;
                } else if (weapon.hasChamber) {
                    const chamber = weapon.system?.loaded ?? {};
                    if (Number(chamber.count) > 0 && chamber.appliedOil?.name) {
                        shotAmmoForOil = { uuid: chamber.uuid ?? "" };
                        shotAmmoOil    = chamber.appliedOil;
                    }
                }
            }
            const flavor = attackRollFlavor({
                ...
                ammoUuid:  shotAmmoForOil ? (shotAmmoForOil.uuid ?? "") : "",
                ammoOilBonusDamage: (shotAmmoOil && shotAmmoOil.name) ? (Number(shotAmmoOil.oilBonusDamage) || 0) : 0,
                ammoOilTarget:      (shotAmmoOil && shotAmmoOil.name) ? String(shotAmmoOil.oilTarget || "")  : "",
                ammoOilName:        (shotAmmoOil && shotAmmoOil.name) ? String(shotAmmoOil.name)             : "",
                subtitle,
            ...
            result = await extendedRoll(
                shotFormula,
                {
                    ...
                    flags: (r) => ({
                        [SYSTEM_ID]: {
                            category: "combat",
                            /* Always stamp the attack total — a targetless card
                             * needs it later so "Assign Target" can re-resolve
                             * the HIT/MISS verdict against the picked defender
                             * without re-rolling the attack. */
                            attackTotal: r.total,
                            ...(decl._engagementId
                                ? { engagementId: decl._engagementId }
                                : {}),
                            ...(_defenderActor?.uuid
            ...
            if (isFeintRoll) {
                ...
                if (decl?._defenseChoice) {
                    ...
                    try {
                        const formula = skillMod ? `1d10 + ${skillMod}` : "1d10";
                        ...
                        decl._defenseChoice.defenseChips = [
                            ...
                            { label: t("WITCHER.Common.Rank", "Rank"), value: rank },
                            modifier ? { label: t("WITCHER.Common.Mod", "Mod"), value: (modifier >= 0 ? "+" : "") + modifier } : null
                        ].filter(Boolean);
            ...
            try {
                if (isFeintRoll && feintSucceeded) {
                    ...
                    const summaryMsg = await ChatMessage.create({
                        speaker,
                        content: tFormat(
                            "WITCHER.Doc.WeaponAttackMixin.Chat.FeintPlus3",
                            { attacker: esc(this.name), target: esc(_defenderActor?.name ?? t("WITCHER.Doc.WeaponAttackMixin.Text.Target", "target")) },
                            "<em><strong>{attacker}</strong> feints — next attack against <strong>{target}</strong> at <strong>+3</strong>.</em>"
                        ),
                    ...
                    await ChatMessage.create({
                        speaker,
                        content: tFormat(
                            "WITCHER.Doc.WeaponAttackMixin.Chat.FeintSeenThrough",
                            { attacker: esc(this.name) },
                            "<em><strong>{attacker}</strong> feints — target saw through it (no advantage).</em>"
                        ),
                        flags: { [SYSTEM_ID]: { category: "combat" } }
                    });
                } else if (feintAdv) {
             ...
             * with a lucky 20 is still capped at 10 while helpless.
             *
             * CE exception: a PINNED-but-otherwise-active defender MAY block
             * (weapon parry / shield block / arm brawl-block) a non-grappling
             * attack, and that intercept roll COUNTS — it is not capped at the
             * pinned floor. Truly helpless states (paralyzed / unconscious /
             * stunned / dead) still cap regardless. A pinned defender who took
             * no defense, or couldn't block (grappling attack → gate gives no
             * options → action "none"), still lands on DC 10. */
            const _defAct2 = defChoice?.action;
            const _intercepted = _defAct2 === "parry" || _defAct2 === "block" || _defAct2 === "brawlBlock";
            const _pinnedActive = _defenderActor?.statuses?.has?.("pinned")
                && !["paralyzed", "unconscious", "stunned", "dead"].some(s => _defenderActor?.statuses?.has?.(s));
            if (!(_pinnedActive && _intercepted)) {
                try {
            ...
            if (result?.message && Number.isFinite(result.total) && (hasDefense || isFumble)) {
                ...
                /* brawlBlock = wrestle defense: show the grappling WEAPON's name
                 * when one was chosen (choice.itemId), else plain "Brawling" —
                 * never the raw "BrawlBlock" action key. */
                if (defActionRaw === "brawlBlock") {
                    const _gw = defChoice?.itemId ? _defenderActor?.items?.get?.(defChoice.itemId) : null;
                    defActionTitle = _gw?.name ?? t("WITCHER.Common.Brawling", "Brawling");
                }
                ...
                const fumbleBanner = isFumble
                    ? `<div class="wdm-attack-fumble"><i class="fa-solid fa-triangle-exclamation"></i> <span class="wdm-attack-fumble-k">${t("WITCHER.Doc.WeaponAttackMixin.Text.FumbleBanner", "FUMBLE")}</span> <span class="wdm-attack-fumble-v">${t("WITCHER.Doc.WeaponAttackMixin.Text.FumbleNatural1", "natural 1 — consult the fumble table")}</span></div>`
                    : "";
                const verdictHtml =
                    `${fumbleBanner}` +
                    `${defenseLabel}` +
                ...
                let brawlBlockNote = "";
                /* Captured for the damage-button rewrite below — the arm the
                 * strike is redirected to, its label, and its ½ location
                 * multiplier (ATTACK_LOCATIONS carries arm mult = 0.5). */
                let brawlBlockArm = null, brawlBlockArmLabel = "", brawlBlockArmMult = null;
                if (defChoice?.action === "brawlBlock" && delta <= 0 && hasDefense) {
                    const arm = Math.random() < 0.5 ? "leftArm" : "rightArm";
                    const armLabel = arm === "leftArm" ? t("WITCHER.Doc.WeaponAttackMixin.Text.LeftArm", "Left arm") : t("WITCHER.Doc.WeaponAttackMixin.Text.RightArm", "Right arm");
                    brawlBlockArm      = arm;
                    brawlBlockArmLabel = armLabel;
                    brawlBlockArmMult  = ATTACK_LOCATIONS[arm]?.mult ?? 0.5;
                    brawlBlockNote = `<div class="wdm-attack-note" style="color:#8b8;"><i class="fa-solid fa-hand-fist"></i> <strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.BodyBlockSucceeds", "Body Block succeeds")}</strong> — ${tFormat("WITCHER.Doc.WeaponAttackMixin.Text.StrikeRedirectedToArm", { arm: armLabel }, `strike redirected to <strong>${armLabel}</strong>. Apply damage against arm SP (×½ multiplier).`)}</div>`;
                }
                ...
                if (delta > 0 && sev && wrHeroic(this) === "deadlyFocus" && sev !== "deadly") {
                    /* The button just opens a picker — the actual tier
                     * step + cost is chosen by the player in the dialog.
                     * `data-current-severity` seeds the picker with what
                     * the crit resolved to; the handler reads live
                     * adrenaline/STA to compute affordable tiers. */
                    deadlyFocusRider =
                        `<button type="button" class="wdm-attack-damage-roll wdm-attack-deadly-focus" data-action="wdm-deadly-focus" ` +
                            `data-attacker-uuid="${esc(this.uuid)}" ` +
                            `data-attack-msg-uuid="${esc(result.message.uuid)}" ` +
                            `data-current-severity="${esc(sev)}" ` +
                            `data-wr-rider="deadlyFocus">` +
                            `<i class="fa-solid fa-crosshairs"></i> Deadly Focus` +
                        `</button>`;
                }
                ...
                /* Strong-strike keys that Forceful Blow triggers on:
                 *   "strong"        — RAW plain strong strike.
                 *   "strongAttack"  — Combat Extended's rename of same.
                 *   "charge"        — RAW charge is a movement action
                 *                     that FORCES its resulting attack
                 *                     to be a strong strike; this
                 *                     codebase collapses both into a
                 *                     single "charge" strike type, so
                 *                     it inherits Forceful Blow
                 *                     eligibility just like any other
                 *                     strong strike would. */
                const isStrongStrike = decl.strike === "strong"
                                    || decl.strike === "strongAttack"
                                    || decl.strike === "charge";
                ...
                /* Forceful Blow is now hoisted into the summary action slot
                 * (see the summary block below) so it sits next to the Roll
                 * Damage button in the collapsed header. Keep it out of the
                 * body verdict fragment. */
                const verdictHtmlWithBrawl = verdictHtml + brawlBlockNote + deadlyFocusRider;
                try {
                    const cur = String(result.message.content ?? "");
                    let next = cur;
                    if (delta > 0) {
                         ...
                         * rewrite the Roll Damage button's location to the
                         * blocking arm (key + label + the arm's ½ multiplier)
                         * so rolling damage actually applies vs ARM SP. Without
                         * this the button kept the originally-rolled location,
                         * so an arm block still dealt damage to the head. */
                        let redirected = cur;
                        if (brawlBlockArm) {
                            redirected = cur.replace(/<button[^>]*data-action="wdm-roll-damage"[^>]*>/, (btn) => btn
                                .replace(/data-loc-key="[^"]*"/,   `data-loc-key="${brawlBlockArm}"`)
                                .replace(/data-loc-label="[^"]*"/, `data-loc-label="${esc(brawlBlockArmLabel)}"`)
                                .replace(/data-loc-mult="[^"]*"/,  `data-loc-mult="${brawlBlockArmMult}"`));
                        }
                        next = redirected.includes("wdm-attack-damage")
                            ? redirected.replace(/<div class="wdm-attack-damage">/, `${verdictHtmlWithBrawl}<div class="wdm-attack-damage">`)
                            : `${redirected}${verdictHtmlWithBrawl}`;
                    } else {
                    ...
                    const flagPatch = {
                        ...
                        critDelta:    delta,
                        /* Persist the fumble so a later re-render (which rebuilds
                         * the verdict pip from stored flags) keeps showing FUMBLE
                         * instead of reverting to MISS. */
                        fumble:       isFumble
                    };
                    if (next !== cur) {
                    ...
                    if (isHit) {
                        try {
                            ...
                            /* Witchers Reborn — Forceful Blow rides ALONGSIDE
                             * Roll Damage in the summary slot on a strong-strike
                             * hit. Extract just the <button> from the rider div
                             * (already built above with all the inherited data-*
                             * attrs) and append it to summaryActionHtml so both
                             * buttons live in the collapsed header side-by-side.
                             * Nothing to strip from the body — verdictHtmlWithBrawl
                             * above no longer includes it. */
                            if (forcefulBlowRider) {
                                const rTmp = document.createElement("div");
                                rTmp.innerHTML = forcefulBlowRider;
                                const fbBtn = rTmp.querySelector('button[data-action="wdm-forceful-blow"]');
                                if (fbBtn) {
                                    fbBtn.classList.add("wdm-card-sum-roll");
                                    summaryActionHtml = (summaryActionHtml ?? "") + fbBtn.outerHTML;
                                }
                            }
                        } catch (_) { /* best-effort */ }
                    }
                ...
                if (defChoice?.action === "parry" && delta <= 0 && !hasWRPerk(this, "swiftRecovery")) {
                    try {
                        ...
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: { label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.Staggered", { name: this.name }, `${this.name}: Staggered`), kind: "status", icon: "fa-shield-halved" }
                        });
                    } catch (err) {
                            ...
                            if (buttons) {
                                const fragment =
                                    `<div class="wdm-attack-rider wdm-wr-rider" data-wr-rider="riposte">` +
                                        `<i class="fa-solid fa-shield-heart"></i> ` +
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.Riposte", "Riposte")}</strong> — ` +
                ...
                try {
                    ...
                    if (delta > 0 && strikeStatus && _defenderActor?.uuid) {
                        ...
                        if (HOLDS.includes(sid)) {
                            const { applyHoldLink } = await import("../../mechanics/holdLink.mjs");
                            /* CE: a Grappling weapon already passed the melee REACH
                             * gate above, so the hold may form at the weapon's reach
                             * (a whip/catch-pole grabs at range). RAW keeps the
                             * adjacency requirement (allowReach false). */
                            await applyHoldLink(this, _defenderActor, sid, { allowReach: isCombatExtendedEnabled() });
                        } else {
                            await emitApplyStatus({
                                targetUuid: _defenderActor.uuid, statusId: sid, action: "apply", sourceActorUuid: this.uuid
                            });
                        }
                        const statusDef = (CONFIG.statusEffects ?? []).find(s => s.id === sid);
                        const statusLabel = statusDef?.name ? game.i18n.localize(statusDef.name) : sid;
                ...
                /* Reverse Grapple (melee, weapon with the Grappling quality).
                 * The strike carries no `appliesStatus` — on a win it SWAPS the
                 * grapple pair so this actor becomes the grappler. reverseHold
                 * verifies the defender is the foe currently grappling this
                 * actor (no-op otherwise), strips the old statuses, and stamps
                 * the swapped ones. */
                if (delta > 0 && decl.strike === "reverseGrapple" && _defenderActor) {
                    try {
                        const { reverseHold } = await import("../../mechanics/holdLink.mjs");
                        const ok = await reverseHold(this, _defenderActor, "grappled");
                        if (ok) {
                            const fragment =
                                `<div class="wdm-attack-rider">` +
                                    `<i class="fa-solid fa-rotate"></i> ` +
                                    `<strong>${esc(this.name)}</strong> reverses the grapple — ` +
                                    `<strong>${esc(_defenderActor?.name ?? "Foe")}</strong> is now grappled.` +
                                `</div>`;
                            await appendAttackResult(result.message, { fragment });
                        }
                    } catch (err) {
                        console.warn("witcher-ttrpg-death-march | melee reverse grapple failed", err);
                    }
                }
                ...
                if (delta > 0 && (decl.strike === "disarm" || decl.strike === "meleeDisarm")) {
                    try {
                        ...
                        /* Grapple Disarm ("disarm") lets you snatch the weapon on a
                         * DC 18 Brawling check; Melee Disarm ("meleeDisarm") only
                         * knocks it away — you're at weapon's length, not grabbing. */
                        const stealTxt = (decl.strike === "disarm")
                            ? ` — or <strong>${esc(t("WITCHER.Doc.BrawlMixin.Text.DC18Brawling", "DC 18 Brawling"))}</strong> to snatch it instead`
                            : "";
                        const fragment =
                            `<div class="wdm-attack-rider">` +
                                `<i class="fa-solid fa-hand-fist"></i> ` +
                                `<strong>${esc(_defenderActor?.name ?? "Defender")}</strong>'s weapon is knocked ` +
                                `<strong>${dist}m ${esc(dirLabels[dir] ?? dir)}</strong>${stealTxt} ` +
                                `<span style="opacity:0.7;">(1d8=${dirRoll.total} → ${esc(dir)}, 1d6=${dist})</span>.` +
                            `</div>`;
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: {
                                label: tFormat("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.DisarmXmX", { dist: dist, dir: dir }, "Disarm: {dist}m {dir}"),
                                kind:  "status",
                                icon:  "fa-hand-fist"
                            }
                        });
                ...
                /* CE Push (grappling weapon) — on a hit, shove the target
                 * ⌊PHY/3⌋ metres (Physique base, rounded down) directly away.
                 * Push deals NO damage — the shove is the whole effect. Mirrors the unarmed
                 * brawl Push; the weapon roll itself is noDamage, so this rider
                 * is the whole effect. */
                if (delta > 0 && decl.strike === "push" && _defenderActor) {
                    try {
                        ...
                        const meters = Math.max(0, Math.floor(phy / 5));
                        const aTok = this.getActiveTokens?.()?.[0] ?? null;
                        const dTok = _defenderActor.getActiveTokens?.()?.[0] ?? null;
                        let moved = 0, clipped = false;
                        if (meters > 0 && aTok && dTok?.document?.uuid) {
                            /* The pusher chooses which way to shove. emitPushToken
                             * shoves AWAY from sourcePoint, so a point opposite the
                             * chosen direction sends the target that way. Cancel →
                             * default straight away from the attacker. */
                            const tc = { x: Number(dTok.center?.x ?? dTok.x), y: Number(dTok.center?.y ?? dTok.y) };
                            let sourcePoint = { x: Number(aTok.center?.x ?? aTok.x), y: Number(aTok.center?.y ?? aTok.y) };
                            ...
                            try {
                                ...
                                const dir = await pickPushDirection(tc, { lengthPx: meters * (gs / (gm || 1)) });
                                if (dir) sourcePoint = { x: tc.x - dir.x * gs, y: tc.y - dir.y * gs };
                            } catch (_) { /* keep the away-from-attacker default */ }
                            const { emitPushToken } = await import("../../setup/socketHook.mjs");
                            const res = await emitPushToken({
                                tokenUuid: dTok.document.uuid,
                                sourcePoint,
                                distanceMeters: meters
                            });
                        ...
                        /* Push breaks any hold the shoved target is in (grapple
                         * / pin / choke, either role) — they're knocked away. */
                        try {
                            const { clearHoldLink } = await import("../../mechanics/holdLink.mjs");
                            await clearHoldLink(_defenderActor, "push");
                        } catch (_) { /* best-effort */ }
                        /* Shoved into a wall — 1d6/2 ramming per full 2m travelled,
                         * bludgeoning to the torso, bypassing armour. */
                        let wallDmg = 0;
                        if (clipped && moved >= 2) {
                            try {
                                const inc = Math.floor(moved / 2);
                                const rr  = await new Roll(`${inc}d6`).evaluate();
                                wallDmg = Math.floor((Number(rr.total) || 0) / 2);
                                if (wallDmg > 0) {
                                    await emitApplyDamage({
                                        targetUuid:    _defenderActor.uuid,
                                        weaponDamage:  wallDmg,
                                        silverDamage:  0,
                                        damageTypes:   ["bludgeoning"],
                                        locationKey:   "torso",
                                        locationLabel: game.i18n.localize(ATTACK_LOCATIONS.torso?.labelKey ?? "torso"),
                        ...
                        const distTxt = moved > 0 ? `${moved.toFixed(1)}m${clipped ? " (cut short on a wall)" : ""}` : "held ground";
                        const fragment =
                            `<div class="wdm-attack-rider"><i class="fa-solid fa-arrows-left-right-to-line"></i> ` +
                            `<strong>${esc(_defenderActor?.name ?? "Defender")}</strong> is shoved <strong>${esc(distTxt)}</strong>` +
                            (wallDmg > 0 ? ` — <strong>${wallDmg}</strong> ramming damage into the wall (torso, ignores armor).` : ` — no damage.`) +
                            `</div>`;
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: {
                                label: tFormat("WITCHER.Doc.WeaponAttackMixin.Text.PushXm", { m: meters }, "Push {m}m"),
                                kind:  "status",
                                icon:  "fa-hand-fist"
                            }
                        });
                    } catch (err) {
                        console.warn("witcher-ttrpg-death-march | melee push rider failed", err);
                    }
                }

                /* CE Slam (grappling weapon) — on a hit: DOUBLE kick damage to a
                 * random location (not weapon damage — the *** rule swaps only the
                 * skill), knock the target prone, shove them 1d6m back (which
                 * breaks the hold), and force a Stun save. Mirrors the unarmed
                 * brawl Slam; the weapon roll itself is noDamage, so this rider is
                 * the whole effect. */
                if (delta > 0 && decl.strike === "slam" && _defenderActor) {
                    try {
                        /* Double kick = (1d6 + MeleeBonus + 4) × 2, to a random
                         * location (location multiplier still rides via the
                         * calculator, same as any hit). */
                        const mb   = Number(this.system?.derivedStats?.meleeBonus) || 0;
                        const flat = mb + 4;
                        const dmgRoll  = await new Roll(`(1d6 ${flat >= 0 ? "+" : "-"} ${Math.abs(flat)}) * 2`).evaluate();
                        const dmgTotal = Math.max(0, Math.floor(Number(dmgRoll.total) || 0));
                        const { loc: locKey } = await rollHitLocation("human");
                        const locDef   = ATTACK_LOCATIONS[locKey];
                        const locLabel = game.i18n.localize(locDef?.labelKey ?? locKey);
                        await emitApplyDamage({
                            targetUuid:    _defenderActor.uuid,
                            weaponDamage:  dmgTotal,
                            silverDamage:  0,
                            damageTypes:   ["bludgeoning"],
                            locationKey:   locKey,
                            locationLabel: locLabel,
                            nonLethal:     true,   // double kick → Stamina (still soaked by SP)
                            kind:          "weapon",
                            /* Fold the damage breakdown into THIS attack card
                             * instead of a separate chat message. */
                            attackMessageUuid: result.message?.uuid ?? null
                        });
                        /* Knock prone. */
                        await emitApplyStatus({ targetUuid: _defenderActor.uuid, statusId: "prone", action: "apply", sourceActorUuid: this.uuid });
                        /* Knockback = ⌊Physique base (BODY + Physique) / 5⌋ metres,
                         * then break the hold (the throw separates you). */
                        const slamPhy = (Number(this.system?.stats?.body?.value) || 0) + (Number(this.system?.skills?.body?.physique?.value) || 0);
                        const meters = Math.max(0, Math.floor(slamPhy / 5));
                        const aTok = this.getActiveTokens?.()?.[0] ?? null;
                        const dTok = _defenderActor.getActiveTokens?.()?.[0] ?? null;
                        let moved = 0, clipped = false;
                        if (meters > 0 && aTok && dTok?.document?.uuid) {
                            /* The slammer chooses which way to hurl the target.
                             * emitPushToken shoves AWAY from sourcePoint, so a
                             * point placed opposite the chosen direction sends
                             * the target that way. Cancel → default away from
                             * the attacker. */
                            const tc = { x: Number(dTok.center?.x ?? dTok.x), y: Number(dTok.center?.y ?? dTok.y) };
                            let sourcePoint = { x: Number(aTok.center?.x ?? aTok.x), y: Number(aTok.center?.y ?? aTok.y) };
                            const gs = canvas?.dimensions?.size ?? 100;
                        ...
                        try {
                            const { clearHoldLink } = await import("../../mechanics/holdLink.mjs");
                            await clearHoldLink(_defenderActor, "slam");
                        } catch (_) { /* best-effort */ }
                        /* Ramming — the SLAMMED person takes the impact: 1d6
                         * bludgeoning to their torso, bypassing armor (on top of
                         * the double kick). */
                        let ramDmg = 0;
                        try {
                            const rammingRoll = await new Roll("1d6").evaluate();
                            ramDmg = Math.max(0, Math.floor(Number(rammingRoll.total) || 0));
                            if (ramDmg > 0) {
                                await emitApplyDamage({
                                    targetUuid:    _defenderActor.uuid,
                                    weaponDamage:  ramDmg,
                        ...
                        /* Slammed INTO a wall — extra 1d6/2 ramming per full 2m
                         * travelled, torso, bypassing armour (on top of the fixed
                         * impact ramming above). */
                        let wallDmg = 0;
                        if (clipped && moved >= 2) {
                            try {
                                const inc = Math.floor(moved / 2);
                        ...
                        /* Stun save — forced automatically on the slammed foe. */
                        try { await _defenderActor.rollStunSave?.({ modifier: 0 }); }
                        catch (err) { console.warn("witcher-ttrpg-death-march | slam stun save failed", err); }
                        /* Display the ACTUAL distance the token travelled (grid
                         * snapping can differ from the rolled request). */
                        const shownM  = moved > 0 ? Math.round(moved) : meters;
                        const distTxt = moved > 0 ? `${moved.toFixed(1)}m${clipped ? " (cut short on a wall)" : ""}` : `${meters}m`;
                        const fragment =
                            `<div class="wdm-attack-rider"><i class="fa-solid fa-hand-fist"></i> ` +
                            `<strong>${esc(_defenderActor?.name ?? "Defender")}</strong> is slammed for ` +
                            `<strong>${dmgTotal}</strong> (double kick) to the <strong>${esc(locLabel)}</strong>` +
                            (ramDmg > 0 ? ` + <strong>${ramDmg}</strong> ramming (torso, ignores armor)` : "") +
                            (wallDmg > 0 ? ` + <strong>${wallDmg}</strong> into the wall` : "") +
                            `, knocked <strong>prone</strong>, thrown back <strong>${esc(distTxt)}</strong>, and must save vs Stun.</div>`;
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: {
                                label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.Slam", { dmg: dmgTotal, m: shownM }, `Slam: ${dmgTotal} dmg, ${shownM}m + prone`),
                                kind:  "status",
                                icon:  "fa-hand-fist"
                            }
                        });
                    } catch (err) {
                        console.warn("witcher-ttrpg-death-march | melee slam rider failed", err);
                    }
                }

                /* CE Takedown (grappling weapon) — on a hit: NON-LETHAL punch
                 * damage (to stamina, still soaked by SP — no armour bypass) and
                 * BOTH fighters go prone. The grapple hold is MAINTAINED (prone
                 * doesn't break it). Weapon roll is noDamage; this rider is the
                 * whole effect. Usable by either role in the grapple. */
                if (delta > 0 && decl.strike === "takedown" && _defenderActor) {
                    try {
                        /* Punch = 1d6 + MeleeBonus (not weapon damage — the *** rule
                         * swaps only the skill). */
                        const mb = Number(this.system?.derivedStats?.meleeBonus) || 0;
                        const punchRoll = await new Roll(`1d6 ${mb >= 0 ? "+" : "-"} ${Math.abs(mb)}`).evaluate();
                        const punchDmg  = Math.max(0, Math.floor(Number(punchRoll.total) || 0));
                        const { loc: locKey } = await rollHitLocation("human");
                        const locDef   = ATTACK_LOCATIONS[locKey];
                        const locLabel = game.i18n.localize(locDef?.labelKey ?? locKey);
                        if (punchDmg > 0) {
                            await emitApplyDamage({
                                targetUuid:    _defenderActor.uuid,
                                weaponDamage:  punchDmg,
                                silverDamage:  0,
                                damageTypes:   ["bludgeoning"],
                                locationKey:   locKey,
                                locationLabel: locLabel,
                                nonLethal:     true,   // stamina; SP still applies
                                kind:          "weapon",
                                attackMessageUuid: result.message?.uuid ?? null
                            });
                        }
                        /* Both go prone; grapple maintained (no clearHoldLink). */
                        await emitApplyStatus({ targetUuid: _defenderActor.uuid, statusId: "prone", action: "apply", sourceActorUuid: this.uuid });
                        await emitApplyStatus({ targetUuid: this.uuid,           statusId: "prone", action: "apply", sourceActorUuid: this.uuid });
                        const fragment =
                            `<div class="wdm-attack-rider"><i class="fa-solid fa-hand-fist"></i> ` +
                            `<strong>${esc(_defenderActor?.name ?? "Defender")}</strong> is taken down — ` +
                            `<strong>${punchDmg}</strong> non-lethal to the <strong>${esc(locLabel)}</strong> (stamina); ` +
                            `both fighters go <strong>prone</strong>, grapple maintained.</div>`;
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: {
                                label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.Takedown", { dmg: punchDmg }, `Takedown: ${punchDmg} non-lethal + prone`),
                                kind:  "status",
                                icon:  "fa-hand-fist"
                            }
                        });
                    } catch (err) {
                        console.warn("witcher-ttrpg-death-march | melee takedown rider failed", err);
                    }
                }

                /* CE Trip (grappling weapon) — like Takedown but only the TARGET
                 * goes prone. On a hit: NON-LETHAL punch damage (to stamina, still
                 * soaked by SP — no armour bypass) and the target falls prone; the
                 * grappler stays upright and the grapple is MAINTAINED. Weapon roll
                 * is noDamage; this rider is the whole effect. Grappler-only. */
                if (delta > 0 && decl.strike === "trip" && _defenderActor) {
                    try {
                        /* Punch = 1d6 + MeleeBonus (the *** rule swaps only the skill). */
                        const mb = Number(this.system?.derivedStats?.meleeBonus) || 0;
                        const punchRoll = await new Roll(`1d6 ${mb >= 0 ? "+" : "-"} ${Math.abs(mb)}`).evaluate();
                        const punchDmg  = Math.max(0, Math.floor(Number(punchRoll.total) || 0));
                        const { loc: locKey } = await rollHitLocation("human");
                        ...
                        /* Only the TARGET goes prone; grapple maintained (no clearHoldLink). */
                        await emitApplyStatus({ targetUuid: _defenderActor.uuid, statusId: "prone", action: "apply", sourceActorUuid: this.uuid });
                        const fragment =
                            `<div class="wdm-attack-rider"><i class="fa-solid fa-person-falling"></i> ` +
                            `<strong>${esc(_defenderActor?.name ?? "Defender")}</strong> is tripped — ` +
                            `<strong>${punchDmg}</strong> non-lethal to the <strong>${esc(locLabel)}</strong> (stamina); ` +
                            `they go <strong>prone</strong>, grapple maintained.</div>`;
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: {
                                label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.Trip", { dmg: punchDmg }, `Trip: ${punchDmg} non-lethal + target prone`),
                                kind:  "status",
                                icon:  "fa-person-falling"
                            }
                        });
                    } catch (err) {
                        console.warn("witcher-ttrpg-death-march | melee trip rider failed", err);
                    }
                }

                /* CE Choke (Strangling weapon) — a HOLD that suffocates. On the
                 * hit that APPLIES it, deal one action's worth of suffocation:
                 * (3 + melee bonus + Strangling flat) × Strangling multiplier (a
                 * garrote's "×3 suffocation" lands here), flat + through armour +
                 * magic shield, to stamina then HP. The chokeheld pair is created
                 * by the generic hold-status rider above; each later turn is dealt
                 * by the upkeep prompt (mechanics/choke) or by re-invoking Choke. */
                if (delta > 0 && decl.strike === "chokehold" && _defenderActor) {
                    try {
                        const { applyChokeDamage } = await import("../../mechanics/choke.mjs");
                        const { mult } = strangleSuffocation(weapon);
                        const dmg = await applyChokeDamage(this, _defenderActor, weapon, { attackMessageUuid: result.message?.uuid ?? null });
                        const fragment =
                            `<div class="wdm-attack-rider"><i class="fa-solid fa-lungs"></i> ` +
                            `<strong>${esc(_defenderActor?.name ?? "Defender")}</strong> is choking — ` +
                            `<strong>${dmg}</strong> suffocation (stamina, then HP; through armour)${mult > 1 ? ` (×${mult} Strangling)` : ""}.</div>`;
                        await appendAttackResult(result.message, {
                            fragment,
                            summaryAdd: { label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.Choke", { dmg }, `Choke: ${dmg} suffocation`), kind: "status", icon: "fa-lungs" }
                        });
                    } catch (err) {
                        console.warn("witcher-ttrpg-death-march | melee choke rider failed", err);
                    }
                }
                ...
                if (defChoice?.action === "block" && delta <= 0 && defChoice?.itemId) {
                    try {
                        ...
                        /* Natural-weapon block (race toggle) — no item, no
                         * reliability to spend; it costs the defender 1 HP
                         * instead (the limb takes the strain). The full block
                         * still negates the hit — this is just the price. */
                        if (defChoice.itemId === "natural" && _defenderActor?.uuid) {
                            await emitApplyDamage({
                                targetUuid:        _defenderActor.uuid,
                                weaponDamage:      1,
                                directHp:          true,
                        ...
                        /* Feeble / Hefty block-through riders (EO p.7), resolved
                         * automatically. Terms:
                         *   Hefty (attacker) — a successful block eats the hit, but
                         *     the EFFORT of stopping a heavy blow drains half the
                         *     rolled damage as NON-LETHAL stamina (directStamina —
                         *     bypasses Quen/armour/resists). EXEMPT if the block item
                         *     is Sturdy / Very Sturdy (counterHefty) — those block a
                         *     Hefty attack normally, no leak.
                         *   Feeble (defender's block item) — a weak guard: half the
                         *     rolled damage leaks THROUGH the block as a real hit
                         *     (armour/resists still apply). vs a non-Feeble attacker
                         *     → half LETHAL. vs a Hefty attacker → half lethal AND
                         *     half non-lethal (the worst case). Never leaks when the
                         *     attacker is itself Feeble. */
                        ...
                        const attackerHefty = attkQ.has("hefty");
                        const blockerFeeble = defQ.has("feeble");
                        const allQCat = { ...WEAPON_QUALITIES, ...ARMOR_QUALITIES };
                        const blockCountersHefty = [...defQ].some(q => allQCat[q]?.counterHefty || allQCat[q]?.counterCrushingForce);
                        /* Roll HALF the weapon's rolled damage once (shared by both riders). */
                        const rollLeakHalf = async () => {
                            try {
                                const dr = mainDamage?.formula ? await new Roll(mainDamage.formula).evaluate() : null;
                                return dr ? Math.floor((Number(dr.total) || 0) / 2) : 0;
                            } catch (_) { return 0; }
                        };
                        const wLeakTypes = weapon?.system?.effective?.damageTypes ?? weapon?.system?.damageTypes ?? [];
                        /* Feeble leak goes THROUGH armour, so it needs a hit location. */
                        const applyFeebleLeak = async (half, nonLethal) => {
                            if (!(half > 0) || !_defenderActor?.uuid) return;
                            const { loc } = await rollHitLocation("human");
                            const locLabel = game.i18n.localize(ATTACK_LOCATIONS[loc]?.labelKey ?? loc);
                            try {
                                await emitApplyDamage({
                                    targetUuid:        _defenderActor.uuid,
                                    weaponDamage:      half,
                                    damageTypes:       wLeakTypes,
                                    locationKey:       loc,
                                    locationLabel:     locLabel,
                                    nonLethal:         !!nonLethal,
                                    kind:              "weapon",
                            ...
                            } catch (err) { console.warn("witcher-ttrpg-death-march | feeble block-through apply failed", err); }
                        };

                        if (attackerHefty && blockCountersHefty) {
                            /* Sturdy / Very Sturdy — blocks the Hefty attack normally. */
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        `<i class="fa-solid fa-shield"></i> ` +
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.SturdyBlocksHefty", "Sturdy")}</strong>: ` +
                                        `<em>${esc(_defenderActor?.name ?? "Defender")}</em> blocks the Hefty attack normally — no block-through (EO p.7).` +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.SturdyNormalBlock", "Sturdy: normal block"), kind: "info", icon: "fa-shield" }
                            });
                        } else if (attackerHefty && blockerFeeble) {
                            /* Feeble guard vs a Hefty attacker: half LETHAL + half NON-LETHAL, both through armour. */
                            const half = await rollLeakHalf();
                            await applyFeebleLeak(half, false);
                            await applyFeebleLeak(half, true);
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        `<i class="fa-solid fa-shield-halved"></i> ` +
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.FeebleVsHefty", "Feeble vs Hefty")}</strong>: ` +
                                        `<em>${esc(_defenderActor?.name ?? "Defender")}</em> takes <strong>${half}</strong> lethal and <strong>${half}</strong> non-lethal through the block (armour applies, EO p.7).` +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.FeebleVsHefty", "Feeble vs Hefty"), kind: "info", icon: "fa-shield-halved" }
                            });
                        } else if (attackerHefty) {
                            /* Plain Hefty block-through — half rolled damage as NON-LETHAL
                             * stamina, bypassing the whole pipeline (directStamina). */
                            const heftyHalf = await rollLeakHalf();
                            if (heftyHalf > 0 && _defenderActor?.uuid) {
                                try {
                                    await emitApplyDamage({
                                        targetUuid:        _defenderActor.uuid,
                                        weaponDamage:      heftyHalf,
                                        directStamina:     true,
                                        attackMessageUuid: result.message?.uuid ?? null
                                    });
                                } catch (err) { console.warn("witcher-ttrpg-death-march | hefty block-through apply failed", err); }
                            }
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        `<i class="fa-solid fa-weight-hanging"></i> ` +
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.HeftyBlockThrough", "Hefty block-through")}</strong>: ` +
                                        `<em>${esc(_defenderActor?.name ?? "Defender")}</em> takes <strong>${heftyHalf}</strong> non-lethal to stamina (half the rolled damage, EO p.7).` +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.HeftyHalfNonLethal", "Hefty: half non-lethal"), kind: "info", icon: "fa-weight-hanging" }
                            });
                        } else if (blockerFeeble && !attkQ.has("feeble")) {
                            /* Feeble guard vs a non-Feeble (non-Hefty) attacker: half LETHAL through armour. */
                            const half = await rollLeakHalf();
                            await applyFeebleLeak(half, false);
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        `<i class="fa-solid fa-shield-halved"></i> ` +
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.FeebleBlock", "Feeble block")}</strong>: ` +
                                        `<em>${esc(_defenderActor?.name ?? "Defender")}</em> takes <strong>${half}</strong> lethal through the block (armour applies, EO p.7).` +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.FeebleHalfDamage", "Feeble: half damage"), kind: "info", icon: "fa-shield-halved" }
                            });
                        }
                        ...
                        if (decl.strike === "charge") {
                            ...
                            await appendAttackResult(result.message, {
                                fragment: chargeFrag,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.ChargeBlockedProneCheckAvailable", "Charge blocked: prone check available"), kind: "info", icon: "fa-person-falling" }
                            });
                        }
                        ...
                        if (defQ.has("bladeCatcher") && delta <= 0) {
                            const frag =
                                `<div class="wdm-attack-rider">` +
                                    `<i class="fa-solid fa-hand-fist"></i> ` +
                                    `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.BladeCatcher", "Blade Catcher")}</strong>: ` +
                                    tFormat("WITCHER.Doc.WeaponAttackMixin.Text.BladeCatcherBody", { defender: esc(_defenderActor?.name ?? t("WITCHER.Doc.WeaponAttackMixin.Text.Defender", "Defender")), attacker: esc(this.name) }, `<em>${esc(_defenderActor?.name ?? "Defender")}</em>'s weapon locks with <em>${esc(this.name)}</em>'s. Opposed Small Blades vs Physique / Sleight of Hand to free both weapons (EO p.7).`) +
                                `</div>`;
                            await appendAttackResult(result.message, {
                                fragment: frag,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.BladeCatcherLocked", "Blade Catcher: locked"), kind: "info", icon: "fa-hand-fist" }
                            });
                        }
                    } catch (err) {
                ...
                try {
                    ...
                    if (onHit && attkQ.has("magicalAnchoring")) {
                        ...
                        for (const sid of ["intangible", "invisible", "teleporting"]) {
                            if (_defenderActor.statuses?.has?.(sid)) {
                                try { await emitApplyStatus({ targetUuid: _defenderActor.uuid, statusId: sid, action: "remove", sourceActorUuid: this.uuid }); }
                                catch (_) { /* best-effort */ }
                                cleared.push(sid);
                            }
                        }
                        const fragMA =
                            `<div class="wdm-attack-rider">` +
                                ...
                                `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.MagicallyAnchoring", "Magically Anchoring")}</strong>: ` +
                                `<em>${esc(_defenderActor.name)}</em> is forced visible and solid` +
                                (cleared.length ? ` — cleared ${esc(cleared.join(", "))}` : "") + `.` +
                            `</div>`;
                        await appendAttackResult(result.message, {
                            fragment: fragMA,
                            summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.Anchored", "Anchored"), kind: "status", icon: "fa-anchor" }
                        });
                    }
                    ...
                    if (onHit && attkQ.has("injector")) {
                        const fragInj =
                            `<div class="wdm-attack-rider">` +
                                ...
                                `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.Injector", "Injector")}</strong>: if charged, the poison is +3 harder to resist or the elixir lasts +3 rounds (EO p.7).` +
                            `</div>`;
                        await appendAttackResult(result.message, {
                            fragment: fragInj,
                            summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.Injector", "Injector"), kind: "info", icon: "fa-syringe" }
                        });
                    }
                    ...
                    if (onHit && this.type === "monster") {
                        ...
                        if (armorTriggered) {
                            ...
                            await appendAttackResult(result.message, {
                                fragment: fragSC,
                                summaryAdd: { label: tFormat("WITCHER.Doc.WeaponAttackMixin.Summary.Staggered", { name: this.name }, `${this.name}: Staggered`), kind: "status", icon: "fa-shield-virus" }
                            });
                        }
                    }
                    ...
                    if (defChoice?.itemId && (defChoice.action === "parry" || defChoice.action === "block") && delta < -4) {
                        ...
                        if (qs.includes("criticalBlock")) {
                            const fragCB =
                                `<div class="wdm-attack-rider">` +
                                    `<i class="fa-solid fa-shield"></i> ` +
                                    `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.CriticalBlock", "Critical Block")}</strong>: ` +
                                    `<em>${esc(_defenderActor.name)}</em> beats by ${Math.abs(delta)} ` +
                                    `— free Shield Strike knocks <em>${esc(this.name)}</em> back 4m and prone.` +
                                `</div>`;
                            await appendAttackResult(result.message, {
                                fragment: fragCB,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.CriticalBlock", "Critical Block!"), kind: "status", icon: "fa-shield" }
                            });
                        }
                    }
                        ...
                        if (qs.includes("criticalRiposte")) {
                            const fragCR =
                                `<div class="wdm-attack-rider">` +
                                    ...
                                    `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.CriticalRiposte", "Critical Riposte")}</strong>: ` +
                                    tFormat("WITCHER.Doc.WeaponAttackMixin.Text.CriticalRiposteBody", { defender: esc(_defenderActor.name), delta: Math.abs(delta) }, `<em>${esc(_defenderActor.name)}</em> beats parry by ${Math.abs(delta)} — free Strike with a held weapon (no extra STA / no penalty).`) +
                                `</div>`;
                            await appendAttackResult(result.message, {
                                fragment: fragCR,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.CriticalRiposte", "Critical Riposte!"), kind: "status", icon: "fa-bolt" }
                            });
                        }
                    }
                            ...
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        ...
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.CriticalFlurry", "Critical Flurry")}</strong>: ` +
                                        tFormat("WITCHER.Doc.WeaponAttackMixin.Text.CriticalFlurryBody", { name: esc(this.name) }, `<em>${esc(this.name)}</em>'s witcher armor grants a free Disarm or Trip at no penalty / STA.`) +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.CriticalFlurry", "Critical Flurry"), kind: "status", icon: "fa-wind" }
                            });
                        }
                        if (hasSpellcast) {
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        ...
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.CriticalSpellcasting", "Critical Spellcasting")}</strong>: ` +
                                        tFormat("WITCHER.Doc.WeaponAttackMixin.Text.CriticalSpellcastingBody", { name: esc(this.name) }, `<em>${esc(this.name)}</em>'s witcher armor grants a free Spellcasting check (Sign cost only).`) +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.CriticalSpellcasting", "Critical Spellcasting"), kind: "status", icon: "fa-wand-magic-sparkles" }
                            });
                        }
                        if (hasMomentum) {
                            await appendAttackResult(result.message, {
                                fragment:
                                    `<div class="wdm-attack-rider">` +
                                        ...
                                        `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.CriticalMomentum", "Critical Momentum")}</strong>: ` +
                                        tFormat("WITCHER.Doc.WeaponAttackMixin.Text.CriticalMomentumBody", { name: esc(this.name) }, `<em>${esc(this.name)}</em>'s witcher armor grants a single free Strike with a held weapon.`) +
                                    `</div>`,
                                summaryAdd: { label: t("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.CriticalMomentum", "Critical Momentum"), kind: "status", icon: "fa-forward" }
                            });
                        }
                    }
                                ...
                                await appendAttackResult(result.message, {
                                    fragment:
                                        `<div class="wdm-attack-rider">` +
                                            ...
                                            `<strong>${t("WITCHER.Doc.WeaponAttackMixin.Text.SetBonus", "Set Bonus")}</strong>: ` +
                                            `<em>${esc(this.name)}</em> wears the full <em>${esc(name)}</em> set — apply its signature bonus.` +
                                        `</div>`,
                                    summaryAdd: { label: tFormat("WITCHER.Doc.WeaponAttackMixin.Dialog.Button.SetX", { name: name }, "Set: {name}"), kind: "info", icon: "fa-medal" }
                                });
                            }
                        }
                    }
        ...
        if (adrenalineDice > 0) {
            try {
                const staPer = hasWRPerk(this, "calmMind")
                    ? Math.floor(adrenalineStaPerDieFor(this) / 2)
                    : adrenalineStaPerDieFor(this);
        ...
        /* Nimble (EO p.7): reduce the extra-action STA COST by the weapon's
         * reduction (2), floored at 0 — NOT a post-hoc refund. The old refund
         * added STA back AFTER the fact, which clamped at max and, at high
         * stamina, turned the extra action into a net STA GAIN. Instead we
         * stash the reduction on the actor for recordExtraAction to subtract at
         * spend time (it consumes/zeroes it). Set here (after the attack
         * commits) so a cancelled attack never leaves a stale reduction; 0 for
         * any non-extra-action attack so it doesn't leak to the next one. */
        this._nimbleExtraStaReduction = (this.nextActionSlot === "extra")
            ? Math.max(nimbleAttackRed, isFastDrawAttack ? nimbleDrawRed : 0)
            : 0;
    ...
    /** Drop a thrown weapon into the world after it's been thrown: it
     *  leaves the actor's hand entirely.
     *
     *    1. Unequip on the actor side. Runs client-side (players own
     *       their own PC's items) so the dock slot clears immediately.
     *    2. Route the "clone-to-world + container-detach + embedded-
     *       delete" cascade through `emitDropItemToWorld`. That helper
     *       creates the world copy under GM permission (players can't
     *       write the world Items collection directly — previously step
     *       2 here failed silently for players, leaving the weapon
     *       vanished from their inventory with no world copy to pick
     *       back up), then runs the delete + container detach GM-side.
     *
     *  Best-effort — the unequip is wrapped so a failure there doesn't
     *  block the drop-to-world; the emitter's handler logs any soft
     *  failures on the GM side. */
    async _dropThrownWeapon(weapon) {
        /* 1. Unequip FIRST so the dock reads empty even before the
         * embedded-delete lands. Also clears any slot-assignment flags
         * the sheet uses (main / off / quick / two-hand). Runs on the
         * player's own client because they OWN their own items. */
        try {
        ...
        /* 2. GM-relay the world-clone + delete + container detach. Same
         * relay as the "Drop to world" button on the inventory dock —
         * shares the auth check (sender must own source actor), the
         * default-OWNER ownership stamp on the world copy, and the
         * container-content list scrub. Direct GM call if we ARE the
         * GM; socket-hop if not. */
        try {
            const { emitDropItemToWorld } = await import("../../setup/socketHook.mjs");
            emitDropItemToWorld({ sourceActorUuid: this.uuid, itemId: weapon.id });
        } catch (err) {
            console.warn("witcher-ttrpg-death-march | thrown-weapon drop-to-world dispatch failed", err);
        }
