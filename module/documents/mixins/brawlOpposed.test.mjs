...
test("brawlMixin imports requestDefenseFromOwner for opposed-check flow", () => {
    ...
    assert.match(src, /import\s*\{[^}]*requestDefenseFromOwner[^}]*\}\s*from\s*"[^"]*setup\/socketHook\.mjs"/);
});
...
test("needsGrapple actions refuse to fire without an existing pair against the target", () => {
    ...
    assert.match(src, /if \(meta\.needsGrapple\)[\s\S]+?attackerHoldsTarget\(this,\s*target\)/);
...
test("All brawl attacks (grapple + plain) request a defense BEFORE the attacker rolls", () => {
    ...
    // Disarm (no hold status) is folded into the opposed check explicitly.
    assert.match(src, /isOpposedAction\s*=\s*\(isGrapple\s*\|\|\s*isPlainAttack\s*\|\|\s*meta\.isDisarm\)/);
    /* Disarm → "disarm"; Choke passes its OWN kind so it hits the Brawling-only
     * gate; other grapples → "grapple"; Push → "push"; everything else → "normal". */
    assert.match(src, /attackKind:\s*meta\.isDisarm\s*\?\s*"disarm"/);
    assert.match(src, /decl\.action\s*===\s*"choke"\s*\?\s*"choke"\s*:\s*"grapple"/);
    assert.match(src, /decl\.action\s*===\s*"push"\s*\?\s*"push"\s*:\s*"normal"/);
});
