...
test("Joint-attack disallow list grows after each parried shot", () => {
  ...
  // the strike type is a joint attack (sMeta.offhand === true), per
  // RAW Core p.163. Gate on the STRIKE (not on whether the off-hand
  // weapon successfully resolved) so a stale/missing off-hand ref
  // can't silently disable the item-reuse restriction.
  assert.match(src, /const _usedDefenseItemIds = \[\]/);
  assert.match(src, /const isJoint = sMeta\.offhand === true/);
...
test("Attack roll stamps engagementId + attackTotal + combat category as flags on the chat message", () => {
  // flags carry the chat-filter category ("combat"). attackTotal is now stamped
  // UNCONDITIONALLY (a targetless card needs it so "Assign Target" can re-resolve
  // the verdict later); engagementId rides its own conditional spread.
  assert.match(src, /flags:\s*\(r\) =>/);
  assert.match(src, /category:\s*"combat"/);
  assert.match(src, /attackTotal:\s*r\.total/);
  assert.match(src, /engagementId:\s*decl\._engagementId/);
});
...
test("rollDamageFromButton reads attack flags + finds matching defense for the delta", () => {
  ...
  /* Callsites pass the house-ruled brackets as the 2nd arg so a GM
   * edit to the crit ladder in HouseRulesConfigApp takes effect on the
   * next attack — the pure helper (see combat/critSeverity.mjs) stays
   * signature-compatible with the RAW default when the arg is omitted. */
  assert.match(src, /critSeverityFromDelta\(delta,\s*hrCritBrackets\(\)\)/);
});
