...
/* Break and Boon tables. Raw data holds a stable `id` (used as internal
 * key / AE marker) and English name/effect as fallbacks. Localized name +
 * effect are resolved at LOOKUP time by mentalBreakForRoll / boonForRoll
 * so we don't capture t() before Foundry has loaded lang. */
const MENTAL_BREAKS = Object.freeze([
    { min: 1,  max: 5,  id: "Indulgent",    name: "Indulgent",    effect: "This is too much. You need your vice. You need comfort. Eat food until you are gorged." },
    { min: 6,  max: 7,  id: "Paranoid",     name: "Paranoid",     effect: "Can you really trust anyone but yourself? Are you being watched? Something is out to get you. Isolate." },
    { min: 8,  max: 9,  id: "Scared",       name: "Scared",       effect: "You're in danger. Everything is uncertain. −1 to every roll." },
    { min: 10, max: 11, id: "Depressive",   name: "Depressive",   effect: "Nothing seems to matter that much anymore. What's the point. −2 WILL." },
    { min: 12, max: 14, id: "Impulsive",    name: "Impulsive",    effect: "Just do something. Anything. Not the time to think about it." },
    { min: 15, max: 16, id: "SelfHarming",  name: "Self-Harming", effect: "Stupid, stupid, stupid. This is all your fault. You did this. Take 1d6 damage on a random body part." },
    { min: 17, max: 18, id: "Selfish",      name: "Selfish",      effect: "This is not the time to think of others. You need to look out for yourself." },
    { min: 19, max: 20, id: "Violent",      name: "Violent",      effect: "You've had enough. Violence is the only answer. +1 REF for the immediate combat." }
]);

const BOONS = Object.freeze([
    { min: 1,  max: 5,  id: "Stoic",         name: "Stoic",          effect: "This is nothing to you. You ignore the next 1d6 points of STRESS." },
    { min: 6,  max: 7,  id: "Optimistic",    name: "Optimistic",     effect: "This isn't so bad. Clear 1 STRESS." },
    { min: 8,  max: 9,  id: "Hopeful",       name: "Hopeful",        effect: "There is a light at the end of the tunnel. Ignore the next 2d6 sources of STRESS." },
    { min: 10, max: 11, id: "Defiant",       name: "Defiant",        effect: "You've had worse. Is this all they have? You roll twice and take the best result on your next mental break roll." },
    { min: 12, max: 14, id: "Focused",       name: "Focused",        effect: "You will have +1 WA for the rest of the day." },
    { min: 15, max: 16, id: "Stalwart",      name: "Stalwart",       effect: "You clear 2 STRESS. You will survive this." },
    { min: 17, max: 18, id: "Determined",    name: "Determined",     effect: "You clear 4 STRESS. If in combat and below wound threshold, you ignore the penalties for 3 turns." },
    { min: 19, max: 19, id: "Unbreakable",   name: "Unbreakable",    effect: "All your STRESS is cleared. You are resolute. Nothing will stop you. You automatically pass 3 death saves if in combat, and ignore penalties from being below your wound threshold and death state for 3 turns." },
    { min: 20, max: 20, id: "SmileAtDeath",  name: "Smile at Death", effect: "You make peace with your demise. Clear all STRESS. Ignore all wound penalties until the end of combat. You gain 2d6 temporary HP and 2d6 temporary STA. You gain +2 REF until the end of combat. When combat ends, you are automatically thrown back into death state." }
]);

/* Lookup helpers — localize name + effect at CALL time (post-i18nInit),
 * so a swap to French takes effect without a reload. */
const localizeRow = (row, ns) => row ? {
    ...row,
    name:   t(`WITCHER.Mech.Stress.${ns}.${row.id}.Name`,   row.name),
    effect: t(`WITCHER.Mech.Stress.${ns}.${row.id}.Effect`, row.effect)
} : null;

const lookup = (table, roll) => {
    const row = table.find(r => roll >= r.min && roll <= r.max) ?? null;
    const ns = table === MENTAL_BREAKS ? "Break" : "Boon";
    return localizeRow(row, ns);
};
...
const STRESS_CONFIG_DEFAULTS = Object.freeze({
    ...
    woundStressDeadly: 4,
    // Failed monster/skill book study readings add stress. Toggle off to
    // let players fail a reading without the mental toll; the amount is
    // GM-configurable (book chrome reads both via the stressConfig setting).
    bookFailStressEnabled: true,
    bookFailStressAmount: 1
});
...
export async function setStress(actor, val, opts = {}) {
     ...
     *
     * opts.render (optional, default true): forwarded straight to
     * actor.update so hot-path callers (e.g. the hunger cascade in
     * foodAndDrink.mjs) can suppress the intermediate sheet re-render
     * and issue one final render at the end of the cascade. */
    const updateOpts = {};
    if (opts.attackMessageUuid) updateOpts.wdmAttackMessageUuid = opts.attackMessageUuid;
    if (opts.render === false) updateOpts.render = false;
    /* HUNGER STRESS floor: total stress can never drop below the actor's
     * current `system.hungerStress` value UNLESS this call is the hunger
     * cascade itself refunding a depth (opts.wdmHungerRefund). Hunger stress
     * cannot be removed by meditation, WILL saves, therapy, roleplay, or
     * sheet tick-down — only by eating back UP through the satiety tier
     * that granted it.
     *
     * The wdmHungerRefund flag is also forwarded to preUpdateActor via
     * updateOpts so the sheet's direct-write path (character.js) skips the
     * same floor during a refund. */
    const hungerLocked = Number(actor.system?.hungerStress) || 0;
    let clamped = Math.max(0, val);
    if (opts.wdmHungerRefund) {
        updateOpts.wdmHungerRefund = true;
    } else if (hungerLocked > 0) {
        clamped = Math.max(hungerLocked, clamped);
    }
    return actor.update({ "system.stress": clamped }, updateOpts);
}
    ...
    if (passed) {
        await post(
            `<div style="border-left:3px solid #4a7c59;padding:4px 8px">
                <b>${actor.name}</b> — ${t("WITCHER.Mech.Stress.Text.StressWillSave", "Stress WILL Save")}<br>
                ${tFormat("WITCHER.Mech.Stress.Text.StressWillNeed", { stress, will, threshold }, `Stress <b>${stress}</b> / Will <b>${will}</b> · need ≤ <b>${threshold}</b>`)}<br>
                ${tFormat("WITCHER.Mech.Stress.Text.RolledResult", { total: saveRoll.total }, `Rolled <b>${saveRoll.total}</b>`)} — <span style="color:#4a7c59"><b>${t("WITCHER.Mech.Stress.Text.HoldsTogether", "Holds together. Barely.")}</b></span>
            </div>`,
            { label: tFormat("WITCHER.Mech.Stress.Summary.StressSavePassed", { name: actor.name, total: saveRoll.total, threshold }, `${actor.name}: Stress save passed (${saveRoll.total}/${threshold})`), kind: "stress", icon: "fa-brain" }
        );
        return;
    }
    ...
    if (owned.size >= cap) {
        await post(
            `<div style="border-left:3px solid #4b0000;padding:6px 10px">
                <b>${actor.name}</b> — ${t("WITCHER.Mech.Stress.Text.StressWillSave", "Stress WILL Save")}<br>
                ${tFormat("WITCHER.Mech.Stress.Text.StressWillNeed", { stress, will, threshold }, `Stress <b>${stress}</b> / Will <b>${will}</b> · need ≤ <b>${threshold}</b>`)}<br>
                ${tFormat("WITCHER.Mech.Stress.Text.RolledResult", { total: saveRoll.total }, `Rolled <b>${saveRoll.total}</b>`)} — <span style="color:#4b0000"><b>${t("WITCHER.Mech.Stress.Text.NoRoomLeft", "NO ROOM LEFT")}</b></span>
                <hr><i>${tFormat("WITCHER.Mech.Stress.Text.OverflowFlavor", { name: actor.name }, `Every break the mind can take is already taken. Control of <b>${actor.name}</b> passes to the GM.`)}</i>
            </div>`,
            { label: tFormat("WITCHER.Mech.Stress.Summary.MentalBreakOverflow", { name: actor.name }, `${actor.name}: Mental break overflow`), kind: "fumble", icon: "fa-skull" }
        );
...
function buildCard({ actor, stress, will, penalty, threshold, saveRoll, header, colour, tableRoll, result }) {
    return `<div style="border-left:3px solid ${colour};padding:4px 8px">
        <b>${actor.name}</b> — ${t("WITCHER.Mech.Stress.Text.StressWillSave", "Stress WILL Save")}<br>
        ${tFormat("WITCHER.Mech.Stress.Text.StressWillPenaltyNeed", { stress, will, penalty, threshold }, `Stress <b>${stress}</b> / Will <b>${will}</b> · penalty <b>-${penalty}</b> · need ≤ <b>${threshold}</b>`)}<br>
        ${tFormat("WITCHER.Mech.Stress.Text.RolledSaveHeader", { total: saveRoll, colour, header }, `Rolled <b>${saveRoll}</b> — <span style="color:${colour}"><b>${header}</b></span>`)}
        <hr style="margin:4px 0">
        <b>${tFormat("WITCHER.Mech.Stress.Text.D20Result", { roll: tableRoll, name: result?.name ?? "???" }, `d20: ${tableRoll} — ${result?.name ?? "???"}`)}</b><br>
        <i>${result?.effect ?? ""}</i>
        <br><small style="opacity:0.7">${tFormat("WITCHER.Mech.Stress.Text.StressClearedTo", { n: will - 1 }, `Stress cleared to ${will - 1}.`)}</small>
    </div>`;
}
...
const BREAK_CONFIG = () => Object.freeze({
    ...
    "Violent":      { img: `${ICON_DIR}/break-violent.svg`,
                      combatEffect: { statusId: "break-violent", name: t("WITCHER.Mech.Stress.Text.ViolentFury", "Violent Fury"),
...
async function applySelfHarmDamage(actor) {
    ...
    try {
        await actor.update({ "system.derivedStats.hp.value": hp - damage });   // HP may go negative (see drainHp)
    ...
    try {
        ...
        await ChatMessage.create({
            ...
            content: tFormat(
                "WITCHER.Mech.Stress.Chat.SelfHarm",
                { name: actor.name, base, mult: multTxt, damage, loc },
                "<div style=\"border-left:3px solid #6b3f3f;padding:4px 8px\"><b>{name}</b> — self-harm: <b>{base}</b>{mult} = <b>{damage}</b> damage to the <b>{loc}</b>.</div>"
            )
        });
...
const BOON_CONFIG = () => Object.freeze({
    ...
    "Focused":        { effect: "duration",          statusId: "boon-focused", durationSeconds: 86400, label: t("WITCHER.Mech.Stress.Dialog.Button.Focused", "Focused") },
...
async function applyAbsorbPlan(actor, plan) {
    ...
    const detail = plan.hopeful
        ? tFormat(
            "WITCHER.Mech.Stress.Chat.HopefulDetail",
            { left: plan.hopeful.remaining - 1 > 0
                ? tFormat("WITCHER.Mech.Stress.Chat.SourcesLeft", { n: plan.hopeful.remaining - 1 }, "{n} sources left")
                : t("WITCHER.Mech.Stress.Chat.Depleted", "depleted") },
            "1 source spent — {left}")
        : tFormat(
            "WITCHER.Mech.Stress.Chat.StoicDetail",
            { spent: plan.stoic.decrementBy, left: plan.stoic.remaining - plan.stoic.decrementBy > 0
                ? tFormat("WITCHER.Mech.Stress.Chat.PointsLeft", { n: plan.stoic.remaining - plan.stoic.decrementBy }, "{n} points left")
                : t("WITCHER.Mech.Stress.Chat.Depleted", "depleted") },
            "{spent} points spent — {left}");
    try {
        await ChatMessage.create({
            speaker: ChatMessage.getSpeaker({ actor }),
            content: tFormat(
                "WITCHER.Mech.Stress.Chat.BoonAbsorb",
                { tint: BOON_TINT, name: actor.name, boon: plan.boonName, absorbed: plan.absorbed, detail },
                "<div style=\"border-left:3px solid {tint};padding:4px 8px\"><b>{name}</b> — <b>{boon}</b> absorbed <b>{absorbed}</b> STRESS<br><small>{detail}</small></div>"
            )
        });
    } catch (_) { /* chat is informational only */ }
}
    ...
    switch (cfg.effect) {
        ...
        case "clear+grit": {
            ...
            if (inCombat && belowThreshold) {
                await actor.createEmbeddedDocuments("ActiveEffect", [{
                    name: t("WITCHER.Mech.Stress.Text.DeterminedGrit", "Determined Grit"),
        ...
        case "smile": {
            ...
            const duration = inCombat
                ...
                : { seconds: 3 * roundSecs, startTime: Number(game.time?.worldTime) || 0 };
            await actor.createEmbeddedDocuments("ActiveEffect", [{
                name: t("WITCHER.Mech.Stress.Text.SmileAtDeath", "Smile at Death"),
            ...
            await ChatMessage.create({
                speaker: ChatMessage.getSpeaker({ actor }),
                content: tFormat(
                    "WITCHER.Mech.Stress.Chat.SmileAtDeath",
                    { name: actor.name, hp: hpRoll.total, sta: stRoll.total },
                    "<i>{name} smiles at Death — Temp HP +<b>{hp}</b>, Temp STA +<b>{sta}</b>.</i>"
                )
            });
            return true;
        }
    ...
    for (const c of combat.turns ?? []) {
        ...
        if (smileAEs.length) {
            try {
                ...
                await ChatMessage.create({
                    speaker: ChatMessage.getSpeaker({ actor }),
                    content: `<em>${tFormat("WITCHER.Mech.Stress.Text.SmileAtDeathClaims", { name: actor.name, sad: t("WITCHER.Mech.Stress.Text.SmileAtDeath", "Smile at Death") }, `${actor.name} — combat ends. <b>${t("WITCHER.Mech.Stress.Text.SmileAtDeath", "Smile at Death")}</b> claims its due. Thrown back into death state.`)}</em>`
                });
            } catch (err) {
...
async function onCreateCriticalWoundStress(item, options, userId) {
    ...
    try {
        ...
        const fragment = tFormat(
            "WITCHER.Mech.Stress.Chat.CriticalRider",
            { tint: BREAK_TINT, name: actor.name, label, delta },
            "<div class=\"wdm-attack-rider\" style=\"border-left:3px solid {tint}\"><i class=\"fa-solid fa-brain\"></i> <strong>{name}</strong> · <strong>{label}</strong> critical wound — Stress <b>+{delta}</b>.</div>"
        );
        ...
        if (attackMsg) {
            ...
            await appendAttackResult(attackMsg, {
                fragment,
                summaryAdd: { label: tFormat("WITCHER.Mech.Stress.Chat.SummaryStress", { name: actor.name, delta }, "{name}: +{delta} Stress"), kind: "stress", icon: "fa-brain" }
            });
        } else {
            await ChatMessage.create({
                speaker: ChatMessage.getSpeaker({ actor }),
                content: tFormat(
                    "WITCHER.Mech.Stress.Chat.CriticalStandalone",
                    { tint: BREAK_TINT, name: actor.name, label, delta },
                    "<div style=\"border-left:3px solid {tint};padding:4px 8px\"><b>{name}</b> — received <b>{label}</b> critical wound.<br>Stress <b>+{delta}</b> from the shock of the injury.</div>"
                )
            });
        }
    } catch (_) { /* chat is informational only */ }
...
export function onPreUpdateActor(actor, changes, options, userId) {
    ...
    /* Hunger Stress floor. Any write to system.stress — sheet ticks, GM
     * panel, direct macros — must NOT lower total stress below the actor's
     * current `hungerStress` value. Those stress points are HUNGER stress:
     * only clearable by eating back UP through the tier that granted them.
     * setStress in this same file already enforces this; enforcing again
     * here catches every other update path (the sheet's +/- clicker writes
     * directly via actor.update). The hunger cascade itself marks its refund
     * calls with opts.wdmHungerRefund which is stashed on `options` when
     * grantStress bubbles it up. */
    const hungerLocked = Number(actor.system?.hungerStress) || 0;
    if (hungerLocked > 0 && !options?.wdmHungerRefund) {
        const raw = Number(changes.system.stress);
        if (Number.isFinite(raw) && raw < hungerLocked) {
            changes.system.stress = hungerLocked;
        }
    }
...
export async function onUpdateActor(actor, changes, options, userId) {
    ...
    /* Reactive Hunger Stress floor guard. Belt-and-suspenders backup to
     * the preUpdateActor mutation — if any code path slipped a stress
     * write past the pre-update floor (nested update, form-submit
     * replacement, direct AE change), correct it here. Skipped for the
     * hunger cascade's own refund path (wdmHungerRefund) which needs the
     * ability to lower stress below the previous floor as it releases the
     * lock. */
    if (!options?.wdmHungerRefund) {
        const hungerLocked = Number(actor.system?.hungerStress) || 0;
        const current = Number(actor.system?.stress) || 0;
        if (hungerLocked > 0 && current < hungerLocked) {
            try {
                // Non-recursive: this is a raw update, and preUpdate will
                // early-out because current === hungerLocked (no diff triggers
                // any further logic). Also cheap — it only runs when the
                // floor was violated, which shouldn't happen normally.
                await actor.update({ "system.stress": hungerLocked }, { wdmHungerFloorGuard: true });
            } catch (err) {
                console.warn(`witcher-ttrpg-death-march | Hunger Stress floor guard failed`, err);
            }
        }
    }
