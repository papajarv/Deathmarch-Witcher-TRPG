...
export function buildWeatherConfig(tags) {
    ...
    /* filter.enabled: false matches Foundry's own preset weather
     * (config.mjs `rainStorm` / `fog` / etc.). Enabling the layer's
     * WeatherOcclusionMaskFilter without an authored occlusion config
     * blacks the scene out — the shader's depth-mask defaults compare
     * against `mapElevation(Infinity)` which zeroes the mask over any
     * area with a nonzero depth reading. Suppression regions still
     * work via the fallback VoidFilter (see #constructEffects: when
     * useOcclusionFilter is false, `#suppressionFilter.enabled` follows
     * `!!this.suppression.children.length`), and the layer's
     * childAdded / childRemoved listeners on `canvas.weather.suppression`
     * re-enable the void filter when a region mesh appears. */
...
export function stopWeatherEffects() {
    ...
    if (c) c.alpha = 1;
    /* Hide the suppression container while there's no weather. With
     * initializeEffects(null) both layer filters go off, and any lingering
     * ERASE-blend suppression meshes (from SuppressWeather region
     * behaviors) render as opaque black boxes against the now-empty
     * layer instead of erasing anything. Re-shown by applyConfig on
     * the next non-empty weather sync. */
    const sup = canvas?.weather?.suppression;
...
async function applyConfig(config) {
    ...
    /* Re-show suppression container (stopWeatherEffects hides it while
     * the layer is empty, to prevent lone ERASE-blend meshes from
     * rendering as opaque black boxes with no weather to erase). */
    if (layer.suppression) layer.suppression.visible = true;

    /* Belt-and-braces re-check of the suppression filter. Foundry's
     * `#constructEffects` sets `#suppressionFilter.enabled` from the
     * suppression container's current child count. When our fade-driven
     * initializeEffects call races with a SuppressWeather region-behavior
     * viewing (e.g. behavior_viewed fires between our fade-out and the
     * rebuild), the filter can end up disabled with children present.
     * Re-firing childAdded on the container invokes the layer's own
     * listener (weather-effects.mjs _draw), which recomputes the enable
     * from the LIVE child count. No-op when no regions are active. */
    if (layer.suppression?.children.length) {
        try { layer.suppression.emit("childAdded", layer.suppression.children[0]); }
        catch (_) {}
    }
