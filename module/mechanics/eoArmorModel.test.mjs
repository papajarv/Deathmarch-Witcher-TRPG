...
test("defenseProperties schema carries the EO fields", () => {
    assert.match(defPropsSrc, /out\.aeSlots\s*=\s*new\s+fields\.SchemaField\(/);
    assert.match(defPropsSrc, /head:\s*num\(\)/);
...
test("Armor item sheet exposes the EO field block (non-shield only)", () => {
    ...
    assert.match(armorTpl, /class="wdm-cfg-row is-checkbox"[\s\S]{0,160}name="system\.difficult"/);
...
test("Chrome EV footer tooltip + pen-label switch text under EO mode", () => {
    ...
    // EO branch text uses STA max + RUN + skills wording (not REF/DEX magic).
...
test("EV skill penalty lands on `skill.total` (NOT mutated onto `skill.modifier`)", () => {
    ...
    assert.match(charSrc, /skill\.total\s*=\s*statVal\s*\+\s*rank\s*\+\s*mod\s*-\s*evSkillPen/);
});
