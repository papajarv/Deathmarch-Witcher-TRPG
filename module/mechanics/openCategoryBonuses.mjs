...
function labelIndex(catalog = WEAPON_QUALITIES) {
    ...
    for (const [key, entry] of Object.entries(catalog)) {
        /* Index BOTH the localized label AND the raw English source
         * (`labelEn`) so world items authored in English still parse
         * under a localized client. Later insertions overwrite earlier
         * ones on collision, so we insert English first, then localized
         * — the localized wins when they differ. */
        if (entry?.labelEn) idx.set(String(entry.labelEn).toLowerCase(), key);
        if (entry?.label)   idx.set(String(entry.label).toLowerCase(),   key);
    }
    _labelIndex = idx;
    return idx;
}
...
export function parseOpenCategoryBonus(text, catalog = WEAPON_QUALITIES) {
    ...
    /* Suffocation (Strangling): a per-turn choke-damage bonus. Two forms:
     *    "+N suffocation"      → suffocation: N   (flat add to 3 + melee bonus)
     *    "xN" / "×N suffocation" → suffocationMult: N (multiply the per-turn total)
     * Both may appear; the flat add is applied before the multiplier. */
    let suffocation = 0, suffocationMult = 1;
    const sufFlat = raw.match(/\+\s*(\d+)\s*suffocation\b/i);
    if (sufFlat) { const n = Number(sufFlat[1]); if (Number.isFinite(n)) suffocation = n; }
    const sufMult = raw.match(/[x×]\s*(\d+(?:\.\d+)?)\s*suffocation\b/i);
    if (sufMult) { const n = Number(sufMult[1]); if (Number.isFinite(n) && n > 0) suffocationMult = n; }
    ...
    return { wa, dmgDice, dmgFlat, suffocation, suffocationMult, grantedQualities, grantedQualityValues, raw };
}
...
/** Strangling suffocation bonus for a weapon: the flat per-turn add and the
 *  multiplier authored in its Strangling open-category text (e.g. a garrote's
 *  "×3 suffocation"). Returns `{ flat, mult }` — choke damage is
 *  `(3 + meleeBonus + flat) × mult` per action. Brawling (no weapon) → {0, 1}. */
export function strangleSuffocation(weapon) {
    let flat = 0, mult = 1;
    if (!weapon) return { flat, mult };
    for (const b of getActiveOpenCategoryBonuses(weapon, { isChokehold: true })) {
        if (b.key !== "strangling") continue;
        flat += Number(b.suffocation) || 0;
        const m = Number(b.suffocationMult);
