...
import { applyDaypartPins } from "./weather-pins.mjs";
...
const FLAG_TAG_KEYS = Object.freeze(["storm", "hail", "lightning", "heat", "aurora"]);
 ...
 *  Applies GM-authored daypart pins so a pinned daypart shows the pinned
 *  conditions (temp/climate flags stay engine-derived). */
export function getDaypartsForTime(worldTime = game.time?.worldTime ?? 0) {
    const absDay = dayIndexForTime(worldTime);
    ...
    return applyDaypartPins(parts, absDay);
}
...
export function getWeatherForTime(worldTime = game.time?.worldTime ?? 0) {
    ...
    const absDay = dayIndexForTime(worldTime);
    const rawParts = generateDayparts({
        absDay,
        seasonName: seasonNameForTime(worldTime),
    ...
    const parts = applyDaypartPins(rawParts, absDay);
