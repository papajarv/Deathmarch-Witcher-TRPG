...
test("weaponAttackMixin pulls the helper and folds wa+tail into the math", () => {
    assert.match(mixinSrc, /import\s*\{\s*getActiveOpenCategoryBonuses\s*,\s*sumOpenCategoryWa\s*,\s*damageTailFromOpenCategory\s*,\s*grantedQualitiesFromOpenCategory\s*,\s*grantedQualityValuesFromOpenCategory\s*\}/);
