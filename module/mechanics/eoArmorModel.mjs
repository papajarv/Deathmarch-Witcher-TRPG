...
/* EO body-zone model (EO p.4, formalized per author ruling): armor
 * enhancements are bought/crafted per ZONE, not per limb — an arms
 * enhancement covers BOTH arms, a legs enhancement covers BOTH legs.
 * ZONES is the authoring granularity (4 zones); LOCATIONS is retained
 * for downstream damage math (still 6 per-side entries because hit
 * location resolves to a specific limb). */
export const ZONES           = Object.freeze(["head", "torso", "arms", "legs"]);
export const ZONE_LOCATIONS  = Object.freeze({
    head:  Object.freeze(["head"]),
    torso: Object.freeze(["torso"]),
    arms:  Object.freeze(["leftArm", "rightArm"]),
    legs:  Object.freeze(["leftLeg", "rightLeg"])
});
export const LOCATION_ZONE   = Object.freeze({
    head:     "head",
    torso:    "torso",
    leftArm:  "arms",  rightArm: "arms",
    leftLeg:  "legs",  rightLeg: "legs"
});
/** Zone-side coverage list for a body location. Used by attachers /
 *  bakers / detachers so an enhancement's SP + effects flow only to
 *  the underlying limbs of the zone it was applied to. */
export function locationsForZone(zone) {
    return ZONE_LOCATIONS[zone] ?? [];
}
/** Map a raw location key ("leftArm") to its owning zone ("arms"). */
export function zoneForLocation(loc) {
    return LOCATION_ZONE[loc] ?? loc;
}
/** Zones the armor PHYSICALLY protects — derived from per-limb
 *  MaxStopping instead of the `location` enum's declared coverage.
 *  This diverges from ARMOR_LOCATION_COVERAGE by intent: a Steel
 *  Breastplate authored as `location: "torso"` has 0 SP on arms, so
 *  offering "Arms" as an AE zone would let the player waste a slot on
 *  a place the armor doesn't protect. Reading MaxStopping is the
 *  ground-truth "what does this piece actually cover" signal, robust
 *  to pack-author enum choices. */
export function coveredZones(armor) {
    const sys = armor?.system ?? {};
    const zones = [];
    for (const zone of ZONES) {
        const limbs = ZONE_LOCATIONS[zone] ?? [];
        const anySp = limbs.some(loc => (Number(sys[`${loc}MaxStopping`]) || 0) > 0);
        if (anySp) zones.push(zone);
    }
    return zones;
}

/* Retained under the old name for internal call sites — now points at
 * ZONES so all authoring / capacity math treats the four EO zones as
 * the enumeration. Legacy 6-loc consumers below use ZONE_LOCATIONS to
 * expand a zone into its physical limbs. */
...
/** Total AE-slot capacity on this armor piece. EO models AE as a single
 *  budget the player allocates across body zones at attach time — see
 *  EO p.4 ("total slot of AEs and players decide what to do with them").
 *  aeSlotCap(armor, _) is retained for legacy call-site compatibility
 *  and returns the same total (per-zone cap doesn't exist in this model). */
export function aeSlotCapTotal(armor) {
    return Number(armor?.system?.aeSlots) || 0;
}
export function aeSlotCap(armor /*, _zone */) {
    return aeSlotCapTotal(armor);
}
...
/** List of body zones covered by this armor piece that can accept
 *  another AE. Under the single-budget model, ANY covered zone is a
 *  valid target as long as the piece's total budget hasn't been spent.
 *  Returns an empty list when the piece is fully allocated.
 *  Each entry: { key: zone, used: perZoneUsed, cap: totalRemaining }
 *  — the `cap` here reports remaining budget so the picker can show
 *  "N slots left" per option without pretending each zone has its own. */
export function locationsWithFreeAeSlots(armor) {
    const totalCap  = aeSlotCapTotal(armor);
    if (totalCap <= 0) return [];
    const totalUsed = aeSlotsUsed(armor);
    if (totalUsed >= totalCap) return [];
    /* Physically-covered zones (SP > 0 on at least one underlying limb),
     * not the enum's declared coverage — see coveredZones() docs. */
    const zones = coveredZones(armor);
    const remaining = totalCap - totalUsed;
    return zones.map(zone => ({
        key: zone,
        used: aeSlotsUsed(armor, zone),
        cap:  remaining + aeSlotsUsed(armor, zone)   // display-only: "you can put another here"
    }));
}
