...
import { sceneWeatherMode, WEATHER_MODES, effectiveWeatherAudio } from "./scene-weather-mode.mjs";
...
// Rainfall + heavy rainfall need a small indoor bump beyond the flat INDOOR_VOL
// so the "storm through the walls" reads at a plausible volume — the bed
// mixes were tuned for outdoor loudness, and the standard indoor duck
// leaves them too quiet to hear over the muffle. Kept smaller than the hail
// boost because rain sits in the low-mid, which the lowpass leaves alone.
// (rain-light isn't in the set — a light shower SHOULD barely reach you
// through walls.)
const RAIN_BOOST_KEYS = new Set(["rain", "rain-heavy"]);
const RAIN_BOOST = 1.35;
...
/* volumeOverride (from a wdmWeatherAudio region behavior) REPLACES the
 * built-in indoor base multiplier. Zero / null means fall back to the
 * default INDOOR_VOL. Hail + medium/heavy rain always get their per-key
 * gentle boost on top so they survive the muffle at either the default
 * or an overridden volume. */
const indoorVolume = (base, key, override) => {
    const mult = (Number(override) > 0) ? Number(override) : INDOOR_VOL;
    let keyBoost = 1;
    if (HAIL_KEYS.has(key))       keyBoost = HAIL_BOOST;
    else if (RAIN_BOOST_KEYS.has(key)) keyBoost = RAIN_BOOST;
    return base * mult * keyBoost;
};
 ...
 * `volumeOverride` applies only in INDOOR mode (from a wdmWeatherAudio region
 * behavior); null / 0 falls back to the built-in INDOOR_VOL multiplier. */
function shapeSound(sound, base, mode, key, fade, volumeOverride = null) {
    ...
    setVolume(sound, indoorVolume(base, key, volumeOverride), fade);   // faded down + muffled
}

function applyToSound(ps, mode, volumeOverride = null) {
    ...
    try {
        if (eff === WEATHER_MODES.OUTDOOR) {
            ...
            _state.set(ps.id, { mode: eff, sound, onPlay: null, base, volumeOverride: null });
            return;
        }

        // INDOOR / OFF. A mode switch, a fresh Sound, OR a volume-override change
        // re-wires the effect chain and FADES to the target — matching the
        // playlist cross-fade, so stems ease in/out instead of popping. When
        // nothing structural changed we leave the running fade ALONE (re-pinning
        // the gain every tick is what made transitions choppy); only a GM volume-
        // slider nudge gets a silent snap.
        const overrideChanged = (prev?.volumeOverride ?? null) !== (volumeOverride ?? null);
        if (prev?.mode !== eff || soundChanged || overrideChanged) {
            shapeSound(sound, base, eff, key, true, volumeOverride);
        } else if (prev?.base !== base) {
            setVolume(sound, eff === WEATHER_MODES.OFF ? 0 : indoorVolume(base, key, volumeOverride), false);
        }
        ...
        if (!onPlay) {
            onPlay = () => {
                try {
                    const { mode: liveMode, volumeOverride: liveOverride } = effectiveWeatherAudio();
                    shapeSound(sound, Number(ps.volume) || 0, liveMode, key, true, liveOverride);
                } catch (_) { /* leave untouched */ }
            };
            sound.addEventListener("play", onPlay);
        }
        _state.set(ps.id, { mode: eff, sound, onPlay, base, volumeOverride });
...
export function syncSceneWeatherAudio() {
    ...
    /* Effective mode blends scene weather mode with per-viewer region
     * overrides (wdmWeatherAudio behavior). volumeOverride is only
     * meaningful for indoor mode; scene-level indoor uses null (default
     * INDOOR_VOL). */
    const { mode, volumeOverride } = effectiveWeatherAudio();
    let pending = false;
    for (const ps of pl.sounds) {
        if (mode !== WEATHER_MODES.OUTDOOR && ps.playing && !ps.sound) pending = true;
        applyToSound(ps, mode, volumeOverride);
    }
...
export function registerSceneWeatherAudio() {
    ...
    /* Viewer-token movement / control triggers re-sync so wdmWeatherAudio
     * region overrides land as the token crosses boundaries. Region
     * behaviors themselves also fire TOKEN_ENTER/EXIT which call in
     * (see weather-audio.mjs), but the token-position hooks below cover:
     *   - GM previewing a token drag (region.tokens updates lazily)
     *   - `controlToken` flipping which token counts as the viewer
     *   - user.character reassignment mid-session (updateUser). */
    Hooks.on("controlToken", scheduleReapply);
    Hooks.on("updateToken",  scheduleReapply);
    Hooks.on("updateUser",   (u) => { if (u.id === game.user.id) scheduleReapply(); });
