...
/** GM-toggled freeze independent of `game.paused`. */
function clockPaused() {
    try { return !!game.settings.get(SYSTEM_ID, "clockPaused"); }
...
/** True iff at least one user (GM or player) is currently connected.
 *  Used to freeze the wall clock only when the world is fully
 *  unattended — nobody signed in at all. When a GM is logged in solo
 *  (session prep, mid-scene review, running AE-expiry tests) the
 *  clock still ticks so time-based effect durations and zone-template
 *  expiries progress the way the GM expects them to. */
function anyUserOnline() {
...
function shouldFlow() {
    ...
    // Fully unattended world: don't advance. See anyUserOnline comment.
    if (!anyUserOnline()) return false;
    if (clockPaused()) return false;
