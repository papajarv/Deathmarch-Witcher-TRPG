...
/** Synchronous variant of getHolds — useful in hooks / dialog builders where
 *  we can't await. Self-warms the cache SYNCHRONOUSLY: finding the existing
 *  registry actor is a plain `game.actors` lookup (no await needed), and the
 *  registry is OBSERVER-visible to players, so this now works on EVERY client
 *  even if the async getHolds path hasn't run yet. Only the CREATE path
 *  (GM-only, first-ever hold) needs async. */
export function getHoldsSync(actorUuid) {
    if (!_cachedRegistry || _cachedRegistry.invalid) {
        try {
            const found = game?.actors?.find?.(a =>
                a?.getFlag?.(SYSTEM_ID, "isHoldRegistry") === true);
            if (found) _cachedRegistry = found;
        } catch (_) { /* game.actors not ready */ }
    }
...
export async function clearHold(actorUuid, partnerUuid = null, kind = null) {
    const reg = await getOrCreateRegistry();
    if (!reg) return [];
    const cur = _readEntries(reg);
    const removed = [];
    const next = cur.filter(p => {
        ...
        /* Kind filter (layered holds): when a `kind` is given, only pairs of
         * THAT kind are eligible for removal — used to peel one layer (e.g.
         * Escape a pin back to a grapple without dropping the grapple). */
        if (kind !== null && p.kind !== kind) return true;
        if (partnerUuid === null) {
