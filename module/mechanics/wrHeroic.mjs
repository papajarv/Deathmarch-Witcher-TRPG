...
import { adrenalineStaPerDie, isAdrenalineEnabled } from "../api/adrenaline.mjs";
...
async function debitAdrenaline(actor, dice) {
    ...
    const staCost = dice * adrenalineStaPerDie();
    if (ae < dice) {
...
export async function flowAndEbb(actor, dice = 1) {
    ...
    if (regen < dice) {
        ui.notifications?.info(`Flow and Ebb: ${actor.name} has only ${spent} vigor spent this round — regen limited to ${regen}.`);
    }
    ...
    const diceLine = regen > 1 ? ` (spent ${regen} adrenaline for ${regen} vigor)` : "";
    await ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ actor }),
        content: `<div class="wdm-attack-rider wdm-wr-heroic"><i class="fa-solid fa-water"></i> <strong>Flow and Ebb</strong> — ${banner}${diceLine}</div>`,
...
export async function lightningFast(actor, dice = 1) {
    ...
    await roll.toMessage({
        speaker: ChatMessage.getSpeaker({ actor }),
        flavor: `<div class="wdm-attack-rider wdm-wr-heroic"><i class="fa-solid fa-bolt-lightning"></i> <strong>Lightning Fast</strong> — ${banner}</div>`,
        flags: { [SYSTEM_ID]: { category: "combat" } }
    });
    return true;
}
...
export async function unrelenting(actor) {
    ...
    if (wrHeroic(actor) !== "unrelenting") {
        ui.notifications?.warn(`${actor.name} does not have the Unrelenting heroic action.`);
        return false;
    }
    if (!game.combat?.started) {
        ui.notifications?.warn("Unrelenting only works during combat.");
        return false;
    }
    ...
    if (!dying && !wounded) {
        ui.notifications?.info(`${actor.name} is neither wounded nor dying — Unrelenting has no effect right now.`);
        return false;
    }
    ...
    if (dying) {
        ...
        await ChatMessage.create({
            speaker: ChatMessage.getSpeaker({ actor }),
            content: `<div class="wdm-attack-rider wdm-wr-heroic"><i class="fa-solid fa-hand-fist"></i> <strong>Unrelenting</strong> — ${actor.name} ignores Death State penalties this round + banks a death-save auto-pass (spent 1 adrenaline).</div>`,
            flags: { [SYSTEM_ID]: { category: "combat" } }
        });
    } else {
        await ChatMessage.create({
            speaker: ChatMessage.getSpeaker({ actor }),
            content: `<div class="wdm-attack-rider wdm-wr-heroic"><i class="fa-solid fa-hand-fist"></i> <strong>Unrelenting</strong> — ${actor.name} ignores Wound State penalties this round (spent 1 adrenaline).</div>`,
            flags: { [SYSTEM_ID]: { category: "combat" } }
        });
    }
    return true;
