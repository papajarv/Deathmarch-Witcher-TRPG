 ...
 *  in the caller. */
/** Parse a #rrggbb flair-override colour to a PIXI int, or null. */
function flairIntOf(item) {
    const hex = item?.system?.flairColor;
    if (typeof hex !== "string") return null;
    const m = /^#?([0-9a-fA-F]{6})$/.exec(hex.trim());
    return m ? parseInt(m[1], 16) : null;
}

/** Tier wash colour as a PIXI int, honouring the GM's world-global rarity
 *  palette override (settings `rarityColors`) before the shipped default. */
function tierHexInt(rarity) {
    try {
        const pal = game.settings.get(SYSTEM_ID, "rarityColors") ?? {};
        const hex = pal?.[rarity];
        if (typeof hex === "string") {
            const m = /^#?([0-9a-fA-F]{6})$/.exec(hex.trim());
            if (m) return parseInt(m[1], 16);
        }
    } catch (_) { /* settings not ready — fall through to shipped default */ }
    return RARITY_HEX[rarity];
}

function drawSlotBackground(g, x, y, size, rarity, flairInt = null) {
    // A per-item flair override wins over the (possibly GM-recoloured) tier.
    const tier = (flairInt != null) ? flairInt : tierHexInt(rarity);
...
const RARITY_SCORE = {
    relic: 110, goetia: 105, elderfolk: 95, experimental: 90,
    witcher: 100, rare: 60, poor: 30, common: 15, everywhere: 5
};
...
function computeFeaturedItems(merchant) {
    ...
    for (const id of merchant.system.featuredPinned ?? []) {
        ...
        result.push({
            ...
            flair: hit.flair, broken: isBrokenForCard(hit.item)
        });
        used.add(id);
    }
    ...
    while (result.length < FEATURED_SLOTS && pool.length) {
        ...
        result.push({
            ...
            flair: pick.flair, broken: isBrokenForCard(pick.item)
        });
    }
    return result;
}
...
async function buildCardContainer(merchant, featured, opacity = 0.9) {
    ...
    for (let i = 0; i < featured.length; i++) {
        ...
        drawSlotBackground(slotBg, x, ROW_Y, SLOT_SIZE, item.rarity, item.flair);
...
class MerchantCardRenderer {
        ...
        el.innerHTML = `
            ...
            ${isGM ? `
                <div class="wdm-card-gm">
                    ...
                    <button type="button" class="wdm-card-gm-btn" data-action="edit" title="${game.i18n.localize("WITCHER.Merchant.EditActor") || "Edit merchant"}"><i class="fas fa-pen"></i></button>
    ...
    _wireOverlay() {
        ...
        el.querySelector('[data-action="edit"]')?.addEventListener("click", (e) => {
            e.stopPropagation();
            // Open the underlying merchant actor sheet — used by GMs to tune
            // stock, pricing, and personality without having to hunt the actor
            // down in the sidebar. Force mode so a closed sheet reopens.
            this.merchant?.sheet?.render(true);
        });
