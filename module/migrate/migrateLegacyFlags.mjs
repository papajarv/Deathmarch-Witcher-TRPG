...
export async function runLegacyMigration() {
    ...
    // v6: Silver became a valueless weapon quality (whole hit counts as
    // silver; no separate silver dice). Scrub the retired `2d6` value box:
    //   • the `param` on the Silver entry in the GM's stored qualities
    //     override (weaponQualitiesOverride), and
    //   • the dead `system.qualityValues.silver` payload on every weapon.
    // Both are already inert at runtime (getActiveWeaponQualities strips the
    // param; the silver roll is gated off), so this is a cosmetic cleanup —
    // it just stops the stale value from lingering in world data.
    if (current < 6) {
        tally(await safe(() => scrubSilverQualityValueAcrossWorld(), "silver quality-value scrub"));
    }
    ...
    if (failed > 0) {
        ui.notifications?.warn(tFormat("WITCHER.Migrate.LegacyFlags.Notify.WithFailures", { migrated, failed }, `Witcher TTRPG: migration applied ${migrated} updates, ${failed} docs failed (see console).`));
    } else if (migrated > 0) {
        ui.notifications?.info(tFormat("WITCHER.Migrate.LegacyFlags.Notify.Complete", { migrated }, `Witcher TTRPG: migration complete (${migrated} updates).`));
    }
...
/**
 * v6 — scrub the retired Silver quality VALUE (the old "2d6" parameter).
 *
 * Silver is now a valueless weapon quality. Two places still carry the dead
 * value in world data:
 *   1. The `param` on the Silver entry inside the GM's stored qualities
 *      override (`weaponQualitiesOverride`) — frozen there when the qualities
 *      editor was last saved, back when Silver still had a value box. Only the
 *      `param` is stripped; any genuine label / description / damageFlags
 *      customization on that entry is preserved.
 *   2. `system.qualityValues.silver` on individual weapons — the number a
 *      player typed into the old value box.
 *
 * Both are already inert at runtime, so this only tidies the persisted data.
 * Walks world items + actor-embedded items (compendium entries are left
 * alone — they're inert too, and unlocking packs is out of scope here).
 * Returns the number of updates applied.
 */
async function scrubSilverQualityValueAcrossWorld() {
    let count = 0;

    // 1. Stored qualities-override setting.
    try {
        const override = game.settings.get(SYSTEM_ID, "weaponQualitiesOverride");
        if (override?.silver?.param) {
            const next = foundry.utils.deepClone(override);
            next.silver = { ...next.silver };
            delete next.silver.param;
            await game.settings.set(SYSTEM_ID, "weaponQualitiesOverride", next);
            count += 1;
        }
    } catch (err) {
        console.warn(`${SYSTEM_ID} | silver override param scrub failed`, err);
    }

    // 2. Per-weapon dead value. Keyed only by the presence of a `silver` entry
    //    in the item's qualityValues map, so it's schema-safe (only items that
    //    actually carry the field are touched).
    const scrubItem = async (item) => {
        const qv = item?.system?.qualityValues;
        if (!qv || typeof qv !== "object" || !("silver" in qv)) return 0;
        try {
            await item.update({ "system.qualityValues.-=silver": null });
            return 1;
        } catch (err) {
            console.warn(`${SYSTEM_ID} | silver value scrub failed on item ${item.id}`, err);
            return 0;
        }
    };
    for (const i of [...game.items]) count += await scrubItem(i);
    for (const a of game.actors)
        for (const i of [...a.items]) count += await scrubItem(i);
