...
/* rAF-coalesced so a burst of updates in one frame (turn change, mass
 * initiative roll) collapses into a SINGLE DOM paint instead of one full
 * `querySelectorAll` + guard-chip rebuild per event. */
let _guardRepaintQueued = false;
function refreshAllTrackers() {
    if (_guardRepaintQueued) return;
    _guardRepaintQueued = true;
    requestAnimationFrame(() => { _guardRepaintQueued = false; paintTrackerRows(document); });
}
