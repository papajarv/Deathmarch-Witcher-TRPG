 ...
 *  which the attack pipeline reads via `getActorTarget()`. Gated on
 *  `isMyTurnNow()` so a player can't tag targets outside their turn. */
async function onTrackerAuxClick(ev) {
    ...
    ev.stopPropagation();
    if (!isMyTurnNow()) {
        ui.notifications?.warn("Not your turn — can't tag a target from the tracker.");
        return;
    }
    ...
    try {
        if (token) {
            ...
            /* releaseOthers:false so middle-click stacks additional
             * targets (matches Ctrl+click on the canvas). */
