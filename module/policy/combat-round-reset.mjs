...
async function resetUpcoming(combat, prior) {
    ...
    try { await checkFastDrawOnTurnEnd(combat, prior); }
    catch (err) { console.warn("witcher-ttrpg-death-march | fast-draw check failed", err); }
    /* Charge cleanup — the charging status is armed by Full Round → Charge
     * and normally clears the moment the next attack commits. If the
     * bearer ends their turn WITHOUT attacking, no attack flow ever fires
     * and the status would otherwise linger into their next turn. Clear it
     * silently here so skipping the turn cancels the charge as expected. */
    try { await checkChargeOnTurnEnd(combat, prior); }
    catch (err) { console.warn("witcher-ttrpg-death-march | charge clear failed", err); }
    /* NB: "clears at own turn end" status auto-clearing is handled by the tick
     * engine (chrome/policy/tick-effects.js) off `updateCombat`, which tracks
     * the outgoing combatant reliably — combat.previous / the combatTurnChange
     * `prior` arg lag across turn/round rollovers. */
}

/* The combatant whose turn just ended. Prefer the `prior` turn-state the
 * combatTurnChange hook passes — it's the authoritative outgoing turn.
 * `combat.previous` can still hold the turn BEFORE that (it updates a beat
 * late), which is what made turn-end effects fire a turn/round too late. */
function outgoingActor(combat, prior) {
    const prevId = prior?.combatantId ?? combat?.previous?.combatantId;
    if (!prevId) return null;
    return combat?.combatants?.get?.(prevId)?.actor ?? null;
}

async function checkChargeOnTurnEnd(combat, prior) {
    const actor = outgoingActor(combat, prior);
    if (!actor?.statuses?.has?.("charging")) return;
    if (!iShouldWrite(actor)) return;
    try { await actor.toggleStatusEffect?.("charging", { active: false }); }
    catch (err) { console.warn("witcher-ttrpg-death-march | clear stale charging failed", err); }
}

async function checkFastDrawOnTurnEnd(combat, prior) {
    const actor = outgoingActor(combat, prior);
    ...
    try {
        await ChatMessage.create({
            ...
            content: tFormat(
                "WITCHER.Policy.CombatRoundReset.Chat.FastDrawNoAttack",
                { name: actor.name },
                "<em><strong>{name}</strong> ended their turn without attacking after a Fast Draw — RAW (Core p.165) requires the snap-draw to be followed by an attack with the drawn weapon. The Fast Draw status has been cleared; rule the move as the GM sees fit.</em>"
            ),
...
async function endCombatForActor(actor) {
    ...
    for (const id of ["stunned", "fastDraw", "charging"]) {
    ...
    try {
        ...
        // Temporary STAMINA (house rule) is likewise a combat-only adrenaline
        // buffer — normally cleared at the start of the next turn, but if the
        // fight ends first it evaporates here too. It has no other source, so
        // the whole sta.temp is cleared.
        if ((Number(actor.system?.derivedStats?.sta?.temp) || 0) > 0) {
            upd["system.derivedStats.sta.temp"] = 0;
        }
...
export function registerCombatRoundReset() {
    ...
    Hooks.on("combatStart", async (combat) => {
        /* Make the started combat the ACTIVE one. Some start paths (the carousel
         * tracker's Start button, toggling a token into a pre-existing combat)
         * leave `active:false`. In Foundry v14 `game.combat` returns the combat
         * tracker's `viewed` while the tracker is rendered and only falls back
         * to `combats.find(c => c.isActive)` otherwise — so a started-but-inactive
         * combat makes `game.combat` collapse to null every time the native
         * tracker briefly de-renders (which it does on each item/actor update).
         * That silently broke every `game.combat` consumer (action-economy
         * charges, camera, movement budget). Activating on start restores the
         * fallback so `game.combat` stays resolved regardless of tracker churn.
         * Only the active GM writes (activate() updates the Combat document). */
        try {
            if (game.user?.isActiveGM && combat && !combat.active && typeof combat.activate === "function") {
                await combat.activate();
            }
        } catch (err) {
            console.warn("witcher-ttrpg-death-march | combat activate-on-start failed", err);
        }
    ...
    Hooks.on("combatTurnChange", (combat, prior) => resetUpcoming(combat, prior));
