...
function injectButton(app, html) {
    try {
        ...
        button.innerHTML = `<i class="fa-solid fa-crop-simple"></i> ${t("WITCHER.Policy.RingPortraitButton.Text.CropButton", "Crop Portrait into Token")}`;
        button.title = t("WITCHER.Policy.RingPortraitButton.Dialog.Title.OpenTheCropperFrameThePortrait", "Open the cropper, frame the portrait, and bake a circular token texture onto this actor (no file upload).");
        button.addEventListener("click", async (ev) => {
            ...
            try {
                ...
                ui.notifications?.error(tFormat("WITCHER.Policy.RingPortraitButton.Notify.CropperFailedToOpenXCheck", { val0: err?.message ?? err }, "Cropper failed to open: {val0}. Check console."));
            }
        });
