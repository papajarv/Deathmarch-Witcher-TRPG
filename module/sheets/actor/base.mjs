...
import { layeringEquipBlock } from "../../mechanics/armorLayering.mjs";
...
import { toggleEffectPause } from "../../chrome/policy/tick-effects.js";
...
export class WitcherActorSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
    ...
    static DEFAULT_OPTIONS = {
        ...
        actions: {
            ...
            woundHealDaysInc: WitcherActorSheet._onWoundHealDaysInc,
            woundHealDaysDec: WitcherActorSheet._onWoundHealDaysDec,
            ...
            pauseEffect:      WitcherActorSheet._onPauseEffect
        }
    };
    ...
    async _promptSkillChoice(profName, choose, optionKeys) {
        ...
        const content = `<div class="wdm-skillpick">
            <p>${tFormat("WITCHER.Sheet.Actor.Base.Text.ChooseNSkillsFor", { n: choose, plural: choose > 1 ? "s" : "", prof: profName }, `Choose <strong>${choose}</strong> skill${choose > 1 ? "s" : ""} for <strong>${profName}</strong>:`)}</p>
            ${rows}</div>`;
        const picked = await DialogV2.wait({
            window: { title: tFormat("WITCHER.Dialog.Skill.Package", { prof: profName }, "Skill Package — {prof}") },
            content,
            buttons: [{
                action: "ok", label: t("WITCHER.Common.Confirm", "Confirm"), default: true,
    ...
    async _prepareContext(options) {
        ...
        const armorLocations = [
            { key: "head",     label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Head", "Head") },
            { key: "torso",    label: t("WITCHER.Common.Torso", "Torso") },
            { key: "rightArm", label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.RArm", "R. Arm") },
            { key: "leftArm",  label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.LArm", "L. Arm") },
            { key: "rightLeg", label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.RLeg", "R. Leg") },
            { key: "leftLeg",  label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.LLeg", "L. Leg") }
        ];
        ...
        /* Race natural armor: full-body SP added ON TOP of worn armor on every
         * location. It is NOT a layer (folded in after layering, no EV cost)
         * and can never be ablated — mirrors buildNaturalArmorShape on the
         * damage side so the sheet total matches what actually soaks. */
        const naturalArmorSP = this.actor.items
            .filter(i => i.type === "race")
            .reduce((s, r) => s + (Number(r.system?.naturalArmorSP) || 0), 0);
        ctx.armorByLocation = armorLocations.map(({ key, label }) => {
            ...
            const candidates = armorPieces
                .map(a => {
                    ...
                    // Effective SP + per-location resistances (visor-aware: a
                    // raised visor drops head SP by 5 and clears head piercing,
                    // folded into `effective` by deriveArmorEffective) so the
                    // sheet readout matches what actually soaks in combat.
                    const locRes = eff.resistancesByLoc?.[key] ?? null;
                    return {
                        name: a.name,
                        sp:   Number(eff.stopping?.[key]?.value ?? a.system?.[stoppingKey]) || 0,
                        type: a.system?.armorType || "light",
                        slashing:    locRes ? !!locRes.slashing    : !!eff.slashing,
                        piercing:    locRes ? !!locRes.piercing    : !!eff.piercing,
                        bludgeoning: locRes ? !!locRes.bludgeoning : !!eff.bludgeoning,
                        fire:        locRes ? !!locRes.fire        : !!eff.fire,
                        lightning:   locRes ? !!locRes.lightning   : !!eff.lightning,
                        cold:        locRes ? !!locRes.cold        : !!eff.cold,
                        acid:        locRes ? !!locRes.acid        : !!eff.acid
                    };
                })
                .filter(p => p.sp > 0)
                .sort((a, b) => b.sp - a.sp);
            ...
            // Natural armor adds flat on top — not a layer, so it doesn't shift
            // the layer bonuses above; it just raises the location's total SP.
            total += naturalArmorSP;
            // A location resists a damage type if ANY piece covering it does.
            const resists = [];
            if (candidates.some(p => p.slashing))    resists.push(t("WITCHER.Sheet.Actor.Base.Text.Slash",   "Slash"));
            if (candidates.some(p => p.piercing))    resists.push(t("WITCHER.Sheet.Actor.Base.Text.Pierce",  "Pierce"));
            if (candidates.some(p => p.bludgeoning)) resists.push(t("WITCHER.Sheet.Actor.Base.Text.Bludgeon","Bludgeon"));
            if (candidates.some(p => p.fire))        resists.push(t("WITCHER.Damage.Fire",      "Fire"));
            if (candidates.some(p => p.lightning))   resists.push(t("WITCHER.Damage.Lightning", "Lightning"));
            if (candidates.some(p => p.cold))        resists.push(t("WITCHER.Damage.Cold",      "Cold"));
            if (candidates.some(p => p.acid))        resists.push(t("WITCHER.Damage.Acid",      "Acid"));
            return {
                key, label, total,
                natural: naturalArmorSP,
        ...
        const armorLocLabels = {
            head:     t("WITCHER.Chrome.HitLocation.Short.head",     "Head"),
            torso:    t("WITCHER.Chrome.HitLocation.Short.torso",    "Torso"),
            leftArm:  t("WITCHER.Chrome.HitLocation.Short.leftArm",  "L.Arm"),
            rightArm: t("WITCHER.Chrome.HitLocation.Short.rightArm", "R.Arm"),
            leftLeg:  t("WITCHER.Chrome.HitLocation.Short.leftLeg",  "L.Leg"),
            rightLeg: t("WITCHER.Chrome.HitLocation.Short.rightLeg", "R.Leg")
        };
        ...
        const slotLabels = {
            right: t("WITCHER.Sheet.Actor.Base.Text.MainHand",       "Main Hand"),
            left:  t("WITCHER.Sheet.Actor.Base.Text.OffHand",        "Off-hand"),
            both:  t("WITCHER.Sheet.Actor.Base.Text.TwoHanded",      "Two-Handed"),
            quick: t("WITCHER.Sheet.Actor.Base.Text.QuickOrOffHand", "Quick / Off-hand")
        };
        ...
        /* Pinned-spell set (dock's pinned-spells row reads the same flag). Lets
         * the magic tab show a pin toggle + highlight the currently-pinned ones. */
        const _pinnedArr = this.actor.flags?.["witcher-ttrpg-death-march"]?.pinnedSpells;
        const PINNED = new Set(Array.isArray(_pinnedArr) ? _pinnedArr : []);
        ...
        ctx.magicGroups = [
            { key: "spells",      label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Spells", "Spells"),      items: spells },
            { key: "signs",       label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Signs", "Signs"),       items: signs },
            { key: "invocations", label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Invocations", "Invocations"), items: invocations },
            { key: "hexes",       label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Hexes", "Hexes"),       items: hexes },
            { key: "rituals",     label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Rituals", "Rituals"),     items: rituals }
        ];
        /* Sub-group the spell-form lists (spells / signs / invocations) by
         * proficiency tier so the magic tab reads Novice → Journeyman → Master.
         * Hexes and rituals have no tier axis, so they get a single unlabelled
         * bucket — the template always iterates `tiers`, printing a subheader
         * only when `label` is set, so one row loop covers every group. */
        const SPELL_TIERS = [
            { key: "novice",     label: t("WITCHER.Spell.Tier.Novice", "Novice") },
            { key: "journeyman", label: t("WITCHER.Spell.Tier.Journeyman", "Journeyman") },
            { key: "master",     label: t("WITCHER.Spell.Tier.Master", "Master") }
        ];
        const byTier = (items) => {
            const known = new Set(SPELL_TIERS.map(x => x.key));
            const groups = SPELL_TIERS
                .map(ti => ({ label: ti.label, items: items.filter(it => (it.system?.spellType || "novice") === ti.key) }));
            const other = items.filter(it => !known.has(it.system?.spellType || "novice"));
            if (other.length) groups.push({ label: t("WITCHER.Spell.Tier.Other", "Other"), items: other });
            return groups.filter(g => g.items.length);
        };
        const TIERED = new Set(["spells", "signs", "invocations"]);
        for (const g of ctx.magicGroups) {
            g.tiers = TIERED.has(g.key) ? byTier(g.items) : [{ label: null, items: g.items }];
        }
        ctx.hasMagic = ctx.magicGroups.some(g => g.items.length > 0);

        /* Focus items for the Magic tab's Foci section (replaces the dead
         * free-text `system.focus1..focus4` inputs, which bound to fields that
         * no longer exist in the schema). Equipped foci sort first. `_availableFoci`
         * gates the cast dialog on `equipped`, so surfacing the equip toggle here
         * is what makes a focus actually reachable at cast time. */
        ctx.fociItems = this.actor.items
            .filter(i => i.type === "focus")
            .map(i => ({
                id: i.id, name: i.name, img: i.img,
                focus:        Math.max(0, Number(i.system?.focus) || 0),
                greaterFocus: Math.max(0, Number(i.system?.greaterFocus) || 0),
                equipped: !!i.system?.equipped
            }))
            .sort((a, b) => (Number(b.equipped) - Number(a.equipped)) || a.name.localeCompare(b.name));
        ctx.hasFoci = ctx.fociItems.length > 0;
        ...
        // Focus meta: surface the two numbers that make a focus worth equipping
        // (STA discount + Greater Focus roll bonus) and whether it's currently
        // worn, so the inventory row reads the same way the cast dialog does.
        const tagFocus = i => {
            const parts = [];
            const f = Number(i.system?.focus) || 0;
            const g = Number(i.system?.greaterFocus) || 0;
            if (f > 0) parts.push(`−${f} STA`);
            if (g > 0) parts.push(`+${g} roll`);
            if (i.system?.equipped) parts.push("equipped");
            const w = i.system?.weight;
        ...
        ctx.gearGroups = [
            { key: "weapons",     label: t("WITCHER.Common.Weapons", "Weapons"),      items: inventoryItems(i => i.type === "weapon" && !i.system?.equipped, tagInvWeapon) },
            { key: "ammo",        label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Ammo", "Ammo"),         items: inventoryItems(i => i.type === "ammo",       tagWeight) },
            { key: "shields",     label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Shields", "Shields"),      items: inventoryItems(i => i.type === "shield" && !i.system?.equipped, tagInvShield) },
            { key: "armor",       label: t("WITCHER.Common.Armor", "Armor"),        items: inventoryItems(i => i.type === "armor"  && !i.system?.equipped, tagInvArmor) },
            { key: "alchemicals", label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Alchemicals", "Alchemicals"),  items: inventoryItems(i => i.type === "alchemical", tagAlch) },
            { key: "mutagens",    label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Mutagens", "Mutagens"),     items: inventoryItems(i => i.type === "mutagen",    tagMut) },
            { key: "components",  label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Substances", "Substances"),   items: inventoryItems(i => i.type === "component",  tagWeight) },
            { key: "diagrams",    label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Diagrams", "Diagrams"),     items: inventoryItems(i => i.type === "diagrams",   tagWeight) },
            { key: "enhancements",label: t("WITCHER.Common.Enhancements", "Enhancements"), items: inventoryItems(i => i.type === "enhancement",tagWeight) },
            // Foci list here regardless of equipped state — unlike weapons/armor
            // they have no combat-tab home, so the inventory row (with its
            // equip/unequip context action) is their only sheet presence.
            { key: "foci",        label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Foci", "Foci"),         items: inventoryItems(i => i.type === "focus",      tagFocus) },
            { key: "food",        label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.FoodDrink", "Food & Drink"), items: inventoryItems(i => i.type === "food",       tagFood) },
            { key: "books",       label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Books", "Books"),        items: inventoryItems(i => i.type === "book",       tagWeight) },
            { key: "dice",        label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Dice", "Dice"),         items: inventoryItems(i => i.type === "die",        tagWeight) },
            ...
            { key: "valuables",   label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Valuables", "Valuables"),    items: inventoryItems(i => i.type === "valuable" || i.type === "remains", tagWeight) },
            { key: "notes",       label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Notes", "Notes"),        items: inventoryItems(i => i.type === "note" || i.type === "map", () => "") }
        ];
        ...
        ctx.effects = Array.from(this.actor.allApplicableEffects()).map(e => ({
            ...
            img:         e.img,
            disabled:    e.disabled,
            // Manual-pause state — the AE-list template swaps the pause icon
            // to a play icon when this is truthy. Flag is set by the pause
            // button (see toggleEffectPause / _onPauseEffect).
            isPaused:    e.getFlag?.(SYSTEM_ID, "pausedRemainingSecs") != null,
    ...
    static async _onRollInitiative(event, target) {
        ...
        /* Refuse when the actor is ALREADY in the active combat. This control
         * exists to roll an actor INTO combat; clicking it again on a combatant
         * re-rolls (or re-adds) their slot, which reshuffles the order mid-fight
         * and desyncs the tracker. Re-rolls belong to the combat tracker, not
         * this button. */
    ...
    static async _onTakeBreath(event, target) {
        ...
        let rec = Number(this.actor.system?.derivedStats?.rec) || 0;
        if (!sta) return;
        /* Restricted / Poor Vision helm (visor down) halves the STA you regain
         * from a Recovery Action — same rule the dock's Recovery Action applies,
         * routed through the shared predicate so both surfaces agree. */
        const visionHalved = armorHalvesStaRecovery(this.actor);
        if (visionHalved) rec = Math.floor(rec / 2);
        ...
        const breathText = visionHalved
            ? tFormat("WITCHER.Sheet.Actor.Base.Chat.TakeBreathHalved", { name: this.actor.name, from: sta.value, to: next }, "<em>{name} takes a breath — STA {from} → {to} (halved by Restricted Vision).</em>")
            : tFormat("WITCHER.Sheet.Actor.Base.Chat.TakeBreath", { name: this.actor.name, from: sta.value, to: next }, "<em>{name} takes a breath — STA {from} → {to}.</em>");
        ChatMessage.create({
    ...
    /** GM: advance a wound's days-elapsed by one (max = full heal). */
    static async _onWoundHealDaysInc(event, target) {
        if (!game.user.isGM) return;
        const wound = this.actor.items.get(target.dataset.itemId);
        if (!wound || wound.type !== "criticalWound") return;
        await wound.system.adjustHealElapsed(+1);
    }

    /** GM: set a wound's days-elapsed back by one (min = 0). */
    static async _onWoundHealDaysDec(event, target) {
        if (!game.user.isGM) return;
        const wound = this.actor.items.get(target.dataset.itemId);
        if (!wound || wound.type !== "criticalWound") return;
        await wound.system.adjustHealElapsed(-1);
    ...
    static _lifeEventDialogContent(entry = {}) {
        ...
        return `
            <div class="wdm-event-form">
              <label class="wdm-event-field"><span>${t("WITCHER.Sheet.Actor.Base.Text.Title", "Title")}</span>
                <input type="text" name="value" value="${esc(entry.value ?? "")}" placeholder="${t("WITCHER.Sheet.Actor.Base.Text.AMomentThatMattered", "A moment that mattered")}" />
              </label>
              <label class="wdm-event-field"><span>${t("WITCHER.Sheet.Actor.Base.Text.Date", "Date")}</span>
                <input type="date" name="date" value="${esc(entry.date ?? "")}" />
              </label>
              <label class="wdm-event-field"><span>${t("WITCHER.Sheet.Actor.Base.Text.Location", "Location")}</span>
                <input type="text" name="location" value="${esc(entry.location ?? "")}" placeholder="${t("WITCHER.Sheet.Actor.Base.Text.WhereItHappened", "Where it happened")}" />
              </label>
              <label class="wdm-event-field"><span>${t("WITCHER.Sheet.Actor.Base.Text.Detail", "Detail")}</span>
                <textarea name="details" placeholder="${t("WITCHER.Sheet.Actor.Base.Text.WhatHappenedWhoWasThereWhyItSticks", "What happened, who was there, why it sticks")}">${esc(entry.details ?? "")}</textarea>
              </label>
            </div>`;
    }
    ...
    static async _onAddLifeEvent(event, target) {
        const result = await foundry.applications.api.DialogV2.prompt({
            ...
            ok: { label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Add", "Add"), callback: (event, button) => WitcherActorSheet._readLifeEventForm(button) }
        });
    ...
    static async _onEditLifeEvent(event, target) {
        ...
        const result = await foundry.applications.api.DialogV2.prompt({
            ...
            ok: { label: t("WITCHER.Common.Save", "Save"), callback: (event, button) => WitcherActorSheet._readLifeEventForm(button) }
        });
        if (!result) return;
        await this._writeLifeEvent(id, result);
    }
    static async _onDeleteLifeEvent(event, target) {
        ...
        const ok = await foundry.applications.api.DialogV2.confirm({
            ...
            content: `<p>${t("WITCHER.Sheet.Actor.Base.Text.RemoveThisLifeEvent", "Remove this life event?")}</p>`
        });
        if (!ok) return;
        await this.actor.update({ [`system.general.lifeEvents.${id}`]: new foundry.data.operators.ForcedDeletion() });
    ...
    static async _onRollWeapon(event, target) {
        ...
        const actor = this.actor;
        /* Monster weapon rolls from the sheet route through the SAME canvas
         * target-select overlay as the dock, so a monster wielding a real
         * weapon picks its target on the canvas like its natural attacks do.
         * Character weapon-tab rolls keep their existing immediate behavior —
         * the dock is the PC's tile-targeting surface, and changing the
         * character sheet's roll button unprompted would be a surprise. The
         * overlay degrades to the immediate attack (returns false) when there's
         * no token / gridless / unparseable range / the setting is off. */
        if (actor?.type === "monster" && weapon.type === "weapon" && isTileTargetingEnabled()) {
            const token = actor.getActiveTokens?.()?.[0] ?? null;
            if (token) {
                try {
                    const engaged = await beginWeaponTargeting(actor, token, weapon, {
                        onPick: (defenderActor) => actor.weaponAttack?.(weapon, { forceDefender: defenderActor, tileTargeting: true })
                    });
                    if (engaged) return;   // the overlay owns the flow now
                } catch (err) {
                    console.warn("witcher-ttrpg-death-march | monster sheet weapon tile targeting failed", err);
                }
            }
        }
        return actor.weaponAttack?.(weapon);
    }
    ...
    static async _onCreatePerk(event, target) {
        ...
        const [perk] = await this.actor.createEmbeddedDocuments("Item", [{
            name: t("WITCHER.Sheet.Actor.Base.Text.NewPerk", "New Perk"), type: "perk", img: "icons/sundries/scrolls/scroll-bound-sealed-red.webp"
        }]);
    ...
    static async _onDeleteItem(event, target) {
        ...
        const ok = await foundry.applications.api.DialogV2.confirm({
            ...
            content: `<p>${t("WITCHER.Sheet.Actor.Base.Text.Remove", "Remove")} <strong>${item.name}</strong> from this actor?</p>`
        });
    ...
    equipMenuEntries() {
        ...
        const isEquippable = (i) => i && (handItem(i) || i.type === "armor" || i.type === "alchemical" || i.type === "focus");
        ...
        return [
            { name: t("WITCHER.Sheet.Actor.Base.Text.DrawMainHand", "Draw — Main Hand"), icon: '<i class="fa-solid fa-hand-fist"></i>',
              condition: (el) => { const i = get(el); return containerWeapon(i) && oneHand(i); }, callback: toHand("right") },
            { name: t("WITCHER.Sheet.Actor.Base.Text.DrawOffHand", "Draw — Off-hand"), icon: '<i class="fa-solid fa-hand-fist"></i>',
              condition: (el) => { const i = get(el); return containerWeapon(i) && oneHand(i); }, callback: toHand("left") },
            { name: t("WITCHER.Sheet.Actor.Base.Text.DrawQuickOffHand", "Draw — Quick / Off-hand"), icon: '<i class="fa-solid fa-bolt"></i>',
              condition: (el) => { const i = get(el); return containerWeapon(i) && oneHand(i) && isQuickItem(i); }, callback: toHand("quick") },
            { name: t("WITCHER.Sheet.Actor.Base.Text.DrawWield", "Draw — Wield"), icon: '<i class="fa-solid fa-hand-fist"></i>',
              condition: (el) => { const i = get(el); return containerWeapon(i) && !oneHand(i); }, callback: toHand(null) },
            { name: t("WITCHER.Sheet.Actor.Base.Text.EquipMainHand", "Equip — Main Hand"), icon: '<i class="fa-solid fa-hand"></i>',
              condition: (el) => { const i = get(el); return looseWeapon(i) && oneHand(i) && !inCombat(); }, callback: toHand("right", { spendAction: false }) },
            { name: t("WITCHER.Sheet.Actor.Base.Text.EquipOffHand", "Equip — Off-hand"), icon: '<i class="fa-solid fa-hand"></i>',
              condition: (el) => { const i = get(el); return looseWeapon(i) && oneHand(i) && !inCombat(); }, callback: toHand("left", { spendAction: false }) },
            { name: t("WITCHER.Sheet.Actor.Base.Text.EquipQuickOffHand", "Equip — Quick / Off-hand"), icon: '<i class="fa-solid fa-bolt"></i>',
            ...
            { name: t("WITCHER.Sheet.Actor.Base.Text.EquipWield", "Equip — Wield"), icon: '<i class="fa-solid fa-hand"></i>',
              condition: (el) => { const i = get(el); return looseWeapon(i) && !oneHand(i) && !inCombat(); }, callback: toHand(null, { spendAction: false }) },
            { name: t("WITCHER.Sheet.Actor.Base.Text.Equip", "Equip"), icon: '<i class="fa-solid fa-shield-halved"></i>',
              condition: (el) => { const i = get(el); return !i?.system?.equipped && (isShield(i) || i?.type === "alchemical" || i?.type === "focus"); },
              callback: (el) => this._setEquipped(get(el), true, "Equip") },
            { name: t("WITCHER.Sheet.Actor.Base.Text.Wear", "Wear"), icon: '<i class="fa-solid fa-shirt"></i>',
            ...
            { name: t("WITCHER.Sheet.Actor.Base.Text.SheatheUnequip", "Sheathe / Unequip"), icon: '<i class="fa-solid fa-box-archive"></i>',
            ...
            { name: t("WITCHER.Sheet.Actor.Base.Text.PutInContainer", "Put in Container…"), icon: '<i class="fa-solid fa-box"></i>',
              condition: (el) => {
                  const i = get(el);
            ...
            { name: t("WITCHER.Sheet.Actor.Base.Text.TakeOutOfContainer", "Take Out of Container"), icon: '<i class="fa-solid fa-box-open"></i>',
    ...
    async _setEquipped(item, equipped, verb) {
        if (!item) return;
        /* Armor layering gate — refuse an equip that would break a layering
         * limit (checked BEFORE the write, so nothing to revert). Runs in both
         * models: CE only enforces the stifling constraint (no count/type caps),
         * RAW enforces all. Unequips are always allowed. */
        if (equipped && layeringEquipBlock(this.actor, item, { ceModel: isCESubsystemEnabled?.("eoArmorModel") }).length) return;
    ...
    async _promptPutInContainer(item) {
        ...
        const containerId = await foundry.applications.api.DialogV2.prompt({
            ...
            content: `<div class="wdm-event-form"><label class="wdm-event-field"><span>${t("WITCHER.Sheet.Actor.Base.Text.Container", "Container")}</span><select name="container">${options}</select></label></div>`,
            ok: { label: t("WITCHER.Sheet.Actor.Base.Dialog.Button.Stow", "Stow"), callback: (_event, button) => button.form.elements.container?.value }
        }).catch(() => null);
    ...
    editItem() {
        return {
            name: t("WITCHER.Sheet.Actor.Base.Text.View", "View"),
    ...
    deleteItem() {
        return {
            name: t("WITCHER.Sheet.Actor.Base.Text.Delete", "Delete"),
    ...
    static async _onCreateEffect(event, target) {
        const [effect] = await this.actor.createEmbeddedDocuments("ActiveEffect", [{
            name: t("WITCHER.Sheet.Actor.Base.Text.NewEffect", "New Effect"),
    ...
    static async _onDeleteEffect(event, target) {
        ...
        const ok = await foundry.applications.api.DialogV2.confirm({
            ...
            content: `<p>${t("WITCHER.Sheet.Actor.Base.Text.Remove", "Remove")} <strong>${effect.name}</strong>?</p>`
        });
     ...
     * Backing implementation lives in chrome/policy/tick-effects.js
     * (toggleEffectPause). Snapshot on pause, restore on resume; the
     * WitcherActiveEffect._prepareDuration override reads the frozen value
     * so Foundry's registry-refresh never expires a paused effect. */
    static async _onPauseEffect(event, target) {
        const id = target.closest("[data-effect-id]")?.dataset.effectId;
        const effect = this.actor.effects.get(id);
        if (!effect) return;
