...
/* -------------------------------------------------------------------------- */
/* New silver-rule poor edge + Silver/Meteorite hybrid                        */
/* -------------------------------------------------------------------------- */

test("New silver rule: silver weapon deals HALF to a non-silver-weak target (poor edge)", () => {
  const r = resolveDamage({
    damageSource: dmgWeapon({ weaponDamage: 10, isSilver: true, newSilverRules: true, location: { key: "torso", mult: 1 } }),
    target:       makeTarget()   // ordinary target, no silver weakness
  });
  assert.equal(r.finalDamage, 5);
  assert.ok(r.stages.some(s => s.stage === "silverPoorEdge"));
});

test("Silver + Meteorite vs ordinary target: full damage (meteorite edge, no poor-edge halve)", () => {
  const r = resolveDamage({
    damageSource: dmgWeapon({ weaponDamage: 10, isSilver: true, isMeteorite: true, newSilverRules: true, location: { key: "torso", mult: 1 } }),
    target:       makeTarget()
  });
  assert.equal(r.finalDamage, 10);
  assert.ok(!r.stages.some(s => s.stage === "silverPoorEdge"));
...
test("Silver + Meteorite vs meteorite-weak monster: full (uses meteorite, no silver poor-edge)", () => {
  const r = resolveDamage({
    damageSource: dmgWeapon({ weaponDamage: 10, isSilver: true, isMeteorite: true, newSilverRules: true, location: { key: "torso", mult: 1 } }),
...
test("Silver + Meteorite vs silver-weak monster: full (uses silver gate)", () => {
  const r = resolveDamage({
    damageSource: dmgWeapon({ weaponDamage: 10, isSilver: true, isMeteorite: true, newSilverRules: true, location: { key: "torso", mult: 1 } }),
    target:       makeTarget({ monsterFlags: { resistNonSilver: true } })
  });
...
test("Improved AP does NOT bypass per-type immunity (immunity is absolute → 0)", () => {
  const r = resolveDamage({
    damageSource: dmgWeapon({ weaponDamage: 10, improvedArmorPiercing: true, damageTypes: ["fire"], location: { key: "torso", mult: 1 } }),
    target:       makeTarget({ monsterFlags: { immuneToTypes: ["fire"] } })
  });
  // Immunity is absolute — no armor piercing (regular or improved) bypasses it.
  assert.equal(r.finalDamage, 0);
});
