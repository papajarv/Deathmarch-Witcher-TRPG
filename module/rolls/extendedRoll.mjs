...
export async function rollWitcherD10() {
    /* Two independent House Rules toggle the RAW explode/collapse
     * chains (Core p.21). Defaults: both enabled. When collapse is
     * off, a nat 1 still counts as a fumble — only the -d10 drain
     * chain is suppressed. */
    let explodeOn = true, collapseOn = true;
    try {
        const { hrD10Explode, hrD10Collapse } = await import("../mechanics/house-rules-config.mjs");
        explodeOn  = hrD10Explode();
        collapseOn = hrD10Collapse();
    } catch (_) { /* config module unavailable at module-init → RAW defaults */ }
    ...
    if (first === 10) {
        ...
        /* Fumble stamp fires regardless of the collapse toggle —
         * a nat 1 always sends the player to the fumble table.
         * Only the -d10 drain chain is gated by `collapseOn`. */
...
export async function extendedRoll(formula, messageData = {}, config = {}) {
    ...
    /* Resolve messageMode for the visibility routing. Foundry v14's
     * canonical pattern (chat-message.mjs _preCreate line 538 +
     * chat.mjs line 875) is to pass `messageMode` as an OPERATION option
     * to ChatMessage.create — _preCreate calls applyMode() which stamps
     * whisper / blind on the data. Legacy `rollMode` was ALSO on
     * operation, but v14 marked it deprecated and only bridges it for
     * roll messages (isRoll gate around line 525) — since our payload
     * carries `rolls: []` we're NOT isRoll, so the legacy `rollMode`
     * path is skipped and only `messageMode` is honored.
     *
     * Resolution order:
     *   1. Explicit `messageMode` on messageData (caller v14 override).
     *   2. Explicit legacy `rollMode` on messageData, mapped forward.
     *   3. Foundry's client `core.messageMode` — SAME setting the
     *      sidebar Public/GM/Blind/Self dropdown writes to.
     *   4. "public" fallback if the setting is unavailable. */
    const shortModeFor = (legacy) => {
        try { return foundry.dice.Roll._mapLegacyRollMode(legacy); }
        catch (_) {
            return { publicroll: "public", gmroll: "gm", blindroll: "blind", selfroll: "self" }[legacy] ?? legacy;
        }
    };
    const messageMode =
        messageData.messageMode
        ?? (messageData.rollMode ? shortModeFor(messageData.rollMode) : null)
        ?? (() => { try { return game.settings.get("core", "messageMode"); } catch (_) { return null; } })()
        ?? "public";
    ...
    const created = messageData.suppressMessage
        ? null
        : await ChatMessage.create(msgPayload, { messageMode });
