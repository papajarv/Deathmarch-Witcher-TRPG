...
/* STA optionally carries `temp` too — a temporary buffer granted by the
 * "adrenaline → temporary stamina" house rule (6 per die, spent BEFORE real
 * STA, expires on the actor's next turn). Mirrors hp.temp; never raises max. */
const staPool = () => new fields.SchemaField({
    value: new fields.NumberField({ initial: 0, integer: true }),
    max:   new fields.NumberField({ initial: 0, integer: true }),
    temp:  new fields.NumberField({ initial: 0, integer: true, min: 0 })
});
