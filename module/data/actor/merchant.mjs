...
const RARITIES = ["everywhere", "common", "poor", "rare", "witcher", "elderfolk", "relic", "goetia", "experimental"];
/* Premium/exotic tiers default OFF in a merchant's stock filter — a random
 * shop shouldn't spawn Relics or Goetia gear unless the GM opts in. */
const RARITY_OFF_BY_DEFAULT = new Set(["witcher", "elderfolk", "relic", "goetia", "experimental"]);
    ...
    static defineSchema() {
        return {
            ...
            pricing: new fields.SchemaField({
                ...
                /* Fraction of an item's base cost this merchant pays when
                 * BUYING an item from a player. Seeds the suggested price in
                 * the sell-to-merchant negotiation dialog. Not affected by
                 * this merchant's sell-side markup: a shop that marks items up
                 * 2× isn't more generous on buybacks, it's just more greedy. */
                buybackPercent:        new fields.NumberField({ required: true, initial: 50, min: 0, max: 100, step: 1, integer: true })
            }),

            // Item TYPES this merchant is willing to BUY from players. Empty =
            // accept every tradeable type (backwards-compatible). Non-tradeable
            // types (pricing.NON_MERCHANT_TYPES) are always excluded regardless.
            buyCategories: new fields.ArrayField(new fields.StringField(), { initial: [] }),

            // Race / profession price adjustments. Matched against the buyer's
            // embedded `race` / `profession` item NAME (case-insensitive).
            // `pricePercent` is a multiplier percent: 100 = no change, 120 = the
            // buyer pays 20% MORE to buy and is paid proportionally LESS on a
            // buyback; 80 = the reverse. Applied symmetrically to buy & buyback.
            demographicModifiers: new fields.ArrayField(
                new fields.SchemaField({
                    kind:         new fields.StringField({ required: true, initial: "race", choices: ["race", "profession"] }),
                    name:         new fields.StringField({ initial: "" }),
                    pricePercent: new fields.NumberField({ required: true, initial: 100, min: 0, step: 1, integer: true })
                }),
                { initial: [] }
            ),
            ...
            stocking: new fields.SchemaField({
                ...
                rarityEnabled: new fields.SchemaField(
                    Object.fromEntries(RARITIES.map(r => [r, new fields.BooleanField({ initial: !RARITY_OFF_BY_DEFAULT.has(r) })]))
                ),
                ...
                rarityWeights: new fields.SchemaField({
                    ...
                    elderfolk:    new fields.NumberField({ initial: 1,  min: 0, max: 100 }),
                    relic:        new fields.NumberField({ initial: 1,  min: 0, max: 100 }),
                    goetia:       new fields.NumberField({ initial: 1,  min: 0, max: 100 }),
                    experimental: new fields.NumberField({ initial: 1,  min: 0, max: 100 })
                }),
