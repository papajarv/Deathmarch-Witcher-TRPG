...
export class SpellData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        return {
            ...
            /* AUTHORED BEHAVIOUR — the block engine (module/magic/).
             *
             * `frame` is law: cost, targeting, defence, element and duration,
             * enforced by the frame in the same order for every castable.
             * `on` maps an entry point ("hit", "success", "takeDamage",
             * "onExpire") to a tree of blocks.
             *
             * Both are ObjectFields rather than a modelled schema, and that is
             * deliberate: the shape is defined by the block REGISTRY, which
             * modules extend. A DataModel here would have to be widened every
             * time somebody adds a block, and the version that failed to be
             * widened would silently drop the argument — which is precisely
             * the failure the engine's own validator exists to catch, moved
             * somewhere it cannot be caught.
             *
             * Empty `on` means "no authored behaviour", and the item casts
             * down the original path untouched. */
            magic:       new fields.SchemaField({
                frame: new fields.ObjectField({ initial: {} }),
                on:    new fields.ObjectField({ initial: {} })
            }),
            ...
            // "Apply Every" (SPELL_APPLY_EVERY) — how often the target-
            // resource effect re-applies over the spell's duration. "instant"
            // = once on cast; "turn"/"minute"/"hour"/"day" spawn a ticking AE
            // (heal/damage action + cadence) on each target that fires until
            // the duration expires. Only meaningful for hp / healHp / ablation.
            applyEvery: new fields.StringField({ initial: "instant" }),
            ...
            statusRiders: new fields.ArrayField(new fields.SchemaField({
                statusId:    new fields.StringField({ required: true, blank: true }),
                ...
                /* Scaling formula for a single rider value.
                 *   offset   — value produced at baseSta AND per-step delta
                 *              (sign is preserved so negative offsets grow
                 *              more negative per step)
                 *   divisor  — stamina per additional step (min 1)
                 *   cap      — absolute value cap; scaling stops here
                 *              (negative caps clamp from above, positive from
                 *              below, 0 = no cap)
                 *   baseSta  — stamina at which scaling ACTIVATES; first step
                 *              lands here. Values below leave rider unscaled.
                 *              Encodes "every N beyond M" — set baseSta=M+1.
                 *              Default 1 = scales from minimum stamina.
                 *   maxSta   — stamina beyond this doesn't scale further; the
                 *              rider caps at whatever value maxSta produces.
                 *              0 = no per-rider cap (spell's own STA range).
                 * Formula (see mechanics/staScale.mjs → resolveScaleAt):
                 *   sta_eff = clamp(sta, baseSta, maxSta || Infinity)
                 *   steps   = 1 + floor((sta_eff - baseSta) / divisor)
                 *   value   = offset * steps, clamped by cap. */
                staScale:    new fields.SchemaField({
                    ...
                    baseSta: new fields.NumberField({ initial: 1, integer: true, min: 1 }),
                    maxSta:  new fields.NumberField({ initial: 0, integer: true, min: 0 })
                }),
                /* Where the resolved staScale value gets routed at cast time.
                 *   magnitude        → replaces the clause's magnitude
                 *                       (Yrden REF/DEX penalty, Static Storm
                 *                       damage tick)
                 *   endCheckModifier → stamped on the applied AE as
                 *                       flags.<sys>.endCheckModifier, added
                 *                       to the target's Stun / Awareness /
                 *                       skill end-check (Axii's harder-to-
                 *                       shake pattern)
                 *   duration         → resolved value REPLACES the rider's
                 *                       authored duration.value (signs that
                 *                       last longer per STA)
                 *   chance           → resolved value REPLACES the rider's
                 *                       chance %. Bigger cast = more likely
                 *                       to apply the status. Clamped 0-100.
                 * Default preserves legacy behavior for every existing rider. */
                staScaleTarget: new fields.StringField({
                    initial: "magnitude",
                    choices: ["magnitude", "endCheckModifier", "chance"]
                }),
                /* When TRUE, a re-cast of this same spell/hex on a target
                 * that already carries this rider's statusId strips the
                 * prior AE FIRST, then applies fresh. Prevents "two Axii
                 * stuns stacked" — the newer one replaces cleanly.
                 * Default FALSE = stack (each cast adds another AE, the
                 * existing behavior for fireball burning etc). */
                refreshOnRecast: new fields.BooleanField({ initial: false })
            })),
            ...
            /* ── Ignore targeting ────────────────────────────────────
             * When TRUE, the cast IGNORES the user's manually-targeted
             * tokens and always uses the AoE template harvest to pick
             * who gets hit. Anyone inside the template at cast time
             * takes it; everyone else is spared, regardless of what
             * the user had reticled. Use for Signs (Yrden, Aard, Igni)
             * and any zone/blast spell where "which token you have
             * clicked on the canvas" isn't the intent — the template
             * IS the intent. Requires a mappable areaShape (cone /
             * radius / cube / line); otherwise falls back to the
             * standard target-driven path. */
            ignoreTargets: new fields.BooleanField({ initial: false }),
            /* ── Damage cadence ───────────────────────────────────────
            ...
            /* ── Secondary armour ablation ────────────────────────────
             * OPTIONAL formula for armour SP eaten ALONGSIDE the main
             * damage (acid bolt: HP damage AND rust armour). Empty = none.
             * Independent of damageType:"ablation" (which ablates INSTEAD
             * of HP). Applies to every equipped armour piece, per-location,
             * same as the primary ablation path. */
            ablateArmor: new fields.StringField({ initial: "" }),
            /* Ablation scope: FALSE (default) = drain SP on EVERY hit location
             * of every equipped armour (RAW Rusting). TRUE = only the struck
             * location (torso for casts, which land on torso). Applies to BOTH
             * the primary damageType:"ablation" and the secondary ablateArmor. */
            ablateHitLocationOnly: new fields.BooleanField({ initial: false }),
            /* ── Multi-attack count ───────────────────────────────────
