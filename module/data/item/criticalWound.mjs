...
export class CriticalWoundData extends foundry.abstract.TypeDataModel {
     ...
     */
    get healingTime() {
        // Prepared BODY total = base + folded AE modifiers (character.mjs
        // prepareDerivedData folds `.modifier` into `.value`). The old `.max`
        // field was dropped from the stat schema, so reading it always yielded
        // 0 — healing time never scaled with BODY. Read `.value` like every
        // other BODY consumer (monster/character data, brawl/attack mixins).
        const body = Number(this.parent?.parent?.system?.stats?.body?.value) || 0;
    ...
    /**
     * GM manual nudge of the days-already-elapsed counter (the numerator of the
     * healing progress). `delta` whole days are added to (or, negative, removed
     * from) healDaysElapsed by shifting the treatedAt anchor: +1 day of elapsed
     * = treatedAt one day earlier. Clamped so elapsed stays within
     * [0, healingTime]; pushing it to the max completes the wound (the autoheal
     * sweep then clears it). Only meaningful once treated — no-op otherwise, or
     * when the click wouldn't change the whole-day count.
     */
    async adjustHealElapsed(delta) {
        const step = Number(delta) || 0;
        if (!step) return;
        if (this.state !== "treated" || this.treatedAt == null) return;
        const currentElapsed = this.healDaysElapsed;
        const targetElapsed  = Math.min(this.healingTime, Math.max(0, currentElapsed + step));
        const diff = targetElapsed - currentElapsed;   // whole days, post-clamp
        if (!diff) return;
        // Elapsed = floor((now - treatedAt) / secondsPerDay); moving treatedAt
        // back by `diff` days shifts the floor by exactly `diff`.
        const nextTreatedAt = this.treatedAt - diff * secondsPerDay();
        await this.parent.update({ "system.treatedAt": nextTreatedAt });
    }
