...
export class AlchemicalData extends foundry.abstract.TypeDataModel {
    static defineSchema() {
        return {
            ...
            /* Alchemy Reborn — running charge count on a bottle whose
             * charges are being spent one-at-a-time (the arrow/bolt
             * coating flow: one charge per coated projectile). Zero
             * means "unused stock" — the ammo coat flow initializes
             * from `oilCharges` on first use, then decrements per
             * coating; the bottle deletes when it hits 0. Weapon
             * coating (whole bottle in one go) reads this too so a
             * partially-spent bottle transfers only its remainder to
             * the weapon (no free charges by coat-arrows-then-weapon).
             * RAW mode ignores this field entirely. */
            currentCharges: new fields.NumberField({ initial: 0, integer: true, min: 0 })
