...
test("defenseMixin and weaponAttackMixin both import and apply guard mods", () => {
    const defSrc = readFileSync(new URL("../../documents/mixins/defenseMixin.mjs", import.meta.url), "utf8");
    const atkSrc = readFileSync(new URL("../../documents/mixins/weaponAttackMixin.mjs", import.meta.url), "utf8");
    assert.match(defSrc, /from\s+"[^"]*combatExtended\/guards\.mjs"/);
    assert.match(atkSrc, /from\s+"[^"]*combatExtended\/guards\.mjs"/);
    // defendWith folds guardDefenseMod into the total
    assert.match(defSrc, /guardDefenseMod\s*\(\s*this\s*,\s*item\s*,\s*null\s*\)/);
    // weaponAttackMixin folds guardAttackMod into grandMod
    assert.match(atkSrc, /\+\s*guardAtk\b/);
});
