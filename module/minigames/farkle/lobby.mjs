...
/* Persist the authoritative match snapshot onto the world-scope table setting
 * so a mid-game refresh re-hydrates the board (points, turn, dice, bank)
 * instead of dropping players into a zero-state view. GM-only writer; the
 * launchGame path reads `matchState` and passes it as `initialSnap`. */
export async function persistMatchSnapshot(matchId, snap) {
    ...
    try {
        ...
        console.warn(`${SYSTEM_ID} | farkle persistMatchSnapshot failed`, err);
    }
}
...
async function launchGame(table) {
    ...
    return openFarkleTableGame({
        ...
        // matchState — the app treats an absent field as "start fresh".
