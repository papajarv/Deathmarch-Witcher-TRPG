...
import { settleAndClose, endTable, notifyLiveClosed, persistMatchSnapshot } from "./lobby.mjs";
...
export class FarkleApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    constructor(options = {}) {
         ...
         * setting after every GM broadcast, so re-launching mid-game hands us
         * the last authoritative state via `options.initialSnap`. Restore on
         * top of the freshly-constructed match so shared invariants land first
         * (seats/dice/config), then the mutable game state overwrites. */
        if (options.initialSnap) {
            try { this.match.restore(options.initialSnap); }
            catch (err) { console.warn(`${SYSTEM_ID} | farkle initialSnap restore failed`, err); }
        }
    ...
    #broadcastState() {
        ...
        send({ matchId: this.matchId, to: null, sub: "state",
            payload: { seq: this._stateSeq, mseq: this._seq, snap } });
 ...
 */
export function openFarkleTableGame({ matchId, seats, config, seatDice, starter, mySeat, initialSnap = null }) {
    const app = new FarkleApp({ mode: "table", matchId, seats, config, seatDice, starter, mySeat, initialSnap });
    app.render(true);
    return app;
}
