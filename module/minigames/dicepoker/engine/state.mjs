...
export class DicePokerMatch {
     ...
     * standard limit-poker rule. The seat must be able to cover BOTH the call
     * AND a full raiseStep on top — partial-raise all-ins are disallowed here
     * (they get folded down to a plain call by the caller: canRaise returning
     * false forces the UI to only expose Check/Call/Fold). Without this floor,
     * a seat could nominally "raise" for just 1 coin and, worse, drop
     * currentBet below its previous value — leaving subsequent callers paying
     * less than they should for what they thought was the standing bet.
     */
    canRaise(seat) {
        if (this.phase !== "bet" || seat !== this.toAct) return false;
        const capped = this.#live().length > 2 && this.raises >= this.maxRaises;
        const cost = this.owed(seat) + this.raiseStep;
        return !capped && (this.players[seat]?.purse ?? 0) >= cost;
    }
     ...
     *  at `maxRaises`. `canRaise` guarantees the seat can cover call + full step,
     *  so `#payInto`'s purse clamp never actually engages here — but `Math.max`
     *  belts the currentBet write so a future edit or partial-raise regression
     *  can never drop the standing bet below its previous value. */
    raise(seat) {
