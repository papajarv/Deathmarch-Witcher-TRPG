...
import { settleNets, endTable, notifyLiveClosed, persistMatchSnapshot } from "./lobby.mjs";
...
export class DicePokerApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    constructor(options = {}) {
        ...
        /* Refresh-restore: the lobby persists the match snapshot to the table
         * setting after every broadcast, so re-launching mid-game hands us the
         * last authoritative state via `options.initialSnap`. Restore it on
         * top of the freshly-constructed match — that gives us the shared
         * `seats/dice/config` invariants first, then overwrites the mutable
         * game state (players/pot/scores/phase/etc.) from the snap. */
        if (options.initialSnap) {
            try { this.match.restore(options.initialSnap); }
            catch (err) { console.warn(`${SYSTEM_ID} | dice poker initialSnap restore failed`, err); }
        }
    }
    ...
    #betLine(seat, action, amount = 0) {
        ...
        // Flash an on-board pill for EVERY seat's action, including the local
        // player's own — the tags accumulate under each seat card through the
        // whole betting phase (cleared in #apply when phase leaves "bet") so
        // the table reads at a glance who has already checked/called/raised.
        if (line) this.#flashAction(seat, line);
    }
    ...
    #flashAction(seat, text) {
        ...
        list.push({ id, text });
        this.#refresh();
        // No auto-clear timer. The tags stay under each seat for the whole
        // betting phase and are wiped by #apply when the phase transitions
        // away from "bet" (i.e. back into throwing / reroll selection / done).
    }
    ...
    #apply(event, args) {
        ...
        // Clear the transient check/call/raise/fold badges under each seat
        // whenever the phase steps out of betting (back into rolling / reroll
        // selection / done). During "bet" the tags accumulate so nobody
        // misses who's already acted this street.
        if (m.phase !== "bet") this._seatAction = {};
        this._seq++;
        this.#broadcastState();
    }
    ...
    #broadcastState() {
        ...
        // UI-only sidecars that live outside the engine state: seat-action
        // pills (check/call/raise/fold feedback) and revealed dice used by the
        // hand-name display. These are populated on the GM from local move
        // handlers; clients need them broadcast or their board reads blank.
        const seatActions = this._seatAction ?? {};
        const reveal = this._reveal ?? {};
        send({ matchId: this.matchId, to: null, sub: "state",
            payload: { seq: this._stateSeq, mseq: this._seq, snap, seatActions, reveal } });
        // Durable persistence so a mid-game refresh re-hydrates the board.
        persistMatchSnapshot(this.matchId, snap);
    }
    ...
    async #applyRemoteState({ seq, mseq, snap, seatActions, reveal }) {
        ...
        // Hydrate the UI sidecars from GM so the client's feedback bubbles and
        // per-seat hand-name display don't read blank when the client either
        // joined mid-game or missed a relay to a race with the state broadcast.
        if (seatActions && typeof seatActions === "object") this._seatAction = { ...seatActions };
        if (reveal && typeof reveal === "object") this._reveal = { ...reveal };
        this._armToken = null;
        return this.#refresh();
    }
 ...
 */
export function openDicePokerTableGame({ matchId, seats, config, seatDice, starter, mySeat, initialSnap = null }) {
    const app = new DicePokerApp({ mode: "table", matchId, seats, config, seatDice, starter, mySeat, initialSnap });
    app.render(true);
    return app;
}
