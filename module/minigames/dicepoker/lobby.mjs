...
/* Persist the authoritative match snapshot onto the world-scope table setting so
 * a refresh mid-game re-hydrates the board (points, pot, phase, dice — all
 * restored) instead of dropping players into a zero-state view. GM-only writer;
 * the syncDicePokerUI boot path reads `matchState` off the launched table. */
export async function persistMatchSnapshot(matchId, snap) {
    if (!game.user.isActiveGM) return;
    const t = getTable();
    if (!t?.started || t.matchId !== matchId) return;
    try {
        await game.settings.set(SYSTEM_ID, SETTING, { ...t, matchState: snap });
    } catch (err) {
        console.warn(`${SYSTEM_ID} | persistMatchSnapshot failed`, err);
    }
}
...
async function syncDicePokerUI({ boot = false } = {}) {
    ...
    // games icon. The live app's match state IS persisted onto the table setting
    // by persistMatchSnapshot after every authoritative broadcast, so a mid-game
    // refresh no longer wipes points/pot/phase. We drop only the per-user "keep
    // open" flag so the board doesn't auto-pop on load; when the user opens it
    // again, launchGame reads table.matchState and restores.
    if (boot) {
...
async function launchGame(table) {
    ...
    return openDicePokerTableGame({
        ...
        starter: seatedIds[0],
        mySeat,
        // Restore mid-game state on refresh. First-launch tables have no
        // matchState — the app treats the field as "start fresh" when absent.
        initialSnap: table.matchState ?? null
    });
}
