...
const RARITY_KEYS = ["everywhere", "common", "poor", "rare", "witcher", "elderfolk", "relic", "goetia", "experimental"];
...
export async function stockMerchant(merchant, { isRestock = false } = {}) {
    ...
    const itemsToCreate = mergeStacks(rolled.map(itemData => {
        let quantity = 1;
    ...
    if (isRestock) {
        // On restock we KEEP existing inventory — coalesce new picks against
        // it by stackKey. Matches bump the existing item's quantity; unmatched
        // picks are created fresh. Without this, a restock that re-draws an
        // item the merchant already carries would spawn a duplicate row.
        const existingByKey = new Map();
        for (const it of merchant.items) existingByKey.set(stackKey(it.toObject()), it);
        const freshCreates = [];
        const qtyBumps = new Map();       // itemId → new quantity
        for (const data of itemsToCreate) {
            const match = existingByKey.get(stackKey(data));
            if (match) {
                const curQty = Number(qtyBumps.get(match.id) ?? match.system?.quantity) || 0;
                const addQty = Number(foundry.utils.getProperty(data, "system.quantity")) || 0;
                qtyBumps.set(match.id, curQty + addQty);
            } else {
                freshCreates.push(data);
            }
        }
        if (qtyBumps.size) {
            const updates = [...qtyBumps.entries()].map(([_id, quantity]) => ({ _id, "system.quantity": quantity }));
            await merchant.updateEmbeddedDocuments("Item", updates, { render: false });
        }
        if (freshCreates.length) {
            await merchant.createEmbeddedDocuments("Item", freshCreates, { render: false });
        }
    } else {
...
/* Merge duplicate rolls into stacks BEFORE createEmbeddedDocuments — without
 * this, the same compendium item drawn twice becomes two separate embedded
 * items with quantity 1 each, cluttering the shopfront (and forcing the buyer
 * to add each row individually). Identity key prefers a compendium source
 * pointer when available (a stable UUID across restocks); falls back to
 * name+type so pool-only merchants (curated item pools without compendium
 * origins) still coalesce. */
function stackKey(itemData) {
    const src = itemData?._stats?.compendiumSource
...
function mergeStacks(itemsData) {
    const byKey = new Map();
    for (const data of itemsData) {
        ...
        const curQty  = Number(foundry.utils.getProperty(existing, "system.quantity")) || 0;
        const addQty  = Number(foundry.utils.getProperty(data,     "system.quantity")) || 0;
        foundry.utils.setProperty(existing, "system.quantity", curQty + addQty);
    }
    return [...byKey.values()];
}
